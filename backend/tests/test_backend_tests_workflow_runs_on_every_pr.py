...
⚠️ **本文件全部注释适用同一约定**：凡写「全仓 PR 死锁 / 卡死 / 合不进去」，
一律指**管理员应用过 canonical 清单之后**；应用之前这些后果都不成立。
（与 spec 抬头那条「全文约定」同义。立一处约定而不是逐句补条件 —— 逐句补漏过四轮。）
"""
...
def test_job_name_equals_canonical_backend_context():
    ...
    （「死锁」这个后果同样以**管理员已应用 canonical 清单**为前提 —— 见本文件抬头那条
    ⚠️；应用之前改名不会卡住任何人，只是盲区悄悄回来。下面那条断言的报错文案里
    写全了两种情形。）
