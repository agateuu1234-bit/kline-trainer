...
# 谓词 2：ship 门 PR11-R1 / W1-R2 标 OPEN（块内全行）；W3-11-R1 已 CLOSED #117（bounce live 接线）；13a-R2 已 RESOLVED（不再标 OPEN）
# 结构守卫（R3-Med）：WAVE3-STATUS 须恰 1 开标记 + 其后有闭合 -->（拒未闭合注释吞后文）
# 谓词 3：高层状态 store-ready=NO + formal-closure=PENDING + feature-completeness=COMPLETE
#         + runtime-matrix=PARTIAL + freeze-tag=NOT-TAGGED（块内全行；feature-complete 合法 claim，但无误 claim 上架/已关闭）
...
# 谓词 2：ship 门 PR11-R1/W1-R2 = OPEN；W3-11-R1 = CLOSED #117（bounce live 接线）；13a-R2 = RESOLVED
require_kv "residual-W3-11-R1-bounce-live-wiring" "CLOSED W3-11-R1 #117"
...
# 谓词 3：高层状态（feature-complete 合法；无 store-ready / 正式关闭 误 claim）
require_kv "store-ready" "NO"
require_kv "formal-closure" "PENDING-runtime-matrix-device-record"
require_kv "feature-completeness" "COMPLETE"
...
echo "[verify-wave3-completion] PASS：A/B/C/D CLOSED + W3-11-R1 CLOSED #117 + PR11-R1/W1-R2 OPEN + 13a-R2 RESOLVED + WAVE3-STATUS 块诚实（feature-completeness=COMPLETE / store-ready=NO / closure·matrix·freeze pending）+ 矩阵 fixture 机制 + §三.3 三连合取（c8b/u2-gesture/帧预算）+ 顺位 2 orientation 指针就位"
