...
| B2 | 粘贴并回车：`bash /tmp/s4a_b1.sh` | 最后几行里有 **`1575 passed`**，并且**没有** `skipped` 或 `failed` 字样 | 数字是 1575 且无 skipped/failed = 通过；出现 failed 或 skipped = 不通过 |
...
2. **这一片没有经过 codex 官方评审通道**（那条通道撞到用量上限，评审没有跑起来，**不是没通过**）。
   本片经过的是另一条评审通道，它**写不了评审账本，不兑现治理条款**。
