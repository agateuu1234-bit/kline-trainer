...
| 9 | 看信息栏颜色（涨 K 线 vs 跌 K 线各一次） | **实时价/开/高/低/收/涨跌/涨跌幅/均价 全按前一根收盘上色**（高=红/低=绿/平=白）；日期·时间/成交量/成交额：黄色 | 价格字段全方向上色 + 非价格黄 = pass；开/高/低/均价 显黄或方向错 = fail |
...
| 19 | 长按上图出光标 → 再长按下图 | 上图光标+侧栏**自动消失**，只下图有光标（同时只一个图有光标） | 切图后旧光标消 = pass；两图同时有光标 = fail |
| 20 | 进训练界面看默认周期 | 上图 60分 / 下图 日线（相邻一档），单指竖滑（一甩）能上下切档 | 默认 60分/日线且竖滑能切 = pass；非相邻档/切不动 = fail |
