...
1. **底栏补 🗑 删除键**，与「类型」键并列。无选中时灰；有选中且该线可删时亮（**精确置灰规则见 D65「删除可用」谓词**：还需线当前可见、未锁定——未来枚举值线可删故不含该分量）。
...
> 但每轮的**修法方向一致且收敛**：把不变量钉在写入边界、让坏状态不可表达，而不是逐个补调用点或依赖 UI 自觉；核心设计（选择态显式 mode / 派生回显 / 写入边界三门）自 R5 起未再变动，后续多为判据精度与覆盖补全。
...
2. **UI 反馈路径可达且必须接**：返 `false`（或 UI 预检直接不调引擎）有**六种**原因（见下表），**它们的选中处置各不相同** —— 故 UI **不得**按返回值决定选中生命期，一律走 **D64 的状态谓词**（`selectedDrawingID` 是否仍在 `drawings` 中）。

   | 返 `false` / 不调引擎 的原因 | 引入决策 | 选中 |
   |---|---|---|
...
**我 R4 的修法（条件派生 `old.textColorToken == old.colorToken ? style.colorToken : old.textColorToken`）被 R7 推翻**——它在**解码后的枚举值**上比较，而**这个比较对 unknown 枚举值失效**：

- **实测**（`Models/Models.swift:327/334`）：`colorToken`/`textColorToken` 解码是 `.flatMap(DrawingColorToken.init(rawValue:)) ?? .orange` → 两个**不同**的 future raw 值（`colorToken:"futureNeon"` / `textColorToken:"futureCyan"`）**双双 fallback 成 `.orange`**，解码后 `==` 成立。
- 于是我的条件派生判「本来就是派生的」→ 覆盖字色 → 丢失。`LossyDrawingArray.swift:220-221` 的注释早已点破：这类值「完全落在 `knownDiskKeys` 内，对未来字段探测不可见——解码出的 `DrawingObject` 已被 fallback 覆盖，任何只读 `DrawingObject` 的逻辑对它不可见」。
- **在 `DrawingObject`（解码值）层，"orange 是本来的值"与"orange 是 futureCyan 的 fallback"根本无法区分。** 判据必须 raw-aware。

**这个问题比字色更大**（实测确认）：持久化走 `loadedDrawingsLossy.encoded()`（每条 `.known` 用原始 `raw` 重发，`LossyDrawingArray.swift:154-164`）。P1a 从不被咬，是因为**它只有 append、从不编辑已有线**；**1b-i 第一次引入"编辑已有线"** → 第一次会 re-merge 一条可能是高版本的线，用 fallback 值覆盖 raw 里**所有** unknown 枚举字节（`colorToken` / `textColorToken` / `lineStyle` / `period` …），不止字色。
...
- ⚠️ **本决策原稿曾写「样式编辑不禁」，已被 D65 推翻**（codex R5-F2）：那仍是一次不可逆的内容变更，且是 D59 教我别做的个案裁量。现在 **🗑 与样式控件同进同退**，统一由 D65 的可用性谓词决定。
- 🗑 置灰的判据与 D58 的预检**必须复用同一个** `visibleGeometry` 求值（同一函数、同一 mapper、同一 `selectedPanel`），不得另写一份。

### D64 选中清空的判据是**状态谓词**（id 还在不在），不是**返回值**

> **来源：codex 对抗性评审 R5 high finding。问题成立且是真矛盾；**⚠️**机制上我不采纳它的 result-enum 处方，理由见下。**

**codex 指出的真矛盾**（我跨轮累积出来的，逐条核对属实）：D54 clause 5 写「`updateDrawingStyle` / `deleteDrawing(id:)` 返 `false` → 清空选中」，但后续几轮又引入了三处**必须保留选中**的失败：

| 失败 | 来源 | 上游/本 spec 要求的选中行为 |
|---|---|---|
| `locked` 拒绝 | D60 | **保留**——上游 §7.2 明写「锁定线仍可被选中（否则无法解锁）」 |
| 样式语义不成立（`withStyle` 返 nil） | D59 | **保留**——线还在，只是这个改动不合法 |
| 几何不可见时删除被拒 | D63 / N16b | **保留**——D63 的全部价值就在「不抖掉选中」 |
...
### D65 选中对象的**所有**写入都绑同一个可用性谓词（不给"改样式"开特例）

> **来源：codex 对抗性评审 R5 medium finding。采纳——它推翻的是我在 D63 里给自己开的一个特例。**

D63 里我写了「样式编辑**不禁**（改一条当前看不见的线的颜色是无害且非破坏性的）」。codex 的反驳成立：**那仍然是一次不可逆的内容变更**（按 D56 立刻 autosave，而 undo 要到 1b-ii），只是"看不见后果"而已。用户可以把线平移出屏、失去高亮、然后改掉一个自己无法检视的对象。
...
改样式可用 ⟺ locked == false
            ...
            且 **该线的未来数据「样式改得到」**（🔄2026-07-26 二次修订，codex R18-F2）：
                 ...
                 `DrawingStyleAvailability.userCoverableFutureKeys`
                 = `{lineSubType, lineStyle, thickness, colorToken, labelMode, isExtended}`
                 ⚠️ **不含 `textColorToken`**（codex R19-F2）：本构建**没有字色控件**（独立字色属 P3），
                    它只会被 D59 派生② **隐式**跟随线色改写 → 算作"可覆盖"就等于允许
                    「用户改线色 → 顺手抹掉一条他看不见、也没碰过的高版本字色」。
...
>   - 未来值落在**用户改不到的位置**（`textColorToken` / `anchors[].period` / `tailAnchor.period`）或存在**未来顶层字段**
...
>   ④ 未来 `textColorToken` 线 → 样式控件**灰**（本构建无字色控件，改线色只会**隐式**抹掉它）、🗑 **亮**。

- **样式控件**按「改样式可用」置灰；**🗑** 按「删除可用」置灰。🔄**2026-07-26 修订（终稿，codex R19-F1 消歧）**：二者共享 `locked` + `visibleGeometry` 两个分量；**样式**另有第三个分量「未来数据**用户改得到吗**」，**删除没有**。即：`改样式可用 = locked + 几何 + 可覆盖性`，`删除可用 = locked + 几何`。可覆盖 → 控件可点（换掉那个值即完成修复，引擎按「看结果」最终裁决）；不可覆盖（未来顶层字段 / `anchors[].period` / `tailAnchor.period` / `textColorToken`）→ **样式控件灰、🗑 亮**，靠删除解封。
...
- **清空选中的全部情形**（每次清空后 🗑 回灰）：
  1. 退出画线模式；
  2. 从选择态切回画线态（点亮工具图标）；
  3. **结构性不可见**：`visibleDrawings(for: selectedPanel)` 不再含 `selectedDrawingID`（切周期、或该线改由另一面板显示）——判据**按 `selectedPanel`**，哪怕同 id 在**另一个**面板可见也照样清空（**几何性**不可见**不**清空，见 D63）；
  4. **`selectedDrawingID` 不在 `engine.drawings` 中**——涵盖「被删除」与「被任何其它路径移除」。
     ⚠️ **这是一个状态谓词，不是"某个 API 返回了 false"**（D64 取代了本条的原始写法；返 `false` 的失败里有三类**必须保留**选中）。
...
- **N17 失败原因 × 选中生命期（D64，codex R5-F1 专项，不可省）**：**逐个失败原因**各一条，断言选中态——
  | 失败 | 触发方式 | `selectedDrawingID` |
  |---|---|---|
  ...
  **并加一条实现形状守卫**：断言清空逻辑**不读**任何 API 的返回值（源码守卫：清空分支里不得出现 `== false` / `!result` 之类的判据），只读 `drawings` 里 id 的存在性——防止实现"顺手"退回按返回值判断、把上表重新写错。

- **N18 不可见 / 锁定时样式控件与 🗑 同进同退（D65，codex R5-F2 专项）**：
...
| 18c | 选中一条线（🗑 亮），然后**左右平移图表**，直到那条线的价位滑出可见范围（线看不见了） | 线看不见时 **🗑 和整个样式面板的控件都变灰、点不动**（看不见的东西不许动）；但**选中没丢**、面板**仍显示着那条线的颜色/线型/粗细**（看得见、改不动）——把图**平移回来**，线重新出现且**仍是高亮选中**的，🗑 与控件**全部自动恢复**又能改能删了（D63 / D65） | |
