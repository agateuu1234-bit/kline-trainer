...
2. **UI 反馈路径可达且必须接**：返 `false` 有**四种**原因（见下表），**它们的选中处置各不相同** —— 故 UI **不得**按返回值决定选中生命期，一律走 **D64 的状态谓词**。

   | 返 `false` 的原因 | 引入决策 | 选中 |
   |---|---|---|
   | `id` 不在 `drawings` 中（线已不存在） | D50 | 清空（谓词自动成立） |
   | 目标 `locked == true` | D60 | **保留**（上游 §7.2：不能选就没法解锁） |
   | `withStyle` 返 nil（样式语义不成立） | D59 | **保留** |
   | ——（UI 预检拒绝时引擎**根本不被调用**，无返回值） | D58 / D65 | **保留** |
...
- **N17 失败原因 × 选中生命期（D64，codex R5-F1 专项，不可省）**：**逐个失败原因**各一条，断言选中态——
  ...
  | UI 预检拒绝 | 锚点在右缘外时改 `.ray` | **原样保留**（引擎根本没被调用） |
  | 删除成功 | 正常删 | 被清空、🗑 回灰 |
