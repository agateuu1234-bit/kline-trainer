...
startTick(forStartDatetime d: Int64, in allCandles) -> Int
  ...
  - 实现 = `min(m3.partitioningIndex { $0.datetime >= d }, m3.count-1)`；空 m3 → 0（make 已先验非空）。
  - degenerate（d 超过所有 m3，损坏数据）→ `partitioningIndex` 返 `count`，`min` 钳到 `count-1 = maxTick`（保 `startTick ∈ 0...maxTick` + 不变量；valid data 不触发）。**不是返 0**——返 0 会违反下方不变量。
```
