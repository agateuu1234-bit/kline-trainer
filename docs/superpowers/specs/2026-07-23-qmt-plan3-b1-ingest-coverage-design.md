...
**（c）daily 全历史严格校验门**（codex R13-F2 high + R15-F2 medium，均**核实为真**）：拷贝完整性门比对的是 `clean` **之前**的 raw 行数、D10 只对账 dense 1m 跨度——**深历史（dense 跨度之外，可能 20 年前）的坏 daily 行两处都够不到**，而 `resample_calendar` 会拿它合成周/月线、`monthly` before-context 取全历史 → 导入报成功却发出**深历史损坏**的训练组。两类坏行都要在导入期拒：
...
- **`clean` 不查、却仍污染聚合的列**（R15-F2）：现有 `clean` **不校验 `amount`、不管 `volume` 正负**（`volume` 在 required 列里、NaN 会被丢，但**负 volume / inf amount 不丢**）。一行 `amount=inf` 或 `volume=-5` 能过 `clean`（len 不变、过上一条）却让周/月线的 Σamount/Σvolume 变垃圾。故对**全历史每行** daily 再断言：`amount` 有限非空、`volume` 有限整数且 `>= 0`，否则 `QmtIngestRejected("daily_bad_amount_or_volume")`。

下游 `resample_calendar` 用的就是这份已验的 daily。**仅 daily 需要此门**：1m 坏行会让当日 ≠ 241 → 落入非-dense 被整日 drop、且 1m 只近一年全在 D10 守区内。
...
- **P3-D9(c) daily 全历史校验门（R13-F2 + R15-F2）**：所有构造行都落在 **dense 1m 跨度之外**、且 raw 行数/端点仍与 export_log 相符（这样拷贝完整性门 + D10 都看不见）→ ① 深历史一行 OHLC 为 0/NaN → 抛 `daily_clean_dropped_rows`；② 重复 datetime → 同样拒；③ 深历史一行 `amount=inf`、④ 一行 `volume=-5`、⑤ 一行 `volume=NaN` → 各抛 `daily_bad_amount_or_volume`（`clean` 不丢负 volume/inf amount，故 len 门放行、必须另有此门）；全净 daily → 放行。mutation：分别去掉 len 相等断言 / amount-volume 断言 → 对应测各自必挂。**这三条必须直接调纯函数 `build_stock_import` 来测**（codex R3-F3：门若只存在于 CLI 里，非 CLI 调用方就绕过去了）；另加一条「不传 entry 就 `TypeError`」的签名测，钉住「没有默认放行的后门」。
