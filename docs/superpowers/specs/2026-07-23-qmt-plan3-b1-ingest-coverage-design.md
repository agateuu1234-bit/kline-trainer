...
> | P3-D9 | (a) 出货可行性预检（复用 B2 `build_training_windows` 全门）+ (b) 拷贝完整性门 | R1-F4 / R2-F3 / R8-F1 / R9-F1 |
> | P3-D10 | ① 作废/版本化 → 拆独立 plan；② 已出货股重导入 fail-closed 互锁（留本 plan） | R2-F2→R5-F1 连提 5 轮；user 两次裁决 |
> | P3-D11 | 通用 CSV 路径 fail-closed | R2-F1 |
> | P3-D12 | 写入器 fail-closed 校验 bundle 形状（period-match + coverage 自洽 + datetime 去重，破坏性 DELETE 前）；B1 锁改非阻塞 | R7-F2 / R7-F3 / R9-F2 / R10-F1 |
...
> **清洗顺序内的位置**：出货可行性预检要**六周期 bar DataFrame** + `month_boundaries`/`dense_dates`/`trading_dates`，故排在 `build_intraday`+`resample_calendar`+`daily`（六周期都成形、dense 覆盖与月边界都算出）**之后**、`to_kline_records` 之前。`build_training_windows` 只按 `datetime` 做窗口切分与 per-day 桶数硬门、不读指标列，故可在 `compute_indicators` 之前跑（省一遍指标计算于必被拒的股）。拷贝完整性门要原始行数，仍在 `clean` 之前。
...
- 每个周期的记录列表**非空**；
- **每条记录的 `period` == 它所在的那个 key，且 ∈ `PERIODS`**（codex R9-F2，high，**核实为真**：`write_qmt_stock` 的 INSERT 用的是 `record["period"]`，而 `klines` 表**没有 period CHECK 约束**——一份把 key 写对、却在记录里塞 `period="wtf"` 的坏 bundle 能过前两条校验、DELETE 掉六个规范周期、再按**错周期/未知周期**塞回去还返回成功。这是破坏性替换里最阴的一条，必须显式挡）；
- 每条记录的 `stock_code` == 传入的 `stock_code`（防串股）；
- **每条记录含必需字段**（`_INT_COLS` + 价格四列非空）——防半成品记录 INSERT 进库；
- **每周期内 `datetime` 无重复**（codex R10-F1，**核实为真**）：INSERT 用 `ON CONFLICT (stock_code,period,datetime) DO UPDATE`，bundle 里若有重复 datetime 会**静默塌成 last-wins**、实际入库行数少于预期，而 coverage 仍按原计数登记 → 行数与 count 悄悄失配。去重前先拒。
- **`coverage` 与 `daily` 记录完全自洽**（codex R10-F1，high，**核实为真**——推翻我 R9 的「不可实现」误判）：我 R9 说 `CoverageArtifact` 没有 dense 日期列表、故 count 查不了，就把一致性 punt 给了提交**之后**的 B2。这是错的两处：① bundle **带着** `records["daily"]`（每条含 `datetime`），写入器完全能从中重建交易日集；② punt 给 B2 = 破坏性 DELETE 已经把旧数据删了、坏 coverage 已经提交，B2 读时才 skip —— **违背「零写入 fail-closed」，还能把一只本来能用的股换成静默产 0**。故改为**在取连接前、从 bundle 自身重建并全查**：
  ...
  assert all(start_date <= d <= end_date for d in dropped_dates)    # dropped 落在区间内
  assert dense_day_count == len(in_span - set(dropped_dates))       # count 就是这么来的
  assert dense_day_count >= 1
  ```

  这套判据**只用 bundle 自己的字段**（daily 记录 + coverage artifact），破坏性 DELETE 之前 bundle 就**完全自洽**，不再 punt。**与 B2 那道 `len(dense_dates)!=dense_day_count` 守卫的关系**：B2 那道是用**已入库的真实 daily klines** 重建、防「写入后被别的路径改脏」，是纵深防御的第二层；本层防「坏 bundle 一开始就自相矛盾还去删旧数据」，是第一层。两层判据同构、但守的是不同时点，都保留。
...
- **P3-D12 bundle 校验**（纯函数直测）：缺一个周期 / 多一个周期 / 某周期空列表 / **某记录 `period` 与 key 不符** / **某记录 `period` 不在 `PERIODS`** / 某记录 `stock_code` 串股 / 某记录缺必需字段 / **某周期有重复 datetime** / coverage `start>end` / **`dense_day_count` 与从 daily 记录重建的 dense 天数不符** / **端点不在 daily 日期集内** / **某 dropped 日落在 [start,end] 外** / `dropped_dates` 非日期列表 → 各一条 `InvalidImportBundleError`；六周期齐全且 coverage 与 daily 记录完全自洽 → 通过。**两条重点断言**：① `period` 不符 → 若无此校验，DELETE 六周期后会按错 period INSERT；② `dense_day_count` 不符 → 若无此校验，坏 coverage 会**提交后**才被 B2 发现、旧数据已毁（mutation：分别去掉这两条断言 → 对应测各自必挂）。另测 `write_qmt_stock` 收到任一坏 bundle → **连 DB 都没连**（校验在取连接前）、零写入。
