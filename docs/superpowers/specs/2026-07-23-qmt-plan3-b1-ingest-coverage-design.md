...
7. **重导入作废该股一切未交付产物**（P3-D10）——修正性重导入后，旧产物不再被 B3 发出去（含正被 lease 持有的）。
...
> | P3-D10 | 重导入作废该股一切未交付产物 | R2-F2（推翻 R1 的不可变政策）/ R3-F1·F2 再收紧 |
...
```
pg_advisory_lock(IMPORT_GEN_LOCK_KEY, stock_key)          ← P3-D8，session 级，BEGIN 之前
BEGIN
...
| 角色 | 取法 | 范围 |
|---|---|---|
| B1 `write_qmt_stock` | `pg_advisory_lock(IMPORT_GEN_LOCK_KEY, stock_key)`（**阻塞式、session 级**） | **在 BEGIN 之前取，直到提交后的文件 unlink 全部做完才在 `finally` 里释放**（见下方 R3-F1）。CLI 连接随即关闭，session 锁必然随连接释放，不会泄漏 |
...
### P3-D10：重导入作废该股**一切未交付的产物**（R1 那条「不可变政策」已被推翻）
...
**处置**：`write_qmt_stock` 成功导入时，在**同一事务内**删除该股**一切尚未交付**的 `training_sets` 行：

```sql
DELETE FROM training_sets
 WHERE stock_code = $1 AND status <> 'sent'
 RETURNING id, file_path
```

**判据 = 「未交付」，不是「当前可领取」**（codex R3-F2 收敛，见下）。事务提交**之后**再 best-effort `unlink` 那些 `file_path`；顺序不能反（先删文件后回滚 = 行还在但文件没了，B3 永久 404）。反过来留下的孤儿 zip 无害——没有任何行引用它，与既有代码里「写完 zip 崩在登记前」的自愈论证同构。

**R3-F1（high，核实为真）：unlink 必须在按股锁内做。** 我上一版让 B1 用**事务级** advisory lock，它在 COMMIT 时就释放了；而 zip 路径是**完全确定**的 `output_dir/{stock_code}_{start_datetime}.zip`（`backend/generate_training_sets.py:364`，已核实）。于是这个序列成立：B1 删行 → COMMIT（锁释放）→ B2 抢到锁、发现该 start 又空出来了、**用新数据重新生成同名 zip 并登记** → B1 那句迟到的 `unlink` **把刚生成的新产物删掉** → 行在、文件没了 → B3 404 / `content_hash` 失配。故 B1 改用 **session 级锁，覆盖到 unlink 做完**（P3-D8 表已改）。**必须有一条并发测钉住这个序列**（导入提交 → 立刻同 start 重生成 → 新文件仍在）。

**R3-F2（high，核实为真）：活跃 lease 不能只是「警告」。** 我上一版保留未过期的 `reserved` 行、把它记成窄残留。codex 指出这不是窄残留：`rollback_expired`（`lease_repo.py:177-179`）会在一个 lease TTL 之后把它翻回 `unsent`，于是**旧导入的产物重新进入库存并被发出去**——恰好抵消了作废库存的全部意义。**我同意，改为一并删除**。

于是判据从「可领取集合」简化成「未交付集合」（`status <> 'sent'`），残留清零。**代价与理由**：

| 类别 | 处置 | 理由 |
|---|---|---|
| `unsent` / `reserved`（无论是否过期） | **删行 + 删 zip** | 尚未交付到任何设备。作废它们是「不发已知坏数据」的全部意义所在 |
| `sent` | **不动** | ① zip 已在用户设备上，追不回也不该追；② 删行会释放 `uq_stock_start`，让 B2 用同一 start 再生成一份 —— 同一用户拿到两份同起点训练组，破坏既有幂等契约 |

**被接受的代价**：重导入会打断该股**进行中的一次下载**（B3 对那个 lease 返回 404）。这是**一次性、可自愈**的（客户端重取即得新产物），而保留它换来的是**长期**把已知坏数据发出去。两害相权取前者。**这条代价要写进 PR body 与验收清单**，不藏着。
...
- 新 `write_qmt_stock(dsn, stock_code, stock_name, bundle) -> WriteResult`（P3-D4 替换语义 + P3-D8 按股 **session** 锁 + P3-D10 库存作废）；返回每周期行数 + 被作废的 `training_sets` id/路径 + 其中原本处于 `reserved` 的 id（这些是被打断的进行中下载，必须显式回报）。
- CLI 新增 `--qmt` 分支（P3-D6），成功时打印：每周期行数 / dense 带 / 日线首末日期 + 月边界数（P3-D9 可观测性）/ 作废了几个库存 / **其中有几个正被 lease 持有（会 404 一次）**。
...
- **P3-D10 库存作废**：造 4 行（`unsent` / 过期 `reserved` / 未过期 `reserved` / `sent`）→ 导入后**前三行全被删、只有 `sent` 保留**；被删行的 `file_path` 在**提交之后**才 unlink（断言顺序，不是只断言结果）；模拟事务回滚 → **一个文件都没被删**。
- **P3-D10 / R3-F1 并发序列测**：导入提交后、`unlink` 之前，模拟 B2 用同一 start 重新生成同名 zip → 断言**新文件在 unlink 之后依然存在**（等价于断言按股锁覆盖到了 unlink；把锁改成事务级 → 此测必挂）。
...
6. **session 锁跨越 COMMIT（R3-F1 的地基）**：连接 A 用 **session** 级 `pg_advisory_lock(K, s1)` 后开事务、提交 → 提交**之后**连接 B 对 `s1` 仍拿不到（证明 session 锁不随 COMMIT 释放，B1 的 post-commit unlink 确实在锁内）；A 显式 unlock 后 B 才拿得到。**同时反向对照**：连接 A 改用 `pg_advisory_xact_lock` 时，提交后 B **立刻拿得到**（证明这两种锁的差别真实存在，而不是脚本写法碰巧都通过）。
...
每一条新增的门测试必须做 mutation：把被测的门改坏 → 对应测试**必须挂**。至少覆盖这几处：`date_set_mismatch` 判据改成恒 True、事务隔离级别改回默认、`readonly` 去掉、coverage 写入注释掉、**`DELETE` 去掉（替换退化成 UPSERT）**、**按股锁改成恒返回 True**、`_MIN_MONTH_BOUNDARIES` 改小、**`export_log_mismatch` 门改成恒放行**、**库存作废谓词改回只删可领取集合**、**通用路径护栏去掉**、**B1 按股锁改回事务级（unlink 掉出锁外）**、**`entry_1m/entry_daily` 给上默认值**。由控制者本人复验，不采信 subagent 自证。
...
8. **重导入后该股一切未交付产物清零**（`unsent` + `reserved` 全删、只留 `sent`）；且**导入提交后立刻用同一 start 重生成的新 zip 不会被迟到的 unlink 删掉**（R3-F1 并发序列测）。「会打断一次进行中的下载」这条代价写进 PR body 与验收清单。
...
| B1 用 **session** 级锁 → 若进程在 unlink 阶段被 kill，锁靠连接断开释放 | PG 在连接终止时释放该 session 的全部 advisory lock，无需清理逻辑。真被 kill 时留下的是「行已删、文件还在」的孤儿 zip —— 无害（无行引用），与既有自愈论证同构 |
| 重导入会打断该股**进行中的一次下载**（B3 对该 lease 404） | 已接受的代价（P3-D10）：一次性、客户端重取即愈；换掉的是「长期把已知坏数据发出去」。CLI 会明确打印被打断了几个 |
| `status='sent'` 的产物仍停留在旧基准 | 已交付到设备、追不回；且删行会释放 `uq_stock_start` 导致同一用户拿到两份同起点训练组。明确不做 |
