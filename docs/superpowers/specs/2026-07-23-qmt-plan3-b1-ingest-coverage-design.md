...
> | P3-D3 | `export_log` status 门必填（+ 重复行拒 + entry 身份绑定） | 初稿 / R8-F3 / R12-F1 |
...
- **entry 身份绑定**（codex R12-F1，high，**核实为真**）：`build_stock_import` 在**任何 status / 拷贝完整性检查之前**先断言 `entry_1m.code == stock_code and entry_1m.period == "1m"`、`entry_daily.code == stock_code and entry_daily.period == "daily"`，否则抛 `QmtIngestRejected("export_log_entry_mismatch")`。否则非-CLI 调用方能塞一条**别的股**的 `ok` 条目（行数/端点碰巧相同），让真文件的 `status=error` 从没被查过——这是 export_log 信任边界上和 R9-F2/R11-F2 同模式（身份参数 vs 数据身份未交叉校验）、但在 `build_stock_import` **输入侧**的漏洞。`ExportLogEntry` 现在带 `code`/`period`（解析器本就按 `(code,period)` 分组、身份现成），绑定零成本。
...
- `build_stock_import(df_1m, df_daily, *, stock_code, stock_name, entry_1m, entry_daily) -> ImportBundle`（P3-D1/D2；两个 entry **必填无默认**，且入口先断言 entry 身份 == (stock_code, 周期)，R12-F1）
...
- 内部顺序：**entry 身份绑定（R12-F1）** → **拷贝完整性门（P3-D9(b) `export_log_mismatch`，clean 前，比对 raw 行数）** → `clean(df_1m)` / `clean(df_daily)` → **daily 无损门（P3-D9(c) `daily_clean_dropped_rows`，clean(daily) 后立刻）** → `reconcile_sources`（含 status 门）→ `build_intraday` → `resample_calendar` ×2 → **出货可行性预检（P3-D9(a) `no_eligible_training_window`）** → 逐周期 `compute_indicators` → `to_kline_records`
...
- **entry 身份绑定（R12-F1）**：给 `build_stock_import` 传一条**别的股**的 `ok` 条目、其 `rows`/`first_time`/`last_time` 与本股 df **碰巧全等**（这样拷贝完整性门也过）→ 仍抛 `QmtIngestRejected("export_log_entry_mismatch")`；把 1m 的 entry 与 daily 的 entry 传反（period 错位）→ 同样抛（mutation：去掉身份断言 → 别股 ok 条目蒙混过 status 门、此测必挂）。
