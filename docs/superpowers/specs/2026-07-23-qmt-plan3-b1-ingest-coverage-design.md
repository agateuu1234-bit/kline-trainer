...
> | P3-D1+ | 四方身份绑定（`QmtSource` 携带 code/period，入口一次校验）——收口身份家族 | R9-F2 / R11-F2 / R12-F1 / R14-F1 |
> | P3-D4 | B1 写入 = 单事务**整体替换** | R1-F2 后改写 |
> | P3-D5 | B2 RR 只读快照读 | 初稿 |
> | P3-D6 | CLI 形态 | 初稿 |
> | P3-D7 | `volume` 不换算 | 初稿 |
> | P3-D8 | 按股导入/生成互斥锁（+ rollout drain 部署要求） | R1-F1 / R8-F2 / R13-F1 |
> | P3-D9 | (a) 出货可行性预检（复用 B2 `build_training_windows` 全门）+ (b) 拷贝完整性门 + (c) daily 无损门 | R1-F4 / R2-F3 / R8-F1 / R9-F1 / R13-F2 |
...
**身份家族根因收口（R14-F1）**：连续 4 轮 high（R9-F2 记录 stock_code、R11-F2 通用路径记录 code、R12-F1 export_log 条目、R14-F1 DataFrame 本身）全是同一病——**身份作为独立参数漂在数据旁边、不与数据里的身份交叉校验**。此前每轮只绑一处。根治 = **身份在解析时绑定到数据、端到端携带**：`parse_qmt_csv` 不再返回裸 df，而是 `QmtSource(code=parse_qmt_filename(路径).code, period, df)`。`build_stock_import` **在任何 status / 拷贝完整性 / D10 检查之前**断言四方一致：

```python
assert src_1m.code == stock_code and src_1m.period == "1m"
assert src_daily.code == stock_code and src_daily.period == "daily"
assert entry_1m.code == stock_code and entry_1m.period == "1m"
assert entry_daily.code == stock_code and entry_daily.period == "daily"
# 否则 QmtIngestRejected("source_identity_mismatch")
```

否则非-CLI 调用方能把 A 股的 1m+daily 帧配上 B 股的 `stock_code` + B 股（行数/端点碰巧全等）的 export_log 条目——D10 只把两个供入帧**相互**对账（A 的 1m vs A 的 daily 自洽），于是 A 的 OHLCV 写到 B 名下、B2 发出贴错标签的 zip。CLI 的文件名 glob 只护 CLI 路径、护不住 pilot 脚本与其它直接调用方。身份一旦随数据携带，这一整类「张冠李戴」在纯 API 入口一次挡死。
...
- **处置 = 部署纪律，非永久代码 fence**：Plan 3 的 B1 与 B2 改动**同批部署**（同一批 PR），且**首次 QMT 导入前，必须重启/drain 调度器**，确保没有 Plan-3-前的 B2/B4 进程在跑。这写进 PR body 与非-coder 验收清单。
- **为什么不加永久全局锁 fence**：让 B1 额外常取 `B2_GENERATION_LOCK_KEY` 能挡旧 B2，但会让 B1 导入与**整个** sweep 永久串行（正是 P3-D8 否掉方案 C 的代价），为一个**部署瞬态**永久牺牲按股粒度，不划算。
- **当前实际风险 = 零**：本系统单用户内网、**调度器/NAS 后端至今未部署**（无任何在跑的 B2/B4 守护进程），偏斜窗口现在就是空的；上面的部署纪律覆盖将来真部署时的窗口。稳态（Plan 3 全部署后）两端都取新锁，不变量成立。
...
**（c）daily `clean` 后严格无损**（codex R13-F2，high，**核实为真**）：拷贝完整性门比对的是 `clean` **之前**的 raw 行数，而 `clean` 会丢 NaN/非正价/`high<low`/非有限 行、并去重。一条落在 **dense 1m 跨度之外**（深历史，可能 20 年前）的坏 daily 行能满足 export_log 行数/端点、过拷贝完整性门，然后被 `clean` 悄悄丢掉——**D10 只对账 dense 跨度、够不到深历史**，而 `resample_calendar` 会拿这条已缺口的日线合成周/月线，`monthly` before-context 又取全历史 → 导入报成功却发出**深历史损坏**的训练组。QMT 前复权数据本应已 clean（flat-fill、无缺口），故任何丢弃 = 损坏。处置：**对 daily**，断言 `clean` 是无损的——`len(clean(df_daily)) == len(df_daily)` 且 `df_daily["datetime"]` 无重复，否则 `QmtIngestRejected("daily_clean_dropped_rows")`；且下游 `resample_calendar` 用的就是这份已验无损的 daily。**仅 daily 需要此门**：1m 的坏行会让当日 ≠ 241 → 落入非-dense 被 `build_intraday` 整日 drop、且 1m 只近一年全在 D10 守区内，已有覆盖。
...
- **`coverage` 与 `daily` 记录自洽**（R10-F1，从 bundle 自身重建、破坏性 DELETE 前查；F1b 修正见下）：
  ...
  assert start_date <= end_date and dense_day_count >= 1
  assert start_date in daily_dates and end_date in daily_dates    # 端点必须真有 daily 行
  assert all(isinstance(d, date) for d in dropped_dates)
  assert set(dropped_dates).isdisjoint(in_span)                   # ← F1b 修正
  assert dense_day_count == len(in_span - set(dropped_dates))     # count 就是这么来的
  ```
...
任一 `validate_import_bundle` 断言不满足 → `InvalidImportBundleError`，**零 DELETE / 零 INSERT**。纯函数、host 单测、先于取锁与 DELETE。
...
- 内部顺序：**四方身份一致断言（R14-F1：`src_1m`/`src_daily`/`entry_1m`/`entry_daily` 的 code+period 全 == (stock_code, 周期)，否则 `source_identity_mismatch`）** → **拷贝完整性门（P3-D9(b) `export_log_mismatch`，clean 前，比对 raw 行数）** → `clean(src_1m.df)` / `clean(src_daily.df)` → **daily 无损门（P3-D9(c) `daily_clean_dropped_rows`，clean(daily) 后立刻）** → `reconcile_sources`（含 status 门）→ `build_intraday` → `resample_calendar` ×2 → **出货可行性预检（P3-D9(a) `no_eligible_training_window`）** → 逐周期 `compute_indicators` → `to_kline_records`
...
- **四方身份一致（R12-F1 + R14-F1）**：给 `build_stock_import` 传 **A 股的 `src_1m`/`src_daily`（df 身份=A）+ B 股的 `stock_code` + B 股的 `entry`**，且行数/端点碰巧全等 → 抛 `QmtIngestRejected("source_identity_mismatch")`（防张冠李戴写错标签）；单把 export_log entry 换成别股 `ok`（df 身份对、entry 错）→ 同样抛；1m/daily 的 src 或 entry 传反（period 错位）→ 抛（mutation：去掉四方断言任一 → 对应张冠李戴/错文件能溜过、此测必挂）。
...
- **P3-D9(a) 出货可行性预检**：构造多类都能过 D10 四门、都写得成 coverage 的输入 → ① 月边界只有 38 → 抛 `no_eligible_training_window`；② 日线 ≥ 39 月**但 dense 1m 只覆盖短短几个月** → 抛（R8-F1：候选过滤级）；③ **候选够、但选中窗口有一天盘中桶数不足**（per-day 硬门失败，无任何候选能过 `build_training_windows`）→ **同样**抛（R9-F1：这一条正是 `eligible_start_indices` 漏掉、只有跑 `build_training_windows` 全门才抓得到）；④ 全部够 → 放行。**断言 B1 预检调的就是 `generate_training_sets.build_training_windows` 本身**（mutation：把该函数的 per-day 门或 before_min 改严 → B1 预检行为跟着变、此测必挂，证明没有第二套判据在漂移）。
...
- **P3-D9(c) daily 无损门（R13-F2）**：构造一份 daily，深历史（dense 1m 跨度之外）有一行 OHLC 为 0/NaN、其 raw 行数/端点仍与 export_log 相符 → `clean` 会丢它 → 抛 `daily_clean_dropped_rows`（若无此门，它过拷贝完整性门 + D10 只守 dense span 看不见 → 周/月线用缺口历史合成、训练组深历史损坏却报成功）；重复 datetime 同样拒；无损 daily → 放行。mutation：去掉 len 相等断言 → 此测必挂。**这三条必须直接调纯函数 `build_stock_import` 来测**（codex R3-F3：门若只存在于 CLI 里，非 CLI 调用方就绕过去了）；另加一条「不传 entry 就 `TypeError`」的签名测，钉住「没有默认放行的后门」。
...
- **P3-D12 bundle 校验**（纯函数直测）：缺一个周期 / 多一个周期 / 某周期空列表 / **某记录 `period` 与 key 不符** / **某记录 `period` 不在 `PERIODS`** / **某记录 `stock_code` ≠ 参数（去重集 ≠ `{arg}`）** / 某记录缺必需字段 / **某周期有重复 datetime** / coverage `start>end` / **`dense_day_count` 与从 daily 记录重建的 dense 天数不符** / **端点不在 daily 日期集内** / `dropped_dates` 非日期列表 → 各一条 `InvalidImportBundleError`；六周期齐全且 coverage 与 daily 记录自洽 → 通过。**F1b 正向测**：`dropped` 落在 `[start,end]` **之外**（合法边界 partial 日，`compute_dense_coverage` 真会产出）→ **必须放行**（防 R11 那条反向断言复活误拒）。**两条重点断言**：① `period` 不符 → 若无此校验，DELETE 六周期后会按错 period INSERT；② `dense_day_count` 不符 → 坏 coverage 提交后才被 B2 发现、旧数据已毁（mutation：分别去掉 → 对应测各自必挂）。另测 `write_qmt_stock` 收到任一坏 bundle → **连 DB 都没连**、零写入。
...
11. **部署纪律（P3-D8 / R13-F1）**：Plan 3 的 B1+B2 同批部署；**首次 QMT 导入前重启/drain 调度器**，确保无 Plan-3-前的 B2/B4 进程在跑。写进 PR body 与非-coder 验收清单。当前无在跑守护进程、实际偏斜窗口为零。
