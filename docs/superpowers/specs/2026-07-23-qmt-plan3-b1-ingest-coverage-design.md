...
6. **日线历史长度门**（P3-D9）——把「导入看似成功、B2 静默不出货」的失败面提前到导入期。
7. 上述各项的 host pytest 覆盖 + 两条一次性**真 PG** 脚本（语义验证 + 真链路端到端）+ CI 内的逻辑链集成测。
...
**训练组不可变政策（回应 codex R1-F2 后半「invalidate or version existing training_sets」）**：训练组 zip 是**生成时刻冻结的自包含产物**，App 端只播放 zip、从不回查数据库。因此——

- **已登记的训练组不会因为后续重导入而被自动作废或改写**。这是**刻意的产品决策**，不是遗漏：每个 zip 内部只含单一复权基准，回放它得到的是一张自洽的图；它只是「按更早那次导出的基准」而已，对练习用途无损。
- 若某次重导入是**修正**（源数据本身错了），作废旧训练组是**运维动作**（删行 + 删 zip + 重新 sweep），不在 Plan 3 范围。
- P3-D4 的替换语义保证的是**数据库当前状态**自洽（不会新旧基准混在同一张表里），P3-D8 的锁保证的是**新产出的训练组**来自单一且当前的导入。两者合起来，坏数据无法进入**新**产物；**旧**产物按上面的政策保持不变。
...
- 内部顺序：`clean` → `reconcile_sources`（含 status 门）→ **日线历史长度门（P3-D9）** → `build_intraday` → `resample_calendar` ×2 → 逐周期 `compute_indicators` → `to_kline_records`
...
- 新 `write_qmt_stock(dsn, stock_code, stock_name, bundle) -> dict[period, int]`（P3-D4 替换语义 + P3-D8 按股 xact 锁）。
- CLI 新增 `--qmt` 分支（P3-D6）。
...
每一条新增的门测试必须做 mutation：把被测的门改坏 → 对应测试**必须挂**。至少覆盖这几处：`date_set_mismatch` 判据改成恒 True、事务隔离级别改回默认、`readonly` 去掉、coverage 写入注释掉、**`DELETE` 去掉（替换退化成 UPSERT）**、**按股锁改成恒返回 True**、`_MIN_MONTH_BOUNDARIES` 改小。由控制者本人复验，不采信 subagent 自证。
...
8. 后端 pytest 全绿、零 skip；PR CI 全绿。
...
| PR | 内容 | 子项 |
|---|---|---|
| **3a** | `qmt_ingest.py` 纯装配层（含 P3-D9 日线历史门）+ `test_qmt_ingest.py` | 2 |
| **3b** | B2：RR 只读事务 + 按股锁（`stock_lock_key`/`IMPORT_GEN_LOCK_KEY`）+ 假 conn 断言测 + `verify_repeatable_read_snapshot.py`（5 条语义断言） | 3 |
| **3c** | B1 写库壳（替换语义 + 按股 xact 锁 + coverage UPSERT + `stock_coverage` 存在断言）+ 假 conn 单测（含替换回归）+ CLI `--qmt` | 3 |
| **3d** | L1 端到端集成测（fixture 生成器）+ L2 `verify_qmt_pg_chain.py` 真链路脚本 | 2 |

**顺序理由**：3b 先于 3c/3d，是为了让后面的端到端跑在最终读路径上，不必写完再改；L1/L2 两条端到端合并成独立的 3d，避免 3c 塞成 4 子项。**3d 的 PR body 必须贴 L2 脚本的完整输出**（§0.2 的 L1+L2 双绿口径）。
