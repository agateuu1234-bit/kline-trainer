...
- 新 `write_qmt_stock(dsn, stock_code, stock_name, bundle) -> WriteResult`（P3-D4 替换语义 + P3-D8 按股 **xact** 锁 + P3-D10 重导入互锁）；返回每周期写入行数。**只读 `training_sets` 一条 `SELECT 1` 作互锁、不删不改、不 unlink zip**；该股已有训练组 → `ReimportBlockedError` 零写入。
...
- `write_qmt_stock`：断言**所有写语句都发生在同一个 transaction 上下文内**；断言顺序 = 按股 xact 锁 → schema 守卫 → **互锁 `SELECT 1`** → `DELETE` → `INSERT`；守卫失败 → 零 DELETE 零 INSERT（**尤其要证明守卫失败时不会先把旧数据删掉**）。
...
每一条新增的门测试必须做 mutation：把被测的门改坏 → 对应测试**必须挂**。至少覆盖这几处：`date_set_mismatch` 判据改成恒 True、事务隔离级别改回默认、`readonly` 去掉、coverage 写入注释掉、**`DELETE` 去掉（替换退化成 UPSERT）**、**按股锁改成恒返回 True**、`_MIN_MONTH_BOUNDARIES` 改小、**`export_log_mismatch` 门改成恒放行**、**通用路径护栏去掉**、**`entry_1m/entry_daily` 给上默认值**、**P3-D10 互锁 `SELECT 1` 去掉（重导入不再被拦）**、**互锁排到 DELETE 之后（先删才发现该拒）**。由控制者本人复验，不采信 subagent 自证。
...
| **L2 真-PG 非 CI 自动门**，靠控制者真跑 + 贴输出 | 已接受（F2，user 裁决）：把真-PG 进 CI（带 postgres service）是独立 CI 治理改动、需单独审 workflow。PR body 写明「CI 绿 ≠ 链路已证，L2 输出见下」 |
...
| **3c** | B1 写库壳（替换语义 + 按股 xact 锁 + coverage UPSERT + 存在断言 + P3-D10 重导入互锁）+ P3-D11 通用路径护栏 + 假 conn 单测 + CLI `--qmt` | 3 |
| **3d** | L1 端到端集成测（fixture 生成器）+ L2 `verify_qmt_pg_chain.py` 真链路脚本 | 2 |

**顺序理由**：3b 先于 3c，让后面的端到端跑在最终读路径上，不必写完再改。两道 fail-closed（P3-D10 互锁 + P3-D11 护栏）都在 `import_csv.py` 写库壳、与 `write_qmt_stock` 天然同批，一并进 3c，PR 数收在 4。**3d 的 PR body 必须**：① 贴 L2 脚本完整输出；② 写明「CI 绿 ≠ 链路已证，凭 L2 输出」（F2 口径）；③ 写明「重导入不作废旧训练组、改以互锁禁止已出货股重导入，作废/版本化拆独立 plan」（P3-D10 口径）。
