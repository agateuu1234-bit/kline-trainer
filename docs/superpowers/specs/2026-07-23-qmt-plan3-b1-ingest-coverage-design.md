...
**另拆独立 plan（非本 plan、非 Plan 4）**：**重导入后旧训练组的作废/版本化**（P3-D10）。需 import 版本号或 `retired` 状态 + B3 `/download`+`/confirm` 语义变更，是产物生命周期的独立改动。本 plan 只登记「旧 zip 不作废」这一已知局限，不设计。
...
> | P3-D10 | 重导入后旧训练组作废 → **拆出 Plan 3**（user 裁决） | R2-F2 提出、R3/R4 反复暴露闭合代价过大 |
...
```
BEGIN
  pg_advisory_xact_lock(IMPORT_GEN_LOCK_KEY, stock_key)   ← P3-D8，事务级
...
**本事务不触碰 `training_sets`、不 unlink 任何 zip**（P3-D10 拆出去了）。
...
| 角色 | 取法 | 范围 |
|---|---|---|
| B1 `write_qmt_stock` | `pg_advisory_xact_lock(IMPORT_GEN_LOCK_KEY, stock_key)`（**阻塞式、事务级**） | 事务开头取，COMMIT/ROLLBACK 自动释放，不泄漏。**事务级足够**：P3-D10 拆出去后 B1 没有任何「提交后的文件操作」需要锁去覆盖，锁只需活到写入提交为止 |
...
于是「B2 的快照 → 建 zip → 登记」这整段期间，该股**不可能**有导入提交（B1 想导入就得先拿这把锁，而 B2 正持着）；RR 保一致、这把锁保新鲜，两者合起来才是完整不变量。B1 的锁只需活到它自己的写入提交——它的作用是「B2 快照时看不到写到一半的导入」，事务级恰好覆盖这段。
...
### P3-D10：重导入后旧训练组的作废——**明确移出 Plan 3，拆独立 plan**（user 2026-07-23 裁决）

**问题是真的**（codex R1-F2 后半 → R2-F2 → R3-F2 → R4-F3，连提 4 轮，每轮都核实为真）：一次**修正性**重导入之后，用旧（错误）数据生成的 zip 仍会被 B3 发给用户，且库里没有字段能区分「当前版本」与「被取代」的产物。R4-F3 更进一步、经核实成立：B3 的 `/download`（`routes.py:56`）**按 id 发文件、不做 status 迁移**，`/confirm`（`routes.py:71`）才迁 `sent`——所以「`reserved` = 未交付」这个前提**是错的**，客户端可能已把旧 zip 下到手，此时删行只会让它的 `/confirm` 撞 404，而坏数据已在设备上。

**为什么拆出去、而不是在本 plan 内闭合**：可靠闭合需要给 `stock_coverage`/`training_sets` 引入 **import 版本号（或 `invalidated`/`retired` 状态）** 并改 **B3 的 `/download`+`/confirm` 语义只发当前版本**——这是 A 类 DDL（migration 0005 + `CONTRACT_VERSION` 再 bump + openapi/iOS 波及）+ 一次 B3 发放契约变更，等于把 Plan 3 从「接通 B1」撑成「接通 B1 + 重做产物生命周期」。半吊子的「同事务删 unsent/reserved 行 + 提交后 unlink」引入的新问题（R3-F1 竞态删新 zip、R4-F2 unlink 信任任意 `file_path`、R4-F3 已下载未 confirm）**比它解决的还多**。

**Plan 3 的实际立场（诚实义务，写进 PR body 与验收清单）**：

- Plan 3 **不作废**任何已存在的 `training_sets` 行，`write_qmt_stock` **不删 `training_sets`、不 unlink 任何 zip**。
- **这是一个已知的、被有意推迟的局限，不是本 plan 造的回归**——「重导入后旧 zip 仍可被领取」这个状态 **Plan 3 之前就存在**（B2 一直在产 zip、从来没有作废机制）。
- P3-D4 的替换语义保证的是**数据库 `klines` 当前状态**自洽（不会新旧复权基准混在同一张表里）；P3-D8 的锁保证**新产出**的训练组来自单一且当前的导入。**坏数据无法进入新产物**；**旧产物**的作废是独立那个 plan 的事。
- 独立 plan 的触发时机 = 真正开始用真数据重导入、且旧库存已积累时（Plan 4 pilot 之后）。这里**只登记，不设计**。

**连带效果**：因为不再有「提交后 unlink」这一步，codex R3-F1（迟到的 unlink 删掉重生成的新 zip）与 R4-F2（unlink 信任任意 `file_path`）**随之消失**；R4-F1（B1 锁范围矛盾）也一并消解——B1 回到事务级锁（见 P3-D8）。
...
1. 取同一把按股锁——这里用**事务级** `pg_advisory_xact_lock(IMPORT_GEN_LOCK_KEY, stock_lock_key(code))` 即可（与 `write_qmt_stock` 的 session 级不同：通用路径**没有提交后的文件操作**，锁不需要活过 COMMIT；事务级还免了泄漏风险）。两者在同一把锁上正常互斥；
...
- `parse_export_log(path) -> dict[(code, period), ExportLogEntry]` + `@dataclass ExportLogEntry`（P3-D3）
...
- 新 `write_qmt_stock(dsn, stock_code, stock_name, bundle) -> WriteResult`（P3-D4 替换语义 + P3-D8 按股 **xact** 锁）；返回每周期写入行数。**不触碰 `training_sets`、不 unlink zip**（P3-D10 拆出去了）。
- CLI 新增 `--qmt` 分支（P3-D6），成功时打印：每周期行数 / dense 带 / 日线首末日期 + 月边界数（P3-D9 可观测性）。
...
- **P3-D9 拷贝完整性门**：`rows` 少一行 / `first_time` 对不上 / `last_time` 对不上 → 各一条 `export_log_mismatch`；`first_time` 格式解析不出 → 报错而非放行。**这三条必须直接调纯函数 `build_stock_import` 来测**（codex R3-F3：门若只存在于 CLI 里，非 CLI 调用方就绕过去了）；另加一条「不传 entry 就 `TypeError`」的签名测，钉住「没有默认放行的后门」。
...
6. **B1 事务级锁足够**：连接 A 用 `pg_advisory_xact_lock(K, s1)` 后开事务写入、提交 → 提交**之后**连接 B 对 `s1` 立刻拿得到（证明 B1 的锁如期随 COMMIT 释放、不多占）；而 A **持锁未提交**期间 B 对 `s1` 拿不到（证明 B2 快照读期间看不到写到一半的导入）。B2 的 session 级锁另由 §5.2 假件测顺序 + 第 5 条互斥语义共同覆盖。
...
docker postgres:15.12 → 应用 schema.sql
  ...
  → 断言：重导前先造一行 unsent training_set → 重导后该行**仍在、其 zip 仍在**（P3-D10 已拆出，Plan 3 不作废旧产物）
...
每一条新增的门测试必须做 mutation：把被测的门改坏 → 对应测试**必须挂**。至少覆盖这几处：`date_set_mismatch` 判据改成恒 True、事务隔离级别改回默认、`readonly` 去掉、coverage 写入注释掉、**`DELETE` 去掉（替换退化成 UPSERT）**、**按股锁改成恒返回 True**、`_MIN_MONTH_BOUNDARIES` 改小、**`export_log_mismatch` 门改成恒放行**、**通用路径护栏去掉**、**`entry_1m/entry_daily` 给上默认值**。由控制者本人复验，不采信 subagent 自证。
...
8. **通用 CSV 路径对已被 QMT 管理的股 fail-closed**；对未被管理的股行为逐字不变。
9. 日线：**只宣称**「月边界 ≥ 39 + 拷贝完整性（export_log 端点/行数）已强制」，**不宣称**「已验证全历史」。
10. **明确不做**：重导入不作废任何已存在的 `training_sets`（P3-D10 拆出 Plan 3）。PR body 写明「旧 zip 仍可被领取」是已知的、被有意推迟的局限，且 Plan 3 之前就存在。
...
| B1 的按股锁是**阻塞式**：B2 正在为该股生成时，导入会等 | 这是期望行为（互斥）。B2 单股的持锁时长 = 快照读 + 建 zip，秒级。若将来发现卡顿，加 `lock_timeout` 是独立的小改动，本 plan 不预设 |
| **重导入后旧训练组仍可被 B3 领取**（用旧/错数据生成的 zip） | **已知局限，有意推迟到独立 plan**（P3-D10）：可靠闭合需 import 版本号/`retired` 状态 + B3 `/download`+`/confirm` 语义变更。且此状态 **Plan 3 之前就存在**，非本 plan 造成的回归。PR body 如实写明，不假装闭合 |
...
| **3c** | B1 写库壳（替换语义 + 按股 xact 锁 + coverage UPSERT + 存在断言）+ P3-D11 通用路径护栏 + 假 conn 单测 + CLI `--qmt` | 3 |
| **3d** | L1 端到端集成测（fixture 生成器）+ L2 `verify_qmt_pg_chain.py` 真链路脚本 | 2 |

**顺序理由**：3b 先于 3c，让后面的端到端跑在最终读路径上，不必写完再改。P3-D10 拆出去后，通用路径护栏（P3-D11）并回 3c（都在 `import_csv.py` 写库壳、天然同批），PR 数从 5 收回 4。**3d 的 PR body 必须贴 L2 脚本的完整输出**（§0.2 的 L1+L2 双绿口径），并写明「重导入不作废旧训练组」这条已知局限（P3-D10 已拆出）。
