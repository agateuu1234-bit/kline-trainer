...
> | P3-D12 | 写入器 fail-closed 校验 bundle 形状（含 period-match，破坏性 DELETE 前）；B1 锁改非阻塞 | R7-F2 / R7-F3 / R9-F2 |
...
- `coverage` 自洽（**只断言 `CoverageArtifact` 实际带的字段**，codex R9-F2 后半，**核实为真**：我上一版写 `dense_day_count == len(去重后的 dense 日期)`，但 `CoverageArtifact = (start_date, end_date, dropped_dates, dense_day_count)` **根本没有 dense 日期列表**，这条不可实现）：`start_date <= end_date`、`dense_day_count >= 1`（0 天必不可能出货，且 P3-D9 预检已保证 ≥1）、`dropped_dates` 是 `date` 列表。**`dense_day_count` 与真实 dense 天数的一致性另有更强的守卫**——B2 `generate_one_training_set` 已有 `if len(dense_dates) != int(dense_day_count): raise`（`generate_training_sets.py:541`，用**真实 daily klines** 重建 dense 日历交叉校验），比在写入器里凭空比对更可靠，故这里不重复、不假装能查。
...
- **P3-D12 bundle 校验**（纯函数直测）：缺一个周期 / 多一个周期 / 某周期空列表 / **某记录 `period` 与 key 不符** / **某记录 `period` 不在 `PERIODS`** / 某记录 `stock_code` 串股 / 某记录缺必需字段 / coverage `start>end` / `dense_day_count == 0` / `dropped_dates` 非日期列表 → 各一条 `InvalidImportBundleError`；六周期齐全且自洽 → 通过。**重点**：`period` 不符那条要断言**若无此校验，DELETE 六周期后会按错 period INSERT**（mutation：去掉 period-match 断言 → 此测必挂）。另测 `write_qmt_stock` 收到坏 bundle → **连 DB 都没连**（校验在取连接前）、零写入。
