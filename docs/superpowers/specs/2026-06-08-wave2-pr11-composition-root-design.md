...
public enum Modal: Identifiable {             // 互斥 sheet；只需 Identifiable（.sheet(item:) 用），不需 Equatable
    ...
    public var id: String {
        ...
        case .history(let r):      return "history-\(r.id)"
        case .settlement(let r):   return "settlement-\(r.id)"
        }
    }
}
@MainActor 状态：
...
| 情形 | 判据 | 行为 |
|---|---|---|
| Normal 正常结束 | `recordId != nil` | router 先 `let rec = try? recordRepo.loadRecordBundle(id: recordId).0`（[fix] 异步预取，弃 ops/drawings；nil→errorMessage+teardown 回 home）→ `activeModal = .settlement(rec)` → SettlementView → confirm → `confirmSettlement()`（含 `endAfterSettlement`）→ 回 home。**注**：此分支 reader 在结算窗期间保持开（正常，confirm 时才 endSession） |
