...
```
offsetBounds(mainFrameWidth: CGFloat, rawVisible: Int, candleCount: Int, currentIdx: Int)
  -> OffsetBounds   // struct { minOffset, maxOffset, candleStep; var bounceEdges: Set<Edge> }
```
**返回 struct（codex R3 branch-diff）**：`OffsetBounds` 含 `minOffset/maxOffset/candleStep`（同旧）+ `bounceEdges`——
**类型级编码 reveal 单边 bounce 非对称**：`bounceEdges == [.max]`（有滚动空间，仅最老边可弹）或 `[]`（无空间），**永不含 `.min`**（最新边硬钳=当前 tick）。R1b-wire 读 `bounceEdges` 单边化 `EdgeBounceModel`，杜绝把最新边当对称弹簧端点（否则负速 fling → 前向揭示）。
