# Wave 3 outline（v2）—— 客户端端到端功能完成 wave 单线顺位
...
**CI baseline（codex R1-F2 校正）**：现有 `.github/workflows/catalyst-build.yml` 仅构建 `ios/Contracts` SwiftPM scheme（business/UI 逻辑包）。**`KlineTrainer.xcodeproj`（app target 外壳：`KlineTrainerApp`/`AppRootView`/`AppContainer`）当前无 CI 编译守护**（PR11-R3 residual）。Wave 3 顺位 2 即补此守护并前置，避免后续实施锚在 app target 编译不过的情况下 merge（见 §三.2）。
...
- **依赖拓扑驱动顺位**：治理前置（1 RFC 钉死契约 + 2 app-target CI 守护早置）→ 手势打磨（3-5，依赖 C5/C6/C7/C8 已落）→ 交易补全（6-8，依赖 RFC + 扩 E5/E6）→ 磨光（9-12）→ 收尾（13）。
- **契约前置**：所有 spec gap（tier 公式 / 结束总资金显示语义 / 夜间调色板 / E5·E6 扩展边界 / replay 结算契约）集中顺位 1 RFC 钉死，实施锚不得现编未治理公共面（沿用 Wave 2 顺位 1 + E2 RFC 先例）。
- **CI 守护前置**：app target 编译守护（顺位 2）必须早于任何改 app target 的实施锚，并对顺位 3-12 强制 required check（codex R1-F2；对齐 Wave 1 1a/1c CI 前置先例）。
...
| 2 | **app-target CI 守护**（`KlineTrainer.xcodeproj` build-for-testing 加入 CI〔trust-boundary workflow〕+ 设为 required check + 对顺位 3-12 实施 PR 强制） | A 治理 + infra | CI workflow 改动；0 业务代码 | — | PR11-R3（前置，codex R1-F2） |
| 3 | **Pinch 缩放**（C8a 去硬编码 `visibleCount=80`/`candleWidthRatio=0.7` + 两指 pinch 手势与 C7 仲裁器集成〔pinch vs 两指周期切换的仲裁归 plan〕 + clamp 边界 + 缩放后视口还原） | B 手势 | ~250-350 行 | 2 | C8a 视口硬编码 |
| 4 | **水平线绘线 MVP**（Phase 2.5：drawing 模式 pan 截获 → `DrawingAnchor{period,candleIndex,price}` → commit → 跨缩放/平移还原） | B 手势 | ~250-350 行 | 2 + 3（跨缩放还原验收需缩放在场） | U2-R2（仅水平线；6 种完整工具属 Phase 4，排除） |
| 5 | **十字光标吸附 + HUD**（snap 到最近蜡烛 + 价格/时间 label 显示） | B 手势 | ~200-300 行 | 2 | Phase 2.5 收口 |
| 6 | **E5/E6 交易扩展**（手动强平方法 + tier 公式 accessor，严格按顺位 1 RFC） | C 交易 | ~250-350 行 | 1 RFC + 2 | U2-R1 / U2-R3（逻辑面） |
| 7 | **U2 交易 UI 接线**（手动强平按钮 + 顶栏「仓位 X/5」+ 结束总资金显示语义，按 RFC） | C 交易 | ~250-350 行 SwiftUI | 6 + 1 RFC | U2-R1 / U2-R3（UI 面）/ E6b-R1 |
| 8 | **Replay 结算窗**（replay 结束触发忠实结算窗，按 RFC 契约；触碰 E5/E6/SettlementView 扩展边界） | C 交易 | ~250-350 行 | 6 + 7 + 1 RFC | PR11-R2 |
| 9 | **夜间模式**（白天/夜间/跟随系统 + 暗色调色板，按 RFC + F2 ThemeController 基础设施） | D 磨光 | ~250-350 行 | 1 RFC | Phase 5 显示模式 |
...
| 11 | **边缘 bounce 动画**（DecelerationAnimator 扩展：到边界回弹） | D 磨光 | ~200-300 行 | — | Phase 5 |
...
| 13 | **Wave 3 收尾**（completion doc + residual 终态回填 + **运行时验收完成作阻塞依赖** + **freeze tag 决策**） | E 收尾 | doc-only（freeze 走则 + tag ceremony） | 全部 + **运行时验收记录**（见 §三.3） | C2/C7/C8 运行时实测回填（codex R1-F3） |

**Phase 划分**：
- A 治理前置（1：spec-gap RFC；2：app-target CI 守护早置）
- B 图表手势/交互打磨（3-5：Pinch 缩放 / 水平线绘线 MVP / 十字光标 HUD）
- C 交易功能补全（6-8：E5·E6 扩展 / U2 交易 UI / Replay 结算窗）
- D 磨光（9-12：夜间模式 / 边界+错误+生产路径 smoke / 边缘 bounce / 性能）
- E 收尾（13：completion doc + 运行时验收阻塞 + freeze tag 决策）

**依赖满足校验**（每锚上游均在更早顺位 merged 或 Wave 0/1/2 已完成）：1 RFC(无依赖) → 2 CI 守护(无依赖；早于所有 app-target 改动) → 3 Pinch(需 2；C7/C8 Wave 2 已落) → 4 绘线(需 2+3；C6 Wave 1 已落) → 5 十字光标(需 2；C5 Wave 1 已落) → 6 E5/E6 扩展(需 1+2) → 7 U2 交易 UI(需 6+1) → 8 Replay 结算(需 6+7+1) → 9 夜间(需 1；F2 Wave 0 已落) → 10 边界+smoke(各模块已落) → 11 bounce(需 2；C2 Wave 1 已落) → 12 性能(全渲染已在场) → 13 收尾(需全部 + 运行时验收记录)。无逆向依赖。
...
- 0 业务代码改动（仅 spec/governance 文档）。顺位 6/7/8/9 据此契约实施，不自行定义公共面。
...
### 3.2 anchor 拆分 / CI 前置 / 排除项理由

- **app-target CI 守护前置（顺位 2，codex R1-F2）**：现有 Catalyst job 仅构建 `ios/Contracts` SwiftPM 包；`KlineTrainer.xcodeproj` app target 无 CI 编译守护（PR11-R3）。若拖到末位，顺位 3-12 任一改 app target 的锚可在 app 编译不过时 merge，集成失败累积到末期才暴露（违 `feedback_swift_local_toolchain_blindspot`「本地绿≠CI绿」）。故前置为顺位 2 并对后续实施锚强制 required check（对齐 Wave 1 1a/1c CI 前置先例）。**trust-boundary**：改 `.github/workflows`，强制 codex review（per `.claude/workflow-rules.json`）。
- **手势三锚（3/4/5）拆点**：Pinch 缩放（视口几何变换）、水平线绘线（drawing 输入状态机 + 坐标存储）、十字光标 HUD（只读叠加层）三者职责正交，各自独立可测；绘线（4）依赖缩放（3）仅为「跨缩放还原」验收点，非编译依赖。
...
- **磨光四锚（9-12）拆点**：夜间模式（主题切换）、边界+错误处理+生产路径 smoke（跨模块健壮性）、边缘 bounce（动画扩展）、性能评审（条件性 Bitmap Cache）四者无强耦合，按风险/领域分锚。Bitmap Cache 为**条件性**引入（仅当 Instruments 实测单帧 >4ms），plan 阶段若实测达标则该子项 no-op，仅交付性能评审 artifact。
- **排除项**（§六明列）：Phase 4 完整 6 种画线工具、NAS 部署/样本数据/生产 endpoint、iPad 横屏。

### 3.3 Wave 3 收尾：运行时验收阻塞 + 完成 claim 诚实 + freeze tag 决策

**运行时验收作收尾阻塞依赖（codex R1-F3）**：Wave 3 为「端到端可玩」wave，**不得**在 C2/C7/C8 运行时无任何证据时关闭。顺位 13 收尾 + 任何 freeze tag **阻塞依赖** = 两份运行时 runbook（`c8b-runtime-acceptance` 减速/帧预算 + `u2-gesture-runtime-acceptance` 手势）的 **device/simulator 实测结果已记录** + Instruments 帧预算实测数值已回填。运行时验收是 user device 职责，但其**完成**是 Wave 3 关闭的硬前提，非「某天再说」。

**完成 claim 诚实（codex R1-F1）**：Wave 3 收尾 doc **不得** claim「可上架商店」。Wave 3 交付 = 客户端 feature 完整 + 端到端 fixture 验证（含顺位 10 生产路径 fixture E2E smoke）+ 运行时验收记录。**真实上架剩余门**（§六）= NAS track 的生产 backendBaseURL（PR11-R1）+ 真实样本数据（W1-R2）——收尾 doc 须显式列此二门为「未完成的 ship 前提」，不计入 Wave 3 完成度。

**freeze tag 决策延至顺位 13**：Wave 3 是客户端功能完成 wave。若届时打 `wave3-frozen` tag，其语义 = **冻结客户端功能完整性**（非 store-ship readiness），且须先满足上述运行时验收阻塞 + 诚实 claim。**是否打 tag 延至顺位 13 收尾时正式定**（视产品完成度 + 是否需冻结点判断）。outline 仅占位顺位 13，不预设打/不打 tag。
...
| U2-R2 画线工具面板（DrawingInputController，水平线） | `project_wave2_completion` §三 | 顺位 4 水平线绘线 MVP（Phase 2.5）；6 种完整工具排除 | 4 |
...
| C2/C7/C8 运行时实测（减速/手势/帧预算） | Wave 2 §二 runbook（user 手动 pending） | 顺位 12 回填判据 + 顺位 13 收尾**阻塞**依赖（记录的 device/sim 结果） | 12 + 13 |

---

## 五、CI / 评审通道说明

- **iOS PR Catalyst CI 强制**：顺位 3-12 均触 `Mac Catalyst build-for-testing on macos-15` required check（Wave 1 1a/1c 已建 always-trigger workflow + required gate）。本地 swift test 绿不等于 CI 绿（per `feedback_swift_local_toolchain_blindspot`）。Catalyst required check 仅验证 build-for-testing（编译 + 链接），**不执行运行时**——顺位 3/5/11 的手势/动画运行时行为须另行 runbook 验收（沿用 Wave 2 §二模式），其完成是顺位 13 阻塞依赖。
- **顺位 2 新增 app target CI 守护**：现有 Catalyst job 构建 SwiftPM 包；顺位 2 补 app target（`KlineTrainer.xcodeproj`）的 CI 构建守护（PR11-R3），并对顺位 3-12 设为 required check（codex R1-F2）。
...
| 2026-06-09 | v2 (codex branch-diff R1 修) | **F1**（high）：「可上架」claim 与排除生产路径（PR11-R1 endpoint + W1-R2 数据）自相矛盾 → 重述 wave 标题/目标为「客户端端到端功能完成」（非 store-ready）+ 顶部加完成 claim 边界块 + §三.3 诚实 claim + §六明列二门为「未完成 ship 门」+ 顺位 10 加生产路径 fixture E2E smoke；**F2**（high）：app-target CI 守护从末位（旧顺位 11 子项）提为**前置独立顺位 2** + 对顺位 3-12 强制 required check + §〇 CI baseline 块 + §三.2 理由；**F3**（high）：运行时验收（C2/C7/C8 device/sim runbook 记录 + Instruments 实测）设为顺位 13 收尾 + freeze tag 的**阻塞依赖**（§三.3）；anchor 12→13，全表/依赖图/Phase/residual 映射同步重排 |
