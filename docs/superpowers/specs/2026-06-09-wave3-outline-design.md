# Wave 3 outline（v6）—— 客户端端到端功能完成 wave 单线顺位
...
7. **finalize 失败保留 session 契约（codex R5-F2）**：现状 `TrainingView.swift:118-119` finalize 任何失败 → `onSessionEnded(nil)` → AppRouter 关 reader + 清 `activeTraining` = 已完成局拆毁丢失（pending 可能 stale/absent，"retry" 不可达）。RFC 须定义 finalize 失败行为：**保留 active session 提供 retry/discard**（不 teardown）或 insert 前 durable checkpoint 精确终态；+ 原子（insert+clear 单事务）+ 幂等（retry 仅记一次 record）。实施落顺位 10。
...
| finalize 失败丢局/重复（`Coordinator:230-231` 分离事务 + `TrainingView:118-119` 失败→onSessionEnded(nil) teardown） | codex R4-F1 → R5-F2 | 顺位 1 RFC 定失败保留契约 + 顺位 10 owns 失败保留 session（retain+retry/discard 不 teardown）+ 原子单事务 + 幂等 retry 仅记一次 + 故障注入 | 1 + 10 |
