...
- **【HIGH residual · codex R4，user 2026-06-16 接受 + attest-override】`.m3` datetime 单调性未在数据边界校验**：合成用 `partitioningIndex{datetime>=…}` 定位 start，依赖 `.m3` datetime 严格递增。reader/engine 仅校验 index/endGlobalIndex 连续 + OHLC 有效、**不**校验 datetime 单调。良性数据 pipeline 保证有序（`import_csv` 按 datetime 排序、`ticket_index` 由 datetime 序派生）故正确；损坏但可加载数据下 start 可能错 → 合成出**有界**错根（成分恒 ⊆ 已揭示 m3：**不崩、不泄漏未来、Y 轴不越界**，仅 OHLC 形态在已揭示区间内可能错=GIGO）。与现有代码对排序轴的信任同类（`currentCandleIndex`/`MarkersLayout` 亦 per-render binary-search on `endGlobalIndex` 不重校验）。**真修 = reader/persistence 边界加「`.m3` datetime 严格递增 + 聚合 open 落其 endGlobalIndex 窗口」校验**（M0.x trust-boundary，独立后续 RFC，超本渲染 RFC 作用域 D5）。本 PR 经 user attest-override 接受。
...
| 2026-06-16 | v1.6 (codex R4 [HIGH] → user 接受 residual + override) | R4 = R1/R2 datetime-单调主题复述：非单调 `.m3` datetime 下 `partitioningIndex` 返错 start → 合成有界错根（不崩/不泄未来/Y 轴不越界，成分 ⊆ 已揭示=GIGO）。真修（reader-boundary datetime 单调校验）属 persistence trust-boundary、超本渲染 RFC（D5）；替代修（弃 partitioningIndex）会回退 R1-H1 clamped-predecessor 正确性。良性数据 pipeline 保证有序故正确；与现有代码对排序轴的 per-render 信任同类。**user 2026-06-16 裁决：接受为 §六 HIGH residual（真修归 persistence 后续 RFC）+ attest-override 合入**（codex 维持 needs-attention，非自我裁定）。 |
