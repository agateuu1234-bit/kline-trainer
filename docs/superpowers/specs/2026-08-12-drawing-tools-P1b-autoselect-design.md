...
### 6.8 D89　发布闸：PR-1 **不单独构成本片的交付**，但**允许先合** —— 依据是「零增量」实测

> **来源：codex spec-R6 high（提出）+ spec-R7 high（窄化重提，指出我上一稿只有散文闸）。**
> R7 的批评有一半成立：上一稿**断言**了「PR-1 可以先合是安全的」，却**没把论据写进来**。现在补上。

#### 6.8.1 论据：PR-1 对「续训丢默认」这条路径的增量是 **0**（已实测）

| 核查项 | 实测结论 |
|---|---|
| main 上画线态有没有选中 | **恒无**（D54 原文：画线态选中恒为空） |
| ⇒ main 上画线态改样式走哪条路 | `TrainingView` 的 `if selectedDrawingID != nil` 分流**恒走 else 支** = `applyDefaultStyleMutation` = `setDefaultStyle` |
| PR-1 后画线态面板**显示**什么 | 仍然是 `session.defaultStyle`（D86 的 `.draw` 行） |
| PR-1 新增了几次 `setDefaultStyle` 调用 | **0** |
| `defaultStyle` 在 `KlineTrainerPersistence/` 的出现次数 | **0**（PR-1 前后都是 0） |

⇒ 「改默认 → 返回 → 续训 → 回落出厂值」这条路径，**在 PR-1 前后的可达性、触发频率、后果完全相同**。
PR-1 **不制造、不加重、也不让它更容易被触发**。它是 main 上的既有缺口，由 PR-2 修。

#### 6.8.2 为什么**不**采纳 codex 的两个处方

| 处方 | 不采纳的理由 |
|---|---|
| 「PR-2 合入前不许合 PR-1」 | 对一个**零增量**改动施加合并闸，安全收益为 0；代价是 PR-1 要挂着等 PR-2 走完 brainstorming → spec → 评审 → plan 全流程，而 PR-1 的评审面（交互语义）与 PR-2（SQL / 迁移 / clean-skip）完全不交叠。这正是 [[feedback_big_pr_codex_noncovergence]] 要避免的形状 |
| 「PR-1 藏在 feature flag 后面」 | **会增加风险而不是降低**：① 引入一条 ship 出去但不执行的死路径，与本仓 **D19 / D24「不 ship 未接线控件」**直接冲突；② 制造一个**没有任何闸门覆盖的构建配置**（host / Catalyst / iOS 三门都只跑一种配置）；③ flag 本身的清理是后续负债。为一个零增量改动付这三样，不成比例 |

#### 6.8.3 真正的闸（用本仓**已有**的发布机制，不是散文）

- **PR-1 可以合进 `main`。**
- **本片在 PR-2 合入前不算交付完成**，具体到可核查的动作：
  1. **不进任何 freeze tag**（本仓的发布单位就是 freeze tag，见进度记录里 Wave3 的先例）；
  2. **不向用户宣布该功能可用**、不出「已完成」结论；
  3. **进度 memory 里该条目必须标「交付未完成，等 PR-2」**，而不是标 ✅；
  4. PR-1 的 **PR 描述顶部**必须带同样的横幅，且**列明未完成的那一条是什么**（本局默认不跨续训，D87）。
- **验收表里不设任何一条把「续训后回落出厂橙」写成正常的项**（原 #23 已删除）。

#### 6.8.4 与 R7 仍存在的分歧（如实记录，不假装收敛）

codex R7 要求「PR-1 不得进入可发布 / 默认启用的构建」。按 §6.8.1 的实测，**PR-1 进不进构建，对这条丢失路径没有任何影响**——
main 今天就是可发布的，且今天就有这条路径。故本 spec **不采纳**该要求，代之以 §6.8.3 的发布闸。
**这是一处判断分歧（remedy 是否成比例），不是事实分歧**：R7 陈述的事实（存在丢失路径、D89 原稿只有散文）我全部接受。
...
纪律沉淀（第七条）：**凡是在 spec 里写「X 是安全的 / 可以先做」，必须当场附上可核查的论据**（本例 = 「PR-1 新增 `setDefaultStyle` 调用 0 次」这类实测数字）。只写结论不写论据，评审只能反复要它 —— 而每一轮重提**看起来都像没收敛**，实际是我一直没交出证据。
