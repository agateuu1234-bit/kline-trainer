...
```
持久化 PR  ──强制──▶  自动选中 PR  ──强制──▶  1b-ii 撤销 PR
   (D89 §6.8)              (§10.1：撤销必须成对回滚「线 + 默认」)
```
**三段全部强制，整条链不可乱序。**

- **「持久化 PR → 自动选中 PR」是强制的**（D89 / §6.8）：自动选中 PR 会新增一条落盘的 `applyStyle` 写入，
  若本局默认尚未持久化，就会产生「线已落盘、默认没落盘」的半持久化坏状态。
- **「自动选中 PR → 1b-ii 撤销 PR」也是强制的**（codex spec-R12 high 纠正了我原先「互不依赖、次序可换」的说法）：
  D86 让画线态的一次改样式**同时写线和默认**，撤销必须把这一对当一个动作**成对回滚**（§10.1）。
  若撤销 PR 先落地，它只会按「单处写入」建栈；等自动选中 PR 再落地，undo 就变成**只回滚线、不回滚默认**
  ⇒ 被撤销掉的样式会在下一笔新画的线上、以及续训之后复活。
...
**undo / redo 恢复回来的线不适用本决策**，一律不自动选中（1b-ii spec `:408` 原样成立）。理由：那条线不是用户此刻「画」出来的，它可能落在屏幕外、可能属于另一个面板，把选中硬塞给它反而重新制造 D37 的陈旧选中。**1b-ii 撤销 PR** 实施时**不得**复用 `commitPendingAndSelect`。
...
- ⚠️ **但这条事实推不出「自动选中 PR 可以先合」**（codex R8 → R9 连续指出，**已实测证实**）：D86 给画线态**新增**了一条
  ...
  **故：自动选中 PR 不得先于持久化 PR 合入**（D89 / §6.8）。本节**不再**为「自动选中 PR 先合」提供任何依据。

### 6.7 D88　持久化 PR **另开一份 spec**，不在本文件内设计
...
理由不是回避评审，恰恰相反——R6 的 critical 说的就是「持久化方案不完整」，
...
**交给持久化 PR spec 的既得事实（已实测，不必重查）**：上表三条 + clean-skip 那条 + 下列两条硬约束：
...
### 6.8 D89　**次序倒过来：持久化 PR 先，自动选中 PR 后**
...
（画线态恒无选中 ⇒ 恒走该分支）。⇒ **持久化 PR 不依赖自动选中 PR，今天就能单独实施**，它持久化的是一个**既有**的值。
...
**因此不需要** codex 提的两个处方：不需要合并闸（倒序后持久化 PR 单独合入即净收益），
...
- undo 把一条线删掉时，若它正被选中 → 由 D54 clause 4（存在性谓词）自动清空，**1b-ii 撤销 PR 不需要**为此新增判据。
- ⚠️ **画线态的一次改样式是「两处写入」，撤销必须成对回滚**（codex spec-R12 high，**已核实为真**）：
  D86 让画线态的一次改样式同时写了 **①那条线** 和 **②本局默认**。
  **撤销栈归 1b-ii 撤销 PR 所有**（不是持久化 PR —— 我上一稿在 R11 的机械改名里把这里错写成了持久化 PR）。
  **1b-ii 撤销 PR 必须**：把这一对当**一个**可撤销动作入栈，undo / redo **一并回滚 / 一并重做**，
  并配一条必测回归 —— 画线态改样式 → undo → 断言**线的样式与本局默认双双回到改动前**；redo → 断言双双回到改动后。
  **若只回滚线不回滚默认**，「已撤销」的样式会在下一笔新画的线上、以及续训之后复活。
...
**本 spec 在 codex R12 的 finding 修复后由用户 override 收口，未取得 `approve`。**

| | |
|---|---|
| **最后一轮** | R12 @ `2a1803b`，`needs-attention`，1 high，**已核实为真并全部修复** |
| **用户裁决** | 「R12 若不是 approve 就 override 收口」（2026-08-13） |
| **共计** | **12 轮，从未 approve**；R1–R12 的 finding **全部核实为真**（唯 R4 的**归因**经实测驳回，其事实照收） |

### override 的边界（**写死，不得扩张**）

**覆盖**：交付次序 / PR 命名 / 文档内部一致性 / 发布闸措辞 这一类 —— 即 R6、R7、R11、R12 那几轮的主题。
它们的共同点是：**改错了会让人做错事，但不会让代码改坏用户数据**。

**不覆盖（一条都不覆盖）**：

- **R9 的两处 base**（画线态必须把字段级改动分别套到默认与线自己，§6.3 #0）—— 它会**静默改掉用户锁定过的线并落盘**；
- **R8 的次序强制**（持久化 PR 必须先于自动选中 PR，D89）—— 它防的是**半持久化状态**；
- **R12 的撤销成对回滚**（§10.1）—— 它防的是**被撤销的样式复活**；
- **D82 / D83 / D84 的全部门与判据**，以及 §7 里**任何一条**变异 / 不变量锁 / 正向档。
...
| **R12** | 同分支 @ `2a1803b`（整支 branch-diff，零 focus 窄化） | `needs-attention` | **1 high**：§10.1 把「画线态改样式的 undo 语义」交给了**持久化 PR**，可**撤销栈不归它管**；且既然一次改样式同时写「线 + 默认」，撤销必须成对回滚 ⇒ 「自动选中 PR ↔ 撤销 PR 互不依赖、次序可换」的说法不成立 | **全采纳**，两点都属实。**ownership 写错是我 R11 机械改名的连带伤害**（原文是「1b-ii 的 PR-2」，裸编号被全局替换成了持久化 PR）。已改：§10.1 归还给 **1b-ii 撤销 PR**，并写死「两处写入必须当一个动作入栈、undo/redo 一并回滚一并重做」+ 必测回归（否则被撤销的样式会在下一笔与续训后复活）；§0.4 删掉「互不依赖、次序可换」，次序图改为**三段全强制** |
...
**R12 的形状 = R11 改名的连带伤害**：我做全局替换时保护了「1b-ii PR-2」这种带前缀的写法，却漏了**裸写的 `PR-2`（实指 1b-ii 撤销）**，于是把撤销栈的归属错记到持久化 PR 名下。
纪律沉淀（第十三条）：**大批量改名必须先按「每个旧名的全部书写形态」分类**（带前缀 / 裸写 / 引号内），逐类确认语义再替换；替换后**必须逐处复核语义**，不能只看替换计数。⚠️ 本轮与 R11 连着两次证明：**机械替换是缺陷源，不是省事手段**。
