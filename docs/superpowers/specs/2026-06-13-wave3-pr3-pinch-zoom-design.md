...
2. `docs/superpowers/specs/2026-06-09-wave3-outline-design.md` §二 表后加 **document-scoped supersession callout（R4-M1）**：顺位 3 行「消费 6 的 zoom/panel-state API」+ 全文所有「engine 契约集中顺位 6 / 不改 engine 契约 / serial neck」表述的 zoom 部分 superseded——**枚举命中（2026-06-13 sweep 实证 + R5-L1 补 L58）**：§二 表 **row 3（L58「消费 6 的 zoom/panel-state API，不自改 engine 契约」+ 依赖格「2 + 6（若需…归 6；纯 render 则仅 2）」条件分支已被裁决 resolve——zoom 落顺位 3）**、row 6（L61）、§三 DAG（L81「6 engine neck」/ L90「engine 写集中 6」）、§二 R8-F1 段（L107）、W1 波次（L111）、关键路径（L124）；版本历史 log 行（L246/L247）不 amend（历史保真）。枚举 = 导航，callout 的 document-scoped blanket = 权威覆盖。callout 写明对全文生效 + 枚举行号，不逐行插注。**neck 目的不破**：仅顺位 3〔轨 G〕改 panelState.visibleCount/zoom 契约，轨 T 不碰，无并发冲突——6b plan 已论证。
3. **modules `kline_trainer_modules_v1.4.md:1743` amendment（R1-H2 修正，原「零改动」声明为事实误判）**：机器锚原文「保持 focus（pinch 中点 candle x 不动，重算 offset）」是无条件的，与裁决 A 相左，须 amendment 为 freeScrolling-only + autoTracking 右锚（user 2026-06-13 裁决注记）。**约束**：amendment 必须完整保留顺位 1 fail-closed gate 的机器锚短语 `pinch/zoom panel-state mutation`（gate 谓词 `grep -cF` 锁该短语，acceptance 2026-06-10-wave3-pr1 #2b）。**gate 复跑范围（R2-M1 校正 + BR1-L1 精确化）**：只复跑**内容锚谓词 (a)/(d)/(e)**（验证 amendment 后机器锚与红线仍命中）；**(f) scope-allowlist 谓词**（allowlist 仅 7 个顺位 1 doc/脚本文件）对本实施分支合法改 Swift 文件**必 FAIL**——它是顺位 1 RFC PR 专属、**不纳入**本核验、不得以「全谓词 PASS」为门。**(g) immutability 谓词仍应 PASS**（本 PR 不触任何 2026-05 冻结 doc）；实跑权威预期 = **(a)-(e)+(g) PASS / 仅 (f) FAIL / exit=1**（与 acceptance #4 + Task 6.6 逐字一致）。
...
| modules amendment 碰 fail-closed gate | D9.3 约束：保留机器锚短语 `pinch/zoom panel-state mutation` + 改后复跑**内容谓词 (a)/(d)/(e)**；权威预期 (a)-(e)+(g) PASS / 仅 (f) FAIL（(f) scope 专属顺位 1 PR，不纳入；(g) PASS 因不触冻结 doc，BR1-L1 精确化对齐 D9.3） |
...
| 2026-06-13 | opus 4.8 xhigh 对抗评审 R5 | **VERDICT: APPROVE**（0C / 0H / 0M / 2L，均判定实质已覆盖、非阻塞）。R4 三条修复核验真实完整正确；评审独立重跑 neck + focus 两类四文件 sweep 与 D9 清单一致——document-scoped callout + blanket caveat **按构造终止**「逐轮漏一处」复发循环；D3 公式（含右缘向量、N′=N 恒等）/ D4 方向与归一 / D6 self-heal 与 guard / D1 drawing 吞没不 bump 全部独立重验无误。R5-L1（outline L58 row-3 枚举补列）已应用；R5-L2（RFC L150 adjacency 覆盖）无需动作。**设计收敛，进 writing-plans。** |
| 2026-06-13 | opus 4.8 xhigh whole-branch branch-diff 终审 BR1（实现后） | **VERDICT: APPROVE**（0C / 0H / 0M / 2L info，非阻塞）。评审亲跑 swift test 896/126 全绿 + Catalyst TEST BUILD SUCCEEDED + gate (a)-(e)+(g) PASS/仅(f) FAIL/exit=1；D3 focus 公式独立重导（wholeShift 非饱和精确抵消）+ 逐向量验算全中 + mutation 删 −1 项杀 8 处断言（端到端非空洞）；跨 commit 集成（pinchBase 归一×右锚 / focus×边缘饱和 / init seed 0→80 parity / reducer 穷尽 / 接线签名）全自洽；D1-D10+两裁决忠实无偏离；四 doc amendment 完整忠实 + 机器锚完好 + ephemeral 不变量（零 saveProgress/finalize/schema）+ 864 baseline 零回归。BR1-L1（D9.3/风险表 prose「(f)/(g) 必 FAIL」暗示 (g) 亦失败，实为仅 (f) FAIL/(g) PASS）→ **本轮已精确化**；BR1-L2（.began 非有限 scale 无专测，由 .changed 自愈 + .ended 清空逻辑完备覆盖）非阻塞不补。**分支收敛，可合并。** |
