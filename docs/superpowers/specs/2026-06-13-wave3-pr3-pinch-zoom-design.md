...
| RFC / outline / modules / plan 四 doc | D9 注记 + modules:1743 与 plan L1037/L1180 amendment（gate 机器锚短语保留 + 复跑内容谓词 (a)/(d)/(e)） |

---

## 四、测试策略（概要；矩阵细化归 plan）

1. **PinchZoomModel 纯函数**（host）：clamp 双端、round 单调、scale 非有限/≤0/0 附近防御、focus 公式手算向量（右缘 offset′=0 / 中点放大缩小双向 / offset≠0 起点 / N′=N 恒等 offset′=offset——**恒等向量必须用非饱和视口构造**：饱和态公式重导出 canonical offset′≠存量 offset 属预期，R2-L3）、**端到端不变量**：构造 candles + makeViewport 前后断言 `u(fx)` 偏差 < 1e-9（连续域主锚）且 fx 下 candle 索引不变（`xToIndex` 离散域副锚；**fx 必须取某 candle 中心、远离边界**——边界处前后 ±1 漂移假阴、中心恒等才有判别力，R1-L4）——按本仓 FP demonstrator 教训须 mutation-verify（故意给错公式确认测试能杀）。
