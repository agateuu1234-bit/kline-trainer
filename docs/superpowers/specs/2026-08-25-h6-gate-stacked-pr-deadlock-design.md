...
| **F26** | 本仓**从未使用过 auto-merge**：#166 / #170 / #172 / #175 / #177 / #178 的 `auto_merge` 字段**全部为 `null`**；ruleset `15660830` 的规则集为 `deletion, non_fast_forward, pull_request, required_status_checks`，**不含合并队列（merge queue）** | `gh api .../pulls/<n> --jq .auto_merge` ×6；`gh api .../rulesets/15660830 --jq '[.rules[].type]'` |
| **F27** | 实际「retarget → 合并」间隔：#170 = **833 秒**、#172 = **431 秒**，比「retarget → 新运行注册」的约 3 秒窗口大**两个数量级** | 两 PR 的 timeline（`automatic_base_change_succeeded`）与 `merged_at` |
...
| retarget 后闸门是**针对 `main`** 算出来的 | ❌ 沿用 retarget 前那次 ← **codex R1 的 [high]** | ✅ | ✅ **除 retarget 后约 3 秒竞态窗口**（R9） |
...
因此**必然产生一次针对新 base 的新运行**，该运行一旦注册为 pending，PR 即不可合并。

> ⚠️ **此处初稿说满了，已按 codex R6 更正**（轮次记录见 §11）。
> 初稿写的是「**retarget 前那盏绿灯不可能单独满足闸门**」。**「不可能」站不住** ——
> 从 retarget 发生、到新运行被注册为 pending，中间有一个**约 3 秒的窗口**
> （F18/F19 实测：retarget 06:49:06 → run 创建 06:49:09）。
> 在该窗口内，旧的绿灯仍然有效，理论上可被合并溜过。
>
> **准确表述**：retarget **必然**触发一次针对新 base 的重跑，
> 使陈旧绿灯**在绝大多数时间里**无法满足闸门；但存在一个约 3 秒的竞态窗口。
> 该窗口在本仓的可达性已量化并接受，见 **R9**；改造方案见 §8 方案 **H**（已否决）。
...
| **R9** | **retarget 与新运行注册之间存在约 3 秒竞态窗口**，期间 retarget 前那盏 `acceptance` 绿灯仍然有效，理论上可被合并溜过 ⇒ 下游 PR 可能未经「针对 `main` 的验证」就进入 `main`（**codex R6 的 [high]**） | **窗口 ≈ 3 秒**（F18/F19 实测）。踩中需要「自动化在 3 秒内完成合并」或「人恰好在这 3 秒里点下合并」。<br>① 本仓**无 auto-merge、无合并队列**（F26）⇒ 无自动化路径；<br>② 实测 retarget→合并间隔为 **431 秒**与 **833 秒**（F27），比窗口大两个数量级；<br>③ 即便踩中，**内容差异近乎为零** —— acceptance 脚本零 base 假设（F11），窗口内验过的树 = 上游+下游，窗口后应验的树 = `main`(已含上游)+下游，二者仅差「这期间 `main` 上另外落了什么」，而这与 R7 描述的**全仓通用陈旧面**同级 | **接受**；消除该窗口的改造方案见 §8 方案 **H**，**已否决**（它会把 `edited` 从保险变成硬依赖，用已发生两次的死锁去换从未发生过的 3 秒竞态） |
...
| **H** | 让必需 context `acceptance` **只在 base=`main` 时发布**；非 main base 时用另一个**非必需**名字（如 `stacked-acceptance`）。或退回「保留 `branches: [main]` + 加 `edited`」（codex R6 的建议） | **否决 —— 它把风险从「已发生的」换成「没发生的」，方向反了**：<br>① 两种写法都使 `edited` 从**加法式保险**变成**硬依赖**。一旦 GitHub 某天不再于 retarget 时发 `edited`（或事件丢失、workflow 未注册成功），必需检查 `acceptance` 将**永远不出现** ⇒ **本次要修的死锁原样复发**。而该死锁在 2026-08-24 **已真实发生两次**（#170 / #172）；R9 的 3 秒竞态则**从未发生过**，且本仓无 auto-merge、无合并队列（F26）、实测合并间隔大两个数量级（F27）。<br>② 动态 job 名（`name: ${{ ... }}`）会让必需 context 的名字随事件负载变化，本身是新的脆弱面：负载字段缺失或表达式求值异常时，context 名可能变成非预期字符串 ⇒ 同样是缺失必需检查 ⇒ 同样死锁。<br>③ 窗口内的内容差异近乎为零（R9 第 ③ 点）。<br>**⚠️ 这是一个真实的取舍，不是纯技术判断**：它在「3 秒陈旧窗口」与「`edited` 失效即死锁」之间选边。本 spec 选前者并已量化理由；若使用者的风险偏好相反，应改选方案 H —— 该分歧已在 R6 轮次记录中明确上报给使用者。 |
...
| 2 | **设计修订**。D1 增加 `types: [..., edited]`，使 retarget 必然产生一次针对新 base 的运行 ⇒ 陈旧绿灯在绝大多数时间无法满足闸门。<br>⚠️ 本轮当时的措辞是「**不可能**单独满足闸门」，**该措辞已被 R6 证伪并更正**（存在约 3 秒竞态窗口）—— 见 §11 R6 与 R9 | D1 修订版；R6 |
...
### R6 · codex `adversarial-review` · 2026-08-30 · HEAD `b780309` → **needs-attention**（1 条 high）

**Finding [high]**：「retarget 前的绿灯不可能满足闸门」这一断言无支撑。
F18/F19 显示 `edited` 触发的运行在 base 变更**约 3 秒后**才出现，
而 F22 又确立了旧结果在未变的 head sha 上**依然有效** ——
这两条合起来意味着**存在一个约 3 秒的竞态窗口**，期间必需 context 仍是绿的，
人或自动化有可能在新运行注册之前完成合并。

**判定：成立。这条是我把话说满了。**

> 注意：R6 的这条 finding 的**前提恰恰来自我自己在 R3 里提交的反驳证据 F22**。
> 我用 F22 证明「旧检查在 retarget 后依然有效」以推翻 R3 的说法；
> R6 则指出 —— 正因为旧检查依然有效，它在那 3 秒里**能**满足闸门。
> **同一份证据，两个方向。** 这也说明 codex R3 与 R6 在此点上**互相矛盾**：
> R3 主张旧检查**无法**满足闸门，R6 主张旧检查**能**满足闸门。
> R6 的方向是对的（与 F22 一致），R3 的方向已被 F22 证伪。

**处置：**

| # | 动作 | 落点 |
|---|---|---|
| 1 | **更正断言**。把「不可能单独满足闸门」改为「必然触发重跑，使陈旧绿灯在绝大多数时间无法满足闸门；但存在约 3 秒竞态窗口」 | D1 该段下新增警示块 |
| 2 | **量化并记账该窗口** | 新增 **R9**；新增 F26（本仓无 auto-merge、ruleset 无合并队列）、F27（实测 retarget→合并间隔 431 秒 / 833 秒，比窗口大两个数量级） |
| 3 | **否决其改造建议**（把 `acceptance` 限定为只在 base=main 发布 / 退回保留 `branches`） | 新增 §8 方案 **H** 并写明三条理由 |

**为什么否决方案 H**：它把 `edited` 从**加法式保险**变成**硬依赖** ——
`edited` 一旦失效，必需检查 `acceptance` 永不出现，**本次要修的死锁原样复发**。
而该死锁**已真实发生两次**（#170 / #172），R9 的 3 秒竞态**从未发生过**，
且本仓无 auto-merge、无合并队列（F26），实测合并间隔大两个数量级（F27）。
**用已发生的故障去换没发生过的故障，方向是反的。**

> ⚠️ **但这是一个真实的取舍，不是纯技术判断** ——
> 它在「3 秒陈旧窗口」与「`edited` 失效即死锁」之间选边，属**风险偏好**。
> 本 spec 选前者并已量化理由；**该分歧已明确上报给使用者裁决**，
> 若使用者偏好相反，应改选方案 H。
> （依据 `feedback_reviewer_verdict_not_authorization`：评审结论不等于用户授权；
> 以及 `feedback_codex_round6_self_contradiction`：轮次到 6 且出现跨轮自相矛盾时应停下上报。）

**本轮的方法论教训**：**用来反驳评审的证据，往往同时是新缺陷的证据。**
F22 是我为推翻 R3 而找的，它证明「旧检查在 retarget 后仍有效」；
这句话反过来读就是「旧检查在那段时间里**还能**满足闸门」。
⇒ 每次拿证据反驳评审后，应当**再问一遍「这条证据还蕴含什么我不想要的结论」**。

---

### R7 · codex `adversarial-review` · 2026-08-30 · HEAD `62c7f3d` → **needs-attention**（1 条 high）→ **user override 收口**

**Finding [high]**：与 R6 **同一点** —— 3 秒窗口内可用陈旧结果满足闸门；
并指出「无合并队列 / 历史无 auto-merge」只降低概率、**不构成不变量**。
建议同 R6：改用 strict required checks，或让 `acceptance` 只在 base=main 发布。

**判定：该点已在 R6 认定成立并记为 R9；本轮为同一分歧的重述。**

**收口方式：user override。**
2026-08-30，已将该分歧作为**风险偏好取舍**（而非技术对错）明确上报使用者，
并给出三个选项与全部量化数据（窗口 3 秒 vs 实测合并间隔 431 秒 / 833 秒；
本仓无 auto-merge、无合并队列 F26；死锁已真实发生两次而竞态从未发生）。
**使用者选择：维持现方案，跑 R7 收口；若 codex 仍坚持同一点则按 override 收口。**
R7 确系坚持同一点 ⇒ 依该授权 override。

> **override 的确切含义（不得含糊）**：
> 本条 [high] **未被修复**，3 秒竞态窗口**依然存在**，已如实记为 **R9**。
> 本线**没有 codex 真 approve**，账本**无本轮条目**。
> PR 正文必须写明本条 override 及其理由（`feedback_codex_convergence_honest_reporting`：
> codex 没 approve 就别写「收敛」）。

**但本轮 codex 另外两条 next-step 是真缺陷，已无条件修复**
（它们与设计分歧无关，纯属我上一轮改漏的同族文本）：

| # | 残留缺陷 | 修复 |
|---|---|---|
| 1 | **计划里预写的 Task 1 提交信息**仍写着「从而不会拿 retarget 前的陈旧绿灯去满足闸门」—— 这是已被 R6 推翻的绝对断言 | 改为准确表述，并在提交信息内直接点明 3 秒窗口与 R9 |
| 2 | **D1 对照表**「retarget 后闸门是针对 main 算出来的」一行标着无保留的 ✅，与 R9 矛盾 | 该格改为「✅ **除 retarget 后约 3 秒竞态窗口**（R9）」 |

**本轮的方法论教训**：**推翻一句断言之后，必须按「这句断言的全部书写形态」穷尽全仓，
