...
**改动**：`:70` 按 §5.1(c) 拆分；`:115` 的 datetime 校验按 §5.1(b) 推广到全部周期；`:130` 那块按 §5.1(c)(d) 改为**逐根精确反算**。同步更新 `PreviewTrainingSetReaderTests.swift`。
...
2. **§6 的全部变异逐条跑过 —— M1、M2、M3、M3b、M4、M4b、M5、M6、M7、M8、M9、M9b、M9c、M10、M10b，外加 M2+M4 组合变异**，共 16 组；每组**报告里写明红的是哪一条测试**（不接受「有测试红了」）。
...
⚠️ **R4 同样无一条被判误报。四轮 9 条，全部属实、零误报。**
