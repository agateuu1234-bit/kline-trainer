...
- **v4**（2026-05-17）：codex R3 3 findings 全修
  - finding 1 (high) → M0.3 inventory v3 还漏 AppState.swift 的 2 个非 Codable struct (TrainingSetFile / AppSettings)；AppState.swift 实际共 **5 struct**（3 Codable + 2 非 Codable），不是 v3 写的 3 struct；新总数 **16 Codable + 7 非 Codable = 23 M0.3 类型**；inventory 表 + 验收 grep 全部改 16/7/23
  - finding 2 (high) → v3 tag 验证用 `|| echo "..."` 静默吞失败，gate 不阻塞；改为 (a) `git verify-tag` 若 signed 失败 **直接 exit 1**；(b) `gh api` 反查 protected tag rule 不命中 **直接 exit 1**；(c) `git ls-remote --tags origin` 反查 remote tag 目标 commit SHA == 本地预期 squash commit，不等 exit 1；三层都 blocking
  - finding 3 (medium) → ledger residual 表加 H6 行（backend deps Wave 1 内 `requirements.txt == 精确锁定`），与 README v1.4 "backend ranges Wave 1 内闭环" 自洽
...
**Tag 创建 + blocking 三层验证（codex R3 finding 2 修：不静默吞失败）：**
...
# Blocking 三层验证（**任意一层失败 exit 1**）：

# 层 1：signed tag 验证（若 signed 失败必爆；若 unsigned，要求层 2 protected namespace 必须命中）
if [ "$TAG_SIGNED" = "1" ]; then
  git verify-tag wave0-frozen-v1.4 || { echo "FAIL: signed tag 验签失败"; exit 1; }
fi

# 层 2：GitHub protected tag namespace 必须有 wave0-frozen-* 规则
PROTECTED=$(gh api "repos/agateuu1234-bit/kline-trainer/rulesets" --jq '[.[] | select(.target=="tag")] | length')
if [ "$PROTECTED" -lt 1 ]; then
  echo "FAIL: GitHub protected tag namespace 未配置；unsigned tag 可被 retarget"
  echo "  修：repo Settings → Rules → New tag ruleset → ref_name pattern: wave0-frozen-* → admin-only push + 禁 force-update"
  exit 1
fi

# 层 3：remote tag target commit SHA == 本地预期 squash commit
REMOTE_TAG_COMMIT=$(git ls-remote --tags origin wave0-frozen-v1.4 | awk '{print $1}')
if [ "$REMOTE_TAG_COMMIT" != "$TAG_COMMIT" ]; then
  echo "FAIL: remote tag 指向 $REMOTE_TAG_COMMIT，与预期 squash commit $TAG_COMMIT 不符"
  exit 1
fi

echo "GATE PASS: tag wave0-frozen-v1.4 三层验证全过（signed=$TAG_SIGNED / protected ruleset OK / remote SHA OK）"
```
