...
**前置 0：从 GitHub 拿 PR 9 真实 squash commit SHA**（codex R5 finding 1 修：不能自 origin/main 派生）
...
# 本 spec 仅描述行为契约
./scripts/governance/verify-freeze-tag.sh --ref "refs/tags/wave0-frozen-v1.4" --layer protected-namespace || exit 1
