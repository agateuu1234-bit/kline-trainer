...
2. **`DefaultDrawingInputController`**：具体 `DrawingInputController`（`tapToAnchor` 逆映射；`shouldCommit` 按 tool 最小锚数判定）。**注（评审 R2-L）**：`shouldCommit(current:tool:)` 收 `DrawingToolType` **enum**，而 `requiredAnchors` 是 `DrawingTool` **实例**属性、非 enum 可达 → MVP 经**显式映射**（`switch tool { case .horizontal: 1 ... }`）把 enum→最小锚数，不臆造"从 enum 派生"。单工具 MVP benign；Phase 4 多工具时该映射随注册表扩。
...
[落锚] 用户单指点击图表 → arbiter.onTap(point)（仅 drawing 模式 fire）
  └─ Coordinator.handleTap(point):
       ...
       ├─ guard inputController.shouldCommit(current: manager.pendingAnchors, tool: .horizontal)  // count >= 该 tool requiredAnchors.lowerBound（horizontal=1）→ true
...
> **【治理记录 — user 2026-06-13 裁决（neck-doctrine superseding-callout，镜像 zoom §4.4d 先例）】**
> RFC §4.4 总纲规定「所有 engine 契约变更集中顺位 6，消费锚不改 engine 契约」；其唯一合法松绑机制 = **documented user 裁决**（zoom §4.4d 即由 user 2026-06-12 显式裁决松绑）。
> **user 2026-06-13 显式裁决**：**画线激活-FSM 编排 handler 家族**（`activateDrawingTool` → `commitDrawing`/`cancelDrawing`）认定为 **C8b（Wave 2 顺位 7 #87）起、顺位 4 收**的一条**独立 handler 家族**，与顺位-6 冻结的业务 API 面（trade/tier/`appendDrawing`/zoom/replay-payload）**正交**，对 neck-doctrine **superseded**。`commitDrawing`/`cancelDrawing` 在顺位 4 落地。
> **事实依据（对抗性 reviewer R1 核实为真）**：`activateDrawingTool` 仅 C8b #87 加，6a #95 / 6b #97 均未碰它（6b 的顺位-6 交付是 `appendDrawing`）→ commit/cancel 确是 `activateDrawingTool` 的兄弟方法，非顺位-6 surface。serial-neck 防的并发改 engine（outline R8-F1）已消解（3/6a/6b/10a/11 全 merged、无 open PR、顺位 4 唯一活跃）。本裁决是 governance 记录、非 spec 自证。

**问题**：reducer `.drawing` 态唯一出口是 `.drawingCommitted`/`.drawingCancelled`；engine 的 `reduce` 是 `private`，且只有 `activateDrawingTool`（进），无方法 dispatch 退出 → 顺位 4 进得去出不来。gap 真实（reviewer R1 核实：消费者无任何已 ship 的 public 出口可 dispatch）。
...
**考虑过的替代（user 2026-06-13 裁决选 supersede，见上方治理记录；以下记录为何不取）**：(a) 把这两方法挪进独立 §4.4c-补微 PR（6c）先 merge——治理最干净但为 ~15 行薄 FSM-dispatch 走整个 PR 周期，与 neck-doctrine 立法意图（防"现编未治理公共面"）不成比例（user 授权 + spec 记录已满足该意图）；(c) 让 Coordinator 绕过 reducer `.drawing` 态、纯靠 UI flag 驱动 drawingMode——放弃 snapshot-freeze（停 animator + 冻视口）机制，违 C6/C1b/C8b 既定架构，吸引更多评审火力。**采纳 = supersede（单 PR），见治理记录。**

### D-PANELFILTER：渲染按 `panelPosition` 过滤
**问题**：`renderState.drawings = engine.drawings`（全部），`KLineView.draw` 在**每个**面板渲染 → 上栏横价线会同时画到下栏（错）。
**决策**：`RenderStateBuilder.make` 内 `drawings:` 改为 `engine.drawings.filter { $0.panelPosition == (panel == .upper ? 0 : 1) }`，使 `renderState.drawings` = "本面板应渲的画线"。语义正确：`panelPosition` 是 `DrawingObject` 既有字段（C6 commit 写入）。
**既有断言语义迁移（非"无回归"）**：`RenderStateBuilderTests.swift:167` `#expect(rs.drawings == engine.drawings)`（makeAssembles, panel:.upper）的语义从「透传全部」变为「透传本面板」；因 `preview()` 不带 drawings（engine.drawings==[]），该断言 `[] == []` **碰巧仍成立**，但须新增**正/负向**断言：注入 panelPosition=0 一条查 .upper 在、注入 panelPosition=1 一条查 .upper 不在（反之 .lower）。组件表已列扩 RenderStateBuilderTests。
...
**completedDrawings 语义（RFC §4.4c 把真相从 manager 降级到 engine.drawings）**：`engine.drawings` 是唯一渲染+持久化真相；`manager.completedDrawings` 是 **C6 遗留的 vestigial 暂存**（commit() 副产，本锚不读、不渲、不持久）。它仅累积本 Coordinator 生命期内的提交（resume 后 manager 空而 engine.drawings 满——二者**故意发散**，因 manager 非真相）。累积**随 Coordinator 生命期单调增长、teardown 时释放、不持久**（非真相、不读、不渲），benign（非泄漏）。
...
水平线锚仅 `price` 承重（横线全宽、`candleIndex` 不参与 render）。`price` 是**周期无关**的纵轴量 → render 用**当前**面板 `mapper.priceToY(anchor.price)`，切周期/缩放/平移后横线仍画在正确价位。故"跨缩放/平移还原"对横线 = render 用实时 mapper 即得，**无需** anchor 重映射逻辑（那是 Phase 4 斜线工具的难题）。`anchor.period`/`candleIndex` 作元数据存储（DrawingObject 结构完整性），render 不消费。
**candleIndex 稳定性声明（评审 R1-M）**：horizontal `anchor.candleIndex = mapper.xToIndex(point.x)` 仅记录**落锚瞬间视口下**的绝对索引（含 startIndex+pixelShift）——同一价位在不同缩放/平移下点击得**不同** candleIndex，**无跨视口稳定性保证**，对横线**无语义**、不做不变量校验。Phase 4 斜线工具才需自定义 anchor 稳定性契约。
...
1. **host 纯逻辑**（macOS swift test）：
   ...
   - `commitDrawing/cancelDrawing`：构 engine 进 .drawing → commit → 验 `interactionMode == .autoTracking` + **revision 不变**（显式 `#expect(after == before)`，钉死「commit/cancel 不 bump revision」契约，防未来误加——核实 Reducer.swift:203-215 commit/cancel case 无 `revision &+= 1`）；**不**断言 offset（commit 不碰 offset，`.drawing` 态吞 offsetApplied，offset 维持激活时刻冻结值，未必为 0）；cancel 同；非 drawing 态 no-op；append 与 commit 独立（append 不改 mode、commit 不改 drawings）。
