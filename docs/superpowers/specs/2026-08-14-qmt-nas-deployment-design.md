...
| NAS `kline-trainer-handoff-20260814/` 三个 zip 重算 CRC32 **同样三个全对**；`kline_trial_20260814.sql.gz` `gzip -t` 通过（⚠️ **历史记述，非判据** —— 这三个指纹与上一行是**同一批 v1 包**；2026-09 切片一起它们同样已作废为 v1 审计归档，见本表下方那行订正） | NAS 上现算 |

→ 「不可再生」的两样东西现在是**本机 + NAS 双份**。⚠️ **2026-09 订正（切片一 P3c）**：P7 的 zip **只能**从 NAS 的 handoff 目录取；Mac 本地 `~/qmt_trial_out/` 那三份自此仅作 **v1 审计归档**，⛔ **不得再 scp 到部署目录**。⛔ **但该 handoff 目录里当前存放的同样是 v1 包** —— 上一行那三个 CRC32 与 Mac 那份**逐字相同**，本就是同一批包的两份副本 ⇒ **在 P4 重建完成、并把新产物同步进该 handoff 目录之前，P7 不得作为第 2 代产物的来源**。两个来源的理由完全一样：切片一把产物升到第 2 代后，v1 包与库里的新指纹对不上，症状是完整性校验失败或 404，而收口闸会全绿、看不出问题。（**判据不变：落到目标目录后必须重算 CRC32 比对**）
