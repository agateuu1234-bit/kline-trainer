...
- `Render/CrosshairLayout.swift`（纯函数）：`resolve` 沿用吸附 + 价/时标；新增「细实线、无偏移」（已是无偏移，只去虚线）。
...
- 竖线贯穿整 panel（main+vol+macd）；横线只在所触子图。左缘价标 = `yToPrice(point.y)`（随横线）；底部时间标 = `candles[snappedIndex].datetime`（随竖线）。
...
**字段（自上而下）**：
1. **光标价**（栏顶醒目行）= `yToPrice(横线 Y)`，纯纵轴读数，**与具体哪根 K 线无关**。
2. 日期 / 时间：`datetime`（UTC+8 / `en_US_POSIX`）；**日内周期（m3/m15/m60）显「时:分」；daily/weekly/monthly 只显日期**（按 `period` 分支）。
...
**颜色规则**（user 定）：
- **方向色（涨=红 / 跌=绿 / 平=白）**：**光标价、收、涨跌、涨跌幅**。基准 = **前一根收盘**（光标价、收均与 prevClose 比；涨跌额/幅同向）。slice 首根无 prevClose → 中性白。
...
|---|---|---|---|
| 1 | 训练页长按上图主图 | 出现细实线十字光标，竖线落最近 K 线中心、横线在手指 Y；**整图冻结**（背景 K 线不平移不缩放） | 十字出现且图不动 = pass；图跟着平移/缩放 = fail |
...
4. 吸附**每根一次短 haptic**。
5. 信息栏字段：**栏顶光标价** + 日期/时间/开高低收/涨跌/涨跌幅/均价/成交量/成交额；**换手率/MA66/可调字号不做**。
6. 颜色：**涨红/跌绿/平白**（光标价/收/涨跌/涨跌幅，基准前收）；**其余黄**。
