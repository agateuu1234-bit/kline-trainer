...
- `Render/CrosshairLayout.swift`（纯函数）：`resolve` **扩参收 `frames: ChartPanelFrames`**；**竖线改为贯穿整 panel**（`mainChart.minY` → `macdChart.maxY`，**改现状** 仅到 `mainChartFrame.maxY`）；**时间标移到 panel 最底部 `macdChart.maxY`**（**改现状** 现在主图区底/成交量上方）；沿用吸附 + 价标；视觉钉死细实线、无偏移。grep 证 `resolve` 仅 `drawCrosshair`+单测调用，扩参安全。
...
每个 `ChartContainerView`(.upper/.lower) 独立一套 arbiter + Coordinator。两个模式标志：`drawingMode`（既有，画线浮动钮切换）、`crosshairMode`（**新增**，长按进入/点击退出）。**二者互斥**：`drawingMode==true` 时长按不进十字光标（drawing 优先；避免锚点与光标冲突）。
...
- 左缘价标 = `yToPrice(point.y)`（随横线 Y）；时间标文本 = `candles[snappedIndex].datetime`（随竖线吸附）。
...
4. 涨跌 / 涨跌幅：`close − prevClose` / `(close−prevClose)/prevClose`（基准 = **前一根收盘**；slice 首根无 prevClose → 显「—」、中性色）。
5. 均价：`amount / volume`，**单位自检**：仅当 `amount != nil && volume > 0 && low ≤ 均价 ≤ high` 才显示，否则该行隐藏（fail-safe，防 A 股「手 vs 元」差 100 倍显假值）。
6. 成交量：`volume`（千分位 + 单位）；成交额：`amount`（`nil` → 隐藏该行）。

**颜色规则**（user 定，**已网查证主流同基准**）：
- **方向色（涨=红 / 跌=绿 / 平=白）**：**栏顶实时价、收、涨跌、涨跌幅**。基准 = **前一根收盘（prevClose）**——主流（同花顺/东财/通达信）涨跌额 = 收 − 前收、涨跌幅 = 涨跌额 ÷ 前收、红涨判定 `CLOSE>REF(CLOSE,1)`，均以前一根收盘为基准（[东财帮助](https://qhweb.eastmoney.com/help/1217204.html) / [涨跌幅百科](https://baike.baidu.com/item/%E6%B6%A8%E8%B7%8C%E5%B9%85/8646206) / [东财指标博客](https://blog.eastmoney.com/benny_511/blog_220706635.html)）。slice 首根无 prevClose → 中性白 + 涨跌显「—」。
- **其余 = 黄色**：日期·时间、开、高、低、均价、成交量、成交额。
...
`CONTRACT_VERSION` 钉持久化/跨端契约。C：0 DDL、0 持久化结构、0 序列化字段、0 后端、0 引擎**行为**、0 新 I/O；仅 iOS 表现层（十字光标视觉/信息栏）+ 手势层（消歧/路由）+ 只读取值（`candles[i]` 既有切片、`yToPrice` 既有几何）。周期切换复用既有 `switchPeriodCombo`（行为不变）。十字光标本就 view-layer 瞬态（不进 engine、不进 RFC 7 契约）。无任何外部可观测契约变化 → **不 bump**（与 RFC-B / PR #122–128 UI 改版一致）。plan/impl 若发现需触持久化（不预期），回本节修订并 bump。
