...
2. **`replay(recordId:)`（从头）补充**（codex spec-R1-F3：**不前置 clear**——易错装配失败会丢旧档）：
   ...
   - 设 `activeStartedAt = now()`（原 `nil`→`now()`，供 `PendingReplay.started_at`）；`activeRecord = record`（已设）；**捕获 `replayBaseline = (tick, ops.count, drawings.count, upperPeriod, lowerPeriod)`**（含周期，R14-F1）+ **`replayHasPersisted = false`**；**`resetAutosaveState()`**（codex plan-R7-F1：重开 autosave 栅栏 `terminating=false`——否则前一会话 endSession 留的 `terminating=true` 会让 fresh replay 的 tick/后台 autosave 全 no-op、crash/后台杀=丢档；与 startNewNormalSession/resumePending 同款）。`activeSessionKey` 保持 nil。
...
3. **新增 `resumePendingReplay(recordId:) async throws -> TrainingEngine?`**（错误纪律镜像 `resumePending`，codex plan-R1-F1/R11-F1）：
   ...
   - **② 本记录槽全量解码（含 position）**：`loadReplay()` **＋ `decodePosition(p.positionData)`**（**codex plan-R18-F1：position 的 PositionManager JSON 解码也是 slot payload，须与 loadReplay 同步在此 do/catch 内**——否则放后面重建块的宽 catch 会被传播、卡死槽；`decodePosition` 已把所有解码错误包成 `.dbCorrupted`）。**`.dbCorrupted`（已验证损坏 payload）→ `try clearReplay()`（**durable，不用 `try?`**，codex plan-R12-F1：清失败=瞬态 DB → 传播可重试、槽留，不伪装"无暂存"）+ 返回 nil（回退从头 fresh）**；其他（瞬态）→ **传播**（不清、不 fresh）。loadReplay 内所有 payload 解码失败统一映射 `.dbCorrupted`（base64/Period/JSON）。
   - **③ 重建块 scalar + period 前置校验（codex whole-branch R1-F1 + R2-F1，关键）**：`loadAllCandles`/`maxTick` 后、`make` **前**，对 pending 的 **scalar + period slot payload** 显式预判——`guard !(allCandles[pending.upperPeriod] ?? []).isEmpty, !(allCandles[pending.lowerPeriod] ?? []).isEmpty, (0...mt).contains(pending.globalTickIndex), pending.cashBalance.isFinite, pending.cashBalance >= 0, pending.accumulatedCapital.isFinite, pending.accumulatedCapital >= 0, pending.drawdown.peakCapital.isFinite, pending.drawdown.peakCapital >= 0, pending.drawdown.maxDrawdown.isFinite, pending.drawdown.maxDrawdown >= 0 else { reader.close(); try clearReplay(); return nil }`。**理由**：`make`（L220/L236-240 对越界 tick / 非有限·负 money·drawdown；**L244-245 `final-R6-F1` 对 saved period 不在 candle map**）抛 `.trainingSet(.emptyData)`，与真训练集故障**不可区分**；若交给重建块宽 catch 传播 → 不清槽 → AppRouter resume-first 每次撞同一损坏槽 → **记录永久 brick**（period 与 scalar 同型 brick：陈旧/损坏槽存了当前 candle set 不含的周期，如训练组被替换）。故须在 make 前把"损坏槽"与"瞬态训练集故障"分流：损坏 scalar/period → `reader.close` + durable `clearReplay`（`try`，清失败=瞬态传播可重试）+ nil（回退从头）。fee 已被 loadReplay 的 `sanitizedForLegacyCorruption` 净化，非 brick 向量，不在此重判。**make 余下抛错（m3 缺/不连续/datetime/commissionRate）= 真训练集故障 → 传播（槽留，可重试，不误清有效槽）**。do/catch 拆三段（candle-load / scalar-guard / make）确保各失败路径 `reader.close()` 恰一次、无双关。
   - 其余错误纪律：`loadRecordBundle` / `loadAllCandles` / `make`（真训练集故障）错误**传播**；**清档点 = openReader `isCorruptTrainingSet`（`cache.delete + clearReplay`）+ 本记录槽 `.dbCorrupted`（`clearReplay`）+ 文件名不一致（`clearReplay`）+ scalar slot 越界/非有限（whole-branch R1-F1，`clearReplay`）+ saved period 不在 candle map（whole-branch R2-F1，`clearReplay`）**。
...
4. **清档时机（关键——区分"续局保留" vs "终局清除"，且终局须 fence）**：
   ...
   - **终局清档须 fence+drain（codex spec-R1-F2）**：replay 走到结算（到 maxTick 自动结算 **或** 手动「结束本局」）→ 终局清档前**必须 `await fenceAndDrainAutosaves()` 再 `clearReplay()`**，否则末根 tick 的 `onChange` 已排队的 autosave 会在 clear 后跑、把 maxTick 终态写回 `pending_replay` → 按钮错显"返回训练"、再进=终态陈旧局。镜像 Normal `finalize`（L362 先 fence 再单事务）。
     - 实现：`replaySettlementPayload` 现为 **sync `throws` 且无 fence**（注释明示原"不触 pending"不变量——本 PR 刻意打破）→ 改为 **async**，**顺序（codex plan-R1-F2/R8-F1/R16-F1）**：①guard 含活跃上下文 **`let recordId = activeRecord?.id`**（缺则 **throw、保留会话**，fail-closed，**不可静默成功留陈旧槽**）→ ②`await fenceAndDrainAutosaves()`（此后无并发写）→ ③`loadMeta` + 构造 `record`（**全部 throwing payload 工作，槽仍在**）→ ④**成功后才** `try clearReplay(ifRecordId: recordId)`（recordId 来自 guard 绑定；**非可选 `if let`**）→ ⑤return record。**清档绝不早于 payload 全部成功**。clearReplay 抛 → 方法抛、record 不返回 → **caller 保留 session+槽、可重试**（不 `onSessionEnded(nil)` 拆毁）。连带 `lifecycle.replaySettlementRecord()` 改 async；`TrainingView` 新增 `runReplaySettlement()`（镜像 `runFinalize` 重入门）+ `replaySettlementFailed` alert（重试/退出本局）；`routeEndOfSession` replay 分支调它。
     - **退出本局 = durable back()（codex whole-branch R3-F1，关键）**：`replaySettlementFailed` alert 的「退出本局」**不能**调 `onSessionEnded(nil)`——`fenceAndDrainAutosaves` 已置 `terminating=true` → autosave 协程见 terminating **退出不落盘**（drain-without-flush，防复活已清槽是刻意设计，不改 fence），故槽仅剩 fence 前的**旧检查点**，终态未写；`onSessionEnded(nil)` 只 `endSession` 不落盘 → 续局回旧态、alert「暂存进度保留，可在历史记录返回训练」承诺落空。故「退出本局」改 **`exitInFlight` guard + `Task { try await lifecycle.back(); onExit() }`**（`lifecycle.back()` = `saveProgress`(显式写当前终态，不受 terminating 门控) + `endSession`），把终态 **durable 落槽**；`back()` 抛（保存失败）→ 重弹本 alert（可重试）。镜像 `backFailed`/`finalizeFailed` 退出按钮同型。
   - **条件清 + fail-closed（codex plan-R3-F1/R8-F1，关键）**：终局/discard 用 **`clearReplay(ifRecordId: recordId)`**（原子 `DELETE WHERE id=1 AND record_id=?`），**仅清属于当前 replay 记录的槽**（因"不前置 clear"——开新 B 未存即到终局/discard 时槽仍=旧记录 A；无条件清会误删 A）。**`recordId` 须纳入活跃上下文 guard**（`guard ..., let recordId = activeRecord?.id else { throw }`）：缺则 **throw、保留会话可重试**（fail-closed，**不可静默返回 record/结束会话却留陈旧槽**）。reset（§A.7）才用无条件 `clearReplay()` 清全部。
   - **丢弃清档**：`discardSession()` 当 `mode==.replay`：`guard let recordId = activeRecord?.id else { throw .internalError }` 再 `clearReplay(ifRecordId: recordId)`（已先 `fenceAndDrainAutosaves`，L460；normal 时清 `pending_training`）。DEBUG 测试钩子 `setActiveRecordNilForTesting()` 验两条 fail-closed。
...
5. **新增查询 `hasResumableReplay(recordId:) -> Bool`**：用轻量 `loadReplaySlotInfo()`（不解码 payload，R11-F1）判 `recordId` 匹配。**display-only / advisory**（仅历史弹窗按钮文案）；**不当路由门**。读失败保守返 false 安全，因路由是 resume-first 权威（见 §A.5）：一次瞬态 false 至多让按钮短暂误显"再次训练"，点击仍走 resume-first、不会丢槽。
...
- **清档（仅"已验证损坏槽"）= ① 训练集 open `isCorruptTrainingSet`（`cache.delete + clearReplay + nil`）；② **本记录槽** 全量 `loadReplay` **或 `decodePosition`** 抛 `.dbCorrupted`（损坏 payload，含 position JSON，`clearReplay + nil` 回退从头，R11/R18-F1）；③ pending 文件名 ≠ 记录文件名（内部不一致，`clearReplay + nil`，R10-F1）；④ **scalar slot payload 越界/非有限**（`globalTickIndex` ∉ `0...maxTick` 或 cash/capital/drawdown 非有限·负 → make 前预判，`clearReplay + nil` 回退从头，**whole-branch R1-F1**）；⑤ **saved period 不在 candle map**（`allCandles[upperPeriod]`/`[lowerPeriod]` 缺/空 → make L244-245 抛 emptyData 同 brick → make 前预判，`clearReplay + nil` 回退从头，**whole-branch R2-F1**）。**
- **传播（瞬态，不清不 fresh）**：slotInfo whole-db 错误、loadRecordBundle、loadAllCandles、**真训练集故障的 make 抛错**（m3 缺/不连续/datetime/commissionRate——**非** pending scalar 越界，后者已在 make 前分流清档）、**非 `.dbCorrupted`** 的 loadReplay 错误 → throw → 路由 setError、不回退从头。
...
| D-A9 | clean replay 不写槽 | replay save 设会话基线`(tick,ops,drawings,upper,lower周期)` + `replayHasPersisted` 标志；**仅 `!replayHasPersisted && 当前态==baseline`** 才跳过写（codex plan-R4/R6/R14-F1） | `back()`/后台 flush 无条件保存→无守卫则 clean fresh B 覆盖别记录 A 槽；仅计数判脏会让"加画线→删画线"残留（故首写后永不跳过）；基线含周期防"切周期被当 clean 丢周期"；resumed 本就拥有槽=true |
...
- `PendingReplayRepository` 协议（`Persistence/PendingReplayRepository.swift`，sync throws：saveReplay/loadReplay/**loadReplaySlotInfo(R11-F1 轻量元数据)**/clearReplay/clearReplay(ifRecordId:)）+ `ReplaySlotInfo` 值类型 + `InMemoryPendingReplayRepository`（`PreviewFakes/InMemoryFakes.swift`，`#if DEBUG`，含 fail-injection/saveCount，slotInfo 不消费 failNext）
...
- `UI/TrainingView.swift`（+`showsReviewControls = canAdvance && !canBuySell`；复盘控件条「下一根」+「快进到结尾」；**replay 终局 `runReplaySettlement()`（async）+ `replaySettlementFailed` 可重试 alert；「退出本局」= `lifecycle.back()` durable 落终态槽（whole-branch R3-F1），不 `onSessionEnded(nil)` 拆毁**；review autosave/maybeAutoEnd 已证安全）+ 新增 `UI/ReviewControlBar.swift`（`ReviewControlBarContent` 纯内容 host-可测 + `ReviewControlAction` 枚举 + SwiftUI 薄壳，codex plan-R15-F2）
...
- Coordinator（in-memory fakes）：
  ...
  - **position JSON 损坏（plan-R18-F1）**：positionData 合法 base64 但非法 PositionManager JSON → `decodePosition` 抛 `.dbCorrupted` → 清槽 + nil（回退从头），不卡死。
  - **scalar slot 越界/非有限（whole-branch R1-F1）**：`pending.globalTickIndex` > 缓存训练集 maxTick（如训练集变更/缩短）→ `resumePendingReplay` 清槽 + nil（回退从头，不传播 brick）；`cashBalance=.infinity`（或 `accumulatedCapital<0`）→ 同样清槽 + nil。两测均断言返回 nil **且** `loadReplaySlotInfo()==nil`（槽已清）。
  - **saved period 不在 candle map（whole-branch R2-F1）**：槽存 `upperPeriod: .monthly`（训练集 candle map 仅含 m3/m60/daily/weekly）→ `resumePendingReplay` 清槽 + nil（回退从头，不到 make/不 brick）；断言返回 nil **且** `loadReplaySlotInfo()==nil`。
  ...
  - **切周期算脏（plan-R14-F1）**：replay 切周期组合（tick/ops/drawings 不变）→ saveProgress 不跳过、`pending_replay.upper/lowerPeriod` 更新；续局恢复改后周期。
  ...
  - **结算失败 durable 退出（whole-branch R3-F1）**：replay 推进两根形成 firstTick→currentTick 终态，结算失败（`failNextClearReplay`，fence 后抛）→ 槽停 firstTick 旧检查点；「退出本局」= `saveProgress`(当前终态)+`endSession`（= `lifecycle.back()`）→ 断言 `loadReplay().globalTickIndex == currentTick`（终态 durable 落槽、可续到终态，**非**旧检查点）。非真空：仅 `endSession`（无 saveProgress）槽停 firstTick。
