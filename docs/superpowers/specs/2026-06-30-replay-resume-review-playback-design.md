...
- **清档（仅"已验证损坏槽"）= ① 训练集 open `isCorruptTrainingSet`（`cache.delete + clearReplay + nil`）；② **本记录槽** 全量 `loadReplay` **或 `decodePosition`** 抛 `.dbCorrupted`（损坏 payload，含 position JSON，`clearReplay + nil` 回退从头，R11/R18-F1）；③ pending 文件名 ≠ 记录文件名（内部不一致，`clearReplay + nil`，R10-F1）。**
