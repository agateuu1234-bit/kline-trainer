...
3. **新增 `resumePendingReplay(recordId:) async throws -> TrainingEngine?`**（错误纪律镜像 `resumePending`，codex plan-R1-F1/R11-F1）：
   ...
   - **③ 重建块 scalar 前置校验（codex whole-branch R1-F1，关键）**：`loadAllCandles`/`maxTick` 后、`make` **前**，对 pending 的 **scalar slot payload** 显式预判——`guard (0...mt).contains(pending.globalTickIndex), pending.cashBalance.isFinite, pending.cashBalance >= 0, pending.accumulatedCapital.isFinite, pending.accumulatedCapital >= 0, pending.drawdown.peakCapital.isFinite, pending.drawdown.peakCapital >= 0, pending.drawdown.maxDrawdown.isFinite, pending.drawdown.maxDrawdown >= 0 else { reader.close(); try clearReplay(); return nil }`。**理由**：`make`（L220/L236-240）对越界 tick / 非有限·负 money·drawdown 抛 `.trainingSet(.emptyData)`，与真训练集故障**不可区分**；若交给重建块宽 catch 传播 → 不清槽 → AppRouter resume-first 每次撞同一损坏 scalar 槽 → **记录永久 brick**。故须在 make 前把"损坏 scalar 槽"与"瞬态训练集故障"分流：损坏 scalar → `reader.close` + durable `clearReplay`（`try`，清失败=瞬态传播可重试）+ nil（回退从头）。fee 已被 loadReplay 的 `sanitizedForLegacyCorruption` 净化，非 brick 向量，不在此重判。**make 余下抛错（m3 缺/不连续/datetime/commissionRate）= 真训练集故障 → 传播（槽留，可重试，不误清有效槽）**。do/catch 拆三段（candle-load / scalar-guard / make）确保各失败路径 `reader.close()` 恰一次、无双关。
   - 其余错误纪律：`loadRecordBundle` / `loadAllCandles` / `make`（真训练集故障）错误**传播**；**清档点 = openReader `isCorruptTrainingSet`（`cache.delete + clearReplay`）+ 本记录槽 `.dbCorrupted`（`clearReplay`）+ 文件名不一致（`clearReplay`）+ scalar slot 越界/非有限（whole-branch R1-F1，`clearReplay`）**。
