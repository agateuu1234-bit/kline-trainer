...
| **D5** | 联动挂载点 | 在**三个具名 gesture 函数的 leader 帧之后**追加 `propagateLinkage`：`beginPan` / `applyPanOffset(deltaPixels:renderBounds:panel:)` drag / `floorOrFullClampedOffsetDelta` decel。**不**包裹通用 `applyOffsetDelta` | **按具名函数挂载（非「offset 唯一入口」）**：offset 不止经 `applyOffsetDelta`——`interruptDeceleration`（在 `beginPan` 内 @664）直接 `reduce(.offsetApplied)` 绕过 applyOffsetDelta（H1）。故联动**锚定具名 gesture 函数**：`beginPan` 尾的 `propagateLinkage` 在 interrupt-clamp 落定**之后**读 leader 新右缘，天然兜住。**根治双驱**：trade/combo 的 `resetOffsetAfterAutoTracking`（经 `applyOffsetDelta`，已对两面板 lockstep）**不在**这三个具名函数内，故不触联动二次驱动（grep 实证 reset caller 仅 trade/switchPeriodCombo，无 gesture 路径）。gesture 入口只在被拖面板触发（Coordinator 传 self.panel）。 |
...
| **D10** | drawing-mode follower | 现有 reducer 在 drawing 态**吞** `.offsetApplied` → follower 不跟（暂 desync），下次 pan/trade/combo 复位 | 画线冻结该面板是既定语义；不为此破例。可接受边界。**含 leader beginPan 时**：若 follower 在 drawing，`reduce(.panStarted, follower)`（D7）在 drawing 态 → `.none`（停留 drawing），后续 `.offsetApplied` 仍被吞 → follower 正确不跟（与 D10 自洽，实测 `Reducer.swift` panStarted/offsetApplied 各模式分支）。 |
...
| **D13** | leader 侧 pinch/draw 起手的瞬时 desync | **登记为已知边界**（不扩范围去联动 pinch/draw）：`interruptDeceleration` 还被 `applyPinch(.began)`@749 / `activateDrawingTool`@846 调（leader 侧），可在 overscroll 中途把 leader offset clamp 回 maxOffset，**不驱动 follower** → 两图右缘瞬时 desync | 仅「正处最老边 overscroll bounce 时又起 pinch/画线」的角落触发；pinch/draw 本就范围外（D2/§1.3）；下次 pan/trade/combo 即复位（同 D10 类）。不为此把联动扩到 pinch/draw 入口（YAGNI + 保 leader 行为不变 D12）。 |
...
`propagateLinkage(fromLeader: L)`（**M2：调用点只传 candles+bounds，几何全在 PanLinkage 内派生，不手算 step/currentIdx → D9 单一几何真相**）：
```
...
let leaderTick = PanLinkage.rightEdgeTick(offset: panelState(L).offset,
                    ...
                    bounds: lBounds, tick: tick.globalTickIndex)         // 内部经 geometryCore/currentCandleIndex 派生 step·currentIdx
let fTarget = PanLinkage.followerOffset(targetTick: leaderTick,
                    ...
                    bounds: fBounds, tick: tick.globalTickIndex)         // 内部派生 geometry + clamp [0,maxOffset]
let fCur = panelState(F).offset
if fTarget != fCur { applyOffsetDelta(fTarget - fCur, panel: F) }        // D6：经现有 .offsetApplied
```
> `endPan(velocity:..panel: L)`：**不改**——仅 leader 起 animator；follower 由 leader 减速帧（floorOrFull 路径）持续驱动，leader settle 即 follower settle（follower animator 全程不启；两面板 animator 是独立 tuple，实证 `TrainingEngine.swift:36,150-157`）。
> `propagateLinkage` 经 `applyOffsetDelta`→`reduce(.offsetApplied)` 是**直接 reduce**，不重入三个具名 gesture 函数 → 单向无环（R4）。follower 在 drawing 态被 reducer 吞（D10）。
> **H1**：offset 也可经 `interruptDeceleration` 的直接 `reduce(.offsetApplied)` 改（非 applyOffsetDelta）；因联动锚定**具名函数**而非 applyOffsetDelta，`beginPan` 尾的 `propagateLinkage` 在 interrupt-clamp 之后读 leader 新右缘，正确兜住。

### 4.2 `PanLinkage` 纯逻辑（平台无关，host 全测）
两函数签名**统一收 `(candles, rawVisible, bounds, tick)`，内部唯一经 `RenderStateBuilder.{geometryCore, currentCandleIndex, offsetBounds, roundTripEdge}` 派生 step/currentIdx/bounds**（M2：不在调用点手算几何标量 → D9 单一几何真相）：
```swift
public enum PanLinkage {
    /// forward：leader 当前 offset → 其右缘可见候选的 endGlobalIndex（= 右缘 tick）。
    /// 内部：step/currentIdx ← geometryCore(mainW(bounds),rawVisible,count,currentCandleIndex(candles,tick));
    ///       wholeShift = floor(offset/step); 右缘候选 idx = clamp(currentIdx - wholeShift, …, currentIdx); 返 candles[idx].endGlobalIndex。
    static func rightEdgeTick(offset: CGFloat, candles: [KLineCandle],
                              rawVisible: Int, bounds: CGRect, tick: Int) -> Int

    /// inverse：目标 tick → follower offset（其右缘候选 endGlobalIndex 对齐 targetTick），clamp 到 follower [0,maxOffset]。
    /// 内部：currentIdx = currentCandleIndex(candles, tick);  targetRightEdgeIdx = currentCandleIndex(candles, tick: targetTick);
    ///       wholeShift = currentIdx - targetRightEdgeIdx;  offset = wholeShift·step;  **clamp 到 offsetBounds(...).[0,maxOffset]**。
    static func followerOffset(targetTick: Int, candles: [KLineCandle],
                              rawVisible: Int, bounds: CGRect, tick: Int) -> CGFloat
}
```
- 两函数是 `makeViewport` offset↔index 的 forward/inverse，**复用** `geometryCore`/`currentCandleIndex`/`offsetBounds`/`roundTripEdge`（零重写几何）。
- 输入纯值（candles + bounds + tick）→ 可逐值 host 断言（跨周期换算 / clamp / 同 tick 对齐 killer）。
- **M1（安全网=clamp，非 wholeShift≥0 保证）**：follower 是**不同周期数组**，`currentCandleIndex(followerCandles, leaderTick)` 在 leaderTick 超过 follower 末根 `endGlobalIndex`（如 review/聚合面板末根 tick 差）时钳 `count-1`，可致 `targetRightEdgeIdx > currentIdx` → **wholeShift 可为负 → offset 预钳为负**；**最终 `clamp[0,maxOffset]` 是 load-bearing 安全网**（不依赖任何「wholeShift≥0」跨周期保证）。host 须测 leaderTick 落在 follower 末根处（钳 0）与远早处（钳 maxOffset）两端。
...
|------|------|------|
| `kline_trainer_plan_v1.5.md` | §双面板独立描述（L109「PanelViewState×2（每面板独立）→ period/interactionMode/visibleCount/**offset**」/ L204「交互模式每面板独立」/ L552「每面板独立 **period/mode/zoom**」〔注：L552 列的是 period/mode/zoom，offset 独立性在 L109〕/ §6.2.3 L941-948） | 增补一句「pan/scroll **时间对齐联动**：拖一面板，另一面板右缘按同一全局 tick 跟随（offset 仍各自独立存储，由引擎跨周期换算驱动）」；**不删**「各自独立」（offset 存储仍独立，仅新增联动驱动）。 |
...
1. **host 纯逻辑 TDD（核心覆盖，红绿）**：`PanLinkageTests` —— `rightEdgeTick` / `followerOffset` 的跨周期换算正确性（如 upper=.m60 offset=X → 某 tick → lower=.daily offset=Y 实算断言）、clamp（follower 够不到 leader tick 时钳 maxOffset；leaderTick 落 follower 末根钳 0，M1 两端）、同 tick 对齐（offset=0→follower=0）、FP round-trip（非整除 step）、空/退化。killer：删换算/反转 clamp 即红。
2. **host 引擎接线 TDD（M3，propagateLinkage `@MainActor` 逻辑可同步 host 测，与现有 `PublicGestureSurfaceTests`/`TrainingEngineDragDampingTests`/`InteractionTests`/`PinchTests`/`BounceWiringTests` 同款）**：驱动 `engine.beginPan/applyPanOffset(renderBounds:)/floorOrFullClampedOffsetDelta` 于 leader，**断言 follower 面板的 `offset`/右缘 tick** 落在 PanLinkage 算出的目标；含 **D7**（leader beginPan → follower 转 freeScrolling）+ **D10**（follower 在 drawing 态被吞、不跟）+ **R7**（trade/combo lockstep reset 不被联动二次驱动）。
3. **host 回归网**：现有 `TrainingEngine`/`RenderStateBuilder` 测试全绿不回归（leader 路径未改，仅追加 follower 驱动；reset/trade/combo lockstep 未动）。
4. **仅真正测不到的部分靠 Catalyst + 人工**：Mac Catalyst `build-for-testing` 编译闸 + iOS app build + **模拟器人工验收**（逐帧 `DisplayLink` 减速节奏与视觉流畅度、跨面板视觉对齐观感、D13 pinch/draw 起手瞬时 desync）。

> 跨周期换算（最易错）+ **引擎接线（follower offset 结果）**都有 host 自动化覆盖（M3：不把可同步 host 测的 `@MainActor` 逻辑推给人工）；仅 DisplayLink 节奏/视觉观感靠编译闸 + 人工。诚实边界。
...
| **R7** | 与既有 lockstep reset 双驱 | 联动**不挂**通用 `applyOffsetDelta`，只挂**三个具名 gesture 函数**（D5）；reset/`interruptDeceleration` 的直接 reduce 不在其中，故不触联动（H1）。host 测 R7 断言。 |
| **R8** | leader 侧 pinch/draw 起手经 `interruptDeceleration` clamp offset → 两图右缘瞬时 desync（H2） | 仅「最老边 overscroll 中途又 pinch/画线」角落；pinch/draw 范围外（D2/D13）；下次 pan/trade/combo 复位。**登记为已知边界**（§8.2 row 8 放宽），不扩范围联动 pinch/draw。 |
...
### 8.1 机器执行
- host：`cd ios/Contracts && swift test` 全量 0 failures；净 = +`PanLinkageTests`（纯逻辑）+ 引擎接线联动 host 测（驱动 leader gesture 断言 follower offset/右缘 tick + D7/D10/R7，M3），现有测试零回归。
...
| 8 | 缩放（pinch）一图 | 仅该图缩放（pinch 不联动，范围外）；**若恰在最老边 overscroll 中途起 pinch/画线，另一图右缘可瞬时不跟（D13/R8 已知边界），下次 pan/trade/combo 即复位** | ☐ |
