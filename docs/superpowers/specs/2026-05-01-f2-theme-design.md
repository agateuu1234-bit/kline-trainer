...
- macOS swift test 跑：纯值层 tests（DisplayMode rawValue + rawValue init 2、AppColorScheme cases + equality 2、resolveColorScheme 4-cell 矩阵 4）—— **共 8 tests**。
- 仅 `swiftc -typecheck` iOS SDK gate 验：UIKit shell tests（ThemeController.displayMode 默认值 + 三态 resolve（system+dark / light forced / dark forced）= 4、AppColor 13 const 全 resolvable / DIF·DEA RGB 字面 / 派生不变量 = 3）—— **共 7 tests**。本 PR 不强制跑 iOS Simulator runtime test；以 `swiftc -typecheck` 编译 + 类型检查为充分条件（D-5）。
...
**测试数 15**（纯值层 8 + UIKit shell 7；macOS swift test 跑 8，UIKit shell 7 由 `swiftc -typecheck` 验语法 / 类型 — 不进 macOS test runner 计数）。
...
### 纯值层 tests（macOS swift test 直跑，8 个）
...
合计 7 tests（ThemeController 4 含 dark-forced + AppColor 3）。
...
- **F. baseline test count drift**：截至 2026-05-01 PR #38 后，Contracts 实跑 108 tests（验证 `swift test` 输出 `Test run with 108 tests in 21 suites passed`）；本 PR 必须保持 8 个新增 macOS test 100% pass，total = 116。任何 baseline test 失败 → root-cause 后再 push。
