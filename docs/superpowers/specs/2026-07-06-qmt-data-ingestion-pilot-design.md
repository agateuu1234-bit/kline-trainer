...
| **D10**（codex R6-F2 / R10-F1） | **源一致性对账门（B1）**：import 某股前——(a) `export_log` 该股 1m+日线 `status=='ok'`；(b) **对称日期集相等门**：共享覆盖窗口内 dense-1m 交易日期集 **== **日线交易日期集，任一方缺日 → **skip**（防日线陈旧缺行、盘中却有该日 = 跨周期漏日）；(c) 该日期集上 1m 聚合日 OHLCV vs 日线 bar 容差内一致（价格 `1e-6`、量精确）。任一不满足 → **fail-closed skip 该股** + 日志。 | 防 R6-F2（不同复权/陈旧 → 值不一致）+ **R10-F1（只查交集会放过日线缺行）**：1m 与日线是两个独立源，日/周/月从日线来、盘中从 1m 来；须**日期集相等 + 值一致**双门，否则产出"盘中含某日、日/周/月漏该日 / close 不符"的跨周期不一致训练组。 |
...
- **源一致性对账（D10，codex R6-F2 / R10-F1）**：import 某股前——(a) 查 `export_log` 该股 1m+日线 `status=='ok'`；(b) **对称日期集相等门**：在**共享覆盖窗口** `[max(1m_dense起, 日线起), min(1m_dense止, 日线止)]`（均经 `trading_date`）内，**dense-1m 交易日期集 == 日线交易日期集**——任一非对称差（1m 有该日但日线缺 / 日线有该日但 1m 不足）→ **fail-closed skip 该股**（否则盘中含某日、日/周/月漏该日 = 跨周期不一致；D10 原只查交集会放过陈旧日线的缺行）；(c) 在该（已相等的）日期集上对账「1m 聚合日 OHLCV」vs「日线 bar」（价格相对 `1e-6`、volume/amount 精确或极小容差）→ 超容差 skip。可选：`export_log` row 数/`last_time` 对交易日历。纯对账逻辑（给定两 df + 容差）可 host 单测。
...
- **B1 源一致性对账（D10，R6-F2/R10-F1）**：**对称日期集**——1m 有某交易日但日线缺该行（陈旧日线）→ **skip**；日线有某日但 1m<241 → skip；**值对账**——日期集相等但某日 close/volume 超容差 → skip；集相等且值一致 → 放行；`export_log` `status=error/partial` → skip（纯对账逻辑 host 单测）。
...
- P/F（D10，R6-F2/R10-F1）：共享窗口 1m/日线**交易日期集不相等**（任一方缺日）→ skip；OHLCV 超容差 → skip；`export_log` 非 `ok` → skip；集相等且值一致放行。
