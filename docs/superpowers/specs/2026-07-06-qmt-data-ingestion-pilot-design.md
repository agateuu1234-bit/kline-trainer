...
| **D5**（codex R2-F2 / R16-F2） | pilot **保持 `training_sets.file_path` 绝对路径**（现状 B3 下载/scheduler/`backfill` 均按绝对 `Path(file_path)` 读）；**列 `VARCHAR(255)→TEXT`**（随 D1 migration，绝对长路径不撑爆）+ **登记失败删 orphan `.zip`**。不在 pilot 半途改相对（只改 B2 写=下载 404）。 | 相对化=跨 B2/B3/scheduler/backfill 横切 + 存量迁移，归未来 NAS 搬迁（§7）；TEXT 防长绝对路径登记失败留 orphan（R16-F2）。 |
...
| **D11**（codex R17-F1） | **持久化 B1→B2 覆盖契约**：新增 `stock_coverage` 侧表（`stock_code` PK、`dense_1m_start_date`、`dense_1m_end_date`、`dropped_1m_dates`（内部 drop 交易日 JSON，通常 `[]`）、`dense_day_count`）。**B1** resample 后写入权威 dense 1m 覆盖；**B2 D2** 从此表读 `dense_dates`（= `[start,end]` 交易日 − `dropped_1m_dates`），**非从 klines 反推**。新表 forward=CREATE / rollback=DROP（可逆对，不丢既有数据、合规 m01）。 | 防 R17-F1：1m 不入库后 B1 的权威 dense 判定丢失、B2 机会主义反推会漂移且失败难诊断。持久化成一等 artifact → B2 消费权威决定、可诊断、跨 retry/周期变更稳定。 |
...
└──────────────────────────────────────────────────────────────────┘
  ...
  compute_indicators  ♻复用（每周期 MA66/BOLL(ddof=0)/MACD(bar×2)，在全精度 close 上算，仍存 round(4/6)）
      ▼
  写 PG klines  ♻复用写库壳（OHLC→DOUBLE；amount/指标列不变；周期集合 6 个；ticket_index 停写留列）
      ▼
...
- **不再计算/写 `ticket_index`**（删 `compute_ticket_index` 函数、`_KLINE_INSERT` 去该列；**列保留、新行 NULL**；D3/R12-F1）。
...
- **`resample_intraday(df_1m, minutes)`** — 按 1m 收盘时间戳 `t`（北京 wall-clock）分桶，**严禁跨午休/跨日**（codex R2-F3：精确规则在 spec 定死，非 plan 延后）：
  ...
  - **分钟级完整性在此强制（B1，1m 在手；codex R4-F1）**：每桶实测 1m 成员数须 == 上面 golden 应有数。**某交易日任一桶不足 → 该日全部盘中周期不入库（drop + 日志），不写半成品桶**。该股 **dense 1m 覆盖带** = 盘中逐日完整的连续日范围（QMT 覆盖-截断的首/末残缺日自然落在带外）。**1m 出库后 B2 已无从判分钟级，故必须在此判**——**B1 把该权威 dense 覆盖（band `[start,end]` + `dropped_1m_dates` + `dense_day_count`）写入 `stock_coverage` 表（D11，R17-F1），供 B2 D2 读，不让 B2 反推**。
...
- **缺日单一不变量（codex R13-F2，全或无、不补零/插值）**：对**有日线 bar 或有任何盘中 1m 的日期**——要么**全部** `3m/15m/60m` 桶都完整（241 根 flat-fill），要么**该日全部盘中周期整体 drop 且踢出 `dense_dates`**；**不允许"缺一桶只丢那一桶、其余照发"的半日 partial**（否则 PG 留下日线-vs-盘中不一致，B2 仅当该日入选窗口才抓、其它消费者/后续选择看不到）。**仅真非交易日（无日线 bar）**才无盘中（正常）。→ **B1 保证 PG 永无 partial 盘中日**。
...
- **`training_sets.file_path VARCHAR(255) → TEXT`（随同一 migration，codex R16-F2）**：D5 存绝对路径，长 temp/NAS/容器挂载根会超 255 → ZIP 建成但登记 INSERT 失败、留 orphan + pilot 晚失败/非幂等。改 `TEXT`（无长度限）随本 DDL migration 一并 forward/rollback（同 `CONTRACT_VERSION` bump、无额外触发；`VARCHAR(255)→TEXT` 是扩容、rollback 需保证现存值 ≤255 或标注）。
- **新增 `stock_coverage` 侧表（D11，codex R17-F1）**：`CREATE TABLE stock_coverage (stock_code TEXT PRIMARY KEY, dense_1m_start_date DATE, dense_1m_end_date DATE, dropped_1m_dates TEXT DEFAULT '[]', dense_day_count INTEGER)`——随本 migration forward=CREATE / rollback=DROP（可逆对，新表不丢既有数据、合规 m01 §Migration Rollback）。B1 写、B2 读（见 §4.2/§4.4）。
...
- `select_start_index(monthly_datetimes, rng, *, dense_dates)`（D2，codex R7-F1）：候选月线下标先按现规则 `[30, n-9]`，**再过滤**「从 `start` 的**交易日期**到 `after_end` 的交易日期，其间（按日线日历）**每个交易日期都在该股 dense 完整 1m 交易日期集 `dense_dates` 内**」；候选为空 → `GenerateSkipException`。**按交易日期比对、非 raw 时间戳**——月/日 bar 标 `00:00`、3m 标盘中(`0933..1500`)，`start`(00:00) < 首根 3m(0933)、`after_end`(≈23:59) > 末根 3m(1500)，raw 时间戳比对会**误杀全部有效候选、产出 0 训练组**。`dense_dates` **从持久化 `stock_coverage` artifact 读**（D11，codex R17-F1）——B1 分钟级完整性判出后写入的**权威 dense 1m 覆盖**，**非从盘中 klines 机会主义反推**（反推会在边角/retry/周期变更下与 B1 决定漂移、且失败难诊断）。**端点必要非充分**（内部洞仍需 D9），完整性由 D9 per-day 桶数硬门兜底（与 artifact 一致性另测）。
...
- `file_path`（D5）：pilot **保持绝对路径写入**，不改现有 B3 `routes.py` 下载 / scheduler / `backfill_content_hash` 的 `Path(file_path)` 读取契约（相对化归 §7 未来搬迁）。列改 `TEXT`（R16-F2，绝对路径任意长）；**登记 INSERT 失败 → 删已建的 `.zip`/`.db`**（防 orphan、保幂等重试）。
...
- **B1 分钟级完整性（R4-F1/R9-F2/R13-F2）**：桶内缺单根/散布缺多根 1m 的**在库交易日**（日线在）→ **该日全部盘中周期整体 drop（非只丢缺的那一桶）+ 记异常日志 + 踢出 `dense_dates`**；构造某日缺一桶 → 断言 **PG 该日盘中行数=0**（永无 partial 盘中日）；停牌**整日缺席**（1m+日线都无）→ 不计入、不误判；1m 覆盖边界 partial 日（199 根）→ dense 带排除；完整日（241）照常 80/16/4 入库。
...
- **B1→B2 覆盖 artifact（D11，R17-F1）**：B1 写 `stock_coverage`（band+dropped+count）；**B2 D2 从该表读 `dense_dates`、断言不从 klines 反推**（mock 一个 klines 与 artifact 不一致的 case → D2 以 artifact 为准 + D9 一致性另测捕获漂移）。
...
- **file_path 长度/orphan 清理（R16-F2，集成测试）**：超长绝对 output 根 → `file_path`(TEXT) 登记成功（不截断/不失败）；模拟登记 INSERT 失败 → 已建 `.zip`/`.db` 被删（无 orphan、可幂等重试）。
...
- P/F：`schema.sql` `klines.open/high/low/close` 为 `DOUBLE PRECISION`；`ticket_index` 列**保留**（停写、新行 NULL——非删列，R13-F1/R12-F1）；amount/指标列不变；空库应用可建成。
...
- P/F（D11，R17-F1）：B1 写 `stock_coverage`（band+dropped+count）；B2 D2 **从 artifact 读 dense_dates**（非反推）；artifact 与 klines 不一致时以 artifact 为准 + 一致性测捕获漂移。
...
- P/F（R16-F2）：`training_sets.file_path` 列为 `TEXT`（随 D1 migration）；长绝对路径登记成功不失败；登记失败清理 orphan `.zip`。
