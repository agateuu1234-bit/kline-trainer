...
- 改：`backend/sql/schema.sql`（`klines.open/high/low/close` → `DOUBLE PRECISION`、去 `ticket_index`）、`backend/generate_training_sets.py`（`select_start_index` 加 1m 覆盖带约束、窗口覆盖完整性校验 D9）、`backend/import_csv.py`（接规整/合成层、取消 `ticket_index`、1m 不入库、写前 schema fail-closed 断言 D8a）。
