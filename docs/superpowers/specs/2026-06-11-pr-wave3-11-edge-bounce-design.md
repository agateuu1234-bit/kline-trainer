...
### In scope（本 PR）
1. **`EdgeBounceModel`**（新纯值类型，`Sendable`/`Equatable`，零 UIKit/零几何）：在注入 offset 边界上的「惯性减速 → 越界临界阻尼弹簧 → 回弹钉边界」完整轨迹物理。**界内减速段直接复用既有 `DecelerationModel.advance`**（不重写积分，parity，codex R7-F1），越界段用临界阻尼解析弹簧（ζ=1）。
...
2. 每帧 `advance(dt) -> FrameOutcome`。记 `frameEntry = offset`；**每个 outcome 的 delta = `offset - frameEntry`**（该 tick 全位移，含同 tick 内减速+弹簧+snap，codex R4-F1：终止帧不丢早期子段位移）。异常 dt（`dt≥1.0`/`dt≤0`）：归一（越界→外溢 `edge-frameEntry`，界内→nil）+ `.finish(.., notifyFinish:true)`（与既有 `DecelerationModel` `dt≥1.0→.stop→onFinish` 一致，codex R6-F3；静默仅留显式 `stop()`/`resetOnSceneActive()`）。否则分两相：
   - **界内减速相（直接复用 `DecelerationModel.advance(dt)`，codex R7-F1）**：**不重写减速积分**——复用既有 `DecelerationModel.advance`（其 per-call 余量即时积分、partition-dependent 是**既有 shipped 行为**）。∴ bounce 的界内减速与**普通 fling 逐帧字节相同**（P4/P7 严格 parity，非「容差内」），且**不比普通滚动更帧依赖**。`.move(d)`：`offset+=d`；若 `offset` 跨出 `[min,max]` → 切弹簧相 seed（`overscroll=offset−edge`〔= DecelerationModel 本帧带过 edge 的真实过冲〕、`velocity=DecelerationModel.velocity`），**本帧仍报 `.move(offset−frameEntry)`**，弹簧自**下帧**起。`.stop`（界内）→ `.finish(nil, true)`（= 既有自然停 + onFinish）。
     - **放弃 decel 相分区不变的追求（codex R7-F1 根因）**：partition-invariant decel 与 DecelerationModel parity **互斥**（后者本就 partition-dependent）。选 parity：bounce 不引入异于普通 fling 的减速行为；分区不变性**只 claim 弹簧相 state**（见 P3）。
...
| P3 | **帧率无关 = 弹簧相 state（codex R1-F3/R5-F1/R7-F1/R7-F2）**：**仅 claim 弹簧相 state 分区不变**（解析闭式 → 任意分区精确）。**减速相不 claim 分区不变**——复用 `DecelerationModel`（partition-dependent 是既有 shipped 行为，与普通 fling 同，不更差，codex R7-F1）。**回调时序 ≤1 帧有界**（codex R7-F2）：模型 zero-crossing state/event-time 解析精确，但 onFinish/terminate 在含 `t_zc` 的 display tick 触发 → 观测 ±1 帧（120Hz 8.3ms vs 60Hz 16.7ms），非严格帧无关 | **模型层**（fake dt，暴露 consumed/event-time）：弹簧 state 60/120/不规则/亚-refInterval → 严格浮点容差；zero-crossing event-time 解析精确；峰值穿透+逐采样轨迹严格容差。**动画器层**：终止 onFinish 帧 ±1 帧有界（不断言严格帧时刻）；减速相 = DecelerationModel parity（不测分区不变） |
| P4 | **界内 = 纯减速（严格复用 DecelerationModel，codex R6-F1/R7-F1）**：起点界内速度不足达边 → **直接复用 `DecelerationModel.advance`**（不重写积分）、零弹簧 → 与普通 fling **逐帧字节相同** | 与同参 DecelerationModel **逐帧 delta 严格相等**（直接复用，非近似/容差）；edge 判定与普通 fling 同；无越界 |
...
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **decel parity（P4，codex R7-F1）**：界内减速 = **直接复用 `DecelerationModel.advance`** → 与同参 `DecelerationModel` **逐帧 delta 严格相等**（非容差，非分区不变——decel 继承既有 partition-dependence，与普通 fling 同）；
