...
2. 每帧 `advance(dt) -> FrameOutcome`（分两相，子步级 handoff）：
   - **界内减速相**：把 dt 按 **refInterval 切子步**（同 `DecelerationModel`），每子步用共享指数衰减规则推进；**逐子步检查是否跨 bounds**。界内自然停（速度 < 阈值，仍在界内）→ `.finish(nil, true)`（无弹簧，P4 与 DecelerationModel 逐帧等价）。
   - **子步级过边 handoff（codex R2-F2，关键）**：在**跨边的那个子步**（时长 ≤ refInterval）把相位切到弹簧，seed 弹簧于 **edge**（`overscroll=0`）、`velocity=` 该子步衰减末速度，再对**该帧剩余时间**（含该子步余下 + 后续子步）走解析弹簧推进。∵子步切分跨帧率一致（60Hz `dt=1/60` = 2×refInterval 子步，与 120Hz 同），spring-start seed **帧率无关至 refInterval 粒度**——消除「整帧 decel 后才切相」造成的 seed 帧率依赖（6.8pt@120Hz vs 14.2pt@60Hz）。
   - **弹簧相**（临界阻尼 ζ=1，**解析闭式传播**，非 Euler 子步——根治 codex R1-F3）：`ω=√k`；由当前 `(x,v)` 解析推进时长 τ：`A=x`、`B=v+ω·x`、`e=exp(-ω·τ)`，则 `x'=(A+B·τ)·e`、`v'=(B(1-ω·τ)-ω·A)·e`。解析解逐段重锚 → **τ 切分精确无关**（推进 τ 一次 ≡ 推进 τ/2 两次，浮点容差内）；已在弹簧相的后续帧直接对整 dt 解析推进一次。
   - **首次过边 clamp（codex R1-F2）**：若弹簧推进使 `overscroll` 跨 0（符号翻转或落到 0）→ clamp 到 edge（overscroll=0）+ settle，**不进内侧**——保证任意初速度（含强内向）均不穿越（见 P2）。残余内向动量在 edge 被吸收（回弹止于 edge；把动量带进内侧滚动属未来增强，out of scope）。
   - **settle**（`|overscroll|<posTol && |velocity|<velTol`，或首次过边 clamp）→ `.finish(snapDelta=edge-offset, notifyFinish=true)`：**同一 tick** 外溢钉边界 delta（offset 精确落 edge）+ 终止 + 触发 onFinish。
3. `FrameOutcome` 处置（动画器 `handleTick`，原子）：`.move(d)` → `onUpdate(d)`；`.finish(fd, nf)` → `if let fd, fd != 0 { onUpdate(fd) }` + `isDecelerating=false` + 失活 driver + `if nf { onFinish?() }`。**末 delta 与停在同一 tick 完成**（P8）。
...
| P3 | **帧率无关至 refInterval 粒度**：弹簧相解析闭式 → 精确无关（R1-F3）；**过边 handoff 在 refInterval 子步解析（R2-F2）→ handoff 速度/时刻帧率无关至 refInterval**；界内减速相沿用既有 `DecelerationModel` refInterval 子步契约 | **(a)** 已活跃弹簧相：60/120Hz/不规则/亚-refInterval → 末 offset+峰值穿透 **严格浮点容差** 内一致；**(b) 端到端（始界内·跨边·回弹）— 比**瞬态量**（codex R3-F3：seed=0/末=edge 是恒等不可作判据）**：60/120Hz/不规则 → **handoff 速度 + 跨边时刻 + 峰值穿透 + settle 时长 + 逐采样轨迹** 在 refInterval 子步容差内一致 |
...
| P8 | **bounce 终止归一（codex R1-F1，原子，codex R3-F1）**：越界中途非自然 settle 终止 → **同一 tick** 外溢 edge-归一 delta + 失活 + **静默不触发 onFinish**（`.finish(edgeNormDelta, notifyFinish:false)`） | 越界中 `resetOnSceneActive()`/大 dt（`dt≥1.0`）→ 单 tick 内 offset 落 edge + driver 失活 + onFinish **未**调；re-grab `stop()` → 保位（见 §五） |
...
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **端到端分区不变（P3b，codex R2-F2/R3-F3）**：**始界内·跨边·回弹**完整轨迹，60/120Hz/不规则 → 比**瞬态量**（**handoff 速度 + 跨边时刻 + 峰值穿透 + settle 时长 + 逐采样轨迹**，**非** seed/末态恒等值）在 refInterval 子步容差内一致（直证 handoff 非整帧漂移：codex 反例「1pt from edge, v=1000」下 handoff 速度不再 60/120Hz 异）；
