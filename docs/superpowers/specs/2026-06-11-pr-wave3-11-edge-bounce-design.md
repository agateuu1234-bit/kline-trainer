...
2. 每帧 `advance(dt) -> FrameOutcome`。记 `frameEntry = offset`；**每个 outcome 的 delta = `offset - frameEntry`**（该 tick 全位移，含同 tick 内减速+弹簧+snap，codex R4-F1：终止帧不丢早期子段位移）。异常 dt（`dt≥1.0`/`dt≤0`）：归一（越界→外溢 `edge-frameEntry`，界内→nil）+ `.finish(.., notifyFinish:true)`（与既有 `DecelerationModel` `dt≥1.0→.stop→onFinish` 一致，codex R6-F3；静默仅留显式 `stop()`/`resetOnSceneActive()`）。否则分两相：
   - **界内减速相（boundary-aware advancement，复用 damp-then-move 律 + 子帧跨边，codex R7-F1 + R8-F1）**：减速**用 `DecelerationModel` 既有 damp-then-move 律**（parity），但**不**用聚合 `advance(dt)`（聚合只在帧末报位移 → 跨边晚一帧 → seed 为**帧率依赖的过冲**：5pt away 下 120Hz≈2.83pt vs 60Hz≈10.20pt，bounce-specific 可见差，codex R8-F1）。改用**新增 boundary-aware 推进 API**（§二，**additive 加在 `DecelerationModel`**，既有 `advance(dt)` 与全测不动）：跑同一子步循环，**子步内若跨 `[min,max]` 则停在 edge**（用 damp-then-move 在该子步求精确跨边点 + 跨边速度），报 `(crossingVelocity, remainingTime)`，其中 **`remainingTime` 是帧相对量**（`= dt − 帧起算的已消耗时间`，**含跨边前所有完整子步 + 跨边子步内消耗**，codex R10-F1）——**不可**用「跨边子步内子时」当帧剩余（否则 60Hz 在第 2 子步跨边会比 120Hz 多喂弹簧 1/120s）。bounce 模型据此 **seed 弹簧于 edge（overscroll=0）、velocity=crossingVelocity，对 `remainingTime` 走弹簧**（消除帧率依赖过冲 + 帧率依赖弹簧时长）。
     - **无跨边时严格 1:1 relay `DecelerationModel` Outcome（含 move-then-stop 时序，codex R9-F1）**：boundary-aware 推进无边界事件时 = 既有 `advance`，其 Outcome **逐帧原样映射**：`.move(d)→.move(d)`；`.stop→.finish(nil, true)`。**关键**：`DecelerationModel` 在速度跌破阈的那帧仍可先返 `.move(累积位移)`、把 `.stop` 推迟到**下一帧**——bounce **必须照搬这个两帧序列**，**不得**在阈值帧提前 `.finish`（否则非 bounce fling 经 bounce API 会早一帧失活 + 早一帧 onFinish，破坏 P4/P7 lifecycle 时序 parity）。∴ `isDecelerating` 翻转帧 + onFinish 触发帧与普通 fling **逐帧一致**。
...
| P4 | **界内无跨边 = DecelerationModel parity（含 lifecycle 时序，codex R6-F1/R7-F1/R9-F1）**：无跨边 → 1:1 relay `DecelerationModel` Outcome（`.move→.move`/`.stop→.finish(nil,true)`），**含 move-then-stop 两帧序列**（阈值帧不提前 `.finish`） | 无跨边时与同参 DecelerationModel **逐帧 delta 严格相等** + **`isDecelerating` 翻转帧 + onFinish 触发帧逐帧一致**（非早一帧）；edge 判定同；无越界 |
...
- **`DecelerationModel` boundary-aware 测试（codex R8-F1/R10-F1）**：新 `advance(dt:within:)` —（a）**无跨边时与 `advance(dt:)` 逐帧 delta 严格相等**（P4/P7 reduce 证）；（b）跨边时停在 edge、报 `(crossingVelocity, remainingTime)`，**`remainingTime` 帧相对**（手算对照含跨边前完整子步）；（c）**子帧跨边帧无关（含多子步，codex R10-F1）**：edge **10pt away（跨边落在第 2 子步，非首子步）**、v=1000，60Hz（dt=1/60=2 子步）vs 120Hz → 报告 crossing state（offset=edge）一致 + **`remainingTime` 一致**（驳「子步内子时当帧剩余」致 60Hz 多喂 1/120s）+ 据此 seed 弹簧的 end-of-frame state 一致。**既有 `DecelerationModelTests` 不改全过**（既有 advance 字节不动）。
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **端到端跨边帧无关 seed（P3 端到端，codex R8-F1/R10-F1）**：**始界内·跨边·回弹**，60Hz vs 120Hz vs 不规则 → seed overscroll **≈0** + 峰值穿透/end-of-frame state 在容差内一致；**必含跨边落在第 2+ 子步的 case（edge 10pt away、v=1000）**——直证 `remainingTime` 帧相对（驳 60Hz 第 2 子步跨边多喂弹簧 1/120s）；
