...
### In scope（本 PR）
1. **`DecelerationModel` additive 累加器 boundary-aware 推进（方案 A，codex Plan-R8-F1）**：新增 `advance(dt:boundaryDistance:)` + additive `carry` 字段（**固定 refInterval 步 + 跨调用携带余量** → 帧率/分区无关）；子步内跨边即停 edge、报 `(crossingVelocity, remainingTime)`。**既有 `advance(dt:)` 与全部 `DecelerationModelTests` 逐字不动**（`carry` 默认 0、既有路径不读它）。
2. **`EdgeBounceModel`**（新纯值类型，`Sendable`/`Equatable`，零 UIKit/零几何）：注入 offset 边界（分离端点）上的「减速（累加器 boundary-aware）→ 子帧跨边 seed 弹簧于 edge → 临界阻尼解析弹簧 ζ=1 → 回弹钉边界」完整轨迹物理。
...
2. 每帧 `advance(dt) -> FrameOutcome`。记 `frameEntry = offset`；**每个 outcome 的 delta = `offset - frameEntry`**（该 tick 全位移，含同 tick 内减速+弹簧+snap，codex R4-F1：终止帧不丢早期子段位移）。异常 dt（`dt≥1.0`/`dt≤0`）：归一（越界→外溢 `edge-frameEntry`，界内→nil）+ `.finish(.., notifyFinish:true)`（与既有 `DecelerationModel` `dt≥1.0→.stop→onFinish` 一致，codex R6-F3；静默仅留显式 `stop()`/`resetOnSceneActive()`）。否则分两相：
   - **界内减速相（持久固定步累加器，user 2026-06-12 裁决方案 A，codex Plan-R8-F1）**：减速**用 `DecelerationModel` damp-then-move 律**，但用**固定 refInterval 步 + 跨 `advance` 调用携带时间余量 `carry`**（additive 字段，既有 `advance(dt)` 不碰）：物理推进与帧边界解耦 ⇒ **到达边界的速度/时刻在任意帧率/分区（含不规则、亚-ref）精确无关**（根治 Plan-R8-F1：聚合复用 `advance(dt)` 使回弹峰值随帧率/速度漂移，v=5000 下 60/120Hz 差 ~5pt）。每固定步检查跨边：子步内匀速 → 解析求跨边子时 `tWithin`，报 `(crossingVelocity, remainingTime= carry−tWithin)`（帧相对，含未消耗余量）。bounce 据此 **seed 弹簧于 edge（overscroll=0）、velocity=crossingVelocity，对 `remainingTime` 走弹簧**。
     - **方案 A 取舍（P3 vs legacy parity，二者对 bounce 减速互斥，codex Plan-R7-F1/R8-F1）**：选 **P3 帧率无关回弹**（磨光初衷）。代价：bounce 减速与既有 `advance(dt)` 仅相差 **sub-refInterval 余量延迟**（< 一固定步、≤8ms 位移，不可感知，P4 within-substep）。**既有 `start(initialVelocity:)` + `DecelerationModel.advance(dt)` 路径零改动、逐帧字节不变（P7）**——累加器仅服务 bounce 路径。`carry` 默认 0、既有 `advance(dt)` 不读它。ref 对齐（dt==refInterval）下累加器与 `advance(dt)` 仍逐帧相等（含 move-then-stop 时序，lifecycle parity）；非 ref 对齐则差余量延迟。
...
3. **共享终止 handler `terminate(finalDelta:notifyFinish:)`（re-entrancy-safe，codex R4-F3 + R5-F3 + R9-F2）**——`handleTick` 的 `.finish` **与** `resetOnSceneActive` 的越界归一**都经此唯一路径**（不得各写一份，否则 reset 绕过保护）：
   1. **先脱离本 run 并 bump run-identity epoch**：`epoch += 1; let myEpoch = epoch`；`isDecelerating=false`；invalidate + nil driver；
   2. **再**回调 `if let fd, fd != 0 { onUpdate(fd) }`（此回调可能重入 `start()`/`stop()`/`resetOnSceneActive()`）；
   3. **仅当 `epoch == myEpoch`**（未被任何重入的 run-ending op 改动）才 `if notifyFinish { onFinish?() }`。
   `.move(d)` → `onUpdate(d)`（既有语义不变）。**run-identity epoch 由每个 run-ending/starting op 都 bump**：`start()`（新 run）、`stop()`（中断）、**`terminate()` 自身（含 reset 路径，codex R9-F2）**——∴终止帧 onUpdate 里重入 `start()`/`stop()`/**`resetOnSceneActive()`** 任一都使 `epoch≠myEpoch`，旧续延的 onFinish 被抑制（reset 静默语义守住）。epoch 独立于既有 `generation` 的 stale-driver 用途（P7 不破）。`resetOnSceneActive` 越界 → `terminate(edge-offset, notifyFinish:false)`；界内 → `terminate(nil, false)`（= 既有静默 reset）。
...
| P6 | **防御（可达 + 分层 + round-trip 固定 cap，codex R7-F3/R10-F2/Plan-R11-F1/R15-F1）**：分离端点 CGFloat（非 `ClosedRange`，后者 NaN 构造即 trap）→ 模型内校验。**offset/bounds 非有限** → 安全停；**仅 velocity 非有限 + offset/bounds 有限** → 净化 0、越界仍归位；**越界归一修正须 round-trippable**（`abs(offset+(edge−offset)−edge)` ≤ **固定亚像素 cap 1e-3** 且 `8·edge.ulp ≤ cap`，否则 consumer 与 model 失步 → inert 拒绝；**固定 cap 不随 edge 量级膨胀**，杜绝大 edge 撑开容差放过点级失步）；绝不外溢 NaN/inf delta | NaN/inf `min/max/offset` → 无 trap；`min>max` → 无 bounce；NaN/inf velocity + 有限越界 → spring 归位；**任意点级失步 → inert（shouldRun false）：量级悬殊（MAX/2 vs 10）或 edge 大到 `8·ulp > cap`（0.99MAX/±1e15/1e21 等）一律拒**；**普通几何（`|edge| ≪ ~10¹¹` 点，1-ULP 噪声 ≪ cap）→ 正常运行、consumer 真到 edge** |
...
| P9 | **回调 re-entrancy 安全（codex R4-F3 + R5-F3 + R9-F2）**：`handleTick.finish` **与 `resetOnSceneActive` 归一** 都经**共享 `terminate`**：脱离 run 时 **`terminate`/`start`/`stop` 均 bump epoch**；onFinish 仅在 `epoch==myEpoch` 时触发 | 终止帧 onUpdate 内重入 `start()` → 新 run 不被旧续延 invalidate + 旧 onFinish 不触发；`stop()` → onFinish 不触发；**`resetOnSceneActive()`（自然 + abnormal-dt 终止帧两路）→ epoch bump → 旧 onFinish 不触发**（reset 静默守住，codex R9-F2） |
...
- **非有限输入（分层处置，codex R10-F2）**：
  - **offset 或 bounds 端点非有限**（几何本身无效）→ 安全停（无 bounce、不外溢非有限 delta，几何无效无可归一目标）。
  - **仅 velocity 非有限、offset+bounds 有限**：`velocity` 净化为 0（mirror `DecelerationModel` isFinite 守门精神）；**若 offset 越界 → 仍 spring 归位/normalize**（**不**因坏速度 strand 一个有效的越界 offset，否则 wiring 后可见滞留）；offset 界内 → 安全停。
...
- **`DecelerationModel` 累加器 boundary-aware 测试（方案 A，codex Plan-R8-F1/R10-F1）**：（a）**ref 对齐**（dt==refInterval）下与 `advance(dt:)` 逐帧相等（无 partial 余量）；（b）**累加器分区不变**：同 elapsed、不同 dt 分区（ref / ref/3 / 不规则）→ velocity + 累计位移一致；（c）**within-substep parity**：累加器总滑行与 `advance(dt:)` 差 < 一固定步位移；（d）跨边停 edge、报帧相对 `remainingTime`；（e）多子步跨边帧无关（60Hz vs 120Hz 跨边速度/remainingTime 一致）。**既有 `DecelerationModelTests` 不改全过**（`carry` 默认 0）。
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **非有限 velocity 不 strand 越界（P6，codex R10-F2）**：offset/bounds **有限且 offset 越界** + `velocity=NaN/Inf` → 净化 velocity=0 + **spring 归位至 edge**（**非**安全停滞留越界）；offset/bounds 非有限才安全停（model + 动画器两层测）；
...
- **`DecelerationAnimator` bounce 测试**：注入 fake `FrameDriving` + fake dt（复用既有 DD-1/DD-5 缝）；验证 bounce 路径外溢 delta 序列累加后 offset 落 edge、自然 settle `onFinish` 恰一次、generation/stale 与既有同守；**无跨边 lifecycle parity（P4，codex R9-F1）**：非 bounce fling 经 bounce API → `.move`-then-`.stop` 两帧序列、`isDecelerating` 翻转帧 + `onFinish` 触发帧与 `start(initialVelocity:)`/`DecelerationModel` **逐帧一致**（不早一帧）；**P8 原子终止 + onFinish 一致（codex R3-F1/R6-F3）**：越界中 `dt≥1.0` → **同一 tick** 外溢 edge-归一 delta + 失活 + **onFinish 触发**；越界中 `resetOnSceneActive()` → 外溢 edge-归一 delta + 失活 + **onFinish 静默**；re-grab `stop()` → 保位不归一；**P9 回调 re-entrancy（codex R4-F3 + R5-F3 + R9-F2）**：终止帧 onUpdate 内重入 `start()` → 新 run 存活 + 旧 onFinish 不触发；`stop()` → onFinish 不触发；**`resetOnSceneActive()` 重入（自然 settle 帧 + abnormal-dt 终止帧两路）→ epoch bump → 旧 onFinish 不触发**（reset 静默守住）；**回调时序 ≤1 帧有界（codex R5-F1/R7-F2）**：弹簧 zero-crossing 模型 state 精确落 edge，动画器 onFinish 在含 `t_zc` 的 display tick 触发 → 断言 **±1 帧有界**；**既有 `DecelerationAnimatorTests` 不改全过**（P7 回归门）。
...
| R9 | branch-diff | needs-attention（**2 med，收敛中**） | **F1[med]** 无跨边路径在阈值帧提前 `.finish` → 比 DecelerationModel 的 move-then-stop 早一帧失活/onFinish，破坏 lifecycle parity → §三改**1:1 relay Outcome（保 move-then-stop 两帧）** + P4 含 isDecelerating/onFinish 时序 parity + 动画器 lifecycle 测。**F2[med]** epoch 仅 start/stop bump，终止帧 onUpdate 重入 `resetOnSceneActive()` 不改 epoch → 旧续延误触 onFinish 破 reset 静默 → §三.3 **`terminate` 自身（含 reset）bump epoch** + P9 + reset 重入测试（自然 + abnormal-dt 两路）。 |
| R10 | branch-diff | needs-attention（**1 high + 1 med，精度细节**） | **F1[high]** `t_cross` 取子步内子时但当帧剩余 `dt−t_cross` → 60Hz 第 2 子步跨边比 120Hz 多喂弹簧 1/120s（5pt 测首子步跨边漏检）→ boundary-aware 改报**帧相对 `remainingTime`**（含跨边前完整子步）+ 多子步测试（edge 10pt away）。**F2[med]** 仅 velocity 非有限 + offset/bounds 有限且越界 → 「安全停」strand 有效越界 offset → §五/P6 分层：净化 velocity=0 + 越界仍归位 + 测。 |
| R11 | branch-diff | **approve** | 无 material finding；spec 契约收敛（防御生命周期/边界/数值/re-entrancy 契约齐备）。ledger 写入。 |

**plan-stage 决策（2026-06-12，user 裁决）：** plan codex R1-R7 收敛后，**R8** 量化指出复用聚合/per-call boundary-aware 推进使**回弹峰值随帧率/速度漂移**（v=5000 下 60/120Hz 差 ~5pt）——P3 端到端帧率无关与「bounce 减速逐字节 match legacy fling」**互斥**（codex 多轮在二者间振荡）。**user 裁决方案 A**：减速段改**持久固定步累加器**（帧率无关回弹，磨光初衷），代价 bounce 减速与 `advance(dt)` 仅差 sub-refInterval 余量延迟（P4 within-substep，不可感知）；既有路径零改动。同步改 §三减速段 + P3 + P4 + §二.1 + §六测试；并授权：若 codex 就此 reuse-vs-accumulator 点继续振荡（permanent-bias）则 accept+override + admin merge。

**方案 A 后续 codex 实现修（均非振荡、真 bug）：** R9 端到端测试改比共时 state（帧采样峰值有 ~0.4pt 采样假差）；R10 累加器 `carry >= refInterval` 浮点丢步 → ULP 容差 + 残留 clamp；R11 越界归一修正须 round-trippable（`offset+(edge−offset)==edge`，量级悬殊如 MAX/2 vs 10 失步）；R12 (a) round-trip 校验**减速相也查**（减速跨边后 snap model 而累积 delta 失步），(b) exact-edge handoff 改 `need==0` 用**已衰减速度** seed（与 edge-ε 极限连续，消幅度跳变，删 `atOrPastOutwardEdge` 特判）；R13 round-trip 裸 `==` 误拒普通几何 → ULP-scaled 容差；R14/R15 容差随 edge 膨胀 → 改**固定亚像素 cap `1e-3` + `8·edge.ulp` fail-closed**（无 edge 量级杠杆）；R16 spec P6 同步固定 cap；R17 测试闸门加 `pipefail`；R18 init 只校验起点 offset → huge velocity 生成 enormous offset → `springStep` 加**动态 round-trip 安全**（钉 edge 不暴露不安全中间态）。

**plan-stage 收敛：** codex R1–R18 + **codex 配额耗尽（至 2026-06-13）→ opus 4.8 xhigh fallback APPROVE**（真实数值仿真确认 R18 fix 不误拒普通输入、无泄漏不安全 offset 路径、编译/P7 就绪）。PR/merge 时仍走 codex required-status-check（治理 backstop）。
