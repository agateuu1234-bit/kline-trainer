...
| P3 | **帧率无关（全程解析 + 事件时刻解析终止，codex R1-F3/R2-F2/R4-F2/R5-F1）**：减速相+弹簧相+过边 handoff 全解析闭式；**闭式事件**（减速 `t_stop`/`t_cross` + 弹簧 zero-crossing `t_zc=−A/B`）**在 dt 内按事件终止 → 终止时刻帧率无关**。**唯一放宽**：渐近 settle-threshold（超越无闭式）在满足阈值的帧边界终止 → **有界 ≤1 帧时序方差**（可见轨迹共时精确，仅 onFinish 帧 ±1） | **(a)** 弹簧相：60/120/不规则/亚-refInterval → 严格浮点容差；**(b) 端到端比瞬态量**（codex R3-F3）：**handoff 速度+跨边时刻+峰值穿透+逐采样轨迹** 严格容差；zero-crossing 终止时刻严格一致；**(c) 抖动 straddle**（codex R4-F2：7.8 vs 7.95，边界 7.9pt away）→ 解析下不改 handoff；**(d)** settle-threshold 终止帧 ±1 帧、共时状态严格一致 |
...
- **bounce 终止归一（codex R1-F1，核心）**：因动画器仅经 `onUpdate(delta)` 通信，越界中途**非自然 settle** 终止会把 `offset` 永久留在界外（wiring 后 → 重启/resume 不连续 + 反复 bounce）。bounce 模式下显式区分两类终止：
  - **`resetOnSceneActive()`（后台/恢复）+ 异常 dt（`dt≥1.0` 或 `dt≤0`）**：若正越界 → 经**共享终止 handler** `terminate(edge-offset, notifyFinish:false)`（§三.3，**先脱离 run 再回调**，re-entrancy-safe，codex R5-F3）外溢 edge-归一 delta 后终止，offset 落 edge，绝不滞留界外（P8），静默不触发 `onFinish`。**不得**另写归一路径绕过该保护。
...
**组件 bounds 契约（codex R5-F2）**：`EdgeBounceModel` 在 `start` **快照单一 `bounds`**（不订阅几何变更）——这是组件**刻意契约**（保持几何无关、隔离）。bounds 在 bounce 运行中保持新鲜是 **wiring 责任**：pinch（顺位 3）改几何须 normalize/stop 当前 bounce 或推新 bounds（§七 W3-11-R1 item 6）。
...
6. **pinch 改几何时 bounds 失效防护（codex R5-F2）**：bounce 在 `start` 快照单一 `bounds`；pinch（顺位 3）改 `visibleCount`/`candleStep` → 有效 offset 边界变 → 活跃 bounce 可能 snap 到**陈旧 edge**（可见跳变/非法 offset）。义务：**pinch-begin 原子 normalize/stop 当前 bounce（再改几何）或提供 bounds-update 机制** + **「bounce→pinch」集成测试**；
