...
2. 每帧 `advance(dt)`（分两相，子步级 handoff）：
   - **界内减速相**：把 dt 按 **refInterval 切子步**（同 `DecelerationModel`），每子步用共享指数衰减规则推进；**逐子步检查是否跨 bounds**。界内自然停（速度 < 阈值，仍在界内）→ `.stop`（无弹簧，P4 与 DecelerationModel 逐帧等价）。
...
| P3 | **帧率无关至 refInterval 粒度**：弹簧相解析闭式 → 精确无关（R1-F3）；**过边 handoff 在 refInterval 子步解析（R2-F2）→ spring-start seed 帧率无关至 refInterval**；界内减速相沿用既有 `DecelerationModel` refInterval 子步契约 | **(a)** 已活跃弹簧相：60/120Hz/不规则/亚-refInterval → 末 offset+峰值穿透 **严格浮点容差** 内一致；**(b) 端到端（始于界内、跨边、回弹）**：60Hz vs 120Hz vs 不规则 → 末 offset + spring-start overscroll 在 refInterval 子步容差内一致（非整帧漂移） |
...
|---|---|---|---|---|
| `beginPan`（re-grab） | `TrainingEngine.swift:524` | 保位（新手势接管） | 否（后续 pan/松手 bounce 收口） | `stop()` 保位语义不变 |
...
**组件 scope 非显现论证（直接答 codex「neither resolved nor tested」）**：本 PR **不把 bounce 接进 engine**（`TrainingEngine` 零改，§二范围判据），故**无任何 live 调用面会停一个真实 bounce**；且 `makeViewport` 当前仍在边缘 pin `pixelShift=0`（未渲染 overscroll，`RenderStateBuilder.swift:86`）→ 即便 offset 滞留界外也不可见。∴该 strand **在本 PR scope 内不可显现、不可测**（无 wiring 即无 live bounce）。`resetOnSceneActive` 的归一（P8）是组件**自身**生命周期所需，已实现 + 测；`activateDrawingTool` 的 normalize 需真实 caller，按 YAGNI 不预造公共 API，整体（normalize 取消 API + caller 改接 + 越界画线激活集成测试）**指派 W3-11-R1**，见 §七。
...
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **端到端分区不变（P3b，codex R2-F2）**：**始于界内、跨边、回弹**的完整轨迹，60Hz vs 120Hz vs 不规则 → spring-start overscroll + 末 offset 在 refInterval 子步容差内一致（直证 handoff 非整帧漂移：codex 反例「1pt from edge, v=1000」下 60/120Hz seed 不再 6.8 vs 14.2）；
