...
2. 每帧 `advance(dt) -> FrameOutcome`。记 `frameEntry = offset`；**每个 outcome 的 delta = `offset - frameEntry`**（该 tick 全位移，含同 tick 内减速+弹簧+snap，codex R4-F1：终止帧不得丢早期子段位移）。异常 dt（`dt≥1.0`/`dt≤0`）：**先归一**（越界 → 外溢 `edge-frameEntry`，界内 → nil）**再** `.finish(.., notifyFinish:true)`——**abnormal-dt 终止触发 onFinish，与既有 `DecelerationModel` `dt≥1.0→.stop→onFinish` 契约一致（codex R6-F3）**；静默无 onFinish **仅**保留给显式 `stop()`/`resetOnSceneActive()`（§五）。否则分两相：
   - **界内减速相（持久固定步累加器，codex R6-F1 + R4-F2）**：**复用 `DecelerationModel` 完全相同的 damp-then-move 律**（每步 `v*=friction^(step/refInterval)` 后 `delta+=v*step`，`step=refInterval`），但**跨 `advance` 调用持久累加 dt + 携带 sub-refInterval 余量**（非每帧重启子步、非连续积分）。∴ (a) **位移/达边判定与 `DecelerationModel` 同律一致**（消除连续积分 ~3% 偏差：v=1000 下 ~130.49pt 而非 134.61pt，edge@132pt 判定与普通 fling 一致，codex R6-F1）；(b) 固定步与帧边界解耦 → **任意帧分区/抖动下达边步一致**（R4-F2）。逐固定步检查跨 bounds + 速度阈：
     - **跨边**（某固定步使 offset 抵/越 edge）→ 在该步内线性求精确跨边子时（固定步 → 帧无关）→ **切弹簧相** seed 于 edge、velocity=跨边速度、对剩余时间解析弹簧推进（见下）。
     - **达停止阈**（`|v|<stopThreshold` 仍界内）→ `.finish(offset−frameEntry, true)`（界内自然停，无弹簧）。
     - 无事件 → 累加器推进可用固定步 → `.move(offset−frameEntry)`（余量留待下帧，offset 量化至固定步、帧无关）。
   ...
   - **事件时刻在 τ 内解析求解、按事件终止（codex R5-F1）**：**不**先推进整 τ 再判。先解析求**首次过边 zero-crossing** `t_zc=−A/B`（`B≠0` 且 `0<t_zc≤τ`，闭式）：若存在 → 在 `t_zc` 终止（offset 精确=edge，clamp，**不进内侧** P2/R1-F2），`.finish(offset−frameEntry, true)`，**终止时刻帧率无关**（消除「整帧后才判」的边界依赖）。残余内向动量在 edge 吸收（带动量进内侧滚动属未来增强，out of scope）。
   - **settle-threshold（渐近衰减无 zero-crossing 的角，超越方程）**：纯外向 `v0` 小/`x0>0,v0=0` 等 `x(t)` 永不过 0 仅渐近趋 0 → settle（`|x|<posTol && |v|<velTol`）时刻为**超越**无闭式 → 在**满足阈值的那个帧边界**终止（`.finish(offset−frameEntry, true)`）。**此终止有界 ≤1 帧时序方差**（P3 据此放宽，见 §四 P3）；可见轨迹（共时采样的 offset）仍解析精确，仅 onFinish 帧索引 ±1 帧（≤8–16ms，不可感知）。
...
| P3 | **帧率无关（减速持久固定步 + 弹簧解析事件终止，codex R1-F3/R2-F2/R4-F2/R5-F1/R6-F1）**：减速=持久固定步累加器（固定步与帧解耦 → 任意分区/抖动达边步一致）；弹簧=解析闭式；**闭式事件**（弹簧 zero-crossing `t_zc=−A/B`）**按事件终止 → 终止时刻帧率无关**。**唯一放宽**：渐近 settle-threshold（超越无闭式）在满足阈值的帧边界终止 → **有界 ≤1 帧时序方差**（可见轨迹共时精确，仅 onFinish 帧 ±1） | **(a)** 弹簧相：60/120/不规则/亚-refInterval → 严格浮点容差；**(b) 端到端比瞬态量**（codex R3-F3）：**handoff 速度+跨边时刻+峰值穿透+逐采样轨迹** 严格容差；zero-crossing 终止时刻严格一致；**(c) 抖动 straddle**（codex R4-F2：边界 7.9pt away，不同分区）→ 固定步累加下达边步不变；**(d)** settle-threshold 终止帧 ±1 帧、共时状态严格一致 |
| P4 | **界内 = 纯减速（与 DecelerationModel 同律，codex R6-F1）**：起点界内速度不足达边 → **复用 DecelerationModel damp-then-move 固定步律**、零弹簧；**位移/达边判定与之一致**（非连续积分 ~3% 偏差） | 与同参 DecelerationModel **总位移/达边判定一致**（差仅 sub-refInterval 余量延迟，**非** 130.49 vs 134.61 的律差）；edge@132pt 与普通 fling 同判定；无越界 |
...
- **`EdgeBounceModelTests`**：P1-P8 逐条 + 复用注入 fake dt 序列（确定性）。关键 killer：
  ...
  - **界内减速与 DecelerationModel 同律（P4，codex R6-F1）**：界内不足达边 → 持久固定步累加器与同参 `DecelerationModel` **总位移/达边判定一致**（差仅余量延迟）；**edge-straddle parity**：edge@132pt（v=1000）下与普通 fling **同判定不跨**（驳 134.61 连续积分误判）；
...
- **`DecelerationAnimator` bounce 测试**：注入 fake `FrameDriving` + fake dt（复用既有 DD-1/DD-5 缝）；验证 bounce 路径外溢 delta 序列累加后 offset 落 edge、自然 settle `onFinish` 恰一次、generation/stale 与既有同守；**P8 原子终止 + onFinish 一致（codex R3-F1/R6-F3）**：越界中 `dt≥1.0` → **同一 tick** 外溢 edge-归一 delta + 失活 + **onFinish 触发**（既有契约一致）；越界中 `resetOnSceneActive()` → 外溢 edge-归一 delta + 失活 + **onFinish 静默**；越界中 re-grab `stop()` → 保位不归一（§五区分）；**P9 回调 re-entrancy（codex R4-F3 + R5-F3）**：终止帧 onUpdate 内调 `start()` → 新 run driver 存活不被旧续延 invalidate + 旧 onFinish 不触发；调 `stop()` → onFinish 静默不触发；**`resetOnSceneActive` 越界归一帧亦经共享 `terminate` → 同样 re-entrancy 安全**（reset 归一 onUpdate 内调 start/stop 同守，codex R5-F3）；**事件时刻终止（codex R5-F1）**：弹簧 zero-crossing 在 `t_zc` 终止，60/120Hz 终止时刻严格一致（settle-threshold 角 ±1 帧）；**既有 `DecelerationAnimatorTests` 不改全过**（P7 回归门）。
