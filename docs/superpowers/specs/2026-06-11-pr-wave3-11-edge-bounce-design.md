...
| P6 | **防御（可达 + 分层 + round-trip，codex R7-F3/R10-F2/Plan-R11-F1）**：分离端点 CGFloat（非 `ClosedRange`，后者 NaN 构造即 trap）→ 模型内校验。**offset/bounds 非有限** → 安全停；**仅 velocity 非有限 + offset/bounds 有限** → 净化 0、越界仍归位；**越界归一修正须 round-trippable**（`offset+(edge−offset)==edge`，否则量级悬殊如 MAX/2 vs 10 使 consumer 落 0 而 model snap edge = 失步 → inert 拒绝）；绝不外溢 NaN/inf delta | NaN/inf `min/max/offset` → 无 trap；`min>max` → 无 bounce；NaN/inf velocity + 有限越界 → spring 归位；**量级悬殊 non-round-trippable（MAX/2 vs 10）→ inert（shouldRun false）；round-trippable 大 edge（0.995MAX vs 0.99MAX，Sterbenz exact）→ 运行且 consumer 真到 edge** |
