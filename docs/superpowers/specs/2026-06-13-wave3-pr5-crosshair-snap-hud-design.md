...
- **精确算法（沿用 `xToIndex` verify-and-correct 精神；杜绝 one-sided / 半开比较 / tie 翻向歧义）**：
...
- **空切片/退化守卫（pure-function precondition，须先于 clamp）**：`resolve` 是 host 可直接调的纯函数（D5.1），须显式守 `candles.isEmpty`（等价 `viewport.visibleCount == 0`）→ 返回 `nil`。否则 clamp 窗口 `[s, s−1]` 反转，`min(max(i, lo), hi)` 返回 `hi = s−1`，`candles[s−1]` 崩。生产路径不可达（`make()` 守 `!candles.isEmpty`，`RenderStateBuilder.swift:23`；且 `.empty` 的 `mainChartFrame == .zero` 致 frame 守卫先返回 nil），但纯函数契约须 total（旧 `timeLabel` 的 `>=startIndex && <endIndex` 自然守卫被 clamp-first 设计移除，须显式补回）。
...
- `CrosshairLayout` 以单一入口替换三个分离函数（`lines`/`priceLabel`/`timeLabel`），返回一个聚合几何值：
  ...
  ```
  - `point == nil`、落 `mainChartFrame` 外、**或 `candles.isEmpty`（= `viewport.visibleCount == 0`）** → 返回 `nil`（D3 空切片守卫）。
...
| 文件 | 改动 |
|---|---|
| `ios/Contracts/Sources/.../Render/CrosshairLayout.swift` | 整合为 `resolve(...)` + 吸附（nearest-center round 两侧校正 + tie 取小 + clamp 到 `viewport.visibleCount`）+ `CrosshairResolved` 值类型；显式守 `candles.isEmpty → nil`（D3）；保留 `priceLabel` 的 `yToPrice` 镜像不变量 + 时间 label 索引界限不变量（结构性，替代旧 `timeLabel` 半开守卫） |
