...
**满足任一路径即 pass**（v5 删 path C 合并到 A/B，R3 F1 fix）：

- **A（approve ledger）**：`.claude/state/attest-ledger.json` 查 key == target 的 entry：
...
**v5 关键变更（R3 F1 fix）**：**删除原 path C**（"Bash 调用 codex-attest.sh + stdout 含 Verdict:approve"）。原因：stdout 证据无法验证 `codex-attest.sh` 实际被调用时的 target 参数是否等于当前 target。Claude 可能 attest 别的文件但 stdout 显示 approve。

**正确做法**：Bash 调用 codex-attest.sh / attest-override.sh 跑完后，**脚本自身会 update ledger / override-log**。下一个响应（或同响应后续 Stop）走 L5 时，路径 A 或 B 自然能抓到新 entry。**不需要 path C**。

**含义**：`Skill gate: codex:adversarial-review` 的响应本身**不保证**能 pass；必须等到 attest 跑完 ledger 写好后，**下一次**声明 L5 才 pass。这符合 codex review 的实际时序（先跑再声明）。
