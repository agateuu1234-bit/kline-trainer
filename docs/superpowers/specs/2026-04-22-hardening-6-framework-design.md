...
Hook 2: skill-invoke-check.sh（v3）
    ...
    │       │   │   ├─ **路径 A（approve ledger）**：attest-ledger.json key == target 且
    │       │   │   │     attest_time_utc 非空 + verdict_digest 非空 + time > SESSION_START → pass
    │       │   │   ├─ **路径 B（override ledger）**：override-log.jsonl 最近行 target 匹配
    │       │   │   │     + time > SESSION_START + kind 正确 → pass
...
- **B（override ledger, revision-bound）**：`.claude/state/attest-override-log.jsonl` 最近行 `target` 匹配、`time_utc > SESSION_START_UTC`、`kind` 正确（file/branch）；**file kind 同样要求 blob_or_head_sha 匹配当前 `git hash-object`**（v7 加）
...
**`SESSION_START_UTC` 获取（v8 R6 F1 fix：去掉 12H fallback，fail-closed）**：
- 首选：`$CLAUDE_SESSION_ID` 是 ULID，前 10 字符时间戳解码
- 退化：env var `$CLAUDE_SESSION_START_UTC`（hook setup 时设置）
- **两者都不可用时**（v8 修正）：
  - **block mode** → SESSION_START_UTC 无法可靠计算 → **BLOCK**（fail-closed）；stderr 提示用户 export `CLAUDE_SESSION_START_UTC` 或确保 `CLAUDE_SESSION_ID` 可读
  - **observe mode** → drift-log `session_start_unknown`，pass（观察期宽松，但**不**用 12H rolling window，该条目不记为"session-scoped"）
- **完全不允许 12-hour rolling fallback**：防止新 session 复用旧 session 的 attest 作为"in-session"证据（R6 F1）
...
| Finding | 严重 | 处理（v8） |
|---|---|---|
| F1 HIGH 12H fallback 可绕（新 session 用旧 session attest）| HIGH | **去掉 12H rolling fallback**。两个 session 变量都不可用时：block mode → BLOCK fail-closed；observe mode → drift-log `session_start_unknown`（pass 但不算 session-scoped 证据）。要求用户 export `CLAUDE_SESSION_START_UTC` 或确保 `CLAUDE_SESSION_ID` 可读 |

新增测试：
- `CLAUDE_SESSION_ID` 未设 + `CLAUDE_SESSION_START_UTC` 未设 + block mode → BLOCK
- 同上 + observe mode → drift-log + pass（WARN）
- 同上 + 12-hour-old ledger entry → BLOCK（v8 fail-closed，不再被 12H 窗口接受）
