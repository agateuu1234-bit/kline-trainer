...
3. **撤销 / 前进（D25）**
   ...
   - 做了新动作 → 前进（↪）置灰。
   - **进画线模式时建栈，退出画线模式时清空**。
   - **撤销条目必须携带原下标**（`(oldObject, index)`），因为 `drawings` 的数组序 **就是** z-order（D33 逆序 hitTest + D40 命中≡渲染）：
     - 撤销「删线」= `drawings.insert(obj, at: index)`，**禁止 `append`**；对应的 ↪ = `remove(at: index)`。
     - 撤销「画线」= `remove(at: index)`（新画的线在末尾）；对应的 ↪ = `insert(at: index)`。
     - 撤销「改样式 / 锁定」= **同下标原地替换**回旧对象，**禁止**「删了再加」。
...
7. **撤销 / 前进都进 dirty 信号（接 1b-i 的 0c）**：`undo` 与 `redo` **各**写一条「调用 → `drawingsRevision` 严格递增 1 → autosave 被触发 → 重新加载后结果正确」的测试；锁定 / 解锁的原地替换 API 同法各一条。**四类动作一个都不能漏**——1b-i 之后已没有 `.count` 兜底（D30 / codex R28-high）。
