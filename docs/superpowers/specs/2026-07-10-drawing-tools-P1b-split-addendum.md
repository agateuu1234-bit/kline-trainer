...
| **D25 撤销语义（含数组序保真）**（1b-ii） | 撤销栈**深度 1**（母 spec §7 已定「各仅一步」）。**入栈动作 = 画线 / 删线 / 改样式 / 锁定·解锁**。做了新动作 → 前进（↪）置灰。**进画线模式时建栈，退出画线模式时清空**。<br>**撤销记录必须同时携带 before / after 两个快照 + 原下标（codex R25-high 定原下标 + R29-high 定 after 快照）**：一条撤销记录 = `(before: DrawingObject?, after: DrawingObject?, index: Int)`。**undo 用 `before`、redo 用 `after`，都在同一个 `index` 上操作**，四类动作统一。**redo 绝不能从「当前默认设置 / 当前选中态」重算**——用户在动作被撤销期间改了默认颜色 / 线型或切了选中，重算就会漂成错值，还会被 `drawingsRevision` / autosave 固化。`drawings` 的数组序 = z-order（D33 逆序 hitTest + D40 命中≡渲染），故 index 必须精确。 | **为何数组序是正确性不变量而非美观问题（codex R25-high，已核实为真）**：D33 规定重叠时选中**最后画的那条**（`drawings` 逆序 hitTest），D40 又要求命中集合 ≡ 渲染集合、共用同一个逆序纯函数——**数组下标就是 z-order**。若撤销删除用 `append` 把线加回末尾，它会**从原位置跳到最上层**：渲染叠放变了，之后的单击也会选中它而不是原本压在它上面的那条。用户看到的是「撤销之后再点，选中的线不对了」，且这次错误的删除会被 autosave 固化。故撤销条目必须携带原下标。<br>画线模式是一个有界的编辑会话，退出即提交。跨模式保留会让用户在交易了几十个 tick 之后意外撤销掉一条线；且跨模式保留会立刻牵出「撤销栈要不要随 autosave 落盘 / 断点续局后 ↩ 是否还灵」的新决策面，与本阶段无关。 |
| **D26 复盘过渡（限定于 UI 外壳）+ 训练 / replay 退役浮动钮**（退役落 1a-iii，**必须与新入口同期**） | 新两行底栏**只在训练 / 再次训练（replay）出现**。**复盘模式的画线「入口与控件」完全不动**：继续使用现有 `DrawingToolFloatingView` 浮动铅笔钮，**控件形态与入口位置一字不改**。P5 时复盘切到 6 键新底栏，届时删除 `DrawingToolFloatingView`。<br>**D26 只管「入口与控件」，不管共享引擎语义（codex R23-medium 澄清）**。以下四项是**全局引擎行为**，**在复盘同样生效**，且**每一项都必须带复盘侧测试**：<br>　• **D29 周期绑定渲染**（1a-i）<br>　• **D42 全局画线会话 + 双面板可画**（1a-ii）<br>　• **连续画线**（1a-ii，不再「画一条就退出」）<br>　• **D32 手势**（1a-iv，画线模式内平移 / 竖滑切周期 / 缩放）<br>换言之：复盘里**长得一样、点法一样**，但**画线的行为跟着引擎一起变好**——这是有意的，不是回归。<br>**1a-iii 起，`DrawingToolFloatingView` 只在复盘渲染（codex R15-medium）**：`TrainingView.swift:69` 现为 `showsDrawingTools = showsTradeButtons || engine.flow.mode == .review`，训练 / replay **今天就显示浮动钮**。否则会 ship 出两个画线入口，老入口绕过新的底栏 / 设置面板语义，还保留着「画一条就退出」的旧行为（D38 正要消灭它）。<br>**但绝不能直接把 `showsDrawingTools` 改成 `flow.mode == .review`（codex R22-high，交易安全）**：该谓词被**复用**在两处——`:186` 的浮动钮，与 **`:423-425` 的 activePanel 红色高亮边框**。而买 / 卖 / 持有全部按 `activePanel` 下单。直接改会在训练 / replay 里**连带抹掉「当前对哪个面板下单」的唯一视觉提示**，显著提高下错面板的风险（且 autosave 后不可逆）。<br>**必须拆成两个谓词**：<br>　• `showsFloatingDrawingTool` = `engine.flow.mode == .review` —— 只管 `DrawingToolFloatingView`。<br>　• `showsActivePanelHighlight`（保留原语义 `showsTradeButtons || engine.flow.mode == .review`）—— 只管 activePanel 高亮。<br>**回归测试**：训练 / replay **不再渲染 `DrawingToolFloatingView`，但 activePanel 高亮边框仍在**；复盘两者都在。<br>**边界澄清（codex spec-R2-medium / R4-high）**：D26 只管**画线模式的入口与控件**。**渲染（D29）与手势编排（D32）都是全局引擎行为，在复盘的浮动钮画线模式下同样生效**。 | 母 spec D19：复盘专属控件（隐藏键、复盘删除、hiddenIds 持久化、clear-saved）全在 P5。若让浮动钮退役而复盘又不上新底栏 → 复盘失去画线入口 = **功能回归**。若复盘上 5 键新底栏 → 其中 🔒/🗑 在复盘必须置灰（复盘删除属 P5、原训练线按母 spec §7 只可隐藏/显示）= **死控件**，两者都不可接受。 |
...
| **D30 内容级 dirty 信号（两处，缺一不可）**（1b-i） | **① 视图触发**：把 `TrainingView` 的存盘触发从 `.onChange(of: engine.drawings.count)` 改为 `.onChange(of: engine.drawingsRevision)`。`TrainingEngine` 新增单调计数器 `public private(set) var drawingsRevision: Int`，**任何**改动 `drawings` 的引擎 API（append / delete / **原地替换（改样式、锁定）** / undo / redo）都 `+1`。<br>**② replay clean-skip：判据一行不改，只加一条「不变量锁」测试**（codex spec-R24-high 修正）。**不得**给 `replayBaseline` 追加字段、**不得**改 `saveProgress` 的 clean-skip 判据。 | codex spec-R2-high + **R4-high**（两条均**已核实为真**）。<br>**① 的必要性**：现触发是 `TrainingView.swift:273` 的 `.onChange(of: engine.drawings.count)`。1a-iii 及之前画线只有增 / 删两种操作，按 count 触发正确；**1b-i 首次引入原地改一条线**（改选中线的样式），**数组长度不变 → 不触发 autosave**。<br>**② 为何反而什么都不改（codex spec-R24-high，本条推翻 R4-high 的结论）**：R4-high 曾断言「fresh replay 里改一条已有线的颜色会被 clean-skip 吞掉」。该场景**不可达**，已逐行核实：<br>　• `TrainingSessionCoordinator.replay(recordId:)`（`:566-573`）构造 fresh replay 时**不传 `initialDrawings`**（方法注释亦明写「不还原标记 / 绘线」）→ `engine.drawings == []`。一局新开的 replay 里**没有任何已有线可改**。<br>　• `resumePendingReplay(recordId:)`（`:913-936`）才恢复画线，但它同时置 `replayHasPersisted = true`（`:936`）→ 而 clean-skip 的守卫是 `if !replayHasPersisted`（`:606`）→ **续局会话永不 clean-skip**。<br>　合起来：**「有已有线可改」与「clean-skip 生效」互斥**。给 `replayBaseline` 加字段是为一条不可达路径加代码（违反 YAGNI），且会误导后人以为该路径有防护。<br>　**真正会丢掉 replay 里样式改动的仍是 ①**：续局 replay 里改样式同样不改数组长度 → `.onChange(of:.count)` 不触发 → 连 autosave 都没发起。①（`drawingsRevision` 触发器）**同时**修好训练与续局 replay 两条路径。<br>　**代之以不变量锁测试**（1b-i 负向测试 0b）：把上述互斥写成断言。将来若有人给 fresh replay 种画线，或让续局不再 `replayHasPersisted = true`，测试立刻变红，届时再补基线字段。<br>**不得**改用 `.onChange(of: engine.drawings)` 或数组值比较：`DrawingObject.==` 刻意**排除 `id`**（P1a 决策，id 是身份非内容）。单调计数器是唯一无歧义的信号。<br>`reviewDrawings` 的 `.count` 触发**保持不变**（P1b 不引入复盘内的原地编辑；P5 引入隐藏 / 删除时同法处理）。 |
...
>
> **本节的范围（codex R30-high 收窄）**：此表只证明**存储形状**——`anchors` 是变长数组，任意锚数都能无损往返，故五个形状**都不需要新列 / 新迁移**。它**不**证明 trend / channel / rect / polyline 可以在 P1b 时代的 1.11 构建里正确**显示**：那是**渲染器注册表**的问题（老构建 `guard let tool = tools[toolType] else { continue }` 会静默丢线），属 **§8 未决的 P1c 版本门**，本节不作结论。下表「落在哪一期」列已如实标注：**只有水平线在 P1b（1a-i），其余四个都在 P1c。**
...
**结论（仅限存储形状）**：`anchors` 变长数组 + `toolType` 已足以**无损存储**全部五个形状的几何，`lineSubType / lineStyle / thickness / colorToken / labelMode / locked` 已在 `style_json` 列（0009 新增）里往返——**没有任何形状需要新列 / 新字段 / 新迁移**。<br>**但这只覆盖「存得下」，不覆盖「显示得出」**：P1b 六个 PR 只**写入** `.horizontal`（故 P1b 确实零版本风险）；P1c 首次写入非水平线工具时的 `CONTRACT_VERSION` / 能力位 / 「可解码但无渲染器」策略**在本 spec 中未决**，唯一权威出处 = **§8**。本节不得被援引为「P1c 可在 1.11 下直接 ship」的依据。
...
2. **画线会话全局化（D42）**
   ...
   - pending 锚（`pendingAnchors`）与其归属面板 **`pendingAnchorPanel` 一并进 1a-ii 的共享容器**（**不得**留各面板 Coordinator 私有，否则下面板清不掉上面板的 pending，codex R31-high）；`pendingAnchorPanel` **与 activePanel 无关**；**下一次落锚 tap 落在 ≠ `pendingAnchorPanel` 的面板**时 → 按 D31 **只丢弃 pending 锚**（`discardPendingAnchors()`，保留 `activeDrawingTool`）。**不得调 `cancel()`**（`DrawingToolManager.swift:76-80` 会连 `activeTool` 一起清掉）。
   ...
   - **切 activePanel 的正确语义**：`drawingModeActive` / `activeDrawingTool` / **pending 锚全部保持不变**（切下单目标面板不产生新落锚，故不丢线）；本期唯一改动 = 退役 observer 顺手调的 `cancelDrawingAllPanels()`。丢 pending 只发生在**下一次落锚 tap 落在别的面板**时（见上）。
...
任何新 UI 控件（顶栏「画图」钮 / 两行底栏 / 设置面板全在 1a-iii）；退役浮动钮（**必须与新入口同期**，1a-iii）；手势改动（1a-iv）；选中 / 编辑（1b-i / 1b-ii）。

**D31 的哪一半不在本期**（codex R28-medium）：本期**落地** `discardPendingAnchors()` API + **「下一次落锚 tap 落在 ≠ `pendingAnchorPanel` 的面板 → 丢 pending 锚」**触发（D42 当期就要用；**注意不是「切 activePanel」**——activePanel 是下单目标面板，见 D42/R30-medium）。**不做**「**周期组合实际改变** → 丢 pending 锚」触发与 `commit` 前的全锚同 period 断言——那两项要等 1a-iv 的 D32 让画线模式**真能切周期**之后才可达（今天 `panPolicyInDrawingMode` 恒 `.drawingTakesOver`，竖滑被吞）。1a-iv **复用**本期这同一个 API，不得另写一份取消语义。
...
3. **D42 跨面板 pending 取消**（`discardPendingAnchors()` **在本期新增**，见 D31 归属切分；**必须跨两个真实 Coordinator**，codex R31-high）：**上面板 Coordinator** 落一个 pending 锚（`pendingAnchorPanel` = 上面板；人造多锚工具场景）后**下一次落锚 tap 打到下面板 Coordinator** → **上面板的 `pendingAnchors` 被清空，而 `activeDrawingTool` / `drawingModeActive` 存活**（走 `discardPendingAnchors()`，不是 `cancel()`）。**测试不得只在同一个 manager 上调两次**——那测不出「私有 pending 跨 Coordinator 不可见」这个真缺陷。<br>另测**下一次落锚 tap 仍在同一面板** → **不丢 pending**（判据是落锚面板，不是 activePanel）。
4. **D42 互斥模型已退役**：激活画线模式**不再**调用 `cancelDrawingAllPanels()`；两面板可同时接受落锚。
4b. **切 activePanel 从不丢 pending（codex R18-high + R30-medium 专项）**：
   - **无 pending 锚时**切 activePanel → `drawingModeActive` 与 `activeDrawingTool` **仍然存活**（会话不倒）。
   - **有 pending 锚时**切 activePanel（**不落新锚**）→ pending 锚 **原封保留**、会话仍在（**切下单目标面板绝不丢线**，R30-medium）。
   - 断言 `TrainingView` 的 `.onChange(of: activePanel)` **不再**调用 `cancelDrawingAllPanels()`。
   - **对照**（丢 pending 的正路径）：有 pending 锚时**下一次落锚 tap 落在 ≠ `pendingAnchorPanel` 的面板** → 只丢 pending 锚，且 `activeDrawingTool` 仍非 nil（codex R20-high，走 `discardPendingAnchors()` 而非 `DrawingToolManager.cancel()`）。
...
6b. **复盘侧同样生效（D26 澄清，codex R23-medium）**：复盘模式下**连续画线**成立（画完一条不退出）、**上下两面板都能画**、切 activePanel 不拆会话。复盘的**入口与控件形态**仍是浮动钮、一字未改。
...
| 8 | 进复盘，用浮动钮画线 | 钮的样子和点法**一字未改**；但同样**能连续画、两个面板都能画**（D26：入口不变、引擎行为一起变好） | |
...
4. **复盘手势同步改善不算回归（D26 澄清）**：复盘浮动钮画线模式下，pan / 竖滑切周期 / 缩放同样可用（D32 全局生效）；复盘的**入口与控件形态**仍是浮动钮、一字未改。
...
3. **选中状态进共享容器并流进渲染（D41，先决）**
   - `selectedDrawingID: DrawingID?` + `selectedPanel: PanelId?` 加入 1a-ii 建立的**同一个共享容器**（与 `activeDrawingTool` 同源）。底栏、设置面板、tap 处理**读写同一份**。**选中态是 `(selectedPanel, selectedDrawingID)` 二元组，渲染与操作都按二元组，不是 id-only**（codex R31-medium）。
   - `RenderStateBuilder` **只在渲染 `selectedPanel` 那个面板时**把 `selectedDrawingID` 带进 `KLineRenderState`（现在**没有** selected 概念）；`KLineView+Drawing` 的 dispatch 据此对该条走**选中蓝**（`DrawingTool.render` 的 D35 入参加 `isSelected: Bool`，或渲染上下文携带 selected id）。
   - **端到端测试**：① 选中一条线 → 强制走一次 `updateUIView` → 渲染出的仍是同一 `(panel,id)` 的选中态，🗑 / 设置面板作用的也是同一条；② **周期迁移清除**（codex R31-medium）：选中一条上面板 60 分线 → 切周期让 60 分改由下面板显示 → 选中被**清空**（不得跟着线迁到下面板续蓝 / 可删）。
...
7. **选中态的转移与生命期（D37）**
   ...
   - **新提交的线不自动选中。**
   - 退出画线模式、从选择态切回画线态、**`visibleDrawings(for: selectedPanel)` 不再含选中 id（因切周期 / 切面板不可见，或线迁到了另一面板）**、选中线被删除 → 均清空选中（判据按 `selectedPanel`，哪怕同 id 在另一面板可见，codex R31-medium）。
   - 每次清空后 🗑 回到灰态。
...
8. **D30 内容级 dirty 信号（两处，缺一不可）**
   ...
   - **② replay clean-skip：一行都不改**（`replayBaseline` `:56` 不加字段，`saveProgress` `:603-610` 判据不动）。
     > **为什么不改**：`replay(recordId:)`（`:566-573`）构造 fresh replay **不传 `initialDrawings`** → 没有已有线可改；`resumePendingReplay`（`:913-936`）恢复了画线但置 `replayHasPersisted = true`（`:936`），而 clean-skip 守卫是 `if !replayHasPersisted`（`:606`）→ 永不触发。「有线可改」与「clean-skip 生效」互斥，故该路径不可达（codex spec-R24-high）。改 ① 的 `drawingsRevision` 触发器**同时**覆盖训练与续局 replay：两者的样式改动都不改数组长度，今天都发不起 autosave。
     > 代之以负向测试 0b **把这条互斥锁死**：将来谁给 fresh replay 种了画线，测试立刻红，届时再补基线字段。
...
0. **内容级 dirty 信号（D30，最高优先，三条测试）**
   - **0a 训练路径**：改一条已有线的样式 → `drawingsRevision` 递增 → 触发 `autosave(immediate:)` → 重新加载后样式仍在。必须是「**不经过数组增删、只改内容**」的落盘往返。
   - **0c 增 / 删不回归（codex R28-high 专项，不可省）**：把触发器从 `.count` 换成 `drawingsRevision`，等于**丢掉了 `.count` 原本白送的增 / 删覆盖**——只要有一个引擎 API 忘了 `drawingsRevision += 1`，新画的线或删掉的线就**再也触发不了 autosave**，杀进程即丢，且没有任何编译期保护。故必须为**每一个**改动 `drawings` 的引擎 API 各写一条：调用它 → `drawingsRevision` **严格递增 1** → autosave 被触发 → 重新加载后结果正确。<br>本期覆盖 `appendDrawing` / `deleteDrawing(at:)` / 原地替换（改样式）；**1b-ii 落地 undo / redo / 锁定时必须把它们补进同一组**（见 §7.3 第 7 条）。<br>**并加一条守门测试**：断言 `TrainingView` 里**不再存在** `.onChange(of: engine.drawings.count)`（防止改一半、两个触发器并存）。
   - **0b replay 不变量锁**（codex R24-high 专项，取代原「fresh replay 改样式不被 clean-skip 吞」的不可达测试）：两条断言，任缺其一则 ② 的「什么都不改」结论失效——
     - `replay(recordId:)` 对一条**画线非空**的记录返回的引擎，`engine.drawings.isEmpty == true`（fresh replay 不种画线）；
     - `resumePendingReplay(recordId:)` 成功续局后，**只改一条已有线的样式**（不推 tick / 不交易 / 不增删 / 不切周期）→ `saveProgress` **真的写盘**，重读槽后样式仍在（即续局会话从不 clean-skip）。
...
5b. **删除确认框「取消 / 关闭」是 no-op（codex R32-medium，本期删除不可逆——undo 要到 1b-ii）**：选中一条线 → 点 🗑 弹框 → **点「取消」**（另测**点框外 / 手势关闭**）→ 断言：那条线**仍在** `engine.drawings`（逐字段不变）、选中态不变（仍是同一 `(panel,id)`、🗑 仍亮）、**`drawingsRevision` 不递增**、**没有任何写盘发生删除**。**不得**只测「弹框 + 确认删除」——cancel 角色接反会造成不可逆数据丢失且现有测试照过。
...
| 13 | 点「删除」 | 线消失，🗑 回灰 | |
| 13b | 选中一条线，点 🗑，在弹框里点**「取消」**（或点框外关掉它） | 线**还在**、仍是蓝的选中态、🗑 仍亮；杀掉 App 重开续局后那条线**也还在**（取消绝不能删线） | |
...
| 17 | 进「再次训练」画**一条线**，退出；从历史弹窗**续这一局 replay**，什么都不做（不下单、不推进、不加线、不切周期），只把那条线改成紫色，**立刻杀掉 App**，再续这一局 | 那条线是紫色的（验续局 replay 里「只改样式」也会触发存盘）<br>注：全新一局 replay 里**本来就没有任何已有线**，只能先画一条、退出、续局才改得到 | |
...
3. **撤销 / 前进（D25）**
   ...
   - 做了新动作 → 前进（↪）置灰。
   - **进画线模式时建栈，退出画线模式时清空**。
   - **撤销记录 = `(before: DrawingObject?, after: DrawingObject?, index: Int)`**（codex R25-high 定 index + R29-high 定 after）。`drawings` 的数组序 **就是** z-order（D33 逆序 hitTest + D40 命中≡渲染），故 index 必须精确；`after` 快照使 redo 不必从当前默认 / 选中态重算。四类动作按同一模板：
     | 动作 | before | after | undo | redo |
     |---|---|---|---|---|
     | 画线 | `nil` | 新线 | `remove(at: index)` | `insert(after, at: index)` |
     | 删线 | 旧线 | `nil` | `insert(before, at: index)` | `remove(at: index)` |
     | 改样式 | 旧对象 | 新对象 | `replace(at: index, before)` | `replace(at: index, after)` |
     | 锁定·解锁 | 旧对象 | 新对象 | `replace(at: index, before)` | `replace(at: index, after)` |
     - 「删线 / 改样式 / 锁定」的 undo / redo **一律同下标操作，禁止 `append`、禁止「删了再加」**。
...
3. **锁定态在续局 replay 里落盘**：`resumePendingReplay` 续局后只锁定一条线（不推 tick / 不交易 / 不增删 / 不切周期）→ `drawingsRevision` 递增 → autosave 发起 → `saveProgress` 真的写盘（D30① 回归保护；fresh replay 无已有线可锁，见 D30②）。
4. **撤销栈深度 1**：连点两次 ↩ 只回退一步；↪ 只前进一步，第二次点无反应。
5. **撤销栈生命期**：退出画线模式再进入 → ↩ / ↪ 均为灰（栈已清空）。做新动作后 ↪ 置灰。
6. **四类动作都可撤销**：画线 / 删线 / 改样式 / 锁定，各写一条「做 → ↩ 复原 → ↪ 重做」的往返测试。
6b. **撤销保序（codex R25-high 专项）**：在**同一价位**依次画三条重合的线 A→B→C（数组序 `[A,B,C]`）。删掉**中间的 B** → ↩ 撤销 → 断言两件事：① `drawings` 数组恰为 `[A,B,C]`（B 回到**下标 1**，不是末尾）；② 在该价位单击，选中的仍是 **C**（D33 逆序 hitTest 的最上层）。再 ↪ → 数组恰为 `[A,C]`。**不得**只断言「B 回来了 / 条数是 3」——那样 `append` 实现也能过。
6c. **redo 不漂移（codex R29-high 专项，改样式 + 锁定各一条）**：把一条线改成**红色**（记为 after）→ ↩ 撤销（回到原色）→ **在设置面板把当前默认色改成绿色、并另选中别的线**（制造「当前态 ≠ after」）→ ↪ 重做 → 断言那条线是 **红色**（after 快照），**不是绿色**（当前默认）也不是原色。锁定同理：锁定一条线 → ↩ → 中途改动别处 → ↪ → 断言回到**锁定**态。**不得**只测「↩ 后 ↪ 能回来」——那种测试在「redo 从当前态重算」的错误实现下也会过，除非中途真的改了当前默认 / 选中。
7. **撤销 / 前进都进 dirty 信号（接 1b-i 的 0c）**：`undo` 与 `redo` **各**写一条「调用 → `drawingsRevision` 严格递增 1 → autosave 被触发 → 重新加载后结果正确（redo 落盘的是 `after` 快照，见 6c）」的测试；锁定 / 解锁的原地替换 API 同法各一条。**四类动作一个都不能漏**——1b-i 之后已没有 `.count` 兜底（D30 / codex R28-high）。
...
| 7 | 进「再次训练」画**一条线**，退出；从历史弹窗**续这一局 replay**，只锁定那条线，**立刻杀掉 App**，再续这一局 | 那条线仍是锁定态<br>注：全新一局 replay 里**没有任何已有线**可锁，必须先画一条、退出、续局（同 1b-i 第 17 条） | |
| 8 | 画一条线，点 ↩ | 线消失 | |
| 9 | 点 ↪ | 线回来了 | |
| 10 | 删一条线，点 ↩，再点 ↪ | ↩ 之后线回来了；↪ 之后线又没了 | |
| 11 | 改一条线的颜色，点 ↩ | 颜色变回改之前 | |
| 12 | 锁定一条线，点 ↩ | 变回未锁定 | |
| 13 | 在**同一个价位**从上往下依次画三条重合的线（就叫 A、B、C），把**中间那条 B** 删掉，再点 ↩ | B 回来了 | |
| 14 | 接第 13 条：切到**选择态**，在那个价位单击一下 | 选中的是**最后画的 C**（和删 B 之前一模一样）——不是刚撤销回来的 B | |
...
**P1c 的契约地位（未决 — codex R23-high → R26-high，本 spec 明确不拍板）**：

- **已确认（R23-high 核实）**：`DrawingToolType` 的 11 个 case 在 **1.11 已全部声明**（P1a merged）。P1c 写入 `trend / channel / rect / polyline` **不扩展枚举值域**，老 1.11 客户端能**解码**它们，lossy 层也不会把它们当 `.unknownRaw`。
- **但可解码 ≠ 可兼容（R26-high，已核实为真）**：渲染派发是 `KLineView+Drawing.swift:23` 的 `guard let tool = tools[drawing.toolType] else { continue }`，而 P1b 时代的 `drawingTools` 注册表**只有 `.horizontal``**。于是一个 **P1b 时代的 1.11 构建**打开 **P1c 写过的数据**时，那些 trend / channel / rect / polyline **会静默消失**：数据在盘上完好、能解码、App 认为「读得懂」，用户却什么都看不到，也没有任何版本告警。这正是 `CONTRACT_VERSION` 本该拦住的版本错位（本 App 可能公开上架，降级 / 多设备错位真会发生）。
- **故 P1c 的 spec 必须先回答**（三选一，或提出更好的）：① 首次写入非 `.horizontal` 工具时 bump `CONTRACT_VERSION`；② 引入一个**持久化的能力位 / feature capability**，老构建读到高于自己能力的画线时显式提示而非静默跳过；③ 保留同版本，但要求任何构建把「可解码但本版本无渲染器」的工具类型当作**受保护的未支持数据**——不渲染、**不静默参与增删改**、且给出显式 UI 门。
- **本 spec 只做两件事**：把该问题钉在这里，并要求 P1c 的三组举证测试（下）里第 3 组必须覆盖它。**任何实现者都不得在 P1c 里默认「不用 bump」。**

**P1c 必须携带的三组举证测试（无论怎么拆，都不得丢）**：
...
3. **跨版本 `toolType` 兼容举证（D28 + codex R26-high）**：
   - 新写入的 `trend / channel / rect / polyline` 经 **finalize（`drawings` 表）/ pending（JSON blob）/ review（wrapper）三条持久化路径**往返不丢；
   - 构造一条**未识别 `toolType`** 的记录，断言 P1a 的 lossy 层走**字节保真 `.unknownRaw`** 而非 crash、也不被普通保存覆盖；
   - **「可解码但无渲染器」路径**：构造一个只注册了 `.horizontal` 的渲染器注册表（模拟 P1b 时代构建），喂进一条 `.trend` 画线，断言 P1c 选定的方案真的生效——**不得**是今天的「静默 `continue`、用户无感」。此测试是 P1c 兼容决策的落地闸门。
