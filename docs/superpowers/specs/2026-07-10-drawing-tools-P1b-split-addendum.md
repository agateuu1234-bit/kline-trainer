...
2. **画线会话全局化（D42）**
   ...
   - pending 锚归属首个落锚的面板，记进独立字段 **`pendingAnchorPanel`（与 activePanel 无关）**；**下一次落锚 tap 落在 ≠ `pendingAnchorPanel` 的面板**时 → 按 D31 **只丢弃 pending 锚**（`discardPendingAnchors()`，保留 `activeDrawingTool`）。**不得调 `cancel()`**（`DrawingToolManager.swift:76-80` 会连 `activeTool` 一起清掉）。
...
3. **D42 跨面板 pending 取消**（`discardPendingAnchors()` **在本期新增**，见 D31 归属切分）：上面板落一个 pending 锚（`pendingAnchorPanel` = 上面板；人造多锚工具场景）后**下一次落锚 tap 点到下面板** → **pending 被清空，而 `activeDrawingTool` / `drawingModeActive` 存活**（走 `discardPendingAnchors()`，不是 `cancel()`）。<br>另测**下一次落锚 tap 仍在同一面板** → **不丢 pending**（判据是落锚面板，不是 activePanel，不得无条件取消）。
