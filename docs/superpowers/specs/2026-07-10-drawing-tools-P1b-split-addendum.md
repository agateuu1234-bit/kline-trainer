...
**D31 的哪一半不在本期**（codex R28-medium）：本期**落地** `discardPendingAnchors()` API + **「落锚面板实际改变 → 丢 pending 锚」**触发（D42 当期就要用）。**不做**「**周期组合实际改变** → 丢 pending 锚」触发与 `commit` 前的全锚同 period 断言——那两项要等 1a-iv 的 D32 让画线模式**真能切周期**之后才可达（今天 `panPolicyInDrawingMode` 恒 `.drawingTakesOver`，竖滑被吞）。1a-iv **复用**本期这同一个 API，不得另写一份取消语义。
...
3. **D42 跨面板 pending 取消**（`discardPendingAnchors()` **在本期新增**，见 D31 归属切分）：上面板落一个 pending 锚（人造多锚工具场景）后点到下面板 → **pending 被清空，而 `activeDrawingTool` / `drawingModeActive` 存活**（走 `discardPendingAnchors()`，不是 `cancel()`）。<br>另测**面板未实际改变**（在同一面板连续落锚）→ **不丢 pending**（不得无条件取消）。
