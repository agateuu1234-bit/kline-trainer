...
>
> **顺序不可乱**：1a-ii 的设置面板改的样式，要靠 1a-i 的渲染迁移才画得出来；1a-ii 单独 ship 时画线模式仍会吞掉平移（**这就是今天的行为，不算回归**），由 1a-iii 修好——而 D29 周期绑定必须与 D32 手势同在 1a-iii，否则就是 R6 抓到的坐标错位。
>
> **1a-i 在模拟器里没有可见变化**（默认色映射到现有的橙色），这是切分的必然代价：「能设样式」与「能画出样式」是一对，拆开必有一半不可见。已向用户说明并获决断。
...
| 决策 | 结论 | 理由 |
|---|---|---|
| **D23 五段切分线** | **1a-i　渲染层**（~300 行，无可见变化）= D35 `DrawingTool` API 迁移 + `HorizontalLineTool` 消费样式字段 + 水平线射线 / 价格标注几何 §5.1 + `colorToken` 主题解析。<br>**1a-ii　外壳与设置**（~450 行，可见）= 画线模式外壳 §2 + 两行底栏骨架（D24）+ 长按设置面板 §3。<br>**1a-iii　手势与周期**（~250 行，可见）= D32 手势最小改动（§14 除节点拖动分支）+ D29 周期绑定渲染 §10 + D31 同 period 钩子。<br>**1b　选中与编辑**（~600 行，可见）= 选中 · 删除 · 锁定 · 撤销 / 前进 §7（**不含节点**）+ D33 命中平局 + D34 复盘门控 + D30 内容级 dirty 信号。<br>**2　多锚与新工具**（~1400 行）= 多锚泛化 §5.0 + 节点模型与拖节点 §6 + §14 剩余的节点拖动分支 + 趋势线 §5.2 / 通道线 §5.3 / 箱体 §5.11 / 折线 §5.4。 | **i↔ii 的缝**：i 只改渲染 / 命中的**数据通路**（让样式抵达渲染层），不引入任何 UI；ii 才给出设置样式的入口。i 单独 ship 时默认色映射到现有橙色，视觉零变化，全部靠单测把关。<br>**ii↔iii 的缝**：ii ship 后画线模式仍会吞掉平移与切周期——**这正是今天浮动钮模式的行为**，不是回归。iii 才解开。<br>**iii 三条为何必须捆一起（R4 / R6-high）**：D32 让画线模式**首次能切周期**；一旦能切，渲染就必须按 `period` 绑定（D29），否则 60 分画的线在切到 15 分后仍留在该面板上、坐标系却已换 = 可见的位置错误；同时多锚采集期必须 fail-closed 取消 pending（D31）。拆开任何一条都 ship 出可见缺陷。<br>**iii↔1b 的缝**：iii 之前不存在**原地修改已有线**的能力，新画线走 append、数组长度变，现有 count 触发即正确。1b 一次性引入「原地改线」及其全部后果（选中 / 改样式 / 锁定 / 撤销 / 内容级存盘 D30）。<br>**1b↔2 的缝**：节点必须与「拖节点」同期落地，否则会 ship 出**拖不动的死节点**（母 spec D19 禁止）。 |
...
## 2. P1b-1a-i 交付范围：「让样式抵达渲染层」（无可见变化）
...
3. **`colorToken` 主题解析**
   - 9 色语义 token → 昼 / 夜两套实际 RGBA。白天的「白」、夜间的「黑」在解析层给出可读的降级色（UI 层的禁选在 1a-ii）。
...
任何 UI（外壳 / 底栏 / 设置面板全在 1a-ii）；手势 / 周期绑定（1a-iii）；选中 / 编辑（1b）。本 PR **不改变任何用户可见行为**。

### 2.3 P1b-1a-i 必须存在的负向测试

1. **视觉零变化**：用**默认样式**构造的 `DrawingObject`，渲染出的描边色与线宽**等于迁移前的常量**（橙色 `(0.82, 0.40, 0.0)`、1.5pt）。
...
5. **昼夜色解析**：同一 `colorToken` 在昼 / 夜主题下解析出不同 RGBA；白天的「白」与夜间的「黑」解析结果**可读**（不等于背景色）。
6. **标注位置**：`.left` / `.right` 标签矩形不与线段矩形相交（不压线）；`.right` + 四位整数价时标签右边界 **≤ 主图右缘**。
7. **`.hidden` 标注**：不产出任何标签绘制调用。

### 2.4 P1b-1a-i 非程序员验收清单

> 本 PR 的设计目标就是**看不出变化**。验收方式是"确认什么都没坏"。

| # | 动作 | 预期 | 通过 / 不通过 |
|---|---|---|---|
| 1 | 进入训练，用现有的浮动铅笔钮画一条水平线 | 线的颜色、粗细**跟改造前一模一样**（橙色细线） | |
| 2 | 退出 App 重进、续上这一局 | 线还在，样子没变 | |
| 3 | 结束一局进复盘 | 原训练线正常显示，样子没变 | |
| 4 | 在复盘里用浮动钮画线 | 行为跟改造前一模一样 | |
| 5 | 找一局改造前就存在的老记录，进它的复盘 | 老画线正常显示，位置和样子都没变 | |
| 6 | 顶栏、底栏、图表 | **没有任何新按钮出现** | |
...
底栏 ②–⑤ 键、选中、删除、锁定、撤销 / 前进（1b）；手势改动 / 周期绑定 / 同 period 钩子（1a-iii）；节点 / 多锚 / 四个新工具（P1b-2）；复盘专属一切（P5）；主页全局默认设置（P6）。
...
| 21 | 画线模式内单指横向拖 | 图表**不平移**（仍被绘线截获）——**这是本期的已知状态**，1a-iii 修 | |

---

## 4. P1b-1a-iii 交付范围：「画线时也能平移、切周期，线跟着周期走」
...
2. **D29 周期绑定渲染（三模式一视同仁，必须与 D32 同期）**
   ...
   - **为何必须与 D32 同期**：D32 让画线模式首次能切周期；若渲染仍按 `panelPosition` 过滤，用户在 60 分画的线会在竖滑到 15 分后继续显示在该面板上，而坐标系已换 = 线的位置是错的（codex R6-high）。
...
2. **周期绑定（D29，含复盘）**：某周期不在任一面板显示时，其画线不进 `RenderStateBuilder` 输出；切回后出现；`revealTick > tick` 的线仍不渲染；`panelPosition` 不再影响输出（造一条 `panelPosition` 与 `period` 冲突的线，断言按 `period` 落到面板）。**必须同时覆盖复盘模式的两层**（`engine.drawings` + `engine.reviewDrawings`），并覆盖 `style_json IS NULL` 的 legacy 行（period 由 `anchors.first.period` 兜底）。
3. **D32 与 D29 的联合不变量**：画线模式内竖滑切周期后，原周期的线**不再渲染在该面板**（R6 指出的中间态回归的直接回归测试）。
4. **D31 钩子，两面都要测**：
...
5. **复盘手势同步改善不算回归**：复盘浮动钮画线模式下，pan / 竖滑 / 缩放同样可用（D32 全局生效）；复盘的**入口与控件**仍是浮动钮（D26）。
...
| 5 | 在上半面板（假设显示 60 分）画一条线，竖滑切周期让 60 分挪到下半面板 | 那条线**跟着 60 分跑到下半面板**显示 | |
| 6 | 继续切周期，让 60 分完全不显示 | 那条线暂时消失 | |
| 7 | 切回来让 60 分重新显示 | 那条线重新出现，位置不变 | |
| 8 | 在**已经是最高周期**时再往上甩一次 | 周期不变，图表不乱（边界 no-op） | |
| 9 | 进复盘，单指竖滑切周期 | 训练时画的线**也跟着周期走** | |
| 10 | 找一局**改造前就存在的老记录**，进它的复盘 | 老画线正常显示，位置不错乱（老数据的周期由锚点兜底） | |
| 11 | 在复盘里用浮动钮进画线模式，单指横滑 | 图表能平移（D32 全局生效） | |
| 12 | 进复盘看画线入口 | 还是浮动铅笔钮，**没有**两行底栏 | |
| 13 | 再次训练（replay）模式 | 手势与周期绑定行为与训练一致 | |
