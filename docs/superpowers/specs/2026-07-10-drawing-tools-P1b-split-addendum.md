...
**P1c 的契约地位（codex R23-high）**：`DrawingToolType` 的 11 个 case 在 **1.11 已全部声明**（P1a merged），P1c 只是开始**写入**这些已声明的值 → **不扩展枚举值域、不 bump**。但必须带**跨版本解码举证**（下第 3 组）。
...
3. **跨版本 `toolType` 解码举证（D28）**：新写入的 `trend / channel / rect / polyline` 经 **finalize（`drawings` 表）/ pending（JSON blob）/ review（wrapper）三条持久化路径**往返不丢；且构造一条**未识别 `toolType`** 的记录，断言 P1a 的 lossy 层走**字节保真 `.unknownRaw`** 而非 crash、也不被普通保存覆盖。
