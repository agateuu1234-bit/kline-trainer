# 划线工具扩充 · P1b 拆分补充 spec（1a-i / 1a-ii / 1a-iii / 1b-i / 1b-ii / 2）
...
> **日期**：2026-07-10。
> **状态**：经 codex 对抗 review R1–R12。D28 举证 / D29–D37 / 六段切分均为 review 结论。**残留**：P1b-2（~1400 行）本 spec 不拆——它明确「不在本次 plan 内」，切一个尚未设计的 PR 是空想；**其 plan 阶段必须先拆**（见 §7）。
...
故 P1b 拆为**六个顺序 PR**：**1a-i → 1a-ii → 1a-iii → 1b-i → 1b-ii → 2**。
...
>
> - **R12-high**：1b（~600 行）与 P1b-2（~1400 行）仍超「≤3 子项 / ≤500 行」。1b 沿「有没有**原地改线**」的缝再切：删除会改数组长度（现有 count 触发够用），改样式 / 锁定 / 撤销都是原地改线（必须带 D30）→ **1b-i**（选中 + 删除 + 改选中线样式 + D30）/ **1b-ii**（锁定 + 撤销 / 前进）。**P1b-2 不在本 spec 拆**（见 §7）。
>
...
底栏 ②–⑤ 键、选中、删除、锁定、撤销 / 前进（1b-i / 1b-ii）；手势改动 / 同 period 钩子（1a-iii）；节点 / 多锚 / 四个新工具（P1b-2）；复盘专属一切（P5）；主页全局默认设置（P6）。
...
选中 / 删除 / 锁定 / 撤销 / 前进 / D30（1b-i / 1b-ii）；节点拖动分支 / 多锚 / 四个新工具（P1b-2）；复盘专属一切（P5）。
...
4. **删除**：点 🗑 → 弹确认 `确定删除划线？[删除][取消]`。（本期没有锁定，故 🗑 只在「无选中」时灰。）
...
6. **D30 内容级 dirty 信号（两处，缺一不可）**
   > 本期首次引入**原地改线**（第 3 条：改选中线的样式），故 D30 必须与它同期。删除会改数组长度，现有 count 触发本就够用；**改样式不会**。
...
锁定 / 解锁、撤销 / 前进、底栏 ②🔒④↩⑤↪（1b-ii）；节点 / 多锚 / 四个新工具（P1b-2）；复盘选中与复盘专属一切（P5）；主页全局默认设置（P6）。
...
2. **选中按 id + 平局规则（D33）**：造两条**同周期、同价格、几何完全重合但 id / 样式不同**的水平线，断言单击选中的是**数组靠后（最上层、后画）那条**的 `id`；随后的改样式 / 删除均作用于该 id，靠前那条**逐字段不变**。
...
4. **D37 选中转移**：选中 A → 单击空白处 → **A 被取消选中**、新线 B 被画出、**B 不处于选中态**、🗑 回灰；此时改样式**只改「下一条线的默认」**，A 与 B **逐字段不变**。
...
| 5 | 选中一条线（它变蓝），然后点图表空白处 | 画出一条新线；**原来那条不再是选中态**；🗑 变回灰；**新画的这条也不是选中态**（D37） | |
| 6 | 接着上一条，直接改颜色 | 改的是「下一条线的默认」——**刚才那两条线都不变色** | |
| 7 | 选中一条线，长按工具图标弹面板，改成绿色 | **那条选中的线**变绿（不是下一条） | |
| 8 | 不选中任何线，改成蓝色，再画一条 | 新线是蓝的，之前那条绿线不变 | |
| 9 | 选中一条线，点 🗑 | 弹出「确定删除划线？[删除][取消]」 | |
| 10 | 点「删除」 | 线消失，🗑 回灰 | |
| 11 | 先画一条绿线，再在**完全一样的价格**上画一条黄线（两条重合），单击这个位置、改成紫色 | 变紫的是**后画的那条黄线**（最上面那条）；先画的绿线**仍是绿的**（D33） | |
| 12 | 点「退出」再单击一条线 | **没有任何反应**（不变蓝、🗑 不出现） | |
| 13 | **改样式后立刻杀掉 App**（不点退出、直接从后台划掉），重开续这一局 | 改过的颜色 / 线型 / 粗细 / 标注**全部还在** | |
| 14 | **进「再次训练」新开一局**，什么都不做（不下单、不推进、不加线、不切周期），只把一条已有线改成紫色，**立刻杀掉 App**，重开续这一局 | 那条线是紫色的（专门验 replay 的 clean-skip 没吞掉改动） | |
| 15 | 选中一条 60 分的线（🗑 变亮），竖滑切周期让 60 分不再显示 | 线消失，**选中自动取消**，🗑 变回灰 | |
| 16 | 进复盘，用浮动钮进画线模式，单击一条训练时画的线 | **不选中**（线不变蓝、什么也没发生），只会落一条新线；原训练线的颜色 / 粗细一点没变（D34 门控） | |
| 17 | 进复盘看画线入口 | 还是浮动铅笔钮，**没有**两行底栏 | |
| 18 | 再次训练（replay）模式 | 行为与训练一致 | |
...
节点 / 多锚 / 四个新工具（P1b-2）；复盘选中与复盘专属一切（P5）；主页全局默认设置（P6）。
...
**P1b-2 的 plan 阶段必须先拆（codex R12-high，accepted residual）**：本 spec 不拆 P1b-2——它「不在本次 plan 内」，切一个尚未设计的 PR 是空想。但 ~1400 行 / 五个风险面（多锚泛化 · 节点模型 · 节点拖动手势 · 四个工具几何 · 折线临时 4 键）**明确超出「≤3 子项 / ≤500 行」**。写 P1b-2 的 plan 之前，**必须先按同样的方法把它拆成若干各自可 ship、各带负向测试的顺序 PR**（建议缝：①多锚泛化 + 趋势线 ②通道线 + 箱体 ③折线 + 临时 4 键 ④节点模型 + 拖节点 + 手势分支）。
...
- `CONTRACT_VERSION` 保持 `"1.11"`；**六个 PR 均无 schema 迁移**；`user_version` 保持 `7`。
