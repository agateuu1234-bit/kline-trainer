...
2. **选中（母 spec §7，去节点）**
   - 画线模式内单击 → 遍历**该面板当前周期所有可见画线**逐个 `hitTest`，命中返回 `DrawingObject.id`（**不用数组下标**）。
...
4. **选中按 id**：两条价格相同的水平线（几何重合）叠在一起时，选中 / 删除 / 改样式作用于正确的那一条。
...
| 14 | 再点 🔒 | 解锁，🗑 恢复可用 | |
| 15 | 画两条**价格完全一样**的水平线，选中其中一条改颜色 | 只有被点中的那一条变色 | |
