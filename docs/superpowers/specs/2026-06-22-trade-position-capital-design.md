...
1. **新增按股数报价（主路径）：**
   ...
   - **卖出校验（D7 清仓奇数股例外，codex R1-high）**：`shares > 0` 且 `shares <= holding`（否则 `.insufficientHolding`）；**当 `shares == holding`（清仓/全平）→ 放行任意股数（含非整手奇数股）**；否则（部分卖）要求 `shares % shareLotSize == 0`（否则 `.invalidShareCount`）。**理由**：`PositionManager` 持久化不变量**不强制整手**（resumed/legacy 仓位可能含奇数股），若清仓也要求整手则奇数残股永远卖不掉。与现有 `forceCloseOnEnd`（不做整手取整、直接平全部持仓）口径一致。host 测：`holding=150` 清仓卖 150 成功；`holding=150` 部分卖 50 拒（非整手且 ≠ holding）。
