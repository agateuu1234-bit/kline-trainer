...
- **`finalize(engine:)` / `DefaultAppDB.finalizeSession(...)`（retry-safe 契约——plan 阶段对抗 review 演进的最终定论，codex R-plan-2/5/9/10/12/13）**：`finalizeSession` 返回 **`(id: Int64, totalCapital: Double)`**；Normal 收尾**同一事务**内：插入记录（幂等锚 `sessionKey`）+ **条件清 pending**（仅当 `pending.session_key == 本次 sessionKey`，防过期重试误清更新 session 的在飞 pending）+ **派生权威资金**。
  ...
  - **非负权威资金不变量（codex R-plan-13-1/16-1，每个 settings 边界都守）**：`setTotalCapital`、`saveSettings.totalCapital` 加 `finite && >= 0` 守卫拒写；`loadSettings`/`parseDouble` 读 `total_capital` `< 0`/非有限 → **fail-closed `.dbCorrupted`**（与现 malformed 同路径；`startingCapital()` 直读 loadSettings 故继承此 fail-closed）；migration 0005 清理 legacy 负/非有限值为默认 10万（无记录也清，避免升级后开局 `.dbCorrupted` brick）。退化局（局终强平 手续费>持仓价值 → 净值<0）→ finalize 派生 **floor 到 0**（"不能欠钱"；冻结的 `performForceClose` 记账不动、记录仍如实记负 profit，user 拍板=持久化边界 floor）。
