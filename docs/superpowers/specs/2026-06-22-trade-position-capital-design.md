...
- **一次性数据迁移 `0005`（user_version 2→3）**：存在记录时 `UPDATE settings SET value = (SELECT total_capital + profit FROM training_records ORDER BY created_at DESC, id DESC LIMIT 1)`；无记录则不动（保持默认 10 万）。**排序口径必须与 `RecordRepositoryImpl.statistics()` 完全一致 = `created_at DESC, id DESC`**（codex R3-medium：未钉排序则同时间戳记录非确定性 → 回填错值 → 后续每局复利从错值滚雪球）。**纯数据 UPDATE，不加列、不改表结构**。host 测含同 `created_at` 多记录 tie-break（id 较大者胜）。保证老用户跨局累计资金不丢（D5）。
...
- **Host 可测纯逻辑**（Swift Testing）：
  ...
  - A4 资金口径：`startingCapital` 读 settings、finalize 写 settings、reset 保留记录+置 10 万、migration 0005 回填（有记录 / 无记录 / **同 `created_at` 多记录 tie-break：`id` 较大者胜**，codex R3）——用 in-memory fake DB（沿用 Wave-1 fixture 范式）逐值。
