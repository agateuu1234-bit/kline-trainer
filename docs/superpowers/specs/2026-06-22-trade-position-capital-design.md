...
> 评审通道：**`codex:adversarial-review` 是唯一权威评审通道**（CLAUDE.md governance backstop §1：所有触碰仓库的 PR 须过 codex:adversarial-review，verdict 作可强制的 GitHub 必需状态检查、非自证）。本 spec / plan / branch-diff 各阶段经 `.claude/scripts/codex-attest.sh` 跑真 Codex。任何非 Codex（如 Opus）review 仅作**辅助证据**，不构成批准通道。
> 性质：触碰 Wave-1 冻结契约 **E3 TradeCalculator**（RFC 授权演进）+ 持久化资金语义；**需 bump `CONTRACT_VERSION` 1.6→1.7**（A 类「改既有语义」，§11；连带触发 CODEOWNERS approve 门）。
...
成功判据（高层）：host swift test + Mac Catalyst build + iOS app build 三绿；§10 模拟器人工验收逐条通过；股数/资金算术由新增 host 测试逐值钉死；E3 契约演进有等价或更强的测试覆盖；无持久化序列化**结构**改动（bump 1.6→1.7 因**既有语义**变更、非结构，§11）。
...
### 2.3 性质与契约
- **0 后端 / 0 DDL 表结构改动**（A4 仅一次性 DML 数据迁移 `0005`：`user_version` 2→3，不加列/不改表结构）。`0005` 范围 = 回填 `total_capital`（floor≥0）+ **有界清理非负 settings 键 `total_capital`/`commission_rate` 的 legacy 腐坏值**（负/非有限/畸形→安全默认，配合 loadSettings 拒负 fail-closed；codex R-plan-19-1/20-1）；每处单键 `key=?` upsert，禁无 WHERE UPDATE，合法值不动。纯 iOS 表现层 + 引擎/计算器逻辑演进 + 资金读写口径变更。
- **bump `CONTRACT_VERSION` 1.6→1.7**（A 类「改既有语义」，§11；连带 CODEOWNERS approve 门）。
...
1. **新增按股数报价（主路径）：**
   ...
   quoteSell(cash: Double, holding: Int, shares: Int, price: Double, fees: FeeSnapshot) -> Result<SellQuote, TradeReason>
   ```
   - **买入校验**：`shares > 0 && shares % shareLotSize == 0` 否则 `.invalidShareCount`；`totalCost <= cash` 否则 `.insufficientCash`。
   - **卖出校验（D7 清仓奇数股例外，codex R1-high）**：`shares > 0` 且 `shares <= holding`（否则 `.insufficientHolding`）；**当 `shares == holding`（清仓/全平）→ 放行任意股数（含非整手奇数股）**；否则（部分卖）要求 `shares % shareLotSize == 0`（否则 `.invalidShareCount`）。**理由**：`PositionManager` 持久化不变量**不强制整手**（resumed/legacy 仓位可能含奇数股），若清仓也要求整手则奇数残股永远卖不掉。与现有 `forceCloseOnEnd`（不做整手取整、直接平全部持仓）口径一致。
   - **卖出可执行性集中契约（codex R-plan-14-1）**：`quoteSell` 入参带 `cash`，success 必须保证「**输出字段全有限** + **`cash + proceeds >= 0`**」——极端价令输出非有限 → `.invalidShareCount`；净现金为负（手续费>持仓价值且现金不够覆盖）→ `.insufficientCash`。**UI（TradeBoxContent）与 engine.sell 都经此 `quoteSell` 判可执行**，杜绝「框显可卖但 engine 拒 / 去格式化非有限值」的两步确认契约破坏。engine.sell 因此不再各自加守卫（quoteSell 单一真相源）。host 测：`holding=150` 清仓卖 150 成功；部分卖 50 拒；低价小手+免5+零现金净负 → `.insufficientCash`；极端价非有限 → `.invalidShareCount`。
...
- 新增 `buy(panel: PanelId, shares: Int) -> Result<TradeOperation, AppError>` / `sell(panel: PanelId, shares: Int) -> Result<TradeOperation, AppError>`：调 `TradeCalculator.quoteBuy(cash: cashBalance, shares:, price: currentPrice, fees:)` / `quoteSell(cash: cashBalance, holding: position.shares, shares:, ...)`（quoteSell 集中可执行性契约，§3.1），成功则 `position.buy/sell` + `cashBalance ±= ...` + 记 `TradeMarker`/`TradeOperation`（沿用现有 `.buy/.sell` 记账 + D6 字段口径：buy `stampDuty:0`，sell `totalCost:proceeds`）+ `advanceAndAccount`。**价仍为全局 `currentPrice`**（§5），`panel` 仍只影响①记录 period ②`stepsForPeriod` 推进步数。
- `forceCloseOnEnd` / `forceCloseManually` / `holdOrObserve` **不改**。
- `buyEnabled` **改**为 fee-aware：`maxBuyableShares(cash:price:fees:) >= shareLotSize`（能买至少 1 手即使能；取代旧 tier/totalCapital 门，配合 D3 删 tier 报价；codex R-plan-20-2）。`sellEnabled` **不改**（仍 `position.shares > 0`）。
...
- **`finalize(engine:)` / `DefaultAppDB.finalizeSession(...)`（retry-safe 契约——plan 阶段对抗 review 演进的最终定论，codex R-plan-2/5/9/10/12/13）**：`finalizeSession` 返回 **`(id: Int64, totalCapital: Double)`**；Normal 收尾**同一事务**内：插入记录（幂等锚 `sessionKey`）+ **条件清 pending**（仅当 `pending.session_key == 本次 sessionKey`，防过期重试误清更新 session 的在飞 pending）+ **派生权威资金**。
  - **权威资金来源 = 持久记录 `total_capital + profit`**（**非** caller 的 `engine.currentTotalCapital`）——因重复 `sessionKey` 重试时 `insertRecord` 返已存 id，用现值会与持久记录发散/回退。
  - **新插入**（当前 session 首次 finalize）→ `setTotalCapital(max(0, 持久 total+profit))` 并随成功返回；**重复/过期重试** → **不写**权威资金、返回**当前** `settings.total_capital`。此分支 `alreadyExisted=true` ⇒ 记录已存在 ⇒ 首次 finalize 必已写过权威值，故 `total_capital` **缺失/畸形/非有限/负 一律 fail-closed `.persistence(.dbCorrupted)`**（codex R-plan-25-2/26-1：缺失=settings 行丢失/迁移偏差的不一致状态，**非**全新安装，**绝不**静默返 10万——否则把累积资金重置回 10万、丢弃已结算盈亏；恢复经 `forceResetAndReload`→`repairAllToDefaults`）。事务内产出权威值随成功返回，**无 fallible 后置读**（杜绝已 commit 却报失败的半态）。
  - `coordinator.finalize` 用 finalizeSession **返回的** `totalCapital` 刷 `SettingsStore` 活缓存（主页结算后即时反映；`SettingsStore.refreshTotalCapital(_:)` 纯缓存、不写库）。
  - **非负权威资金不变量（codex R-plan-13-1/16-1/18-2，每个持久化边界都守）**：`setTotalCapital`、`saveSettings.totalCapital` 加 `finite && >= 0` 守卫拒写；`loadSettings`/`parseDouble` 读 `total_capital`/`commission_rate` `< 0`/非有限 → **fail-closed `.dbCorrupted`**（含 finalize 重复重试分支同口径解析；`startingCapital()` 直读 loadSettings 故继承此 fail-closed）；migration 0005 **对称清理** legacy 腐坏的 `total_capital`（→10万）**与 `commission_rate`（→0.0001 默认）**（负/非有限/畸形、无记录也清，避免升级后 parseDouble 拒负让老用户开局即 `.dbCorrupted` brick，codex R-plan-19-1）。退化局（局终强平 手续费>持仓价值 → 净值<0）→ finalize 派生权威资金 **floor 到 0**；冻结的 `performForceClose` 记账**不动**。**正常（无崩溃）finalize 仍如实记负 profit**（读 `engine.currentTotalCapital` 原值）。**崩溃恢复角的明确契约（codex R-plan-18-2/25-1）**：`saveProgress` 持久化 `pending.cashBalance` 亦 floor `max(0, cashBalance)`（否则负现金 pending → `TrainingEngine.make` 拒负 → resume brick）；其代价 = 若 app 在「局终强平产生负现金」与「finalize 清 pending」之间被杀，**那一局恢复后**按 floored（cash=0）终值入历史，盈亏比真实**少记 ≤ 一次强平佣金**（退化×崩溃窗双重罕见、方向偏保守）。**此恢复角不宣称真实负历史**——按 user「不能欠钱 / 持久化边界 floor」决策接受，测试钉死为定义行为（非静默）。
  - Review/Replay 不写（`shouldSaveRecord()==false`，沿用）。
...
- **一次性数据迁移 `0005`（user_version 2→3）**：存在记录时**只写 `key='total_capital'` 单行**——⚠️ **禁止无 WHERE 的 `UPDATE settings SET value=…`**（codex R2-1-high：settings 是 KV 表，键含 `commission_rate`/`min_commission_enabled`/`total_capital`/`display_mode`；无 WHERE 会把资金串写进**所有键** → 下次加载 bool/主题解析失败 → DB 判损 + 费率/主题被毁）。落地用 `SettingsDAOImpl.setTotalCapital(db, v)` 或 `INSERT OR REPLACE INTO settings(key, value) VALUES ('total_capital', v)`（upsert 单键，覆盖「该键可能不存在」情形），其中 `v = (SELECT total_capital + profit FROM training_records ORDER BY created_at DESC, id DESC LIMIT 1)`。**排序口径必须与 `RecordRepositoryImpl.statistics()` 完全一致 = `created_at DESC, id DESC`**（codex R3-medium：未钉排序则同时间戳记录非确定性 → 回填错值 → 后续每局复利滚雪球）。无记录则不动（保持默认 10 万）。**纯数据 upsert 单键、不加列、不改表结构**。host 测：① 同 `created_at` 多记录 tie-break（id 大者胜）；② **迁移后 `commission_rate`/`min_commission_enabled`/`display_mode` 三键值不变**。保证老用户跨局累计资金不丢（D5）。
...
- **`total_capital` 单写者（codex R-plan-22-1）**：`total_capital` 的写入唯一经 `setTotalCapital`（finalize/reset/migration）；`SettingsDAOImpl.saveSettings` **不再写** `total_capital`（只写 commission/min/display 偏好键），`SettingsStore.update` 提交缓存时**保留当前权威 total_capital**（不被旧偏好快照回滚）。否则延迟的偏好保存（佣金/主题）会用陈旧快照覆盖已 finalize 推进的资金 → 盈亏静默回滚。既有 `saveSettings` 持久化 total_capital 的测试相应更新。**腐坏恢复**（codex R-plan-24-1）：因 saveSettings 不再写该键，`SettingsDAO` 新增 `repairAllToDefaults()`（写全部键含 total_capital=默认，仅恢复用），`SettingsStore.forceResetAndReload` 改调它 → 腐坏/负 total_capital 仍有 in-app 修复路径。
...
- **D7（清仓奇数股例外，codex R1-high）**：`quoteSell(shares:)` 在 `shares == holding`（清仓/全平）时放行任意股数（含奇数），仅部分卖才要求整手。理由 = `PositionManager` 不强制整手不变量，resumed/legacy 奇数仓位必须可清仓（§3.1）。
- **D8（bump `CONTRACT_VERSION` 1.6→1.7，codex R2-medium）**：A4 改 `settings.total_capital` 既有语义 + reset 语义 + 0005 DML migration → 命中 m01 §A 类「改既有语义 / DML 数据 migration」→ 必 bump（同 E2 1.4→1.5 先例）。落地清单见 §11；连带 CODEOWNERS approve 门。
...
- **Host 可测纯逻辑**（Swift Testing）：
  ...
  - **D7 清仓奇数股（codex R1）**：`holding=150` 清仓 `quoteSell(shares:150)` 成功（放行非整手）；`holding=150` 部分卖 `quoteSell(shares:50)` 拒 `.invalidShareCount`（非整手且 ≠ holding）；`holding=200` 部分卖 100 成功。
  ...
  - A4 资金口径：`startingCapital` 读 settings、finalize 写 settings、reset 保留记录+置 10 万、migration 0005 回填（有记录 / 无记录 / **同 `created_at` 多记录 tie-break：`id` 较大者胜**，codex R3 / **迁移只动 `total_capital`，`commission_rate`·`min_commission_enabled`·`display_mode` 三键不变**，codex R2-1）——用 in-memory fake DB（沿用 Wave-1 fixture 范式）逐值。
...
| 13 | 点「买入」在框里填一个数量（如 500），**不关框**直接点 T2「卖出」 | 卖出框重新弹出且数量框**归 0**（非残留 500）；确认键文案随新数量（codex R-plan-21-1：`.id` 绑 strip 请求重置 @State） | 新框为 0 = pass；残留旧数量 = fail |
| 14 | 买框里手输一个非整手/超限数量（如 250 或超「可买」），直接点确认 | 确认键文案 = 将提交的合法股数（如 250→200、超限→可买上限）；成交记录的股数 = 该值；数量框失焦/确认后亦显该值（codex R-plan-23-1：显示==提交） | 文案=记录股数、显示==提交 = pass；显示一个量却成交另一个量 = fail |

---

## 11. `CONTRACT_VERSION` bump 论证（1.6 → 1.7，A 类改既有语义；codex R2-medium 修正）

**结论：必须 bump `CONTRACT_VERSION` `"1.6"` → `"1.7"`。** 权威依据 = `docs/governance/m01-schema-versioning-contract.md` §Bump 策略 **A 类**：「**改既有语义**」「**app.sqlite … DML 数据 migration**」任一即必须联动 bump 顶层。

- **A4 命中「改既有语义」**：`settings.total_capital` 键虽**已存在**、结构不变，但其**语义从「配置起始本金（冷启动地板值）」改为「权威滚动当前资金（每局结束写入）」**；`resetAllTrainingProgress` 语义从「清记录 + 置本金」改为「**保留记录** + 置本金」。同一 DB 键在新旧代码下被**不同解读**（旧码把滚动资金当冷启动地板）→ 跨版本不兼容 → A 类 bump。
- **直接先例**：2026-05-25 Wave 1 E2 把 `position_data` 从「任何字节」收紧为「合法否则拒收」属「改既有语义」、**无 DDL**，照样 bump `"1.4"→"1.5"`（见 m01 §bump 记录）。本 RFC 与之**同类**（既有持久化语义变更、无结构变更）。
- **migration 0005 = app.sqlite DML 数据 migration**（user_version 2→3）→ 亦命中 A 类「DML 数据 migration」联动条件。
- **不属 bump 范围但记录**：E3 `TradeCalculator` 纯计算函数（不序列化）；E2 `PositionManager` 零改动；`TradeOperation` 序列化结构不变（D4 仅定 `positionTier` 取值，不增删字段）；A3 顶栏纯展示（不持久化）。这些**单独**不触发 bump，但 A4 已足够触发。

**bump 落地清单（plan/impl）**：
1. `Models.swift` 顶层 `CONTRACT_VERSION` `"1.6"`→`"1.7"`；同步 `ModelsTests.contractVersionIs1_6()` 断言（改名/改值为 1.7）。
2. `docs/governance/m01-schema-versioning-contract.md` 增一条 bump 记录（A 类「改既有语义」，settings.total_capital 语义迁移 + 0005 DML migration，同 E2 先例）。
3. 本 PR 描述按 m01 §应用范围**点名引用 m01 §Bump 策略 A 类**。
4. ⚠️ **治理门**：更新 m01 治理文档触发 `codeowners_required_globs` → **PR 需 user Approve（CODEOWNERS）**，且本 PR 已过 codex:adversarial-review（本 spec + plan 阶段）。merge ceremony 须含此 approve。
