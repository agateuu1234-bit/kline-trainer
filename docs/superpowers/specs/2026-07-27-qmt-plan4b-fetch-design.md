...
| S2-F46 | **high** | **R16 的「上报 == 登记」写成了无条件相等，把唯一合法的恢复路径堵死** —— `max_bytes_stop()` / `escape_stop()` 的 `staging_recheck` 恒为 `None`（工厂不收该参数，因为决策表这两支根本不看它），而一旦这一轮拉过股就**必然**登记过 `"passed"`（per-stock 的复校闸逼着它登记）⇒ 收尾当场被拒，报的还是「这份账本已不可信，请换新 staging + 新 seed 重拉」。**正是 O4-F1 说「几乎必然」的那个场景**：Run1 撞 `staging_path_escape` 停在 56 只股 → 操作者修好树 → Run2 复校通过、拉股、触到 `--max-bytes`（累计含 Run1）→ **一棵几百只股的健康 staging 被劝报废**，而 O4-F1 特意要产出的 `stopped_reason_secondary: "max_bytes"` 一个字都没写下去；escape 那一支更重：本次**新发生**的信任边界破坏根本记不进账本 | Opus 对抗性评审 [high]，控制者本机端到端复现（决策表本身算得对，端到端被拒）| 一致性检查**只在 `kind == "clean"` 那一支**强制相等（只有它消费前提②）；另加一支：**登记为 `failed` 时只许发布 P2-F3 的具名结局**（否则 `max_bytes_stop()` 会把「复校没过」整个盖掉）。并把「非 clean 结局不得携带 `staging_recheck`」从工厂**下沉到 `__post_init__`**（公开 dataclass 可直接构造，守卫立在工厂等于不存在）。⚠️⚠️ **根因是版本化不变量漏检**：`max_bytes_stop()` 的理由「决策表这一支根本不看它们」对 `resolve_final_lifecycle` 成立，而 R16 给 `outcome.staging_recheck` **新增了第二个消费者**，那句写在注释里的理由没被重核 —— **加消费者时要回头重核所有「因为没人看所以不传」的理由** |
| S2-F47 | medium | **账本上没有那类记录时，`attest_staging_recheck(passed=False)` 被静默丢弃** —— 闸只看**起跑状态**，故 per-stock 提交照样放行；收尾走决策表分支②（上次没有 fatal）返回 `{}`，「这棵树复校没过」**哪都没写**，而 4c 读到的是一份完全干净的账本 | Opus 对抗性评审 [medium]，控制者本机复现两档 | **拒绝登记**，而不是「把它记下来」：pilot 的 fail-closed 判据是「`fetch_fatal_error` 存在」（O4-F1），`stopped_reason` 只是人读附注 ⇒ 只写 reason 是**假安全**，要真生效就得**凭空造一个 fatal**（四字段一个都造不出）。本入口只为前提②存在。⚠️ **已接受的残留**：这一轮的失败由 S4/S5 在**运行级**（rc≠0 + 报告）承担 —— 与 S2-F14 同规格 |
| S2-F48 | medium | **`--skip-existing-verify` 的互斥窄于 spec 原文** —— §4.5:484 写的是「与『manifest 带 **escape 记录**』互斥」，**未限定 kind**；实现只挡 `_needs_staging_recheck` 那两种（staging 侧）。于是带 `source_path_escape` 的账本配上该 flag 能正常起跑，**并在同一轮把 fatal 清掉**（清除 source escape 只要前提①）—— 而这一轮恰恰跳过了「已 staged 的文件与源对不对得上」，正是该 flag 跳过的那件事 | Opus 对抗性评审 [medium]，控制者本机端到端复现（起跑放行 → 同轮 fatal 被清）| 判据改为「`fetch_fatal_error` 在不在」——它才是 O4-F1 定的**粘性信任状态**。⚠️ 顺带确认读侧**双向**都强制了 `REASONS_REQUIRING_FATAL`（实测两值无 fatal 均被拒），故不必再比字面量 |
| S2-F49 | medium | **两条累计字节转移守卫「没人守」** —— S2-F45 新加的固有下限把两条老攻击档（值写 `1` / 加文件而累计值原地不动）**抬到了门槛以下**，固有检查抢先抛且消息里同样含 `committed_bytes`，`match` 照样匹配 ⇒ 两条守卫**单独关、一起关都零红**（本机变异实测 305 全绿）。守卫在生产里仍在做实事（停在下限之上的回滚只有单调守卫拦得住），故是**测试判别力缺陷**，但它让将来任何一次改动都能悄悄把守卫删掉 | Opus 对抗性评审 [medium]，控制者本机变异复核三档全零红 | 两档都**抬到固有下限之上**（退到恰好等于下限 / 预留 500 万字节余量），并把断言从裸词 `committed_bytes` 改成判据专属措辞「至少涨够」。⚠️⚠️ **判据（对 S4/S5 全适用）**：**收紧一条判据时，要回头重跑「原本靠它的邻居档」的变异** —— 新下限会把老攻击样本挪到新判据的射程内，两条判据从此互相掩盖 |
| S2-F50 | low | **「只许一处判定」的 AST 守卫自己漏了一半** —— 它只扫 `Compare.comparators` 且只认 `staging_path_escape` 一个字面量。于是 `"staging_path_escape" == kind`（常量在**左边**）、`kind in ("staging_path_escape",)`（比较项是 Tuple 不是 Constant）、以及任何拿 `staging_recheck_failed` 另判一处，**三种都查不出来** —— 而 `_needs_staging_recheck` 正是按**两个**字面量判定的（O4-F3 解耦） | Opus 对抗性评审 [low]（读代码提出），控制者**注入两个诱饵函数**实测坐实（旧守卫两档全绿）| 扫 `left` + `comparators`，展开 Tuple/List/Set，字面量集合补上 `staging_recheck_failed`；两个诱饵档现在都能把它打红 |
| S2-F51 | medium | **复校闸只立在 per-stock 那个入口** —— `commit_final` 从不看「这一轮欠不欠复校」，它唯一的进度冻结只在**上报** `staging_recheck == "failed"` 时触发，而一次运行**完全可以从不登记任何结论**。本机端到端复现（**同一份 payload、同一次运行**）：`commit_stock` 被拒 ✅ 而 `commit_final` **被接受** ❌，`cursor` 1→3、`files` 4→8、`committed_bytes` +490 万。于是一棵**已被证明动过**的 staging，只要把进度全走收尾入口，就能一轮一轮把冻结宇宙与 `--max-bytes` 预算烧光 —— 正是 P2-F3 存在的理由 | Opus 对抗性评审第 2 轮 [medium]，控制者本机端到端复现 | 闸的对象从「**入口**」改成「**进度**」：`start_needs_recheck and recheck != "passed"` ⇒ 收尾 payload 的进度指纹必须与**运行起点**逐项相同。⭐ 新判据**完全涵盖**旧的那条（上报 failed ⇒ 登记 failed ⇒ 欠复校且未通过），两条合并成一条。⚠️ 冻结的是**进度**不是**提交** —— 这一轮仍要能把「我看见 fatal 了、我停了」记下去（已配正向档，「一律拒绝收尾」那个修过头档红 4 条）。另给 `RunLedger` 加只读属性 `owes_staging_recheck`：不暴露时 S4 只能自己重推 `_needs_staging_recheck`，正是 AST 守卫要禁的「同一件事判在两处」|
| S2-F52 | low | **进度指纹四个分量里只有 `files` 被钉住** —— P2-F3 那条档用 `_with_more_stocks` 一次推进 files+pool+cursor+committed_bytes，**任一分量单独满足不等式就够**。实测把 `cursor` 或 `committed_bytes` 从 `_progress_of` 的元组里删掉，全量 1301 条**一条都不红**；而 `cursor` 恰是 P2-F3 点名的第一个，`committed_bytes` 写在本模块自己的报错文案里 | Opus 第 2 轮 [low]，控制者变异复核两档均存活 | 补两条**单分量推进**的参数化档（只推 cursor / 只涨 committed_bytes），变异下各红 1 条、互不掩盖 |
| S2-F53 | low | **`F_FULLFSYNC` 的「文件内容」那半没人守** —— 三处断言都是「`F_FULLFSYNC` 出现过」或「replace **之后**有屏障」，**改名后那道目录屏障自己就能满足它们**。实测把文件内容那半退回普通 `os.fsync`，全量一条不红 —— 而那条档的标题与 docstring 恰恰声称钉的就是这一半。⚠️ 因为 `F_FULLFSYNC` 是**设备级**屏障，`os.fsync(文件)+F_FULLFSYNC(目录)` 按 man page 原文仍是耐久的 ⇒ 这是**测试主张缺陷**，不是活的耐久性洞 | Opus 第 2 轮 [low]，控制者变异复核存活 | 两条一起加强：①`full_sync=True` 时断言普通 `os.fsync` **零次**（macOS 上两处都走 `full_fsync`）；②时序档改为断言 `replace` **两侧各有**一道屏障。变异下两条一起红 |
| S2-F54 | low | **`_STOCK_CODE_RE` 的 `\Z` 没人守** —— 模块注释声称「三个正则都改成 `\Z`，按判据本身穷尽」并给了实测依据（不加 `re.MULTILINE` 时 `$` 容忍**恰好一个**尾随换行），而三个里只有 `_SHA256_RE`/`_GMT_TOKEN_RE` 被钉住 | Opus 第 2 轮 [low]，控制者变异复核存活 | 走 `RecoveryScope` 补档 —— manifest 里那三个调用点**没有一个隔离得开**（后缀比对 / 冻结名单锚点 / 文件名解析对表都会抢先接住换行）。⚠️⚠️ **补的第一版还是假的（控制者变异实测抓出）**：`RecoveryScope` 另有第四条 `endswith("." + market)`，同样拒尾随换行，而它的文案里也含 `stock_code` ⇒ `match="stock_code"` 照样匹配、档照样绿。改为断言**判据专属措辞**（正则报「形如 600000.SH」/ 后缀报「后缀与 market 不一致」）后判别力才成立。**同一族错误在修它的过程中原地又踩一次 —— 见 S2-F49** |
| S2-F55 | low | **池内 `universe_idx` 唯一性判据造不出专属档**（如实登记，非缺陷）—— 重复的 `universe_idx` 必然蕴含重复的 `code`，而冻结名单锚点 `universe[mk][idx] == code` 会**抢先**拒掉。它本就在 sweep 那 9 条「邻居兜底」名单里、且已机械归因 | Opus 第 2 轮 [low] 建议「补档或如实登记」，控制者核实为结构性等价 | **如实登记，不造假档**（与 files/pool 那几处同规格）|
| S2-F56 | medium | **「先 open、后查 `S_ISREG`」对 unix socket 整个失效** —— `O_NONBLOCK` 解决了 FIFO 阻塞，但 socket 上 `open(2)` **直接失败**（macOS `ENOTSUP`=102，别处 `EOPNOTSUPP`/`ENXIO`），于是**根本走不到**那句 `S_ISREG`。本机复现：FIFO 与目录都拿到干净的 `ManifestInvalidError` + 恢复指引，**socket 抛裸 `OSError [Errno 102]`** —— 突破了 `read_manifest` 文档声明的异常集合（`ManifestInvalidError`/`ManifestVersionError`/`PathEscapeError`），照文档写 `except` 的 S4/S5 接不住，操作者拿到 traceback 而不是「请换新 staging + 新 seed 重拉」。⚠️ 而模块注释白纸黑字写着这条枚举是「FIFO / 目录 / 设备 / **socket**」—— **列了四项只兑现两项**，正是「穷尽性主张必须按字面量逐条定性」那一族。**不是洗白洞**（没有任何东西被放行），故 medium | Opus 对抗性评审第 3 轮 [medium]，控制者本机三档对照复现 | **修在共用探测器 `_open_regular_probe` 本身**（它有**四个**调用点：取锁 ×2 / 读归属标记 / 读账本，同一形态）：打不开就回头 `lstat` 问文件系统「那到底是个什么东西」，不是普通文件即抛新的 `NotARegularFileError`。三条设计判断：①**不枚举 errno**（各平台不同、还会变）；②新异常**继承 `OSError`** ⇒ S1 那三个既有调用点**一个字不改、行为不变**（从裸 `OSError` 变成带清楚文案的子类，严格改善）；③实测确认 **`PathEscapeError` 不是 `OSError` 的子类**，故新加的 `except OSError` 不会吞掉路径逃逸那条线。另把「不是普通文件」的文案抽成模块常量 —— 现在有**两条路**通向同一结论，文案写死一处免得指引漂移。⚠️ **已接受的残留（超出本片）**：`qmt_fsroot.acquire_lock` 等 S1 调用点仍缺**领域专属**的翻译（它们现在拿到的是 `NotARegularFileError` 而不是 `LockDisciplineError`）—— 那是 S1 既有代码、**不在本次 diff 里**，本片不动 |
| S2-F57 | low | **收尾入口剥掉生命周期三键那一步没人守** —— 实测把 `basis = {k: v … if k not in LIFECYCLE_KEYS}` 换成 `dict(manifest)`，全量 1307 条一条都不红。而它真承重：一棵**干净**的 staging 上，调用方内存里塞一条伪造的 `fetch_fatal_error`，收尾就把它**写上磁盘**（`basis.update(on_disk)` 只挡得住「抹掉」方向，挡不住「注入」方向）——一棵健康的 staging 被一条假警报永久锁死。方向是 fail-closed 故 low。⚠️ `commit_stock` 那一侧的同族档**是有的** ⇒ **测试集的不对称，不是代码的不对称** | Opus 第 3 轮 [low]，控制者变异复核存活 | 补 `commit_final` 的注入档，与 `commit_stock` 那条对称 |
| S2-F58 | low | **进度指纹的 `files` 与 `pool_order` 两分量互相掩盖** —— 实测把其中**任一个**从 `_progress_of` 的元组里删掉，全量一条不红：每个推进进度的档都是「加一只股」，两者必然**同时**变化，留下的那个自己就让不等式成立。而 P2-F3 的原文恰是「不得新增 `files`/`pool_order`」| Opus 第 3 轮 [low]，控制者变异复核两档均存活 | ⚠️ **端到端档造不出隔离**（R21-F3 把两者结构性绑死：每个池条目恰配两条 files），故直接对**纯函数** `_progress_of` 下参数化断言 —— 四个分量各一档。变异实测：四条各杀各的，互不掩盖 |
| S2-F59 | low | **三条小守卫没有任何测试守着**（实测关掉后全量皆绿）：①`RunLedger._state()` 的注册表查找 —— 而 `commit_stock` 的 docstring 自称「真正的防线在句柄的**属性访问**上」，唯一那条伪造句柄的档走的却是**入口**那道，被称为主防线的一道反而没人守；②`commit_stock` 的 `isinstance(recovery, RecoveryScope)`；③`_validated_escape` 的 `isinstance(escape, Mapping)` | Opus 第 3 轮 [low]，控制者变异复核三档均存活 | 三条各补专属档（伪造句柄逐个属性访问 / 鸭子类型的 recovery / 非映射的 escape 证据）|
| S2-F60 | medium | **不可哈希的 JSON 值让写侧新守卫抛裸 `TypeError`** —— 四处拿 JSON 值直接当键：重名检查的集合、`_attempts` 的字典键、`_by_id` 的元组键、`_progress_of` 的 frozenset。而 `failures` 是**扩展字段、读侧 `validate_manifest` 完全不碰它**（实测：`stock_code` 是 list 的账本读侧照样放行），且这些守卫**排在 `_write_manifest` 的读侧校验之前** ⇒ **没有任何上游判据接得住**，调用方拿到 `TypeError: unhashable type: 'list'` 的 traceback 而不是恢复指引。「**守卫自己被它该抓的那种损坏弄坏了**」在本片是**第四次**（S2-F9 / S2-F27 同族）| Kimi(K3) 评审 K1 [medium]，控制者本机四档复现 | 两层：①按本仓既有做法「**先验类型再比取值**」——`failures[].stock_code` 必须是字符串（读侧不管它，这里是唯一把关处）；②`_hashable()` 让四处守卫**总是算得出键**。⚠️ **不是「跳过坏值」而是「换一个稳定的键」**：相等的坏值仍映到同一个键（dict 按键名排序归一），重名/单调判据照常成立（S2-F29）；正常取值**原样返回**，故 `_proved_success` 那类「拿键回去比对原值」的逻辑不受影响。⚠️⚠️ **控制者自查补档**：`_hashable` 注释里claim 的**另一半**「不同的坏值必须映到不同的键」**没人守** —— 退化成「所有坏值返回同一常量」全量一条不红，而它失守的方向是**放行**（两条不同的坏记录被折叠成一个键、后者覆盖前者 ⇒ 单调判据对被覆盖那条整个不生效）。已补两条纯函数档，变异下红 3 条 |
| S2-F61 | low | **写侧大小上限那条档其实钉的是读侧** —— `test_the_write_side_limit_is_measured_on_the_bytes_actually_published` 的后半段把上限调到 `exact-1` 后仍在**同一棵**目录上提交，而盘里已有那份恰好 `exact` 字节的账本 ⇒ `begin_run` 一读就先被**读侧**上限拒掉，`commit_stock` 根本没被调用。实测把写侧那段检查整个删掉，本档仍绿 | Kimi K1 [low]，控制者变异复核坐实 | 换一棵**空**的 staging（引导态下读侧无从插手），并把 `match` 从两侧共有的「过大」换成写侧专属的「**拒绝发布**」。⚠️ **拿两条判据共有的措辞做断言**正是 S2-F49 / S2-F54 那一族，本片**第三次** |
| S2-F62 | low | 注释引用了**已不存在的** `_lifecycle_from_disk`（改名为 `_expect_from_disk` 后没跟上），读者按注释找不到该函数 | Kimi K1 [low]，grep 全仓确认仅此一处生产代码命中 | 改正 |
| S2-F63 | medium | **整族坏值扫描只覆盖读侧，写侧守卫一个都没被扫过** —— `test_no_field_of_any_json_type_can_escape_as_a_raw_exception` 只喂 `validate_manifest`，而 S2-F60 那条 medium 恰恰出在**写侧**。控制者把同一套扫描对准写侧后，2988 次探测（0.05 秒）**当场又挖出 7 类 34 个逃逸，全在 `_pool_ids` 一处**：①`.get(mk, [])` 的默认值**只在键缺席时生效** ⇒ `pool_order.SH = null` 交出来的就是那个 `None`，`for e in None` 抛裸 `TypeError`（6 值 × 3 层 = 18）；②集合成员没过 `_hashable` ⇒ `code` 是 list/dict 时集合推导自己崩（16）。⚠️⚠️ **这两个 bug 我都已经在它旁边那个帮手 `_pool_seq` 里修对过**（上一轮加它时写的就是 `isinstance(lst, list)`），只是没回头看隔壁 —— **同一件事判在两处、只改对了一处，本片第三次** | 控制者自查（把 S2a 那套扫描扩到写侧），非评审报出；线索来自 Kimi K2 被额度掐断前留下的分析轨迹 | 两个帮手**合并成一个访问器**（`_pool_ids` 从 `_pool_seq` 派生并过 `_hashable`）；顺带修 `sorted(gone)` —— 混着字符串与包装元组时比大小会抛 `TypeError`，改 `key=repr`。**扫描固化成常驻测试**（读侧那条的写侧对偶，带防空转下限）。⚠️ 不直接跑 `commit_stock`：每次真落盘（`F_FULLFSYNC`）会把整套测试拖慢一个量级；**入口确实接上了这些守卫**由四条走真入口的点测钉住，分工写进 docstring。变异实测三档全中，其中「把枚举器改空转」让**读侧与写侧两条扫描一起红** |
| S2-F64 | **medium** | **单调守卫被「上一份的值必须已经是良型 int」整条关掉** —— R10 那次修的是**新值**，而**旧值仍是整条检查的前置条件**（`if v is None: continue` 排在最前）。于是一条 `attempts` 缺席/坏型的 failures 记录，**连「凭空移出必须自证成功」都一起被跳过**。⚠️⚠️ **不需要任何篡改，用本模块自己的公开 API 就走得通**：读侧完全不碰 `failures`，而写侧当时只校验 `stock_code` 的类型 ⇒ 本模块自己就写得出没有 `attempts` 的记录；下一次提交把它整个删掉，那只股既不在池里也没有 files 记录（**毫无成功证据**）却被凭空移出 ⇒ 补拉重新挑中它 = codex R10/R12 定为 high 的「**复活一个已耗尽重试的候选**」。同族另两处（`committed_bytes` / `inflight_rollbacks`）需外部写入的账本才触发 | Opus 第 4 轮 [medium]，控制者本机端到端复现三档 + 良型对照档 | ①写侧把 `attempts` 提到与 `stock_code` **同等待遇**（必须存在且为非负整数）；②「凭空移出」的判定**前移**，与旧值是不是数字无关；③上一份的单调值坏型时**拒绝而不是跳过**（三处）。⚠️⚠️ **两条我自己的判据被评审推翻/补全**：(a) 我给 `stock_code` 加校验时写的理由是「它是『哪只股失败过、**还剩几次重试**』的唯一标识」—— 而记着还剩几次的是 `attempts`，**按字段穷尽又漏一次**；(b) 我引 O2-F8 说「加进读侧构成新增必需字段、要 bump 版本」—— **「键存在时校验它的类型」既不是新增必需字段、也不需要 bump**，那条理由盖不住这个缺口 |
| S2-F65 | **medium** | **复校凭据与进度冻结只锚定「运行起点」，而 `commit_final` 不是每轮只能调一次** —— 本轮**自己发布**的 fatal 不会重新武装它们。两档本机复现：①账本带旧逃逸 → 登记复校通过（针对**旧**那条路径）→ 收尾发布一条**新路径**的 staging 逃逸 → **同一轮**再收尾一次，fatal 被清光（**那份凭据早于它要清掉的那条逃逸**）；②干净起步 → 收尾发布 escape → `commit_stock` 照样推进 files 4→6、cursor 1→2，而盘上带着未解除的逃逸（spec:1035 / P2-F3 明禁）| Opus 第 4 轮 [medium]，控制者本机复现两档 | 在 `_advance` 里按「**生命周期变了且需要复校**」重新武装（`start_needs_recheck=True` / `recheck=None` / `start_progress` 重锚）。⚠️ **判据特意不是「现在需不需要复校」**：per-stock 提交把三个字段**原样带过去**，那样写会让一次登记通过的运行提交第二只股时把自己的凭据清掉、**从此卡死** —— 已配修过头档（变异实测：正向的「一轮三次 per-stock 提交」当场红）|
| S2-F66 | low | **权限错误逃出 `read_manifest` 文档声明的异常集合** —— 一个**普通文件**打不开（`EACCES` / `EMFILE`）时原样上浮 `PermissionError`，而 docstring 只声明了三族 | Opus 第 4 轮 [low]，控制者本机复现（用 monkeypatch 而非 chmod：CI 可能以 root 跑，chmod 拦不住，本仓零跳过不能靠 skip 绕）| **不翻译**，而是把 `OSError` 一族**如实写进声明**并写明理由：`ManifestInvalidError` 那族的指引是「换新 staging + 新 seed 重拉」，而这里正确的动作是 `chmod` —— 报错族会让操作者**报废一棵几百只股的健康目录**。⚠️ 探测器的兜底因此**必须**先问「是不是普通文件」（评审实测该判据无人守，已补专属档）。**对 S4/S5 的硬性要求**：`except` 清单里把它与另三族分开处置 |
| S2-F67 | low | **恢复豁免里的「必须是那一只」没人守** —— 删掉 `and key[0] == recovery.stock_code` 全量一条不红。它真承重：一次合法移除在途股 R 的提交，可以**同时**把**另一只**已提交股 X 的两条 `files` 记录改绑到别的路径（codex R13 定为 high 的原状）。既有那条改绑档改的是**恢复目标本身**，被 `scoped_removed` 保持 False 接住，**走不到这条豁免** | Opus 第 4 轮 [low]，控制者变异复核存活 | 补「移除 R 的同时改绑 X」的专属档 |
| S2-F68 | low | **`_FROZEN_KEYS` 里 `staged_export_log` 那一项没人守** —— 删掉它全量一条不红：`sha256` 的漂移被冻结的 `source_snapshot` 掩盖，而 `relative_path` 与 `bytes` **没有**，把 `bytes` 调小会让 `--max-bytes` 的下限**少算一整份 export_log** | Opus 第 4 轮 [low]，控制者变异复核存活 | 补两档（`relative_path` 被换 / `bytes` 被调小）|
| S2-F69 | **high** | **发布 P2-F3 明令要发布的那个具名结局，会把本轮自己的「复校失败」凭据清掉** —— 上一轮（S2-F65）把重新武装绑在 `_needs_staging_recheck` 上，而 `staging_recheck_failed` + 保留 fatal **本身就满足它** ⇒ `st.recheck` 被重置为 `None`，「结论不许改口」那道闸凭空消失。全程公开 API、没有篡改也没有说谎：`attest(False)` → `commit_final(staging_recheck="failed")`（盘上正确记下）→ **`attest(True)` 被接受** → 再收尾一次 → **fatal 与 stopped_reason 全被删掉**，一棵**已被证明复校没通过**的 staging 带着干净账本出货，pilot 的 fail-closed 判据（O4-F1）没了 | Opus 第 5 轮 [high]，控制者本机端到端复现 | ①`failed` 改为**粘性**（`_advance` 永不重置它）；②另加一条不看 kind 的硬判据「**本轮自己发布的 fatal 不许在同一轮里清掉**」兜底。⚠️ 既有那条 `..._cannot_be_flipped_once_attested` 之所以是绿的，只因为它把两次 `attest` **背靠背**放、中间没有 `commit_final` —— 有意思的次序它的 setup 到不了 |
| S2-F70 | **high** | **重新武装对 `source_path_escape` 完全不生效** —— 判据是 `_needs_staging_recheck`，而它对源侧逃逸为 False。可 spec:1025 与 spec:1035 **措辞一致**（不记 failure / 不加 attempts / 不推进 cursor，**整次 fetch 致命**）⇒ 这是**代码与 spec 不符**，不只是不对称。两档复现：(a) 发布 `source_path_escape` 后 `commit_stock` 照样把 files 4→6、cursor SH 1→2、累计字节 733 万→980 万；(b) 同一轮第二次收尾（声称重走过源路径）把 fatal 与 stopped_reason **全清光** | Opus 第 5 轮 [high]，控制者本机复现两档（(b) 第一版复现脚本自己写错了 —— payload 过期被回滚守卫以**别的理由**拒掉，改用与磁盘一致的 payload 后确认成立）| **把两件事拆开**：①「整次致命 ⇒ 冻结进度 + 本轮不许再清」对**任何** kind 成立，由新的 `fatal_published` 标志承担；②`_needs_staging_recheck` 只回答「**清除它要哪一道前提**」。⚠️ **一个谓词回答两个问题就是缺陷的温床** —— 本片两条 high 同源于此 |
| S2-F71 | medium | **`--skip-existing-verify` 的互斥只在启动时判一次** —— 一次**干净起步**的运行带该 flag 合法启动，随后**自己发布**一条 staging 逃逸，此时状态与「启动时就带着 escape 记录」完全等价，而模块什么都没复判：`attest(passed=True)` 被接受、下一次收尾把 fatal 清掉。而该 flag 的定义就是**跳过对既有文件的校验**，这样的一轮**不可能**产出「全量复校通过」这个结论 | Opus 第 5 轮 [medium]，控制者本机复现 | `attest_staging_recheck` 直接拒绝任何声明了该 flag 的运行（`SkipVerifyWithEscapeError`）。⚠️ 这是「调用方口头上报的不能当凭据」那条原则**唯一能交叉核对却一直没核**的那个调用方声明 |
| S2-F72 | low | **决策表的配对纵深防御只查了一个方向** —— 它 fail-close「有 fatal 却没 reason」，而 `_require_escape_pairing` 在 `fatal is None` 时直接返回 ⇒ 反方向（reason ∈ `REASONS_REQUIRING_FATAL` 却没 fatal）掉进「上次没有 fatal」那一支返回 `{}`，**静默丢掉一个「已证明复校没通过」的结论**。走 `commit_final` 到不了（磁盘那份先过读侧），纯粹是 docstring 自称提供的那层没做全 | Opus 第 5 轮 [low]，控制者直接调用复现 | 在决策表里补上反方向那一半 |
| S2-F73 | low | **上一份的「容器类型」不对时整条判据被跳过** —— 同一次提交里刚给 `committed_bytes` 与**内层取值**改成「拒绝」（注释自称「三处同族」），而 `isinstance(容器)` 不成立时 `batches` / `inflight_rollbacks` 整条仍被**跳过**。两者都是扩展字段、读侧写侧都不校验 ⇒ 本模块自己就可能把坏容器写上盘，下一次提交把历史抹空 | Opus 第 5 轮 [low]，控制者本机复现两档 | 容器坏型一律拒绝，与内层取值同规格 |
| S2-F74 | **high** | **武装判据问错了问题：用「记录变没变」代替「这一轮撞没撞逃逸」** —— 决策表有**三种**方式产出一份与旧记录相等的生命周期，每一种都让 `fatal_published` 整个不触发：①分支①**保住更严的旧 fatal**（拒绝降级）—— 一个**发现源树被换过**的运行最后交出一份**完全没有 fatal** 的账本；②本轮**再次撞上磁盘上一模一样的那条逃逸**（没修好就重跑，现实中最常见）；③`ledger.lifecycle` 返回的是**内部 dict 本身**，调用方一行赋值即可让判据为假。⚠️ **反方向也错**：分支③给**别人的** fatal 加 `max_bytes` 附注时被误判成「本轮自己发布了 fatal」，把已登记的 `passed` 清掉、并印出一句**假的**「本次运行已经自己发布过…」 | 独立评审（守卫层增量收口轮）[high]，控制者本机复现（对照实验：换条路径武装就正常触发 ⇒ 坐实判据问错了问题）| 判据改为 `hit_escape = (outcome.kind == "escape")` —— **由本轮自己的结局决定**，与记录变没变无关；`commit_stock` 恒传 `False`（它把三个字段从磁盘原样带过去，产不出新 fatal）。`ledger.lifecycle` 改返回**深拷贝**（与 R1「`lifecycle_snapshot` 只拷顶层」是同一教训的第二处）。变异实测：退回旧判据红 7 条；改成「只要带 fatal 就武装」红 2 条（含 `test_full_round_trip`，证明没修过头）|
| S2-F75 | medium | **写侧能造出读侧从此永远拒绝的容器状态 ⇒ staging 永久砖化** —— 上一轮只给**转移守卫**（看 `previous`）加了容器类型检查，写侧固有检查没加 ⇒ **首次提交**就能把 `batches: {...}` / `batches: null` / `inflight_rollbacks: ["x"]` 写上盘，此后每次提交（含崩溃恢复）都被拒，只能手改 JSON 救回来。正是本模块自己 docstring 点名的失败形态（「引导态第一次提交就把…写了进去…这棵 staging 当场卡死」）| 独立评审 [medium]，控制者本机复现四档 | 写侧固有检查补上对称的容器类型断言。⚠️ **这是我自己上一轮捅的「过紧」** —— 加拒绝时只加了一半，另一半让模块能铸造出自己拒绝的状态 |
