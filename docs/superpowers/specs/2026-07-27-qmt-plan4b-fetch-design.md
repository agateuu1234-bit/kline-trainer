...
- **市场从 code 后缀派生**：判据用 **`qmt_manifest.STOCK_CODE_RE`**（`^\d+\.(SH|SZ|BJ)\Z`），后缀即市场。
  ⚠️ **不是** `qmt_normalize._STOCK_CODE_RE`（最终评审 F10 更正）——后者用 `$`，**容忍一个尾随换行**（`"600000.SH\n"` 会被判合法）。写侧与读侧必须用**同一个对象**；S3 Task 4 已把 `qmt_manifest` 那份提为公开，正是为了不让两份近似实现继续漂开。
...
**外加第四条：两个周期缺一不可（S3-F4）**。缺 `daily` 就没有 `k_daily` 可算；
不单列成一条的话，这只股要么在算月数时炸、要么被静默算进别的计数器 ——
两者都会让下面那份统计自相矛盾。真实导出上这一档命中 **0** 只
（11216 = 5608×2 恰好配平），但它是**判据**该有的完整性，不是为当前这份数据服务的。

**`prefilter_stats` 的形状（S3-F4）**——写进 manifest 的**非必需**顶层键，
靠 O4-F10 的「原样返回、不做白名单投影」通道流转，**不 bump `manifest_version`**：

```jsonc
{"total": 5608, "rejected_incomplete": 0, "rejected_status": 1,
...
| S3-F4 | low | **实现有四条剔除路径、统计是六字段，而 §4.3 只写「三条判据」「各条命中数」** —— S4 要**写** `prefilter_stats`，它读的是 spec，照 spec 实现会少一条判据、并且不知道那六个键叫什么、也不知道有「四条之和 + passed == total」这条不变量 | 全支最终评审逐条对照 §4.3 原文与 `qmt_pool.prefilter` 的实现 | §4.3 补第四条判据与 `prefilter_stats` 的六字段形状 + 不变量。**不 bump `manifest_version`**（走 O4-F10 的非必需键通道，与 `quota`/`failures` 同规格）|

---

## S4a 实施轮（2026-09-18）—— 决定不在本文件内

**S4a（单股拷贝事务）的全部决定、证据、验收判据与交接，在
`2026-09-18-qmt-4b-s4a-contract.md`。本文件不复述它们**（复述是本片被反复咬到的缺陷源）。

该契约**仅在 S4a 范围内**优先于本文件，且**逐句列名**了它覆盖的本文件语句（见其 §5）——
**不列名即不覆盖**。本文件其余部分一字未改、照常有效。

证据（可原地复跑的探针脚本）在 `docs/superpowers/evidence/2026-09-18-qmt-4b-s4a/`。
