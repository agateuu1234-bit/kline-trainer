...
**settings.json `deny` 新增条目**（F1 defense-in-depth + F2 状态保护 + R4-F1 secret 保护）：

```
# --- attest-override script + state 文件写保护（F1/F2）---
...
# --- secret 文件读写保护（R4-F1）---
# catch-all Read/Edit/Write 放行后，这些 deny 覆盖 credential 类路径
Read(**/.env)
Read(**/.env.*)
Read(secrets/**)
Read(**/*.pem)
...
**例外**：`**/.env.example`（样例不敏感）不在 deny 里；`**/.env.*` 会匹配到 `.env.example`，所以在 deny 列表加更具体的 `Read(**/.env.example)` / `Edit(...)` / `Write(...)` 放在更高精度的 allow 里覆盖 deny（Claude Code 权限语义：更具体 > 更泛），或改用不匹配 example 的 pattern。本 plan 阶段确认最终写法。
