...
| 18 | **非破坏性边界确认**：读取 `.claude/settings.json.hooks.PreToolUse[0].matcher`（应 == `"Bash"`）+ 读取 `.claude/hooks/guard-attest-ledger.sh` 的 shebang 和输入处理（应依赖 Claude tool_use stdin JSON）；验证 hook 只能接收 Claude Code 工具调用，不可能收到"网页 Merge"事件 | 两个读取结果与 §1.3 覆盖表"网页 Merge ❌ 不拦"一致（matcher 为 Bash；hook 从 stdin 读 Claude tool JSON） | 两条都满足 = PASS；任一不满足说明覆盖表错标 = FAIL。**⚠ 原版本要求实际点 Merge 会造成 main 不可逆污染，已废弃**（对应 round6 R6-F1）|
...
## 9. 开放问题（待 plan 阶段解决，不再延后到新一轮 spec 评审）
...
### 从 Round 6 降级（用户 A 方案授权，进 plan 而非新一轮 spec 评审）

6. **R6-F2（high）credential 可通过 `git show` / `git diff` / `git grep` / `git cat-file` 打印内容绕过**：
   - 现状 `Bash(git show:*)` / `Bash(git diff:*)` / `Bash(git log:*)` / `Bash(git ls-tree:*)` 在 allow，对任何路径的 content 读都放行
   - 走过的 pattern：`git show HEAD:secrets/foo.pem` / `git diff HEAD -- **/.env` / `git cat-file -p <sha>`
   - **Plan 阶段的 Task**：在 settings.json `deny` 新增一组 Bash 子模式：
     - `Bash(git show *.env*)` / `Bash(git show *secrets/*)` / `Bash(git show *.p12)` / `Bash(git show *mobileprovision*)` / `Bash(git show *GoogleService-Info.plist*)` 等覆盖 credential glob
     - 同组 `git diff` / `git grep` / `git cat-file` / `git ls-files -s`（附带 blob sha 可后续读）
     - 实施时通过单元测试（Task 会产出）验证每个 pattern 是否如预期拒绝
   - 接受风险：若有未列 credential 类型仍会漏 → 进入 hardening-3 的 server-side 审计堵死

7. **R6-F3（high）scenario B/C 复用 push 的 `commits-to-push` 文件发现导致 already-pushed 分支漏拦**：
   - 现状 §2.4 scenario B/C 只说"执行 scenario A 的文件级检查"，但 scenario A 的文件发现依赖 Claude session 里的 commits-to-push
   - 若用户在普通终端先 push、再用 Claude 跑 `gh pr create` → Claude session 无 push 动作 → 无 commits-to-push → 文件检查走空
   - **Plan 阶段的 Task**：hook 改写 scenario B/C 的文件发现：
     - 不依赖 commits-to-push，直接 `git diff --name-only origin/<base>...<head_sha>` 列出 PR 完整 diff 文件集
     - 对每个 plan/spec 文件独立检查台账，blob 读自 head_sha 下的 `git ls-tree`
     - 对 branch-diff 检查仍按 head_sha 算 fingerprint，与文件级检查独立触发
   - Plan Task 里会有对应单元测试：模拟"已远端推送、Claude session 无 push 动作"的路径，断言 PR create 仍被拦
...
### Round 6（commit affc1cb，用户方案 A 授权——只修 F1，F2/F3 降级 §9 + plan）

| Finding | 处置 |
|---|---|
| R6-F1 high: §7 #18 是真实 destructive merge | §7 #18 完全重写为**非破坏性配置读取**：检查 `hooks.PreToolUse[0].matcher == "Bash"` + 检查 hook 脚本从 stdin 读 Claude tool JSON；与 §1.3 覆盖表逻辑一致即 PASS。**原 destructive 版本标注废弃** |
| R6-F2 high: credential 可通过 `git show`/`diff`/`grep`/`cat-file` 内容读绕过 | **降级至 §9 开放问题 #6**，plan 阶段作为具体 Task 落地：settings.json deny 新增一组 `Bash(git show *secret-glob*)` 等模式，配单元测试验证 |
| R6-F3 high: scenario B/C 复用 push-based 文件发现 | **降级至 §9 开放问题 #7**，plan 阶段作为具体 Task 落地：hook 改写 B/C 为独立 `git diff --name-only origin/<base>...<head_sha>` 文件发现，配回归测试 |

**评审循环终止（用户授权）**：经 6 轮 adversarial-review（超出 `workflow-rules.adversarial_review_loop.max_rounds=3`，用户先后授权 round 4/5/6），本 spec 进入 plan 阶段。R6-F2/F3 不作为 spec 阻塞项，而是作为 plan 的具体 Task 需求输入。
