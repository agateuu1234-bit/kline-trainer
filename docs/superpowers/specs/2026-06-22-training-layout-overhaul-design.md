...
2. **标的名（R2-H2 修正：不用 `activeMeta`、不加新 I/O）**：名字**只在 review/replay 显示**，而 review/replay **已把含名 `TrainingRecord` 载入内存**——
   - `review(recordId:)`（coordinator:244）按 id 加载 record → `.review(record:)` flow（:260）；replay 经 `replaySettlementPayload` 构造含名 record（:405-410）；`TrainingRecord.stockName/stockCode` 为**非可选 `String`**（Models.swift:117-118）。
   - 因此名字源 = **已在内存的 record**，经一个**只读 handle** 透到顶栏（plan 定具体出口：flow `.review(record:)` 关联值，或 coordinator `activeRecord`/lifecycle 透出）。**绝不**新增 `loadMeta()`/磁盘读，**绝不**在 review 加载路径加 I/O（R2-H2 的反面教材）。
   - normal（盲测）名字隐藏=占位，**不需要任何名字源**。
...
- 只读访问器（§3.1，零行为、零新 I/O）：`TrainingEngine.swift`（加 `public var currentPrice` 只读镜像）；review/replay 标的名复用**已加载的 `TrainingRecord`**（plan 定只读 handle：flow 关联值 / coordinator `activeRecord` / lifecycle 透出），**不**新增 meta 加载。
...
- **D5 标的名隐显**（R2-H2 修正：名字源自已加载 record，非新增 meta 注入）：
  - 规则：`engine.flow` 正常训练（盲测）→ 显示占位「训练标的 · 盲测」（不泄露真名，**无需任何名字源**）；**review / replay** → 显示 `formatStock(record.stockName, record.stockCode)`（复用既有 helper，全角括号）。
  - 名字源 = review/replay **已在内存的 `TrainingRecord`**（§3.1.2）：`record.stockName/stockCode` 为非可选 `String`，**恒有值**（不存在 nil 情形）；经只读 handle（plan 定出口）透到顶栏。**零新 I/O、零引擎行为/写入**。
  - 注：因 `stockName` 非可选，review/replay 名字恒在 → 不设「nil fail-closed」分支（那将是不可达死代码，R2-M）；§10#10 只验真名显示。
...
- 新增只读访问器（§3.1）：`TrainingEngine.currentPrice`（public 只读镜像，供 T2 显示）。review/replay 标的名复用**已加载 record**（只读 handle，plan 定出口），不新增 meta 加载。均零行为、零新 I/O。
