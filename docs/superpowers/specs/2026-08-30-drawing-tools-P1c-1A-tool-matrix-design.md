...
> ⇒ 这是**知情决策**，不是遗漏，也不是基于错误事实的决策。重提本条前，请先给出**上表路径之外的新触发路径**，或指出订正后的事实仍有错 —— 仅重述「备份会迁移数据」不构成新信息。
>
> **重提记录（逐轮登记，防止无限循环）**：
> - **codex R2-high**：指出「放弃」= `clearPending()` 永久删除整局 ⇒ **事实成立，已采纳**（本条理由 1 下方那段「残留的真实代价」就是它逼出来的）。**决定未变。**
> - **codex R6-high**：指出 `.unknownRaw` 的来源不可证 ⇒ **事实成立，已采纳**（§2-F4b + 路径 f）。**决定未变** —— 新增的路径 f 与既有的「数据库/存储损坏」同族，而那一族已在本条的取舍范围内。
> - **codex R7-high**：**未给出新触发路径、也未指出订正后的事实有错**；其唯一实质新点是「§8-Q13 挂着『未排期』读起来像**无主**」⇒ **该点已采纳**：Q13 归属改写为「**user 认领 · 已明确延后**」，并记录了 codex 建议的备选触发点（第 4 片开工前设闸），**留给 user 届时决定**。**决定未变。**
> - **codex plan-R1-high（第四次重提）**：**未给出新触发路径、也未指出订正后的事实有错**。它列举的四点 —— 磁盘满 / DB 损坏 / IO 错误可达、界面文案与行为相反、唯一非破坏性出路是强退 App、「延后处理不降低风险」—— **前三点全部是它从本 spec 自身读出的内容再复述**（分别出自 D113 的弹窗可达性表原因 5、§8-Q13、D113 理由 1 的补写段），第四点是对**决定**的异议而非事实。**按本条自设门槛：不构成新信息，决定未变。**
>   ⚠️ **但如实登记两件事**：① 这是**第四个**独立轮次把本残留判为不可发布 —— 频次本身值得 user 知道；② 其中最有分量的一半（**界面声称「进度保留」却删掉整局**）**就是 §8-Q13**，而 Q13 已由 user 认领。⇒ **降低本争议的最小动作 = 做掉 Q13**（很可能只需把那句文案改对，不必改行为），且它**与 P1c 无关、应独立成 PR**。
...
> **⛔⛔ 本节的纪律（codex R5–R10 六轮换来的，务必遵守）**
>
> 本节每一条**只负责三件事**：① **已核实的事实**（哪几处代码、什么行号、会出什么症状）；② **为什么不能不管**（尤其症状是否**静默**）；③ **必须存在的测试**。
>
> **⛔ 本节不开药方。** 上一稿曾为第 4 片写出具体设计方案，结果连续三轮被指出方案本身有洞（Q18 → 撤销/重做后门 → 二元组 → 与 Q17 冲突）—— 根因是**在没有那一片代码的情况下隔空推演解法**。本仓的规矩是**每片写自己的 spec、走自己的评审**，第 4 片届时面对真代码，判断力远强于此刻。
>
> ⇒ 下文出现的任何方案一律标注为「**备选思路（未经验证，第 4 片须自行论证或推翻）**」，**不构成本 spec 的处方**；其中**已被证伪**的会明确标出，供后来者避坑。
...
- ⛔ **箱体 / 折线档不归第 4 片**（codex R10-medium，**已核实为真**；本 spec 上一稿把它派给了第 4 片，**那是不可能完成的**）：箱体属**第 5 片**、折线属**第 6 片**（上游 D109），第 4 片手里根本没有这两个工具，而本 spec §9 又明令禁止合并切片 ⇒ 「箱体数据落线成功」这条真测试**在第 4 片写不出来**，硬写只能写成一个证明不了任何事的桩。
  ⇒ **归属订正**：**箱体档 → 第 5 片**、**折线档 → 第 6 片**，各自与该工具的落地同片交付。每一档仍是**两半都要** —— **数据落线成功** + 三个线型按钮**全灰**（只测落线成功 → 漏掉「面板错误可点」；只测全灰 → 漏掉「该工具画不出来」）。
  ⇒ **第 4 片保留的是**：趋势线档（上一条）+ **UI 可选性这一维本身建立起来**（含面板三处各接各的维度）。
...
| **Q18** | **切工具 / resume 后共享默认样式失配 ⇒ 落锚被静默吞掉**（codex R7-medium，**已核实为真**）。三条实测事实叠起来：① `DrawingSession.defaultStyle` 是**会话全局一份**、**不按工具分**（`DrawingSession.swift:154`）；② `activate(tool:)` 切工具时**只换工具 + 丢 pending，不归一化默认样式**（`:201-208`）；③ `commitPending` 里 **`discardPendingAnchors()` 在 `withStyle` 之前执行**（`:270-271`）—— 样式不成立时返回 nil，而锚点**已经被丢掉了**。⇒ 第 4 片之后：选趋势线 → 线型设成 `.segment` → 切回水平线 → 默认仍是 `.segment` → **每次点击都被吞、屏幕上什么都不出现、零反馈**，直到用户自己去改样式。<br>⚠️ **必须与 Q17 一起解，⛔ 不得单独修任何一个**：今天 `DrawingDefaultStyleColumn.decode` 那句 `sanitized(for: .horizontal)`（Q17 要改掉的那处）**恰好挡住了 resume 后的这个死状态**（把 `.segment` 强行改回 `.straight`）。若第 4 片按 Q17 把解码改成工具中立、却不做切工具归一化，**这个死状态会从「仅本次会话」放大到「重启后依然存在」**。<br>**⛔ 不变量必须覆盖「默认样式的每一个写入点」，不能只盖激活与 resume**（codex R8-medium，**已核实为真**）：撤销 / 重做是**第三个入口，能绕过激活时的归一化** —— ① 会话已开时切工具**不清撤销栈**（`TrainingEngine.swift:1443-1448`：只有 `drawingModeActive` **翻转**才清，切工具时它恒为 true）；② 撤销 / 重做把快照**无条件**写回（`TrainingEngine.swift:1638` `setDefaultStyle(direction == .undo ? dd.before : dd.after)`，**完全不看** `activeDrawingTool`）；③ 快照结构 `DrawingDefaultStyleDelta` **不带工具身份**（`DrawingUndoEntry.swift:57-62` 只有 `before` / `after`）。⇒ 趋势线下改成 `.segment` → 切水平线（归一化生效）→ **按 ↩ 或 ↪** → `.segment` 被写回 → 下一次点击照样被吞。

**写入点全表**（`grep -rn "setDefaultStyle" ios/Contracts/Sources` 穷尽，**5 处**）：

| # | 写入点 | 场景 |
|---|---|---|
| 1 | `Drawing/DrawingEditRouter.swift:267` | 面板改样式（无选中 ⇒ 改本局默认） |
| 2 | `Drawing/DrawingEditRouter.swift:287` | 面板改样式（另一条路径） |
| 3 | `TrainingEngine/TrainingEngine.swift:1638` | **撤销 / 重做**（codex R8 指出的那个后门） |
...
**⇒ 由此得到的**事实**是：只盯住「默认样式被写」那一半不够 —— 工具那一半会独立变化。**
**⚠️ 但这不等于「必须在切工具时改写存储的值」** —— 那正是下面 (i) 被证伪的原因。兼容性也可以在**使用那一刻**投影而完全不动存储值（见 (ii)）。**本 spec 只陈述事实与结果要求，不规定在哪里加代码。**

**第 4 片必须定一条「工具 × 默认样式」不变量，且必须对下列**全部**触发点成立。⛔ **方案由第 4 片自己定**，下面三条只是**备选思路（未经验证）**，其中 (i) **已被证伪**：

- **⛔ (i) 「切工具时归一化改写共享默认」—— 已证伪，不得采用**（codex R10-high）：进入画线模式**必然**激活一个工具，而当前入口写死 `.horizontal`（`TrainingEngine.swift:1412`）⇒ 归一化必在那一刻把存着的 `.segment` 改成 `.straight`，**正好摧毁 Q17 要保住的那个值**，随后还可能被 autosave 落盘。⚠️ 更阴险的是：**Q17 与 Q18 的规定测试此时可以全绿** —— Q17 的往返断言在激活之前就通过了，Q18 的「水平线点击成功」恰恰因为值被改掉才通过。⇒ **一份合法的趋势线默认被静默丢失，而没有任何测试变红。**
- **(ii) 备选思路：投影而非改写** —— 不动存储的默认样式，只在**用它造一条线的那一刻**按当前工具投影（`commitPending` 里 `defaultStyle` → `sanitized(for: tool)`）。⚠️ **未经验证**：需第 4 片自行确认它同时满足 Q17 与 Q18 的全部测试，并回答「面板此刻显示什么」（当前工具下该值是灰的）。
- **(iii) 备选思路：按工具各存一份默认** + 持久化当前工具 + 撤销快照带工具身份。语义最干净，但改动面大（持久化格式 + 撤销结构，可能触发契约 bump）。

**⛔⛔ 必须区分「兼容性投影」与「改写规范值」—— 这是本条唯一容易写混的地方**（codex R12-high：本 spec 上一稿把上一版的机制细节降级保留，结果与新加的 R-B / 测试⑧ **正面矛盾**，实施者无法同时满足；那段已整块删除）：

| | 兼容性投影 | 改写规范值 |
|---|---|---|
| 做什么 | **取用**默认样式去造一条线时，按当前工具把它投影成一个该工具用得了的值 | 把**存着的那份**默认样式本身改掉 |
| 存储里的值 | **一个字不动** | **被覆盖** |
| 满足哪条 | R-A（能画） | —— |
| 违反哪条 | 无 | **R-B**（用户设的值没了） |
...
**⛔ 撤销 / 重做的语义在本条中被明确钉死**：**撤销 = 原样恢复 `before` 快照，重做 = 原样恢复 `after` 快照，两者都不得做任何「兼容性强制转换」。**
理由有两条，缺一不可：① 它与仓内**既有契约一致** —— `TrainingEngine.applyUndoEntry`（`:1638`）的设计本就是把那条线与本局默认**当作同一个动作的两半原样回滚**（D102）；② 若允许撤销时顺手「转换成对当前工具合法的值」，规范值就在用户按 ↩ 的那一刻被悄悄换掉，**正是 R-B 要防的事**。
⇒ 撤销之后若当前工具用不了那个值，**由投影解决（R-A），不是由改写解决**。

**⛔ 硬要求写成「结果」，不写成「在哪几处加代码」**（codex R11-high，**已核实为真**；本 spec 上一稿仍留着一句「两侧都要改」的**机制级**规定，它与刚加的备选 (ii)「不改写、只投影」**直接打架** —— 一条不改写任何存储值的实现是合法的，却过不了那句规定）。**第 4 片必须同时满足下面两条结果，怎么实现由它自己定**：

- **R-A 可用性**：**当前工具下必须能正常落线** —— 任何时刻用当前默认样式落一条线，都不得出现「锚点被吞、屏幕无变化」。
- **R-B 保真性**：**用户设过的规范值必须活下来** —— 切到别的工具、进画线模式、autosave、重启、撤销 / 重做，**都不得把它改掉**。

⚠️ **两条缺一不可，且缺任何一条都有一个「测试全绿」的坏实现在等着**：只满足 R-A ⇒ 就是已被证伪的方案 (i)（把值改掉换取能画）；只满足 R-B ⇒ 就是今天的现状（值保住了但点击被吞）。

**必须配的测试**（**八条都要**；⚠️ 上一稿只有前五条 —— 它们**全部只验 R-A**，对 R-B **零判别力**，所以那个「静默丢默认」的坏实现能全绿通过）：

**验 R-A（能不能画）**
① 趋势线 `.segment` → 切水平线 → **第一次点击就落线成功**（不是被吞）；
② resume 一个存着 `.segment` 默认的存档 → 进画线模式（默认水平线）→ **第一次点击就落线成功**；
③ 趋势线 `.segment` → 切水平线 → **按 ↩ 撤销** → 第一次点击**仍然落线成功**；
④ 同上但**按 ↪ 重做**；
⑤ **反向对照**：趋势线 `.segment` 在**趋势线自己**下落线仍成功（防「归一化过头、把合法组合也改掉」）。

**验 R-B（值有没有被弄丢）—— ⭐ 本组是抓「方案 (i) 那类坏实现」的唯一手段**
⑥ 趋势线 `.segment` → 切水平线 → **再切回趋势线** → 默认**仍是 `.segment`**；
⑦ 存一份 `.segment` 默认 → resume → **进画线模式**（会激活水平线）→ **触发一次 autosave** → **重新加载** → 默认**仍是 `.segment`**（⚠️ 这一条同时钉死「被改掉的值不得被 autosave 固化到磁盘上」）；
⑧ 撤销 / 重做之后，**规范值仍是用户设的那个**（不得被替换成「对当前工具合法的最近值」）。

⚠️ ⑦ 必须**真的走一次存盘 + 重新加载**：只在内存里断言会与「改了内存没落盘」和「落盘了但读回时被改」两种坏实现同时为绿。 | **第 4 片**（与 Q17 同一动作） | 症状是**零反馈的点击失效** —— 用户无从理解、也不会去想「是样式的问题」 |
...
| **Q13** | **⚠️ 与 P1c 无关的既有缺陷（本片只登记、不修）**：`结算入账失败` 弹窗的正文写着「进度保留至最近存档」（`UI/TrainingView.swift:180`），而「放弃」按钮实际执行 `discardSession()` → `pendingRepo.clearPending()`（`TrainingSessionCoordinator.swift:1000-1018`）**永久删除整局 pending**。⇒ **用户看到的话与按钮做的事相反**，会让他放心点下破坏性动作。触发面**不限于**未支持数据 —— 上表原因 5（磁盘满 / DB 损坏 / IO 错误）是现实可达的。⛔ **不得并进 P1c 任何一片**（它属结算流程，与划线无关）；最小修法可能只是**把那句文案改对**（如「放弃将丢弃本局进度」），未必需要改行为 | **user 认领 · 已明确延后** | ⚠️ **归属订正**（codex R7-high 指出上一稿写「未排期」读起来像**无主**）：它**不是无主**——user 2026-08-30 明确决定「与 P1c 分开、单独记成待办，等收拾结算流程时再处理」，**排期权在 user**。⛔ 后续评审不得把它当作「被遗忘的残留」重提。<br>**备选触发点（记录，不在本片决定）**：codex R7 建议把它设成**第 4 片开工前的闸** —— 理由是第 4 片是第一个真正写出新工具数据的片。此建议**留给 user 在第 4 片开工时决定**，本片不代为设闸 |
