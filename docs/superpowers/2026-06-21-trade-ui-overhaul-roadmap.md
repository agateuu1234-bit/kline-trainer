...
- **4 处偏离主流的处理**：
  - #8 双图上下滑切周期：**保持双图特色**（已实现 §4.4 两指上下滑）；**但手势从两指改/加单指竖滑**（user 觉得两指别扭）→ 归 RFC-C 一并做（见顺位 4）。两指版当前可用。
...
- 点一下屏幕退出十字标 + 收侧栏。
- **🆕 并入：单指竖滑切周期（2026-06-21 user 提）**：把周期切换从「两指上下滑」改/加成「单指竖滑」（两指有点别扭）。归 C 是因为 C 本就重做单指手势路由——单指届时有 3 义：横滑=平移、竖滑=切周期、长按=十字光标，需在精细的 `ChartGestureArbiter`(C7,15 轮评审) 里**一次性统一消歧 + 防误触阈值**，避免分两次碰 arbiter。`switchPeriodCombo` action 复用、零引擎/契约改动。**待 C 阶段敲定的子决策**：①单指竖滑「替换」两指 还是「单双指都保留」；②竖滑做成一甩切一档（翻页式）。→ #8 决策从「保持两指·无需开发」修订为「C 阶段改单指」。

### 顺位 5 · E — 设置 popover（小）
- 主页设置齿轮点开 = 齿轮旁小 popover 菜单（锚齿轮），非底部大 sheet。

## 已确认「无需开发」的项
- **#8 周期上下滑切换**：**两指版已实现**（spec §4.4，两指上滑=周期变大/下滑=变小；模拟器 ⌥+竖直拖、真机两指）。⚠️ user 要改单指竖滑 → 已并入 RFC-C（非「无需开发」，C 阶段做）。
