...
- [x] **1. 两个脚本各跑一遍** → lifecycle **37 档全绿**（连跑三轮稳定）、
  concurrency **7 档全绿**，`EXIT=0`
- [x] **2. 4a-1 那个脚本复跑** → 仍是 **28 档全绿**（harness 抽取没把它改坏）
- [x] **3. host 全量仍绿** → `test_qmt_pilot_db.py` **429 passed**
- [x] **4. 把档数与断言数写进交接单第三节** → 已写，并把**两个生产缺陷**单列一张表
- [x] **5. 非 coder 验收清单补两项** → 补了第 11 条（三连跑，读末行 `✅ N 档`）
  与第 12 条（lifecycle **连跑三遍**比对末行，专查间歇性 flake），
  并附「L2 三连跑」的逐行命令
...
| 项 | 数 |
|---|---|
| 新增 L2 脚本 | 2（`verify_pilot_db_lifecycle.py` 37 档、`verify_pilot_concurrency.py` 7 档）|
| 抽出的共用安全护栏 | `_pilot_verify_harness.py`（6 个函数，4a-1 那个脚本改为引用）|
| 真 PG 档位合计 | 28 + 37 + 7 = **72** |
| L2 档位的 mutation | 24 条：**22 RED**，2 条如实登记（㉜ 被遮蔽、㉜b 的预检是纯纵深）|
| **挖出的生产缺陷** | **2 个**（`name[] = text[]` 恒抛；DROP 预检把 autovacuum worker 当占用者）|
| 挖出的我方判别力缺陷 | 5 个（⑯ 夹具、㉗ 恒真、㉖ 假红变异、⑳/`columns_ok` 互相遮蔽、局部 import 漏删两处）|

⚠️ **两个生产缺陷都改了 `backend/qmt_pilot_db.py`。** 上一轮 codex 的 R1–R8 结论
**不覆盖**这两处改动 —— 2b 开 PR 前必须 `git rebase origin/main` 后**重跑对抗性评审**。
