...
14. **branch-diff codex（实施后第二道）finding**：
   ...
   - **branch-diff R4 `~DEFAULT_BRANCH` 未核实 default==main（high）→ 接受为 minor residual（user 决策 escape-hatch）**：`_matches_main` 把 `~DEFAULT_BRANCH` 无条件当 main，未核实仓库当前默认分支真是 main。**对本交付是理论问题**：本仓 default 分支确为 `main`，且脚本 `REPO` 硬编码 `agateuu1234-bit/kline-trainer`、1b/1c 用途即护本仓 main，故 `~DEFAULT_BRANCH`=main 成立。**完整修**（live 查 `repos/$REPO` default_branch==main + offline 加 `--default-branch` 参数贯穿 runbook 3 处 offline 调用 + 测试，~20-30 行）推到**将来跨仓复用 / default 改名场景**再做。owner @agateuu1234-bit；影响半径=仅当脚本被移植到 default≠main 的仓或本仓 default 被改名时；rollback N/A（不改本仓 prod 行为）；属 codex governance-runbook permanent edge-mining（plan-stage 7 轮 + branch-diff 4 轮，findings 逐条修至仅剩理论边界）。
   - **收敛决策**：branch-diff codex 4 轮（bypass 修 / param 修 / R7-F3 实施 / default-branch residual），per `feedback_codex_round6_self_contradiction` + `feedback_openai_quota_ci_pattern` 走 user TTY override + admin merge；required checks 真绿，仅 codex 此一理论边界不收敛。
