...
- **不改名**：维持 `HistoryActionSheet`（文件/类型/init 签名 `(record:onReview:onReplay:onCancel:)` 全不变，D3）。全仓 13 处 `HistoryActionSheet` 引用中，**仅** `ios/Contracts/Sources/KlineTrainerContracts/UI/HistoryActionSheet.swift` 与 `ios/Contracts/Sources/KlineTrainerContracts/App/AppRootView.swift` 因本 RFC 改动（且类型名不变）；其余 11 文件（历史交付凭证/sibling design）零触碰。
...
Expected: 全量 0 failures（新增 8 测试；`HistoryActionContentTests` / `AppRouterTests` 等未改全绿）。
...
```swift
        ...
        // RFC #2：.history 居中弹窗（半透明遮罩内含居中卡片，由 HistoryActionSheet body 自绘）
        .overlay {
...
- [ ] 动作：`cd ios/Contracts && swift test --filter HistoryDialogPresentationTests`
      预期：`HistoryDialogPresentation routing` 8 个全绿。
- [ ] 动作：`cd ios/Contracts && swift test`
      预期：全量 0 failures；相对基线净 = +HistoryDialogPresentation(8)，`HistoryActionContentTests`/`AppRouterTests` 零改动全绿。
