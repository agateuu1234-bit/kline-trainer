...
期望：`Test run with 57 tests in 14 suites passed`（baseline 49 + PositionManager 8）。若 baseline 数字与本机 49 不一致，以 PR 提交前 main 分支跑出的实际数为准 +8。
...
| # | 动作 | 期望结果 | 通过/失败 |
|---|------|----------|-----------|
| 1 | 在终端 cd 到 worktree 后跑 `swift test 2>&1 \| tail -5` | 最末行包含 `Test run with 57 tests in 14 suites passed` | ☐ |
...
| 7 | 在 PR 页面的 commit 列表 | 看到 3 个独立 commit（Task 1 / Task 2 / 不含 Task 3，因 Task 3 仅 push & PR） | ☐ |
...
cd "/Users/maziming/Coding/Prj_Kline trainer/.worktrees/pr1-e2/ios/Contracts"
swift test 2>&1 | tail -5
# 期望：Test run with 57 tests in 14 suites passed
