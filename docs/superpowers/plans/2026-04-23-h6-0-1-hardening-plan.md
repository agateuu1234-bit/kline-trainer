...
**Review base SHA (pinned, codex Gate-4 round-8 finding):** All diff-audit and codex-attest commands in this plan use the fixed SHA `893b83222435a0ea4d9ce4f30d077c4cd4480ed7` as the base, NOT the moving ref `origin/main`. Rationale: if other PRs merge to `main` between spec approval and Gate 5/6/7 execution, `git diff origin/main` would include unrelated changes, breaking the exact-4-file A3 check and the 32-test A4 count. The pinned SHA keeps every gate auditing the same delta this spec/plan was approved against.
...
class TestR53F1BashArgs:
    ...
    def test_behavior_neutral_bash_rg_without_path_blocks(self, tmp_path):
        ...
        validates Task 3's 2-arg rule (not Task 1 sensitive-name helper)."""
        tp = _write_transcript(
            tmp_path,
            "Skill gate: exempt(behavior-neutral)\n\n",
    ...
    def test_single_step_bash_rg_without_path_blocks(self, tmp_path):
        """Gate-4 round-9 finding: single-step must share same no-path invariant."""
        tp = _write_transcript(
            tmp_path,
            "Skill gate: exempt(single-step-no-semantic-change)\n\n",
            tool_uses=[{"name": "Bash", "input": {"command": "rg TODO"}}],
    ...
    def test_behavior_neutral_bash_jq_filter_only_blocks(self, tmp_path):
        """Gate-4 round-9 finding: filter `.foo` is non-sensitive non-root,
        test validates Task 3's 'jq needs ≥2 args' rule specifically."""
        tp = _write_transcript(
            tmp_path,
            "Skill gate: exempt(behavior-neutral)\n\n",
    ...
    def test_single_step_bash_jq_filter_only_blocks(self, tmp_path):
        """Gate-4 round-9 finding: single-step shares same invariant."""
        tp = _write_transcript(
            tmp_path,
            "Skill gate: exempt(single-step-no-semantic-change)\n\n",
            tool_uses=[{"name": "Bash", "input": {"command": "jq .foo"}}],
...
```bash
git diff 893b83222435a0ea4d9ce4f30d077c4cd4480ed7 --name-only
```
...
```bash
cd "/Users/maziming/Coding/Prj_Kline trainer/.worktrees/hardening-6.0.1"
bash .claude/scripts/codex-attest.sh --scope branch-diff --head hardening-6.0.1 --base 893b83222435a0ea4d9ce4f30d077c4cd4480ed7
```
