...
- **DD-8 数值校验（修 R1-F2）**：
  ...
  - `start(initialVelocity:)`：`guard initialVelocity.isFinite, abs(initialVelocity) >= model.stopThreshold else { return }`（非有限或低于停止阈值的初速度不启动；后者为自审 I1：避免零/微速度 start 触发 spurious onFinish）。
...
| Create: `ios/Contracts/Tests/KlineTrainerContractsTests/DecelerationAnimatorTests.swift` | 接线测试（@MainActor，`.serialized`）+ 内置 `FakeFrameDriver`：handleTick 逻辑 + 代次 + initialVelocity guard（含 sub-threshold no-op）+ fake 驱动生命周期/清理 + 1 macOS 真 Timer smoke（14）。 |
...
Run: `swift test --package-path ios/Contracts --filter DecelerationAnimator`
Expected: PASS（macOS 宿主 14 tests = 13 确定性/fake + 1 真 Timer smoke；Catalyst 编译 13，0 failures）
...
git commit -m "feat(C2): DecelerationAnimator + 可注入 FrameDriving + RealFrameDriver + 14 tests"
```
...
Expected: 既有测试（PR #59 后约 297+ 个）+ 新增 28 个（model 14 + animator 14）全 PASS，0 failures。
...
| 2 | 运行 `swift test --package-path ios/Contracts --filter DecelerationAnimator` | 终端输出含 `14 tests`、`0 failures` | failures = 0 → 通过；否则不通过 |
