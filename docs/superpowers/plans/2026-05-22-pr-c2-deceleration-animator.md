...
| Create: `ios/Contracts/Tests/KlineTrainerContractsTests/DecelerationAnimatorTests.swift` | 接线测试（@MainActor，`.serialized`）+ 内置 `FakeFrameDriver`：handleTick 逻辑 + 代次 + initialVelocity guard（含 sub-threshold no-op）+ fake 驱动生命周期/清理 + elapsedDelta 真经过时间 + 1 macOS 真 Timer smoke（15）。 |
...
Expected: 既有测试（PR #59 后约 297+ 个）+ 新增 29 个（model 14 + animator 15）全 PASS，0 failures。
