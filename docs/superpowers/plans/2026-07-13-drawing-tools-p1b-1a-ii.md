...
- **不碰 `switchPeriodCombo`**：画线模式下竖滑被 `singlePanStep(drawingTakesOver:)` 吞掉（每个 return 都 `periodSwipe: nil`），两指切周期未接线 → 切周期在会话开启时**不可达**；本期不得在那里加会话逻辑（那是 1a-iv 的 D32，且必须用 `discardPendingAnchors()` 而非整场取消）。
...
（`switchPeriodCombo` 派的 `.periodComboSwitched` 有同样的破坏力，但**画线模式下切不了周期**——已实证，见下——故本期**不碰它**，改用 Task 2 的 `periodSwitchUnreachableWhileDrawing` 测试把这条不可达性钉死；1a-iv 放开竖滑时那条测试会变红，逼实现者按 D31 处理。codex plan-R6-high。）
...
### D46：面板级 FSM 原语降 internal —— 兼容性反驳（codex plan-R6-high 的 pushback，**证据在此**）

codex R6 担心「`activateDrawingTool` / `commitDrawing` / `cancelDrawing` 从 public 降 internal 会打断包外消费者，而测试用 `@testable` 看不出来」。**逐条核实后不成立**，故维持降级（这也是 codex R2-high 自己要求的修法）：

1. **包外零消费者**：`grep -rn "commitDrawing\|cancelDrawing(\|activateDrawingTool" ios/KlineTrainer/ ios/Contracts/Sources/KlineTrainerPersistence/` → **零命中**。本仓是单 App 私有仓，不是对外发布的库；`KlineTrainerContracts` 的消费者只有 App target 与测试。
2. **冻结 spec 没有约定访问级别**：`kline_trainer_modules_v1.4.md:1640` 写的是 `func activateDrawingTool(_: DrawingToolType)`（**裸 `func`，无 `public`**）；`commitDrawing` / `cancelDrawing` 在该文件里**根本没出现**（它们来自 reducer effect handler 家族）。`SpecLiteralGuardTests` 也**没有**锁这三个 API 的 public 性（只锁 `DrawingToolManager` 的 `@MainActor`）。
3. **非-`@testable` 客户端构建证明**：三绿门第 ③ 条 `xcodebuild build -scheme KlineTrainer` **就是**codex 要求的那个「non-@testable client build」—— App target 以普通 `import KlineTrainerContracts` 消费本包。它绿 = 没有任何受支持的消费者被打断。

**结论**：维持 internal。若后续真出现包外消费者，再按需开 public 包装（而不是现在为一个不存在的消费者留一扇漂移窄门）。
...
struct TrainingEngineDrawingSessionTests {
    ...
    @Test("codex plan-R6：画线模式下**切不了周期** —— 故 switchPeriodCombo 不必（也不许）碰会话；1a-iv 放开时本测试变红")
...
`switchPeriodCombo`（`:378-393`）：**不动**（codex plan-R6-high）。
理由（**已实证**，不是引 spec）：画线模式下**根本切不了周期** —— `ChartGestureArbiter.handleSinglePan` 把
`drawingMode` 喂进 `singlePanStep(drawingTakesOver:)`，而该纯函数在 `drawingTakesOver` 分支的**每一个 return
都带 `periodSwipe: nil`**（`GestureClassifiers.swift:113-121`）；两指切周期的 `onTwoFingerSwipe` 在
`ChartContainerView` **根本没接线**（RFC-C 起就没接）。故 `switchPeriodCombo` 在会话开着时**不可达**。
在这里挂 `endDrawingSessionIfActive()` 会（a）写下一段**永不执行**的死代码，（b）提前实现 spec §3.2 明确
**划给 1a-iv** 的「周期改变 → 处理 pending」语义，而且用的还是最粗暴的「整场取消」——正是 D31 禁止的
`cancel()` 式语义。**不可达性由 Task 2 的 `periodSwitchUnreachableWhileDrawing` 测试钉住**：1a-iv 一旦放开
竖滑（D32），那条测试立刻变红，逼实现者按 D31 用 `discardPendingAnchors()` 正经处理，而不是静默漂移。
...
| 5 | 切周期组合（竖滑） | 画线模式下**竖滑被吞、切不了周期** → `switchPeriodCombo` 不可达，不变量不受威胁 | Task 2 `periodSwitchUnreachableWhileDrawing`（纯函数实证 + 非画线模式对照）|
