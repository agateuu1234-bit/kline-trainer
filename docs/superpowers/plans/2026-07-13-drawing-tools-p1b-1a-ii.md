...
1. **DEBUG 不变量断言**（`commitDrawing` / `cancelDrawing` 体内）：`assert(!drawingSession.drawingModeActive)` —— 谁在会话开着时调面板级原语，**不管他在包内还是包外**，DEBUG / `swift test` / Catalyst 当场炸。这比访问级别覆盖面更广。
