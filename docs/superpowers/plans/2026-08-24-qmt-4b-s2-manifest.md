# QMT 4b 切片 S2：manifest 结构 + 读侧闭合校验 —— 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立 `fetch_manifest.json` 这份 pilot 唯一真相源的结构定义、fail-closed 读侧闭合校验、`manifest_version` 三档出路，以及 `stopped_reason` / `fetch_fatal_error` 的生命周期——**纯数据结构层，不碰网络、不碰拷贝、不碰数据库**。
...
**拆片预案（user 2026-08-24 拍板：先按一片写，超了当场拆）**：若实施中发现规模超过 S1（915 行代码 / 94 条测试 / codex 10 轮未收敛），拆点在 **Task 13 与 Task 14 之间**——
- **S2a** = Task 1–13：纯函数，零文件系统（常量/版本/全套读侧校验/聚合指纹）
- **S2b** = Task 14–17：碰磁盘（S1 小改、读回、两个提交入口、生命周期决策表）
...
**做完 S2 的正确表述**：manifest 这一层建好了，全部验证跑在临时目录里；`qmt_fetch.py` 仍然**零实现**。
