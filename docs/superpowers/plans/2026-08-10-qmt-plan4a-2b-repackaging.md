...
## ⚠️ 实施 S1 时实测挖出的修正（写计划时没料到）

1. **⑭ 必须归 S2 —— 数据依赖，符号扫描看不见。** 它断言的是 `leaked` / `right_token`，
   由 ⑪⑫ 捕获。删掉 ⑪⑫ 之后 ⑭ 会 `NameError`。
   → S1 是 **16 档**不是 17，S2 是 **18 档**。
2. **⑯ 之后 S1 要自己收尾。** 原本由 ⑰ 把 db15 删掉重建；⑰ 归 S2，
   S1 不补一句 `harness.drop_database` 就会每跑一次留下一个 `state=initializing` 的库。
3. **`c4` 必须进白名单，虽然扫描器不要求它。** ①②③④ 用 `f"{_PREFIX}c4"` 拼出，
   **不是源码字面量** → `assert_every_selfcheck_db_is_whitelisted`（只认字面量）不会要求它；
   但清场认不得它时，崩溃残留会被当成 stranger 把整个脚本挡住（返回 4）。
4. **lifecycle 的 DSN2 依赖是假的（codex S1-R1-F2）**：它读了 DSN2、过了破坏性闸、
