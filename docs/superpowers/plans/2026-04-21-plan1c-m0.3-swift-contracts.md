...
**Scope 说明**：
- 本 plan 只冻结 **Swift 数据模型**（struct/enum + Codable/Equatable/Sendable + CodingKeys）
...
# ---- Swift 测试（必须 ≥ 22 tests passed；codex round 4 finding: 低阈值会让
# 缺失测试套件悄悄通过。parse summary 提取 test 数并断言 ≥22）----
run "swift test: ≥22 tests passed" \
    bash -c 'cd ios/Contracts && swift test 2>&1 | tee /tmp/plan1c-swift.out && N=$(grep -oE "Test run with [0-9]+ tests passed" /tmp/plan1c-swift.out | grep -oE "[0-9]+" | head -1); [[ -n "$N" && "$N" -ge 22 ]] || { echo "FAIL: expected >=22 tests passed; got N=$N"; exit 1; }; echo "swift test count=$N OK"'
...
### 验收项 2：Swift 测试全绿（≥22 tests）

| | 内容 |
|---|---|
| Action | 运行 `cd ios/Contracts && swift test 2>&1 \| tail -5 && cd ../..` |
| Expected | 输出含 `Test run with NN tests passed`（NN ≥ 22），无 `failed` 或 `error:` |
| Pass / Fail | `passed` 行出现且 NN ≥ 22 = PASS；其它 = FAIL |
