...
# ---- Swift 测试 ----
# 必须确切 passed；若将来 test 数变化 label 必须同步更新
run "swift test: all Suites pass" \
    bash -c "cd ios/Contracts && swift test 2>&1 | tee /tmp/plan1c-swift.out && grep -qE 'Test run with [0-9]+ tests passed' /tmp/plan1c-swift.out"
...
Expected: 末行 `Plan 1c (M0.3 Swift Contracts) acceptance: 9 passed, 0 failed` + `PLAN 1c PASS`，退出码 0。
...
| Expected | 末尾依次出现 `Plan 1c (M0.3 Swift Contracts) acceptance: 9 passed, 0 failed` 和 `PLAN 1c PASS` |
