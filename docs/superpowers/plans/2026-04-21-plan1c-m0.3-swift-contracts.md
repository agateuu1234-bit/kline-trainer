...
- Plan 1b（M0.2 OpenAPI）已开 PR #24（待 merge）—— 本 plan 对齐 snake_case CodingKeys
- Plan 1c 本身不依赖 Plan 1b merge 到 main；若 Plan 1b 在 Plan 1c 实施期间 merge，本 plan 会一并 merge origin/main 以维持跨契约一致性测试（acceptance 项 4/5 会 grep 这 3 文件）
...
run "content_hash pattern alignment (3 files)" \
    bash -c "grep -E \"'\\^\\[0-9a-f\\]\\{8\\}\\$'\" backend/sql/schema.sql backend/openapi.yaml && grep -E '\\[0-9a-f\\]\\{8\\}' ios/Contracts/Tests/KlineTrainerContractsTests/RESTDTOsTests.swift"
...
| | 内容 |
|---|---|
| Action | 运行 `grep -lE '\[0-9a-f\]\{8\}' backend/sql/schema.sql backend/openapi.yaml ios/Contracts/Tests/KlineTrainerContractsTests/RESTDTOsTests.swift` |
| Expected | 输出**恰好 3 行**，每行是以上一个文件的路径 |
| Pass / Fail | 恰好 3 行 = PASS；< 3 或有多余 = FAIL |
