...
| | 内容 |
|---|---|
| Action | 先确认 PR #24 已 merge（`git log origin/main --oneline \| grep -E "(Plan 1b\|M0.2)"`）。然后运行 `grep -lF '[0-9a-f]{8}' backend/sql/schema.sql backend/openapi.yaml ios/Contracts/Tests/KlineTrainerContractsTests/RESTDTOsTests.swift` |
| Expected | grep 输出**恰好 3 行**，每行是以上一个文件的路径 |
