...
actionlint .github/workflows/catalyst-build.yml .github/workflows/swift-contracts-smoke.yml && echo "ACTIONLINT OK"
```
Expected: `ACTIONLINT OK`。若本机彻底无法安装 actionlint，则**不**视为通过——以 **Task 5「push 后 required check-run 实际出现」**（权威 ground-truth = acceptance #6 的 `gh pr checks`）作为强制替代验证，必须 binary 勾选。
...
|---|---|---|---|
| 1 | 运行 `actionlint .github/workflows/catalyst-build.yml .github/workflows/swift-contracts-smoke.yml; echo $?`（actionlint 未装则先 `brew install actionlint`） | 末行打印 `0`（Actions schema 校验通过；比 yaml 语法更严） | ☐ |
