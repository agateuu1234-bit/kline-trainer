...
# 每条真 throw 语句必须含 AppError（锚定行首 throw，避免误匹配注释里的 "throw" 字样）
...
# public 方法体内不得有 raw .decode（须在 private helper）
grep -nE "\.decode\(|JSONDecoder" TrainingSessionCoordinator.swift
# 期望：.decode 仅出现在 decodePosition private helper 内
```
Expected: 第一条 `PASS`；第二条仅命中 `decodePosition` helper 内一处（`JSONDecoder().decode`）。
