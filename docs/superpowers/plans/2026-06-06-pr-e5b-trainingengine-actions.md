...
- `PanelViewState.reduce(_:) -> ChartReduceEffect`（**非 `@discardableResult`**，故全部 callsite 用 `_ =` 丢弃）；`.tradeTriggered` 返 `.none`、`.periodComboSwitched` 返 `.clearPendingDrawing`——E5b **有意丢弃**返回 effect（画线 handler 在顺位 7 C8，E5b 无 pending drawing 可清，D8/L344）。`.period` 是 `var`（可直接赋值）✅；`ChartAction.{tradeTriggered,periodComboSwitched,panStarted}` ✅。（R1-L1 登记）
