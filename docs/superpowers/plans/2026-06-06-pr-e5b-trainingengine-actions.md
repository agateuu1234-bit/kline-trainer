...
- `PanelViewState.reduce(_:) -> ChartReduceEffect`（**非 `@discardableResult`**，故全部**引擎内** callsite 用 `_ =` 丢弃）；`.tradeTriggered` 返 `.none`、`.periodComboSwitched` 返 `.clearPendingDrawing`——E5b **有意丢弃**返回 effect（画线 handler 在顺位 7 C8，E5b 无 pending drawing 可清，D8）。`.period` 是 `var`（类内可直接赋值）✅；`ChartAction.{tradeTriggered,periodComboSwitched}` ✅。（R1-L1 登记）
- **R2-C1 关键约束**：`upperPanel`/`lowerPanel` 是 `public private(set) var`（E5a 冻结，外部只读）。`@testable import` **不放开 private setter**，故**测试侧不能**对其调 `mutating` 的 `reduce`（会编译失败 setter inaccessible）。测试只能**读** panel 状态——「硬切两面板」靠断言 `revision` 自增（reducer 内唯一改 revision 的路径）+ `interactionMode == .autoTracking` 间接证明 `.tradeTriggered`/`.periodComboSwitched` 已派发。reduce 仅在**引擎方法内部**调用。
- **R2-C2 关键约束**：`tradeEngine` helper 参数声明顺序 `(closes:, cash:, capital:, position:, mode:)`；所有 callsite 实参须按此顺序（`position` 在 `mode` 前），Swift 不允许尾随实参乱序。
