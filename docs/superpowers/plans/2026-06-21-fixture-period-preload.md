...
> **M2 注（loadMeta 新增 throw 语义）**：`loadMeta()` 失败落在内层 `do/catch`（与紧邻的 `loadAllCandles()` 同位）→ `reader.close()` + rethrow，**不**触发外层「损坏文件 delete+retry」循环（那只包 `openReader`）。后果：meta 损坏但 candles 正常的训练组会 surface error 而非删+重试——与既有 `loadAllCandles` 失败语义一致，可接受，无需额外处理。
...
> **H2 注（穷举边界完整性）**：除上述 4 处，共享 fixture `PersistenceIntegrationFixtures.swift`(`PIFixtures`) 被 6 个文件用（`TrainingSessionAutosaveTests`/`...FenceTests`/`...CrossFeatureTests`/`AutosaveBannerTests`/`...ProvenanceTests`/`CacheTouchOnUseTests`），其 `makeCoordinator` 复用 **本任务已 re-align 的 `TrainingSessionPersistenceTests.validCandles()`** + 默认 meta(startDatetime=1) → re-align 后 `m3[0].datetime=1` ⇒ startTick=0，**这 6 文件全部保持绿**（无需改）。`KlineTrainerPersistenceTests` 模块的真 seeded 端到端断言见 Task 4（那里 startTick=12000）。Step 5 跑全套件即验证此「全绿」断言（不靠预判）。
...
`DebugFixtureDataTests.swift` 内提及 `9600` 的注释（如 L112「满载根数（9600）」、L133「m3=9600..monthly=80」）改为 `19200`（monthly=80 不变，仅 m3 数字更新）。`DebugFixtureData.swift` 旧推导注释（L44-48 整块，含 L45「9600 根 m3」+ L48）已在 3a 整块替换。**勿动**与本 fixture 无关的 `9600`/`96000` 字面：`FixturePriceSeriesTests.swift:42/48/61`（直接 `FixturePriceSeries.generate(count: 9600)`）、`TrainingEngineActionsTests.swift:482`（tier 数学注释）——它们不引用 `fullLoadM3Count`/fixture 总数。`DebugTrainingSetWriterTests.swift:12/35` 用 `make(m3Count: 240)`，受默认 `beforeM3Count=0` 保护不受影响。
...
- [ ] **Step 1: 写失败测试（真 seeded 端到端：开局 tick = 12000）**

本文件既有 `test_after_reset_freshStart_startsAtDefault`（~L181）已在 reset 后 `let engine = try await c.coordinator.startNewNormalSession()` 并断言 `engine.currentTotalCapital == 100_000`（shares=0 → 不依赖 startTick，**保持绿**）。**在该断言后加一行**端到端证明（串起 Task 2 coordinator 派生 + Task 3 fixture before + Task 4 seed 接线）：

```swift
        #expect(engine.tick.globalTickIndex == DebugFixtureData.fullLoadBeforeM3Count)  // 开局=起始点 12000（真 seeded→coordinator 端到端，app 开局非空）
```

（`DebugFixtureData.fullLoadBeforeM3Count` 为 public，`AppContainerDebugSeedTests` 已 import `KlineTrainerPersistence`。这是「app 开局显历史」的最强测试级证明。）

- [ ] **Step 2: 跑测试确认失败**

Run: `cd "ios/Contracts" && swift test --filter AppContainerDebugSeedTests/test_after_reset_freshStart_startsAtDefault 2>&1 | tail -20`
Expected: FAIL —— 当前 seed 调用 `make(m3Count: fullLoadM3Count)`（beforeM3Count 默认 0）→ meta.startDatetime=m3[0] → 派生 startTick=0，实得 `globalTickIndex == 0`，期望 12000。
...
Expected: `test_after_reset_freshStart_startsAtDefault` PASS（tick==12000 + capital==100_000）；该文件既有 seed 测试（`seededFixture_isFullLoad` 等）不回归。
