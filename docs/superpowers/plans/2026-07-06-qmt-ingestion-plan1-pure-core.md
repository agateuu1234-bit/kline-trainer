...
Run: `cd backend && python -m pytest tests/test_qmt_resample.py -v`
Expected: PASS（21 passed；含 final whole-branch review 补测的 5 个 OHLCV 分支测试：open/high/low/volume/amount）
...
# ===== final whole-branch review Finding 1 补测：D6 per-period before/after 硬门（Task8 删旧测试造成的
...
Run: `cd backend && python -m pytest tests/test_generate_training_sets.py -v`
Expected: PASS（5 passed；含 final whole-branch review 补测的 D6 gate 2 个测试：before_context_too_thin / after_window_empty）
