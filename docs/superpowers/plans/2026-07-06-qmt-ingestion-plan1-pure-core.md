...
- Modify: `backend/generate_training_sets.py` — B2 纯逻辑（`compute_after_end` / `select_period_window`（改纳入规则）/ `eligible_start_indices`（按交易日历+dense）/ `select_valid_window`（有界重试）/ `per_day_intraday_complete` / `intraday_span_trading_days`）。
...
- Produces（均纯函数）：
  ...
  - `intraday_span_trading_days(win_3m, trading_dates) -> list[datetime.date]`：3m 窗口跨度内交易日（供 Plan 2 组 `try_assemble` 时取 D9 span）。**`try_assemble` 的组装**（切窗 `select_period_window` → `intraday_span_trading_days` → `per_day_intraday_complete` 硬门 → 起点唯一性 → SQLite/zip）**在 Plan 2 用真实 bars 组**（复用本 plan 已测的 `select_period_window`+`per_day_intraday_complete`；不在本 plan 单测，因需多日 fixture 满足 per-period `before≥30`）。
  ...
  - `per_day_intraday_complete(windows: dict[str, pd.DataFrame], daily_dates_in_span: list[datetime.date], expected: dict[str,int] = {"3m":80,"15m":16,"60m":4}) -> bool`：D9 per-day 硬门——盘中窗口跨度内每交易日**每个盘中周期**桶数精确 == 应有数（3m=80/15m=16/60m=4）；**任一周期任一日不符 → False**（codex PF1-R2：不能只校 3m）。
...
def test_per_day_intraday_complete_all_periods_hardgate():
    from generate_training_sets import per_day_intraday_complete
    def _mk(day, n):
    ...
    def _win(n3, n15, n60):
        return {"3m": pd.DataFrame(_mk((2026,7,1),n3)+_mk((2026,7,2),n3)),
                "15m": pd.DataFrame(_mk((2026,7,1),n15)+_mk((2026,7,2),n15)),
                "60m": pd.DataFrame(_mk((2026,7,1),n60)+_mk((2026,7,2),n60))}
    assert per_day_intraday_complete(_win(80,16,4), days) is True
    assert per_day_intraday_complete(_win(80,15,4), days) is False   # 3m 全但 15m 短 → False
    assert per_day_intraday_complete(_win(80,16,3), days) is False   # 60m 短 → False
    assert per_day_intraday_complete(_win(79,16,4), days) is False   # 3m 短 → False
```

- [ ] **Step 2: 跑测试确认失败**

Run: `cd backend && python -m pytest tests/test_generate_training_sets.py -k "after_end or eligible or valid_window or hardgate" -v`
...
def eligible_start_indices(month_boundaries, rng, *, dense_dates, trading_dates) -> list:
    ...
    for idx in candidates:
        ...
        d1 = trading_date(compute_after_end(month_boundaries, idx))
...
def select_valid_window(month_boundaries, rng, *, dense_dates, trading_dates,
                        try_assemble, max_retries: int = 8):
...
def intraday_span_trading_days(win_3m, trading_dates) -> list:
    """盘中 3m 窗口跨度 [首日, 末日]（含 before-context）内的交易日
    （供 Plan 2 组 try_assemble 时的 D9 span；Plan 2 组: select_period_window 切窗 →
    intraday_span_trading_days 求 span → per_day_intraday_complete 硬门 → 唯一性）。"""
    if win_3m.empty:
        return []
    d0 = trading_date(int(win_3m["datetime"].min()))
    d1 = trading_date(int(win_3m["datetime"].max()))
    return sorted(d for d in trading_dates if d0 <= d <= d1)

_INTRADAY_EXPECTED = {"3m": 80, "15m": 16, "60m": 4}

def per_day_intraday_complete(windows, daily_dates_in_span, expected=None) -> bool:
    """D9 per-day 硬门（codex PF1-R2）：跨度内每交易日**每个盘中周期**桶数精确 == 应有数
    （3m=80/15m=16/60m=4）；任一周期任一日不符 → False。"""
    expected = expected or _INTRADAY_EXPECTED
    for period, need in expected.items():
        win = windows.get(period)
        counts = ({} if win is None or win.empty else
                  pd.Series([trading_date(e) for e in win["datetime"]]).value_counts().to_dict())
        if not all(counts.get(d, 0) == need for d in daily_dates_in_span):
            return False
    return True
```
...
Expected: PASS（现有测试 + 新增全绿；若现有测试因 `select_period_window` 纳入规则改动而红，同步更新调用点——现有 `assemble_training_set`/`select_start_index` 在 Plan 2 正式重接线；本 plan 只保证新纯函数与其单测绿，可临时保留旧 `select_start_index`/`monthly_after_end` 不删以免破坏现有测试）
...
- **B2 纯逻辑**（§4.4）：after_end=第8完整月末 ✅、窗口纳入(周straddle) ✅、`eligible_start_indices` 按交易日历+dense ✅、`select_valid_window` 有界重试(R12-F2) ✅、per-day 硬门(3m/15m/60m) ✅(Task7)。
