...
- Modify: `backend/generate_training_sets.py` — B2 纯逻辑（`compute_after_end` / `select_period_window`（改纳入规则）/ `eligible_start_indices`（按交易日历+dense）/ `select_valid_window`（有界重试）/ `per_day_intraday_complete`（各周期自身跨度硬门））。
...
- Produces（均纯函数）：
  ...
  - **`try_assemble` 的组装**（切窗 `select_period_window` → `per_day_intraday_complete`（各周期自身跨度硬门）→ 起点唯一性 → SQLite/zip）**在 Plan 2 用真实 bars 组**（复用本 plan 已测的 `select_period_window`+`per_day_intraday_complete`；不在本 plan 单测，因需多日 fixture 满足 per-period `before≥30`）。
  ...
  - `per_day_intraday_complete(windows: dict[str, pd.DataFrame], trading_dates: set[datetime.date], expected: dict[str,int] = {"3m":80,"15m":16,"60m":4}) -> bool`：D9 per-day 硬门——**每个盘中周期**在**其自身窗口日期跨度**内、每交易日桶数精确 == 应有数（3m=80/15m=16/60m=4）；**各周期各自校验**（before-context 深度不同，短的高周期 before-context 不漏检；codex PF1-R2/PF1-R4-F2）；任一不符 → False。
...
def test_per_day_intraday_complete_catches_short_60m_before_context():
    # 3m/15m 只覆盖 D1,D2(完整)；60m 多一更早 before-context 日 D0 只 2 根 →
    # 各周期按**自身跨度**校验 → 60m 跨度含 D0(2≠4) → False（codex PF1-R4-F2）
    D0, D1, D2 = (2026,6,30), (2026,7,1), (2026,7,2)
    trading = {dt.date(*D0), dt.date(*D1), dt.date(*D2)}
    ...
    assert per_day_intraday_complete(windows, trading) is False
    windows["60m"] = pd.DataFrame(_mk_bars(D0,4)+_mk_bars(D1,4)+_mk_bars(D2,4))
    assert per_day_intraday_complete(windows, trading) is True
```
...
def per_day_intraday_complete(windows, trading_dates, expected=None) -> bool:
    """D9 per-day 硬门（codex PF1-R2/PF1-R4-F2）：**每个盘中周期**在**其自身窗口日期跨度**内、
    每个交易日（∈ trading_dates）桶数精确 == 应有数（3m=80/15m=16/60m=4）——各周期 before-context
    深度不同（150 根 3m≈2 日、15m≈9 日、60m≈37 日），须各自校验，否则短的高周期 before-context 漏检。
    任一周期任一日不符 → False。（Plan 2 组 try_assemble：切窗 → per_day_intraday_complete → 唯一性。）"""
    ...
    for period, need in expected.items():
        ...
        span = [d for d in trading_dates if d0 <= d <= d1]          # 该周期自身跨度内的交易日
...
- **B2 纯逻辑**（§4.4）：after_end=第8完整月末 ✅、窗口纳入**两侧**周边界校验(after trailing 跨月周 + before 跨 start 周，PF1-R5 无 lookahead) ✅、`eligible_start_indices` 按交易日历+dense ✅、`select_valid_window` 有界重试(R12-F2) ✅、per-day 硬门各周期自身跨度(3m/15m/60m) ✅(Task7)。
