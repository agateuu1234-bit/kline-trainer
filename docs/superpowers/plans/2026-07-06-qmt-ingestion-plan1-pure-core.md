...
def compute_dense_coverage(df_1m: pd.DataFrame) -> DenseCoverage:
    """dense = 某交易日 1m 分钟集**精确等于** _EXPECTED_SORTED（无重复/缺失/盘外，codex PF1-F1）——
    行数==241 不充分（重复+缺失、盘外替盘内 也能凑 241）。"""
    if df_1m.empty:
        return DenseCoverage([], [], None, None)
    df = df_1m.copy()
    df["_d"] = df["datetime"].map(trading_date)
    df["_m"] = df["datetime"].map(_mins_of_day)
    complete, dropped = [], []
    for d, g in df.groupby("_d"):
        (complete if sorted(g["_m"].tolist()) == _EXPECTED_SORTED else dropped).append(d)
...
- Produces（均纯函数）：
  ...
  - `select_start_index(month_boundaries, rng, *, dense_dates: set[datetime.date], trading_dates: set[datetime.date]) -> int`：候选下标 `[30, len-9]`（随机顺序）中，过滤「`start`→`after_end` 之间**该股交易日历（`trading_dates`，= daily 日期集）**里的每个交易日 ∈ `dense_dates`」——**按交易日遍历、非日历日**（周末/假期不在 `trading_dates`，不误拒；codex PF1-F2）；无候选抛 `GenerateSkipException`。
...
import random
from generate_training_sets import select_start_index
...
def test_select_start_index_selects_when_trading_days_all_dense():
    # ≥39 月边界（覆盖 2020-01..2023-08 首日午夜），trading_dates=全程工作日，dense=全部工作日
    bounds, y, mo = [], 2020, 1
    for _ in range(44):
        bounds.append(_mid(y, mo, 1)); mo += 1
        if mo > 12: y += 1; mo = 1
    d0, d1 = dt.date(2020,1,1), dt.date(2023,12,31)
    trading = _weekday_trading_dates(d0, d1)   # 交易日历（工作日）
    dense = set(trading)                        # 全 dense（含跨周末/月，工作日全覆盖）
    idx = select_start_index(bounds, random.Random(0), dense_dates=dense, trading_dates=trading)
    assert 30 <= idx <= len(bounds) - 9         # 选出有效候选（不因周末误拒）

def test_select_start_index_skips_when_a_trading_day_missing_from_dense():
    bounds, y, mo = [], 2020, 1
    for _ in range(44):
        bounds.append(_mid(y, mo, 1)); mo += 1
        if mo > 12: y += 1; mo = 1
    trading = _weekday_trading_dates(dt.date(2020,1,1), dt.date(2023,12,31))
    dense: set = set()                          # 空 dense → 每个候选窗口都有非-dense 交易日 → skip
    with pytest.raises(GenerateSkipException):
        select_start_index(bounds, random.Random(0), dense_dates=dense, trading_dates=trading)
...
def select_start_index(month_boundaries, rng, *, dense_dates, trading_dates) -> int:
    """按**交易日历**约束（codex PF1-F2）：[start日期..after_end日期] 内该股每个交易日
    （∈ trading_dates）都 ∈ dense_dates；周末/假期不在 trading_dates、不误拒。"""
