...
|---|---|---|---|
| 1 | `swift test --filter currentPositionTier` | 输出含 `Test run with` 行且 `0 failures`；5 个 `currentPositionTier*` 测试全 ✔ | 全 ✔ 且 0 failures = 通过；任一 ✘ 或非零 failures = 不通过 |
...
| 4 | `swift test`（全量回归） | `Test run with N tests`，`N ≥ 810`（基线 799 + 新增 11），`0 failures` | 0 failures = 通过；≥1 failure = 不通过 |
...
- §4.4b/§4.1 tier accessor：市值/当前总资金基准 ✓；round ✓；空仓 0 ✓；total<=0 守 0 ✓；满仓 5 ✓；RFC acceptance 锁向量（4→×2→卖2/5→3）✓（`UsesMarketValueBasisNotStatefulBuyTier`）。
