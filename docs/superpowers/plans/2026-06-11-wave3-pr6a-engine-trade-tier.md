...
```swift
    ...
    /// 幂等：空仓短路。**返回 `position.shares == 0`** —— 顺位 7 caller 的「安全可结算」信号
    /// （codex plan R2：只在确认平仓后才路由结算；溢出/非法报价 no-op 时返 false，caller 不结算）。
    /// ended UI 态 / 结算路由本身归顺位 7/8，非 engine 契约。
    @discardableResult
    public func forceCloseManually() -> Bool {
        guard flow.canBuySell() else { return position.shares == 0 }
...
> **Scope 边界（codex plan R2 回应）**：同一「有限极端价 → notional 溢出」性质在 `buy`/`sell`（经 `quoteBuy`/`quoteSell`/`makeSellQuote`）**预先存在**（E3/E5b 已冻结契约，非本锚引入）。6a 仅加固它正在抽出的 force-close 共用体（顺带改善既有 auto `forceCloseIfEnded`）；buy/sell/quote 层的全面 finite 校验属独立健壮性关注（顺位 10/12），**不在 6a scope**——避免改动冻结的 E3 quote 契约与 buy/sell 签名。

- [ ] **Step 4: 跑新测试 + 既有强平回归测试确认通过**

Run: `cd ios/Contracts && swift test --filter forceClose 2>&1 | tail -30`
Expected: PASS —— 包含新 9 个 `forceCloseManually*`（6 行为 + 3 失败模式 `NoOpOnZeroPrice`/`NoOpOnNonFinitePrice`/`NoOpOnFiniteOverflowPrice`）**以及**既有 `advancingToEndWithHoldingForceCloses`/`advancingToEndWithoutHoldingDoesNotForceClose`/`forceCloseIsIdempotentAcrossRepeatedEndAdvances`/`buyThatAdvancesToEndTriggersForceClose`（证抽体重构未改 auto 行为；`advancingToEndWithHoldingForceCloses` 的 `maxDrawdown == 10` mutation killer 仍绿）。0 failures。
...
| 2 | `swift test --filter forceCloseManually` | 9 个 `forceCloseManually*` 测试全 ✔；含 `DisabledInReviewMode`（Review no-op 返 false）、`UsesCurrentTickPriceNotEndPrice`（价 10 非 99）、`NoOpOnFiniteOverflowPrice`（极端价 → 现金不被写 NaN、原子 no-op） | 全 ✔ 且 0 failures = 通过；否则不通过 |
...
| 4 | `swift test`（全量回归） | `Test run with N tests`，`N ≥ 814`（基线 799 + 新增 15），`0 failures` | 0 failures = 通过；≥1 failure = 不通过 |
...
- §4.4a on-demand 强平：去 maxTick 门 ✓（Task 2 `forceCloseManually`）；前置 `canBuySell()` ✓（review no-op / replay allowed 测试）；当前价非末根 ✓（`UsesCurrentTickPriceNotEndPrice`）；幂等 ✓（`IsIdempotent` + flat no-op）；共用体不漂移 ✓（抽 `performForceClose`，既有 auto 测试回归）；非法/溢出报价原子 no-mutation + 安全可结算返回信号（codex plan R2-high）✓（`NoOpOnZeroPrice`/`NoOpOnNonFinitePrice`/`NoOpOnFiniteOverflowPrice` + `@discardableResult -> Bool`）。
