...
> 3. **根因 + 结构性硬化（顺位 10，非 6a）**：病态价能进引擎的根因 = **reader 接受任意有限正价**（无量级上界）；其结构性修复——(a) reader 价格量级校验、(b) init 拒非有限派生 startTotal、(c) advance 路径 drawdown 污染、(d) buy/sell quote 溢出、(e) **finalize fail-closed gating**（持仓未平 / 派生量非有限 → 拒持久化、不清 pending）——是 **RFC §4.7 + 数据校验层 charter 给顺位 10a/10b** 的契约。6a **不**改 `reader` / E5a `init` / `advanceAndAccount` / `buy`·`sell` / 冻结的 `TrainingSessionCoordinator.finalize` / `TrainingView` 路由。
> 4. **codex「单一完整终态校验器」(R5 next-step) = RFC §4.7 顺位 10，按名对应**：codex 反复建议「一个完整 terminal-state validator，manual+auto finalize 都依赖它」——这**正是** RFC §4.7 钉死的 **顺位 10a「单事务 session-finalization port + 失败保留 + 终态 fence」**：在 finalize 边界对**真正持久化的输出**（totalCapital/profit/returnRate/**maxDrawdown 比率**——比率换算在 E6 finalize 内，6a 不拥有）做 fail-closed 校验并对 auto/manual 双路强制。engine 无法从 1.8e308 病态价产出理智结算，亦不拥有 finalize 的比率换算——理智化 + 输出校验 + 双路强制属 finalize 边界（10）。6a 止于「**engine 内部操作不腐蚀/不 trap**（force-close 整笔原子 + tier 不 trap）+ **engine 级结算门诚实**（isSettlementSafe 对引擎级派生量全 finite 校验，非有限即拒报可结算）」。**finalize-gating 已于 R3-high + R5#1 两次提出，均按此 charter fence 给顺位 10**（非 6a scope；6a 实现它将侵入冻结 coordinator + 10 的 §4.7 plan，且不完整）。

- [ ] **Step 4: 跑新测试 + 既有强平回归测试确认通过**

Run: `cd ios/Contracts && swift test --filter "[Ff]orceClose" 2>&1 | tail -45`（注：`--filter forceClose` 大小写敏感、漏 `autoForceClose*`，故用 `[Ff]orceClose`）
Expected: PASS（`27 tests ... 0 failures`）—— 包含新 15 个测试（行为 + 失败模式：`NoOpOnZeroPrice`/`NoOpOnNonFinitePrice`/`NoOpOnFiniteOverflowPrice`/`NoOpOnCashSumOverflow`/`ReturnsFalseInFlatReviewMode`/`ReturnsFalseWhenDrawdownNonFinite`/`ReturnsFalseWhenReturnRateNonFinite`/`autoForceCloseOnOverflowPriceLeavesCashFinite`/`autoForceCloseNoOpOnCashSumOverflow` 等）**以及**既有 `advancingToEndWithHoldingForceCloses`/`advancingToEndWithoutHoldingDoesNotForceClose`/`forceCloseIsIdempotentAcrossRepeatedEndAdvances`/`buyThatAdvancesToEndTriggersForceClose`（证抽体重构未改 auto 行为；`advancingToEndWithHoldingForceCloses` 的 `maxDrawdown == 10` mutation killer 仍绿）。0 failures。
...
| 2 | `swift test --filter "[Ff]orceClose"`（大小写都匹配：`forceCloseManually*` 与 `autoForceClose*`/`*ForceClose*`） | `Test run with 27 tests ... 0 failures`；且以下新增测试均出现且 ✔：`forceCloseManuallyNoOpOnFiniteOverflowPrice`、`forceCloseManuallyNoOpOnCashSumOverflow`、`forceCloseManuallyReturnsFalseWhenLiquidationGoesNegative`、`autoForceCloseNoOpOnCashSumOverflow` | `27 tests` 且 `0 failures` 且上述 4 个 named 测试 ✔ = 通过；否则不通过 |
| 3 | `swift test --filter advancingToEndWithHoldingForceCloses`（既有局终自动强平 killer） | `Test run with 1 test ... 0 failures`——该测试 `maxDrawdown == 10` 断言证抽 `performForceClose` 共用体未改 auto 强平行为 | `1 test` 且 `0 failures` = 通过；该测试 ✘ = 不通过 |
...
- §4.4a on-demand 强平：去 maxTick 门 ✓（Task 2 `forceCloseManually`）；前置 `canBuySell()` ✓（review no-op / replay allowed 测试）；当前价非末根 ✓（`UsesCurrentTickPriceNotEndPrice`）；幂等 ✓（`IsIdempotent` + flat no-op）；共用体不漂移 ✓（抽 `performForceClose`，既有 auto 测试回归）；**整笔原子 + finite 全守**（quote 字段 + newCash 和；codex R2/R4）✓（`NoOpOnZeroPrice`/`NoOpOnNonFinitePrice`/`NoOpOnFiniteOverflowPrice`/`NoOpOnCashSumOverflow` + auto 对偶）；**结算安全 Bool 诚实**（flat && total finite && returnRate finite && drawdown finite，Review 恒 false；codex R3-medium/R4/R5#2）✓（`ReturnsFalseInFlatReviewMode`/`ReturnsFalseWhenDrawdownNonFinite`/`ReturnsFalseWhenReturnRateNonFinite`）；auto-end 不腐蚀 cash（codex R3-high）✓（`autoForceCloseOnOverflowPriceLeavesCashFinite`/`autoForceCloseNoOpOnCashSumOverflow`）；结构性根因 + finalize 输出完整校验/双路强制（codex「单一 validator」R5#1）= RFC §4.7 顺位 10（Scope 边界 3/4，finalize-gating 已 R3+R5 两次 fence）。
...
**3. Type consistency：** `forceCloseManually() -> Bool`（public，`@discardableResult`，无参，返 `isSettlementSafe`）/ `performForceClose() -> Bool`（private，`@discardableResult`）/ `isSettlementSafe: Bool`（private computed）/ `forceCloseIfEnded()`（private，保留，丢弃返回）/ `currentPositionTier: Int`（public computed）—— 全文一致。`TradeCalculator.forceCloseOnEnd`、`TradeMarker`、`TradeOperation`、`PositionManager`、`DrawdownAccumulator`（`peakCapital`/`maxDrawdown`/`update`）签名均取自现有代码。✓
...
### plan-stage codex 结果 + accept-residual override（user 2026-06-12）

plan-stage codex 跑 **6 轮**（R1-R6，全程真实 finding）。R1-R5#2 全部范围内 finding 已修并收敛（见各 Task 注 + Scope 边界）。R6 余 2 条经核实**越界**，user 2026-06-12 裁决 **「接受 residual + override」**：

| residual | codex 轮次 | 性质 | 归属 |
|---|---|---|---|
| **R-1 finalize fail-closed gating**：force-close 失败/非有限/未平仓时阻断 auto+manual finalize（单一完整 terminal-state validator） | R3-high → R5#1 → **R6#1（3 次复述同补救 = permanent-bias）** | 需改**冻结** `TrainingSessionCoordinator.finalize`/`TrainingView`；codex 自述「单一 validator at finalize」**按名 = RFC §4.7 顺位 10a 单事务 finalization port** | **顺位 10a/10b**（RFC §4.7） |
| **R-2 低面值清仓负 proceeds 语义**：fee>proceeds（如 100×0.01 + 最低佣金 5）→ 负现金的处置（拒绝/负债/floor） | R6#2（新，但预先存在于 auto force-close + `sell`） | **spec 级非负资金不变量决定**，涉冻结 E3/E5b；6a 已加 `currentTotalCapital >= 0` 结算门守卫使**门诚实**，但 mutation 侧语义未定 | **治理 / 顺位 10**（spec 决定） |

**override 机制（最终 branch-diff 闸门）**：6a 实施后整体 codex 评审若复述上述 2 越界 residual → TTY attest-override + admin merge（per `feedback_codex_round6_self_contradiction` + `feedback_codex_distributed_reliability_drilldown`；不绕 Catalyst required check，Catalyst 须真绿）。6a 自身范围内（§4.4a/§4.4b engine API + 内部不腐蚀/不 trap + 结算门诚实）已收敛。

**branch-diff codex R1 结果（2026-06-12，实施后）**：verdict=needs-attention，2 high——经核对**正是上述 2 个 user-accepted residual 的复述**，无新增 6a 范围内 finding：
- finding-1（`TrainingEngine.swift:432-434` auto-finalize 忽略 force-close 失败）= **R-1 finalize fail-closed gating**（plan-stage R3/R5/R6 已 3 次 fence，本轮第 4 次）→ RFC §4.7 顺位 10a。
- finding-2（`:485-499` 手动平仓 commit 负现金后报 false）= **R-2 低面值负 proceeds 非负不变量语义**（plan-stage R6#2）→ 治理/顺位 10。
两者均 user 2026-06-12 明确接受为 residual。按 override 机制：TTY attest-override + admin merge（Catalyst 真绿不绕）。spec+quality+final code review 三道均 APPROVE（仅 1 Minor acceptance 命令大小写已修）。
```
