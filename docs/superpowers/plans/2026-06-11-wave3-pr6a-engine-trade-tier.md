...
| 2 | `swift test --filter forceClose` | 15 个新测试全 ✔（含 `NoOpOnFiniteOverflowPrice`/`NoOpOnCashSumOverflow` 现金不被写 NaN/inf、`ReturnsFalseWhenDrawdownNonFinite`/`ReturnsFalseWhenReturnRateNonFinite` 安全降级、`autoForceCloseNoOpOnCashSumOverflow`）；既有 4 个 auto 强平测试仍 ✔ | 全 ✔ 且 0 failures = 通过；否则不通过 |
...
| 4 | `swift test`（全量回归） | `Test run with N tests`，`N ≥ 820`（基线 799 + 新增 21），`0 failures` | 0 failures = 通过；≥1 failure = 不通过 |
