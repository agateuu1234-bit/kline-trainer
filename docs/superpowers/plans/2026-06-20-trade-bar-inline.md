...
**Interfaces:**
- Consumes: `PositionTier`（`Models.swift:25-31`，`allCases` = tier1…tier5，`rawValue` = `"1/5"…"5/5"`）。
...
把文件头注释里引用旧模态 PositionPicker 的 D9 行（约 L13 `D9 PositionPicker 全档启用，buy 返 failure 兜`）改为反映新设计，例如：

```swift
// - D9（RFC #1 改）：买卖改内联 TradeBarView（点买/卖悬浮小条，非模态 PositionPicker）；
//   全仓/清仓 = tier5 强调档；buyEnabled/sellEnabled 门控小条打开，失败仍走 TradeFeedback toast。
```

（只改这一行 stale 注释；不动其余无关注释。）
