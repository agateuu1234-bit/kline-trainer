...
- ~~**MAJOR-1**~~：**已修复（codex round 1 HIGH-2）**。`DefaultTrainingSetReader.loadAllCandles` 已改为通过内部 `KLineRow: FetchableRecord, Decodable` 走 GRDB throwing decode 路径：列类型 mismatch / NULL 出现在 NOT NULL 语义列 → 抛 `RowDecodingError`，外层 catch 翻译为 `AppError.persistence(.dbCorrupted)`，不再 fatalError。新增 2 个 corrupt fixture tests 覆盖（`test_loadAllCandles_wrongTypeInColumn_throwsDbCorrupted` + `test_loadAllCandles_nullInRequiredColumn_throwsDbCorrupted`）。Reader test 总数 5 → 7，全 PR test 总数 15 → 17。
