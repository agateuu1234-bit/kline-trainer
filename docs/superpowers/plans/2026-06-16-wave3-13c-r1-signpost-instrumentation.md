...
在 `KLineView.swift` 的 `renderState` 闭包后（约 L21 之后）插入：
...
| 3 | `grep -c "RenderSignposter.beginMake\|RenderSignposter.beginMakeCrosshair\|RenderSignposter.beginDraw" ios/Contracts/Sources/KlineTrainerContracts/Render/ChartContainerView.swift ios/Contracts/Sources/KlineTrainerContracts/Render/KLineView.swift` | ≥ 3（三调用点接线：make + crosshair-make + draw） | ☐ Pass / ☐ Fail |
...
grep -c "RenderSignposter.begin" ios/Contracts/Sources/KlineTrainerContracts/Render/ChartContainerView.swift ios/Contracts/Sources/KlineTrainerContracts/Render/KLineView.swift
grep -n "13c-R1" docs/acceptance/2026-06-14-wave3-pr13c-completion.md
bash scripts/governance/verify-wave3-completion.sh
