...
Run（**authoritative = CI workflow `.github/workflows/catalyst-build.yml`**：scheme `KlineTrainerContracts`，cwd `ios/Contracts`）：
```bash
cd ios/Contracts && xcodebuild build-for-testing \
  -scheme KlineTrainerContracts \
  -destination 'platform=macOS,variant=Mac Catalyst' -derivedDataPath /tmp/derived 2>&1 | tail -15
```
...
xcodebuild build-for-testing -scheme KlineTrainerContracts -destination 'platform=macOS,variant=Mac Catalyst' -derivedDataPath /tmp/derived 2>&1 | tail -3
```
