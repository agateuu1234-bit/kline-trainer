...
- **F6（codex R6，high）—— 技术成立，但在本仓结构上不可达**：
  R6 指出 §C-5 那道「独立的必需门」并不真的独立 —— `pull_request` 事件下 checkout
  取的是**合并后的内容**，即 PR 自己的改动。所以一个 PR 可以**同时**收窄
  `backend-tests.yml` 的两个触发器 **并且** 删掉 `codeowners-config-check.yml` 里
  那两步（或改坏那个测试文件），只要 job id 不变，必需检查照样绿。

  **这条完全成立，而且对任何「写在仓库里的守卫」都成立** —— 守卫和被守卫的东西住在
  同一个可被同一个 PR 修改的树里，这是仓库内 CI 的固有属性，不是本次实现的疏漏。
  再加一层守卫只是把同样的问题往外挪一格（无限递归）。

  R6 给的两条出路，本仓**都不可达**（已逐条核实，2026-09-01）：
  1. *「外部托管的必需工作流 / GitHub App」* —— 需要组织级设施，本仓是个人仓库；
  2. *「对这个 workflow 及其检查器强制不可绕过的审查」* —— 本仓
     **协作者数量 = 1**。GitHub 不允许作者批准自己的 PR，所以把
     `required_approving_review_count` 设成 ≥1 会让**唯一的维护者再也合不了任何 PR**。
     本仓 memory `feedback_branch_protection_single_dev` 也正是据此明确记录
     「单人仓库应当 `required_approving_review_count: 0`」。

  **另外核出一条**（与本 PR 无关，单独记）：当前 ruleset 的 `bypass_actors` 里有
  `{"actor_type":"RepositoryRole","bypass_mode":"always"}`（actor_id=5，按 GitHub 的
  角色编号应为仓库管理员，未逐字核对该映射）。也就是说**本仓任何必需检查对管理员
  本来就是可绕过的** —— 而管理员就是那唯一一个人。这让「不可绕过的强制」在本仓
  更加无从谈起。建议单独排查这一项是否符合预期。

  **威胁模型的诚实表述**：能做出 R6 那个改动的，只有仓库唯一的那个人、而且必须是
  **刻意**为之（我对 `.github/workflows/**` 是硬 deny，每次落地都得由 user 亲手 `cp`）。
  本 PR 这套守卫的目的是防**意外**（无意中漏掉某个外部输入、或为省 CI 时间顺手加回
  过滤器），这个目的已经达到：所有非协同的单点改动都会在合并前被拦住（变异 32 组验证）。
  它不打算、也无法防御「所有者刻意改自己的 CI 策略」。
