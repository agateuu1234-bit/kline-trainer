...
**判定 F1（采纳 codex「throwing factory」—— additive，非 breaking 契约变更）：** 新增 `public static func make(...) throws -> TrainingEngine`，对**全部数据派生输入**逐项校验 → 抛**已存在**的 `AppError.trainingSet(.emptyData)`（`isRecoverable==true`，由 UI 呈现），再委托 `init`：① `maxTick>=0`（最先——`0...maxTick` 范围 maxTick<0 时访问即 trap，故先 guard）② `flow.allowedTickRange.upperBound == maxTick`（flow/maxTick 版本错位）③ 非空 `.m3` ④ `.m3` 覆盖 maxTick ⑤ resolved `startTick = initialTick ?? flow.initialTick` 在 `flow.allowedTickRange` 内（陈旧 resume tick 超出被替换的更短训练组）（Stage6 R3-F1 补全：tick/maxTick 同属持久化数据）。**E6 必须经此工厂构造**（可恢复路径**强制于代码**），并对 P5 缓存数据**重新校验**。`init` 保留同名 precondition 作「已校验数据」契约末线不变量（直调 init = trust-boundary，trap = 调用方程序 bug，同 `NormalFlow` 风格）。**不改 init 为 throwing**（spec init 非 throwing，modules L1607-1616），**不新增 AppError 分支**（复用 `.emptyData`）→ additive 扩展，登记待 E6 RFC（同 D5/D7）。**残留：** flow 以负 maxTick 构造但 param 不一致这种 E6 程序 bug 无法在 factory 防（访问其 range 即 trap），属 init precondition territory。
