...
> **⚠️ STATUS: COMPLETED — DO NOT RE-EXECUTE ⚠️**
>
> 本 plan 已于 PR #49（branch `worktree-pr7b2-stale-drift-helpers-cosmetic`，commits `3aedfd1`→`91701ae`→`b0fcdfe`→`821939f`→`8083dbb`）完整实施落地，所有 task checkbox 已勾选标记完成。**未来 agent 看到此文件请视作只读 archival 记录**，不要按 task 步骤再次插入 helper / 增 test / 写 acceptance—— `makePanel` / `makeDrawingMode` / `ReduceStaleDrawingSnapshotTests` / `*ReadsSnapshotNotRevision` tests / acceptance doc / reducer comment 字面对齐全部已在 PR #49 baseline 中存在，重跑会触发 redeclaration build 失败 + 重复 Suite 名 + acceptance 文件 overwrite。
>
> （原 plan stage 用途：codex adversarial-review 8 轮收敛 APPROVE 的 source-of-truth；branch-diff stage 用途：PR #49 reviewer 复审 plan-vs-impl 对照。）

> **For agentic workers (historic note, no longer actionable):** Was REQUIRED SUB-SKILL `superpowers:subagent-driven-development`. Steps used checkbox (`- [x]`) syntax for tracking — all boxes marked complete below. Per project memory `project_executing_plans_excluded`: 本项目只用 subagent-driven-development。每个 batch 派一个 fresh sonnet 4.6 high-effort subagent；批与批之间主线 review。
...
- [x] 在 `ReducerTests.swift` L12（`@testable import KlineTrainerContracts` 后）加：
...
- [x] L200-203（`ReducePanStartedTests`）
- [x] L243-246（`ReducePanEndedTests`）
- [x] L284-287（`ReduceTradeTriggeredTests`）
- [x] L324-327（`ReducePeriodComboTests`）
- [x] L364-367（`ReduceOffsetAppliedTests`；这一个是 3-param 形态）
- [x] L408-411（`ReduceActivateDrawingTests`）
- [x] L455-458（`ReduceSetDrawingSnapshotTests`）
- [x] L527-530（`ReduceDrawingCommittedTests`）
- [x] L570-573（`ReduceDrawingCancelledTests`）
...
- [x] L413-418（`ReduceActivateDrawingTests`）
- [x] L460-465（`ReduceSetDrawingSnapshotTests`）
- [x] L532-537（`ReduceDrawingCommittedTests`）
- [x] L575-580（`ReduceDrawingCancelledTests`）

> 同上：实际按 `grep -n 'private func drawingMode' ReducerTests.swift` 应返回 4 行（删前）→ 0 行（删后）。

#### Step 1.5: 全文 `make(...)` callsite rename 到 `makePanel(...)`

- [x] 全文搜索 `var s = make(.` 和 `var s = make(drawingMode(` 形式，逐处改 `make` → `makePanel`、`drawingMode(` → `makeDrawingMode(`。
...
- [x] 修改 `Reducer.swift`：定位 PR7b1 落的（约 L168-169）
...
- [x] 修改 `Reducer.swift`：定位 PR7b1 落的（约 L183-184）
...
- [x] 修改 `Reducer.swift`：定位 PR7b1 落的（约 L187-190）
...
- [x] 在 `ReducerTests.swift` 末尾（`@Suite("revision UInt64 overflow")` 之前）插入：
...
- [x] 在 `ReduceDrawingCommittedTests` Suite 内（已有 `drawingMatchedExits` + `drawingUnmatchedKeepsSession` 之后）插入：
...
- [x] 在 `ReduceDrawingCancelledTests` Suite 内（已有 `drawingMatchedExits` + `drawingUnmatchedKeepsSession` 之后）插入镜像测试：
...
- [x] 运行（新 stale Suite 5 + 2 个 distinguishing tests = 7 个）：
