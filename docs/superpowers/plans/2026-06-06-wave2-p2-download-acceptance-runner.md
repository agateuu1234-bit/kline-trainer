...
6. **temp 文件清理契约**：`DefaultZipExtractor` L39-49 把 sqlite 平展进独立临时目录 `ZipExtract-<UUID>/`，故 `sqliteURL.deletingLastPathComponent()` = 解压临时目录。runner cleanup 传 `[下载zip, 解压临时目录]`（删目录连带 sqlite）。**cache 副本是 `cache.store` 另建的拷贝，不在 cleanup 范围**（`InMemoryCacheManager` L349-365 + `DefaultFileSystemCacheManager` 内部 stage，store 读 zip 自建副本）。
...
8. **schema 版本**：`openAndVerify(expectedSchemaVersion:)` 传**客户端可读常量** `TRAINING_SET_SCHEMA_VERSION = 1`（spec §11.3 L2202「双方共享常量」，当前代码未定义 → 本 PR 定义），**不传 `meta.schemaVersion`**（后者是服务端声明；若服务端给 schema≠1 而客户端只会读 1，传 meta 值会误放行不可读文件）。
...
- **Create** `ios/Contracts/Tests/KlineTrainerContractsTests/DownloadAcceptanceRunnerTests.swift`
  - 测试套件 + 测试本地 helper：`FakeAPIClient` / `StubDBFactory` / `StubReader` / `RecordingJournalDAO`
...
```swift
    public func run(meta: TrainingSetMetaItem, leaseId: String) async -> AcceptanceResult {
        ...
        do {
            ...
            // Step 6：存入 cache（cache 从 zip 自建副本）
            let file = try cache.store(downloadedZip: zipURL, meta: meta)
...
3. 状态机硬约束：`stored` 不能直跳 `confirmed`（指向 happy-path 序列断言含 `confirmPending`）。
...
7. Catalyst CI required check 真绿（给出 PR 页 check 名 `Mac Catalyst build-for-testing on macos-15` + 期望 ✅）。
