...
> ⛔⛔ **次序铁律（2026-08-20 事故后新增，全 task 适用）：先提交实现，再跑变异。**
>
> **原来的次序（先变异、后提交）是错的**，Task 6 第一次实施时被会话额度中途掐断，
> 留下一个**跨 5 个文件、含函数签名改动**的未复原变异态（生产代码 + 4 个测试文件的调用点），
> 且它在变异态下**还在继续编辑测试** ⇒ 「真实工作」与「为配合变异而做的改动」已无法可靠分离，
> 只能整体丢弃重来。
>
> **新次序**：
...
>
> **收益**：任何时刻被打断，恢复动作都只是 `git checkout -- .`（丢掉的必然只有变异本身，
> 因为真实工作已经在 commit 里）。这条纪律把「被杀」从**灾难**降级为**无损**。
>
> ⚠️ **若某条变异需要改函数签名**（如 M11），它会波及所有调用点 —— **这类变异必须放在最后一条**，
> 且复原后必须**逐个文件**核对 `git status` 为空，不能只看被改的那一个文件。
