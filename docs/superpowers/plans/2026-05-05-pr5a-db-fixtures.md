...
**预估 prod LOC（硬规则 ≤500；R1 / R2 / R3 / R4 / R5 修订上调）：**
- `InMemoryFakes.swift` 净增 ≈ 230 LOC（含 R5 factory doc comment scope limitation +5 LOC）
- `PreviewTrainingSetReader.swift` 新增 ≈ 115 LOC（R5 init `internal` 不增行数 = 删 `public` keyword + 注释 +3 行）
- 合计：**≈ 345 prod LOC** ✓ 仍低于 500 上限

**预估 test LOC**：≈ 670（无 R5 新增 test 用例——R5 只改访问控制 + scope clarification）
...
**R5 修订（codex round-5 high-1）—— 显式声明 scope = preview-path-only**：

`PreviewTrainingSetDBFactory` 的目标用例是 **preview 渲染 + happy-path 单测**，不是「production 错误路径模拟」（versionMismatch / fileNotFound / SQLite IO 错误）。如果 consumer 测试需要走 versionMismatch / fileNotFound 路径，应：
- 直接抛构造好的 `AppError.trainingSet(.versionMismatch)` 在测试 Stub 里
- 或为该测试用例自己写小 mock factory（XCTestCase 内 `class FailingFactory: TrainingSetDBFactory`）

**plan 在 §10 `不在本 PR 范围` 表新增一条**：「production 错误路径模拟（versionMismatch / fileNotFound 等）的 factory mock」→ 留 backlog；不属 PR5a fixture-port-domain（spec §11.3 line 2200）的 PreviewTrainingSetDBFactory 范畴。

文件头注释 + `PreviewTrainingSetDBFactory` doc comment 加显式 limitation 声明：
```swift
...
**R5 修订（codex round-5 med-2）—— reader init 改 `internal` 镜像 production 访问控制**：

Production `DefaultTrainingSetReader.init` (line 18) 是 **internal**（无 `public` 修饰）—— 唯一构造路径是 `DefaultTrainingSetDBFactory.openAndVerify`，工厂在调用 init 前已 validate meta。**外部调用者 永远不能直接 bypass meta validation**。

Fake 必须镜像同访问控制：
- `PreviewTrainingSetReader.init(meta:candles:)` 改 **internal**
- 唯一公开构造路径 = `PreviewTrainingSetDBFactory.openAndVerify`（已含 `validateMeta`，R4 修订）
- 单测如需直接构造 reader（绕过 factory），通过 `@testable import KlineTrainerContracts` 拿 internal 访问 —— 这是测试 internal 实现的合法 escape hatch；与 production 模块内 factory 调 init 同语义

**fake-internal 单测的 meta 也要满足 sanity**：现有单测如 close-then-read / 数据校验，都直接构造 reader（绕过 factory）。R4 已统一改用 `startDatetime: 1, endDatetime: 1`（满足 sanity），R5 这条进一步固化「**任何直接构造 reader 都用合法 meta**」契约。
...
**8. R5 修订对接（codex round-5 findings 吸收）：**
- finding 1 (high) factory 跳 file/schema verify → §3 加 R5 段：显式声明 fake scope = preview/happy-path only；error-path mock 进 backlog；不在本 PR 范围表加一条；factory doc comment 加 limitation 声明
