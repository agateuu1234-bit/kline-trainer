...
def test_pool_order_string_element_containing_the_key_names_is_still_rejected():
    ...
    再往下执行 `item["code"]` 就会泄漏裸 `TypeError`（英文报错，
    操作者读不懂，也不带「该怎么办」）。

    `isinstance(item, dict)` 排在第一道挡住了它——本条就是给那条判据配的
    专属档（2026-08-25 控制者变异时发现它此前**无档**：删掉类型判据，
    现有测试一条都测不出来）。
...
| # | 变异 | 必红的测试 |
|---|---|---|
| M18 | **在类型判据之前**把裸字符串转成对象（模拟真正的「兼容实现」） | `..._bare_string_elements_are_rejected` |
| M18′ | 把 `isinstance(item, dict)` 挪到键存在性判据之后、或删掉它 | `..._string_element_containing_the_key_names_is_still_rejected` |

> ⚠️ **M18 必须模拟「真正的兼容实现」，不能只放宽 `isinstance`**（2026-08-25 实测教训）：
> 只放宽类型检查时**全绿**——因为裸字符串被**下一条判据**接住了（对字符串
> `"code" in item` 是**子串检查**，`"code" in "600000.SH"` 为 False）。
> 那是**变异没对准判据**，不是测试没判别力。本仓纪律：**变异必须对准你要证明的那条判据**。
...
> ⚠️⚠️ **M30 的归因，三轮才定案**（2026-08-25：控制者变异 → 评审质疑 → 控制者再实测）：
> - 我第一版写「五个坏路径**统统被文件名判据接住**、路径判据**从未被求值**」——**错**。
> - 评审改说「只有 `""` 是被路径判据拒的，另外 4 个被文件名判据接住」——**也错**。
> - **实测定案**：真实实现下那五个**全部**由路径判据拒下，它一直在工作。
>   零判别力的真正机理是——**变异掉路径判据之后，文件名判据会兜底**（五个末段本身都不合规），
>   于是测试**仍红**、变异什么都没证明。
>
> 新补那一组的末段完全合规，绕过路径判据后**无人兜底**，判别力才真正落在它身上。
> **这条判据是防逃逸的防线**，此前零覆盖。
>
> ⭐ **这一条本身就是「归因要单独核」的最好例子**：三方给了三种说法，
> 结论（旧档零判别力）三方一致，**机理三方都不同**，只有真跑才定案。
> 而写进代码注释里的是**机理**——错的机理会误导下一个读它的人。
...
> ⚠️⚠️ **M43/M44 的关系已实测定案（2026-08-25）：不是「互相掩盖」，是「前者蕴含后者」**。
>
> 单独跑的结果：
> - 只删「聚合与重算相符」（循环内）→ `..._must_match_recomputation_...` 红 ✅ **专属档成立**；
> - 只删「两趟聚合相等」（循环后 `aggs[0] == aggs[1]`）→ **全绿**。
>
> 推演：循环内已要求**每一趟**都 `== expect_agg`；两趟都过 ⇒ `aggs[0] == aggs[1]`
> **必然成立** ⇒ 循环后那条是**恒真断言**。而 `..._rejects_two_passes_with_different_aggregates`
> 把 pass2 的聚合改成别的值后，它在**循环内**就被拒了、**根本走不到**循环后那条。
>
> **裁决：保留判据但据实标注，不删**——①spec 明写 full 级「两趟聚合摘要相等」，删掉是偏离；
> ②它是**防御性冗余**（若将来放宽了循环内那条，它还在守）。
> **但它此刻挡不住任何循环内那条挡不住的东西**，注释里必须写明，否则下一个读它的人
> 会以为它在守着什么。
>
> ⭐ **这正是本仓那条「读代码发现不了恒真断言，只有变异能」的实例。**
...
**期望**：最后一行是 `127 passed in X.XXs`（秒数无所谓）。

**通过判定**：数字恰为 **127** → 通过；比它少 → 不通过（有测试没被收集到）。
...
**期望**：第一条输出一串**从 `S2-F1` 开始、连号不跳**的编号（今天是到 `S2-F9`，
以后还会更多）；第二条输出的数字**等于第一条的行数**。

> 这两条是**互相印证**的，所以不用记住今天是几条：
> · 第一条列的是**全文提到过**的编号 —— 跳号说明有更正只在正文里被提及、却没建表格行；
> · 第二条数的是那张表里**以 `| S2-F` 开头的表格行** —— 比第一条少，
>   说明有行**掉出了表格**（这份 spec 上我两次把新增行前面多打一个空行，就会这样）。

**通过判定**：编号连号不跳、且两个数字相等 → 通过；任一不满足 → 不通过。
