...
```
`endSession()`（:425-439，与其它 `active* = nil` 同处，约 :437）加（R2-M：teardown 一并清，防 review 结束后 stale 名）：
```swift
        activeRecord = nil
```
...
加到 `TrainingSessionPersistenceTests.swift`（该文件已有 `Self.makeCoordinator(candles:)`/`Self.validCandles()` + review 用例 setup，**逐字复用真实 fixture**，见 :151-160）：
```swift
...
```
> fixture 字段 = `TrainingSessionPersistenceTests.swift:153-158` 既有 review 用例逐字镜像（仅把 stockCode/stockName 改为可断言值）。`makeCoordinator`/`validCandles` 是该测试类的 `Self.` 静态 helper。
...
public struct TrainingTopBarContent: Equatable, Sendable {
    ...
    public let position: String            // "仓位 3/5"（保留兼容）
    public let positionShort: String       // "3/5"（顶栏「仓位」格数值，免字符串截取 smell）
...
- [ ] **Step 4: 迁移既有 13 个用例（必做，否则编译红）**

`TrainingTopBarContentTests.swift` 既有用例**全部用旧 init** `(totalCapital:holdingCost:returnRate:positionTier:)`（约 11 个 `@Test` / 12 处 `TrainingTopBarContent(` 构造），且 2 处断言 `c.holdingCost`（:16/:22）。**逐个迁移**（grep `TrainingTopBarContent(` 与 `.holdingCost` 全覆盖，不漏）：
- 旧 `TrainingTopBarContent(totalCapital: A, holdingCost: B, returnRate: C, positionTier: D)` → 新 `TrainingTopBarContent(totalCapital: A, averageCost: B, shares: 0, returnRate: C, positionTier: D, stockName: nil, stockCode: nil)`（旧 `holdingCost` 入参语义本是总额，迁移后该参数位放原值即可——既有用例只测格式化口径，不测每股/总额语义，故沿用原数值不破坏断言）。
- 断言 `#expect(c.holdingCost == ...)` → `#expect(c.holdingCostPerShare == ...)`（字段重命名）。
> 这些既有用例验证 `¥`/千分位/±0 归一口径不回归，必须全绿。

- [ ] **Step 5: 跑测试看通过**

Run: `swift test --filter TrainingTopBarContent`
Expected: PASS（5 新用例 + 13 迁移用例全绿）
...
`AxisGridLayoutTests.swift:61` 现硬断言价标贴**右**缘：
```swift
...
```
同时扫该文件其余断言价标/周期标/量标/MACD标 x 坐标的用例（grep `maxX`/`minX` 在 priceTicks/volumeAxis/macdZero/periodLabel 相关用例），按新坐标（价/量/MACD=左缘、周期=右上）同步更新。
> 注（R2-L）：`cornerPlacement`/「角标定位左上角」类测试名/注释在周期标移右上后语义过时——若该测试用宽松 `>=`/`<=` 边界仍 PASS（不破），顺手把名/注释改为「右上」即可（非必须，不破不阻）。

- [ ] **Step 6: 跑测试看通过**

Run: `swift test --filter AxisGridLayout`
Expected: PASS（2 新用例 + 既有迁移用例全绿）

- [ ] **Step 7: `drawLabelBox` 去底框 + 阴影（透明可读）**
...
> 注：Task 5 已从 topBar 移除画线钮（旧 :165-168）；本步整体重写 topBar（:148-178，自然不含画线钮）。`metricCell` 用 Task 2 新增的 `bar.positionShort` 取「X/5」（不再字符串截取）。
...
> **编译说明（R2-H 裁决）**：`Picker(selection: $activePanel)` + `.tag(PanelId.upper/.lower)` 要求 `PanelId: Hashable`。`PanelId`（`Models.swift:45` `enum PanelId: Equatable, Sendable { case upper, lower }`）**无 associated value → Swift 隐式 Hashable**，可直接做 Picker selection。**不要**加 `extension PanelId: Hashable {}`——对无 payload enum 显式再声明会触发「redundant conformance to Hashable」编译错误。即不动 `Models.swift`、不碰契约。
