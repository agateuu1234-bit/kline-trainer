...
func activeRecord_setAfterReview_nilAfterNormal() async throws {
    ...
    await coord.endSession()                          // D10 precondition：normal 前先清会话
    _ = try await coord.startNewNormalSession()        // 无参
...
`TrainingTopBarContentTests.swift` 现有 **13 个用例全部用旧 init** `(totalCapital:holdingCost:returnRate:positionTier:)`，且 2 处断言 `c.holdingCost`（约 :16/:22）。**逐个迁移**：
