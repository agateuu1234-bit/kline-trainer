...
- preview 两项镜像影响**所有 `PreviewTrainingSetReader` 路径**测试（PreviewTrainingSetReaderTests + 任何用 `PreviewTrainingSetDBFactory` 的 preview/seed 测试）。
...
| 8 | `cd ios/Contracts && swift test --filter "m3DatetimeMustStrictlyIncrease\|aggregateOpenMustBeWithinWindow" 2>&1 \| tail -2` | 两条均 PASS（preview reader 镜像两项校验，维护契约不漂移） | ☐ |
