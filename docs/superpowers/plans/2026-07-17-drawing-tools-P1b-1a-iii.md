...
- Modify: `Sources/KlineTrainerContracts/TrainingEngine/TrainingEngine.swift`（`buy:521` / `sell:550` 加画线守卫）
- Test:
  - `Tests/KlineTrainerContractsTests/TrainingEngineTradeBoundaryTests.swift`（host：画线中 buy/sell 拒绝且零 mutation）
...
- [ ] **Step 8c: 引擎层交易边界——`buy`/`sell` 画线中一律拒绝（root 强制，codex plan-R13-high）**

> UI 层门控（清 tradeStrip + overlay + onConfirm）只挡当前这条 SwiftUI 路径。`TrainingEngine.buy/sell`（`:520/:549`）现只 `guard flow.canBuySell()`，任何**直接引擎调用 / 未来 UI 路径 / 竞态**都能在画线中成交、不可逆 + autosave。把「画线期间不下单」钉在**变更点本身**（矛盾不可表达）。

先写失败测试（**mutation-focused**：画线中下单必须零改动）：

```swift
// Tests/KlineTrainerContractsTests/TrainingEngineTradeBoundaryTests.swift
// Spec: split-addendum §4.1「画线期间不能下单」（codex plan-R13-high）：引擎层拒绝 + 零 mutation。
import Testing
import Foundation
import CoreGraphics
@testable import KlineTrainerContracts
...
@MainActor
@Suite("引擎层交易边界：画线中 buy/sell 拒绝且零 mutation")
struct TrainingEngineTradeBoundaryTests {
    static let bounds = CGRect(x: 0, y: 0, width: 800, height: 600)

    /// 真进入全局画线会话（两面板都 arm → drawingModeActive==true）。
    static func drawingEngine() -> TrainingEngine {
        let (e, _) = TrainingEngineInteractionTests.engine()
        e.recordRenderBounds(bounds, panel: .upper)
        e.recordRenderBounds(bounds, panel: .lower)
        e.beginDrawingSession(tool: .horizontal)
        return e
    }

    @Test("画线中 buy → .trade(.disabled)，cash/position/markers 零 mutation")
    func buyRefusedWhileDrawing() {
        let e = Self.drawingEngine()
        #expect(e.drawingSession.drawingModeActive)            // 前置：真在画线
        let cash = e.cashBalance, shares = e.position.shares, markerN = e.markers.count
        let result = e.buy(panel: .upper, shares: 1)
        guard case .failure(.trade(.disabled)) = result else { Issue.record("应 .trade(.disabled)"); return }
        #expect(e.cashBalance == cash && e.position.shares == shares && e.markers.count == markerN)
    }

    @Test("画线中 sell → .trade(.disabled)，零 mutation")
    func sellRefusedWhileDrawing() {
        let e = Self.drawingEngine()
        let cash = e.cashBalance, shares = e.position.shares
        let result = e.sell(panel: .upper, shares: 1)
        guard case .failure(.trade(.disabled)) = result else { Issue.record("应 .trade(.disabled)"); return }
        #expect(e.cashBalance == cash && e.position.shares == shares)
    }

    @Test("非画线：buy 不被 drawing 守卫误伤")
    func buyNotBlockedWhenNotDrawing() {
        let (e, _) = TrainingEngineInteractionTests.engine()
        #expect(!e.drawingSession.drawingModeActive)
        if case .failure(.trade(.disabled)) = e.buy(panel: .upper, shares: 1) {
            Issue.record("非画线态不应因 drawing 守卫 disabled")
        }
    }
}
```

Run: `cd ios/Contracts && swift test --filter TrainingEngineTradeBoundaryTests` → FAIL（守卫未加）。再在 `TrainingEngine.buy`（`:521` canBuySell 守卫**之后**）与 `TrainingEngine.sell`（`:550` 之后）各加一行：

```swift
        // codex plan-R13-high：画线期间不下单——钉在变更点，任何调用路径都拦得住（UI 门控是外层纵深）。
        guard !drawingSession.drawingModeActive else { return .failure(.trade(.disabled)) }
```

> **hold/observe 不加守卫**：`holdOrObserve` 不是「下单」（不改仓、无不可逆成交），且画线模式底栏是 `DrawingModeBar`、无 hold 入口 → 无路径。本步只封 buy/sell 两个真·下单口。
...
git add ios/Contracts/Sources/KlineTrainerContracts/UI/TrainingView.swift \
        ...
        ios/Contracts/Tests/KlineTrainerContractsTests/TrainingEngineTradeBoundaryTests.swift \
        ios/Contracts/Tests/KlineTrainerContractsTests/Drawing/DrawingSessionSourceGuardTests.swift
git commit -m "1a-iii Task5：TrainingView 集成——画图入口+底栏切换+退役浮动钮(谓词拆分)+交易边界(引擎root守卫+overlay+onConfirm三重)+设置卡片+退役 noNewDrawingUI"
```
