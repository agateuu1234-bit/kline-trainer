        ...
        VStack(alignment: .leading, spacing: 14) {
            group("线型") {
                seg(LineSubType.allCases, current: style.lineSubType,
                    ...
                    title: { subLabel($0) }) { picked in commit { $0.lineSubType = picked } }
            }
            group("线样式") {
...
**Files:**
- Modify: `Sources/KlineTrainerContracts/UI/TrainingView.swift`（谓词 `:69` / 浮动钮 `:186` / 底栏 `:199` / activePanel 高亮 `:425` / 顶栏 `:328` / tradeStrip overlay `:401` / onChange 群 `:234`）
- Test:
  - `Tests/KlineTrainerContractsTests/Render/TrainingViewShellSourceGuardTests.swift`（谓词拆分 + 退役 + 交易边界结构守卫）
  - `Tests/KlineTrainerContractsTests/Render/TradeBoundaryTests.swift`（UIKit-gated：进画线作废买卖框、`engine.buy/sell` 零调用）
...
- Produces: 新私有谓词 `showsFloatingDrawingTool` / `showsActivePanelHighlight`；`@State private var typeRowExpanded`、`@State private var showingStyleCard`。
...
struct TrainingViewShellSourceGuardTests {
    ...
    @Test("交易边界：drawingModeActive 变 true 即清 tradeStrip + overlay 门控")
...
- [ ] **Step 9: 交易边界纵深——TradeBox overlay `:401` 加 `!drawingModeActive`**

把 `if showsTradeButtons, let strip = tradeStrip, strip.panel == id {` 改为：
...
> 说明：`tradeStrip` 清空与 overlay 门控属 View 层，纯 host 测不到点击；本行为测试锁「进画线不改持仓」的引擎侧不变量，**View 层由 Step 1 的结构守卫 + 模拟器验收覆盖**（见收尾）。
...
git commit -m "1a-iii Task5：TrainingView 集成——画图入口+底栏切换+退役浮动钮(谓词拆分)+交易边界+设置卡片"
```
...
**本 design 额外新增负向测试**（超出 §4.3 原 12 条，codex design review 逼出）：
- 原子样式落盘（Task1 `defaultStyleFlowsIntoCommit`：落锚→append 间 `drawings` 只写一次、样式原子构造）。
- 交易边界（Task5 `TradeBoundaryTests` 引擎侧不变量 + `TrainingViewShellSourceGuardTests.tradeBoundary` 结构守卫 + 模拟器人工验收）。
