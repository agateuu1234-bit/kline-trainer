...
```swift
    ...
    func atomicStyleConstruction() throws {
        ...
        #expect(!rc.contains("drawings["))               // 无 drawings[/reviewDrawings[ 元素替换（后者含前者子串）
...
| 9 | 样式往返（autosave 重载五字段相等） | ✅ Task1 `styledDrawingSurvivesSaveProgress`（**端到端**：defaultStyle→commitPending→routeDrawingCommit→saveProgress→读回 pending，5 字段+revealTick 保真+原子性）+ `defaultStyleFlowsIntoCommit`（提交产出带样式）+ 🔁 `DrawingModelP1aTests:45`（Codable 往返） |
