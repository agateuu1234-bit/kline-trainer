...
struct RubberBandTests {
    ...
    func asymptote() {
        ...
        #expect(RubberBand.damp(over: 100_000, dimension: 800) > 790)         // 已逼近 d
...
Expected: `dragPastEdgeDamps`/`reverseDragUnwinds`/`newestEdgeHardClamp` 失败（当前硬钳，offset 停 maxOffset 或 0）。`inBoundsLinear`/`noScrollRoomHardClamp` 可能已绿（界内/无空间硬钳行为偶合）。
