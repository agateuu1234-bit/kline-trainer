...
| # | 位置 | 性质 | 对栈的表态 | 结构性写入计数 |
|---|---|---|---|---|
| 1 | `init` 的 `self.drawings = seededLossy.drawings` | **构造**（不是替换） | 无需动作（新引擎的栈按定义为空） | 1（整体赋值，计入结构性写入） |
...
| 7 | `injectDrawingsForTesting` | 仅测试可达 | **作废整个栈** | 1（`drawings = ds`，整体赋值，计入结构性写入） |
| 8 | `applyUndoEntry` | 撤销执行单点 | **既不入栈也不作废** | **3**（`remove(` + `insert(` + 下标赋值） |
| | | | **合计** | **10** |

**关于整体赋值计入结构性写入（整支终审②/Important-4 订正，2026-08-21）**：上表第 1、7 行的整体赋值
（`drawings = <表达式>`）**计入**结构性写入——它是最彻底打乱数组下标的一种写法，本表原先把它排除在外
（旧版此处写「0，不是结构性写入」）是本表自身的判定缺陷，不是既有事实：本表的头注就声称要穷尽「全部
会打乱数组内容/顺序的方法」，漏掉整体赋值等于在根因层（D79 第一层）留了一个口子——一处会让栈里已存
下标必然错位的写入面，却因为「不算结构性写入」而从没被要求对栈表态。守卫 L12b 与 U-G4 已按此订正为 **10**。
