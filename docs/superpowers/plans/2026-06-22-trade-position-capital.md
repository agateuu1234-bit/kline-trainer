...
```swift
        let id = try finalization.finalizeSession(record: record,
                                                  ...
                                                  sessionKey: key)
        // R-plan-3-1：刷活缓存→主页即时反映。R-plan-4-2：用**内存** engine.currentTotalCapital
        // （= 持久记录 total_capital+profit，纯内存计算不可抛），**不**在 finalize 成功后做 fallible DB 读
        //  → 杜绝「记录已 commit 却因后续读失败而 finalize 抛错」的半失败窗口。
        settings.refreshTotalCapital(engine.currentTotalCapital)
        return id
```
