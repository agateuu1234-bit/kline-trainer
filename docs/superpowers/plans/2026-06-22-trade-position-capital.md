...
- `ios/Contracts/Sources/KlineTrainerContracts/TrainingEngine/TrainingSessionCoordinator.swift` — `startingCapital()` 直读 DB + `finalize()` 传 newTotalCapital（Task 3）。
- `ios/Contracts/Sources/KlineTrainerContracts/Persistence/SessionFinalizationPort.swift` + `ios/Contracts/Sources/KlineTrainerPersistence/DefaultAppDB.swift` — `finalizeSession` 加 `newTotalCapital` 参数 + 事务内写（Task 3）；`resetAllTrainingProgress` 去 `deleteAll`（Task 3）。
...
**Files:**
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/Persistence/SessionFinalizationPort.swift`（`finalizeSession` 签名加 `newTotalCapital`）
- Modify: `ios/Contracts/Sources/KlineTrainerPersistence/DefaultAppDB.swift`（`finalizeSession` 事务内 `setTotalCapital`；`resetAllTrainingProgress` 去 `deleteAll`）
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/TrainingEngine/TrainingSessionCoordinator.swift`（`finalize` 传 newTotalCapital；`startingCapital` 直读 DB）
- Test: `ios/Contracts/Tests/KlineTrainerPersistenceTests/TrainingResetPortTests.swift`（加「reset 保留记录」）+ `SessionFinalizationPortTests.swift`（加「finalize 写 total_capital」）

**Interfaces:**
- Consumes: `SettingsDAOImpl.setTotalCapital`、`RecordRepositoryImpl.insertRecord`、`PendingTrainingRepositoryImpl.clearPending`、`SettingsDAO.loadSettings`。
- Produces（Task 5 依赖）: `settings.total_capital` 成为权威当前资金；`finalizeSession(record:ops:drawings:sessionKey:newTotalCapital:)`。
...
`SessionFinalizationPortTests.swift` 加：

```swift
func test_finalize_writes_total_capital() throws {
    let id = try db.finalizeSession(record: someRecord, ops: [], drawings: [],
                                    sessionKey: "k1", newTotalCapital: 123_456)
    XCTAssertGreaterThan(id, 0)
    XCTAssertEqual(try db.loadSettings().totalCapital, 123_456)
}
```

（`someRecord`/insert 用本文件现成构造范式。）
...
Expected: 编译失败（`finalizeSession` 无 `newTotalCapital` 参数）+ reset 测试 FAIL（仍 deleteAll → totalCount==0）。

- [ ] **Step 3a: 改 `SessionFinalizationPort` 协议签名**（加 `newTotalCapital: Double`）

在 `SessionFinalizationPort.swift` 把 `func finalizeSession(record:ops:drawings:sessionKey:) throws -> Int64` 改为追加末参 `newTotalCapital: Double`。

- [ ] **Step 3b: 改 `DefaultAppDB.finalizeSession`**（事务内加写 total_capital）

```swift
    public func finalizeSession(record: TrainingRecord, ops: [TradeOperation],
                                drawings: [DrawingObject], sessionKey: String,
                                newTotalCapital: Double) throws -> Int64 {
        do {
            return try dbQueue.write { db in
                ...
                try SettingsDAOImpl.setTotalCapital(db, newTotalCapital)   // A4：权威当前资金（原子）
                return id
            }
...
- [ ] **Step 3c: 改 `DefaultAppDB.resetAllTrainingProgress`**（去 `deleteAll`，保留记录）
...
- [ ] **Step 3d: 改 `TrainingSessionCoordinator`**（`startingCapital` 直读 DB；`finalize` 传 newTotalCapital）

`startingCapital()` 改为（直读 DB，绕开 SettingsStore 缓存陈旧）：

```swift
    /// RFC-A A4：起始资金 = 权威 settings.total_capital（直读 DB，反映上局 finalize 写入）。
    private func startingCapital() throws -> Double {
        try settingsDAO.loadSettings().totalCapital
    }
```

`finalize()` 内调用处把 `finalizeSession(record:…, sessionKey: key)` 改为追加 `newTotalCapital: engine.currentTotalCapital`：
...
```swift
        let id = try finalization.finalizeSession(record: record,
                                                  ...
                                                  sessionKey: key,
                                                  newTotalCapital: engine.currentTotalCapital)
```
...
Run: `swift test` 全量（确认 finalizeSession 签名改动的其它调用点/fake 同步——若 `InMemory*`/fake 实现了 SessionFinalizationPort 需同步加参；编译错会指明）。

- [ ] **Step 5: 提交**

```bash
git add ios/Contracts/Sources/KlineTrainerContracts/Persistence/SessionFinalizationPort.swift ios/Contracts/Sources/KlineTrainerPersistence/DefaultAppDB.swift ios/Contracts/Sources/KlineTrainerContracts/TrainingEngine/TrainingSessionCoordinator.swift ios/Contracts/Tests/KlineTrainerPersistenceTests/TrainingResetPortTests.swift ios/Contracts/Tests/KlineTrainerPersistenceTests/SessionFinalizationPortTests.swift
...
**Interfaces:**
- Consumes: `DatabaseMigrator` 注册范式（同 `0004`）；`AppDBFixture.makeFreshDB`/partial-migrator 升级路径范式（同 `AppDBMigrationsTests` 的 `test_0004_upgrade_*`）。
...
final class AppDB0005MigrationTests: XCTestCase {
    ...
    override func setUp() async throws { dbURL = try AppDBFixture.makeFreshDB() }
    ...
    func test_full_migrator_sets_user_version_3() throws {
        let q = try AppDBFixture.openRaw(at: dbURL)
    ...
    func test_0005_backfills_total_capital_from_last_record_and_keeps_other_keys() throws {
        // 模拟「升级前」：partial migrator 跑到 0004（user_version=2），插 2 条记录 + 写 settings 其它键
        let q = try DatabaseQueue(path: dbURL.path)   // 注：用一个只注册到 0004 的 partial migrator（见下方 helper）
        try Self.migrateTo0004(q)
        try q.write { db in
            // 旧 settings：total_capital 默认 10 万 + 费率/主题
    ...
    // helpers：migrateTo0004 注册 0001/0003/0004（不含 0005）；insertRecord 插一条最小合法 training_records 行。
...
- Modify: 审计出的消费者（如主页统计 `HomeContent`/其驱动），改读 `settingsDAO.loadSettings().totalCapital`（或经已有的 settings 读取路径），而非 `statistics().currentCapital` 派生。
- Test: 对应消费者的 host 测（若是纯值 `*Content`，加「显示值 = 注入的权威资金」断言）。

**Interfaces:**
- Consumes: Task 3 的权威 `settings.total_capital`。
- Produces: 重置后显示不与权威值背离。

- [ ] **Step 1: 审计 + 写失败测试**

Run 审计：`grep -rn "currentCapital" ios/Contracts/Sources`。对每个「显示跨局当前资金」的点（预期：主页统计），确认它当前读 `statistics().currentCapital`（派生）。为其纯值层写测试：注入「有记录但 settings.total_capital 已被 reset 为 10 万」场景 → 期望显示 10 万（权威），而非末条记录派生值。

> 注：若审计发现**没有**任何 UI 实际显示 `statistics().currentCapital`（仅 `startingCapital` 用过、已在 Task 3 改走 settings），则本任务收敛为：在 plan 执行记录中**显式声明「无额外消费者」**（非静默），`statistics().currentCapital` 字段保留供胜率统计的同伴查询但不再作资金真相源；跳到 Step 5 提交一个 doc/注释说明。
...
把审计出的消费者改读权威 `settings.total_capital`。（具体改动随审计结果定；每处 = 把 `statistics().currentCapital` 换成 `loadSettings().totalCapital` 注入。）
...
> 风险（执行时注意）：Task 3 改 `SessionFinalizationPort` 签名会波及所有实现该协议的 fake（编译报错会指明，逐个加 `newTotalCapital`）；Task 5 D6 消费者集合以审计结果为准（若无消费者则显式声明非静默）；Task 9 删冻结契约方法须 Task 8 先完成 UI 迁移。
