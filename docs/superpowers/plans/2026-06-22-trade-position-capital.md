...
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/TrainingEngine/TrainingSessionCoordinator.swift`（`startingCapital` 直读 DB；`finalize` 调用点不变）
...
- Produces（Task 5 依赖）: `settings.total_capital` 成为权威当前资金；`finalizeSession(record:ops:drawings:sessionKey:)` 签名不变，事务内从持久记录派生写资金（retry 幂等）。
...
（`someRecord`/`someRecordWithProfit`/insert 用本文件现成 `TrainingRecord` 构造范式。）
...
- [ ] **Step 3c: 改 `TrainingSessionCoordinator.startingCapital()`**（直读 DB；`finalize()` 调用点**不变**）
...
`finalize()` 内 `finalizeSession(record:ops:drawings:sessionKey:)` 调用点**保持原样**（不加参数；资金由 `DefaultAppDB` 内部从持久记录派生）。`SessionFinalizationPort` 协议**不改签名** → 无 fake 波及。
...
```bash
git add ios/Contracts/Sources/KlineTrainerPersistence/DefaultAppDB.swift ios/Contracts/Sources/KlineTrainerContracts/TrainingEngine/TrainingSessionCoordinator.swift ios/Contracts/Tests/KlineTrainerPersistenceTests/TrainingResetPortTests.swift ios/Contracts/Tests/KlineTrainerPersistenceTests/SessionFinalizationPortTests.swift
...
> 风险（执行时注意）：Task 3 `finalizeSession` 不改签名（资金从持久记录派生，零 fake 波及）；finalize retry 幂等已由「读持久记录派生资金」保证（codex R-plan-2-1）。Task 5 D6 消费者集合以审计结果为准（若无消费者则显式声明非静默）。Task 9 删冻结契约方法须 Task 8 先完成 UI 迁移。
