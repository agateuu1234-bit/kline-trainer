...
- [ ] **Step 2: 在 §E5 小节末尾插入契约增量块**

在 `### E5 训练引擎模块` 小节内容末尾（下一个 `###` 之前）插入：
...
- [ ] **Step 2: 在 `saveProgress` 契约（L1676 区块）之后插入契约增量块**

在 modules `saveProgress`「U2 退出时 / 每 N tick 自动调用」契约附近（同 §E6 区）插入：
...
- [ ] **Step 2: 在 §F2 小节末尾插入契约增量块**

在 `### F2 主题模块 \`Theme/\`` 小节内容末尾插入：
...
```markdown
> **【顺位 1 RFC 已落地（`docs/superpowers/specs/2026-06-10-wave3-pr1-spec-gap-rfc-design.md`，2026-06-10）**：本节所述「tier 公式未定 / 拒臆造 / 显示语义 dispute / 中断持久化未定 / E5·E6 扩展未治理」等契约**已由 RFC §4.1-§4.7 钉死权威定义**。后续 planner 以 RFC 为准，下方历史描述仅作背景；**本节契约已由顺位 1 RFC 钉死**。】
```
...
- **(c) outline supersede marker 位置**：`h < m < s`（heading `### 3.1 顺位 1` 行 < marker `本节契约已由顺位 1 RFC 钉死` 行 < 首个 `拒臆造` 行）。任一缺/序错 → FAIL。
...
- **(g) 冻结一致性**：`git diff --name-only "$base" HEAD | grep -E "plan.*\.md"` 只允许 `kline_trainer_plan_v1.5.md`（无其他 `*plan*.md`）；且无 `docs/superpowers/plans/2026-05-` 路径。命中违例 → FAIL。
...
**禁用措辞**（per `.claude/workflow-rules.json` forbidden_phrases）：不得出现「验证通过即可 / 看起来正常 / 应该没问题 / should work / looks fine」。
