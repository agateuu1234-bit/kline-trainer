...
Run: `cd ios/Contracts && swift test --filter PanLinkageTests`
Expected: PASS —— 7 个 `@Test` 全绿，0 failures。

- [ ] **Step 5: Run full host suite (regression)**

Run: `cd ios/Contracts && swift test`
Expected: 全量 0 failures（新增 7 测试；现有零回归）。
...
Run: `cd ios/Contracts && swift test --filter TrainingEnginePanLinkageTests`
Expected: PASS —— 7 个 `@Test` 全绿（2 drag + D7 + 减速帧 + R7 健全性 + D10 drawing + M2 跨周期）。

- [ ] **Step 5: Run full host suite (regression)**

Run: `cd ios/Contracts && swift test`
Expected: 全量 0 failures（leader 路径未改，现有 pan/bounce/pinch/drag 测试零回归；新增 Task1(7)+Task2(7)）。
...
- [ ] `cd ios/Contracts && swift test` → 全量 0 failures；净 = +14（PanLinkage 7 + 引擎接线 7），现有零回归。
