...
Run: `cd ios/Contracts && swift test`
Expected: 全量 0 failures（leader 路径未改，现有 pan/bounce/pinch/drag 测试零回归；新增 Task1(8)+Task2(7)）。
...
- [ ] `cd ios/Contracts && swift test` → 全量 0 failures；净 = +15（PanLinkage 8 + 引擎接线 7），现有零回归。
