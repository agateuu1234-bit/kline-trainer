...
**trust-boundary 触点**：本 PR 仅新增 `ios/**/*.swift`（业务模块，非 `.github`/`.claude`/schema/CODEOWNERS）；不改冻结契约文件。属业务模块 anchor，非 governance/trust-boundary 变更。
...
| **D5** | **`finalize` 重入守卫**：`TrainingView` 用 `@State didFinalize` 保证自动结束仅调 `finalizeForSettlement()` 一次（`onChange(of: globalTickIndex)` 在末态可能多次触发）。 | `finalize` 第二次调会因 record 已插/pending 已清而行为异常或重复入账；幂等由调用方一次性闸门保证（U2 壳逻辑，非 coordinator 职责）。 |
...
//
// 平台无关纯值（host 全测）：把 engine 实时数值格式化为顶栏显示串。格式口径对齐 SettlementContent
// （POSIX `¥ ` 千分位 + 2 位小数；收益率带符号 2 位百分比 + -0.0 归一）。SettlementContent 的 formatter
// 为 private static（U3 冻结），本文件独立实现同口径（刻意不抽共享，避免动冻结 U3）。
...
public struct TrainingTopBarContent: Equatable {
    ...
    /// 带符号 2 位百分比；`-0.0` 归一为 `+0.00%`（signed-zero killer）。
    private static func percent(_ rate: Double) -> String {
        let pct = rate * 100
        let normalized = pct == 0 ? 0 : pct           // -0.0 → 0
        let f = NumberFormatter()
        f.locale = Locale(identifier: "en_US_POSIX")
        f.numberStyle = .decimal
        f.minimumFractionDigits = 2
        ...
        let s = f.string(from: NSNumber(value: abs(normalized))) ?? "0.00"
        return "\(sign)\(s)%"
    }
}
```
...
public struct TrainingView: View {
    ...
    // D4/D5：仅可步进且应结算模式（mode != .review）+ 到末态 + 未结算过 → 调 finalize 一次。
    private func maybeAutoEnd() {
        guard engine.flow.mode != .review, lifecycle.isAtEnd, !didFinalize else { return }
        didFinalize = true
        Task {
            do {
...
| # | action | expected | pass/fail |
|---|---|---|---|
| 1 | `cd ios/Contracts && swift test --filter TrainingSessionLifecycle` | 9 测试全 passed，0 failure | pass = 终端打印 `9 tests ... passed` |
| 2 | `swift test --filter TrainingTopBarContent` | 8 测试全 passed | pass = 终端打印 `8 tests ... passed` |
| 3 | `swift test` 全量 | `754 tests ... passed`（737 基线 + 17 新），0 failure | pass = 全量 0 failure |

## 二、生命周期 5 路径矩阵（host 测断言，逐条对应）

| # | 路径 | expected | pass/fail |
|---|---|---|---|
| 4 | back（Normal） | pending 写入 + activeEngine 清空 | pass = `back_normal_savesAndEnds` 过 |
| 5 | back（Review/Replay） | pending **不**写（非保存分支）+ activeEngine 清空 | pass = `back_review_noSaveButEnds`/`back_replay_noSaveButEnds` 过 |
...
**4. 测试计数自检：** Task 1 = 9 测试，Task 2 = 8 测试，合计 17。全量 = 737（基线，本 worktree 实测）+ 17 = **754**。验收 doc #3 / acceptance 数字以此为准（impl 阶段实跑回填，防 stale）。
