...
| D70 | Task 17/18 致命语义 | 重新武装绑在「需不需要复校」上 ⇒ 发布 P2-F3 具名结局会清掉本轮凭据、且对 source 侧不生效（Opus-5 两条 high，spec **S2-F69/F70**）| 拆成两件事：`fatal_published` 管「整次致命」（任何 kind），`_needs_staging_recheck` 只管「清除要哪道前提」；`failed` 凭据粘性；本轮发布的 fatal 不许同轮清 |
| D71 | Task 17 启动闸 | skip-verify 互斥只在启动判一次（Opus-5 [medium]，spec **S2-F71**）| `attest` 直接拒绝声明了该 flag 的运行 |
| D72 | Task 18 决策表 | 配对纵深防御只查一个方向（Opus-5 [low]，spec **S2-F72**）| 补反方向 |
| D73 | Task 16 单调守卫 | 容器类型不对时整条被跳过（Opus-5 [low]，spec **S2-F73**）| 容器坏型一律拒绝 |
| D74 | 流程（非代码）| ⚠️⚠️ **变异脚本被工具超时从外部杀掉，`finally` 不执行 ⇒ 源码残留一条被削弱的生产守卫**（`_require(True,` 在 L400）。而那个站点恰是 9 条「邻居兜底」之一 ⇒ **随后 1362 全绿完全看不出来**，差一步就提交上去 | ①**变异跑一律放后台**（前台超时=外部杀进程，`try/finally` 一概不执行）；②每次变异跑后**逐字节 diff 回 pristine** 并 grep 生产代码里的 `_require(True` / `if False:`；③**临时文件不是持久的** —— `/tmp/sweep.py` 被系统清掉而复用它的命令**退出码仍是 0**，脚本已改放 scratchpad |
...
| **Opus-5** | needs-attention（**2 high** + 1 medium + 2 low）| ⚠️⚠️ **严重度不降反升，而两条 high 都是 Opus-4 那次修复自己捅的** —— 我把「整次致命」与「清除要哪道前提」**用同一个谓词表达**，于是 P2-F3 的具名结局会清掉本轮凭据、而 source 侧逃逸完全不受管。**五条全部本机复现属实**（六轮累计 26 条零误报）。11 组变异全部命中：P3 退回旧写法两条 high 立刻复现（证明修对了）、P6「一律拒绝收尾」打死合法的 fatal 升级路径（证明没修过头）、**P11 首轮零红由控制者自查补档**。⚠️ 过程中控制者自己栽三次：`cannot_be_cleared` 两档第一版**假绿**（`match` 撞上既有一致性检查的文案，同族**第五次**）、新判据写在 `st` 赋值之前导致 37 条一起红（作用域错误非回归）、**变异残留**（见 D74）|
| **收口自查** | 0 违反（控制者，**非独立评审**）| ⚠️ 独立通道连续两次 `Connection lost mid-response` 开局即断（不算轮次；两次都验过 worktree 干净、无变异残留），按「连续两次同样早死就别盲目重跑第三次」改做法。⇒ 对**新守卫层**做**穷尽状态机探索**（不是读代码）：4 种合法起始生命周期 × 2 种 `skip_existing_verify` × 3 种登记 × 10 种操作 × 3 种登记 × 10 种操作 = **5832 条链路**，全部走真入口、真落盘、收场读回磁盘判定。检验这层守卫**自称提供**的两条性质：**I1** 本轮自己发布过 fatal ⇒ 收场时磁盘上必须仍有 fatal；**I2** 无权推进时（自己发布过 fatal，或起跑欠复校且未登记通过）不许推进进度指纹。结果 **I1=0、I2=0 违反**（其中 2187 条被启动闸正当拒绝、未成链，单独计数不混入覆盖量）。⚠️⚠️ **过程中探索脚本自己错了三次，逐条如实登记**：①没接住 `begin_run` 对「skip flag 撞 fatal」的**正当拒绝**（脚本崩）；②I2 第一版写成「一旦有 fatal 就永不许推进」⇒ **虚报 230 条**，把 spec 明令允许的「修好→复校通过→继续拉股」判成违反；③补丁插进 `try:` 块内部造成 `SyntaxError`。**验证工具本身也会错，且错法分虚报与漏报两种，漏报那种不会自己冒出来** —— 故「这次验证是否有效」本身是结论的一部分。⚠️ **不入测试套件**：单跑 38 秒，会把整套从 33 秒拖到 70 秒，与本轮「守卫集冻结、不再加东西」的政策冲突；方法与参数写在此处，将来改动守卫层时按此重建。⛔ **这是自查不是独立评审**，独立性那一档仍然缺着 |
| **独立评审（收口轮）** | needs-attention（1 high + 1 medium + 2 low）| ⚠️ 前两次尝试**开局即断**（`Connection lost mid-response`，不算轮次，每次都验过 worktree 干净无残留），第三次成功。⭐⭐ **它挖出的 high，正是控制者 5832 条状态机探索永远挖不到的那一类** —— 因为**同一个思维盲点同时污染了实现与验证**：我用「记录变没变」判武装，也用「fatal 凭空出现」判自查。四条**全部本机复现属实**（(a) 的「还能提交股票」那半我的复现脚本自己写错了 —— 重复加同一只股被别的守卫拒掉，**如实登记为未验成**；其余各半确凿）。5 组变异全部命中，含两条关键对照：退回旧判据红 7 条（修对了）、改成「只要带 fatal 就武装」红 2 条（**没修过头**）|
| **仪器订正** | 3 个盲区（控制者，非评审）| ⚠️⚠️ 顺着那条 high 回头查自己的状态机探索，发现**三层各错一处**：①**枚举空间**少了一整类操作（「再撞上同一条逃逸」）；②**判据**问错了（把「本轮自己发布了 fatal」判成「fatal **凭空出现**」，于是「盘上本来就有 fatal、本轮又撞一条」永远判不出来 —— 分支①那条 high 正是此形态）；③**时序**弄反（执行完这一步才更新状态，再拿它回头判这一步，且基线取运行起点 ⇒ 虚报 45 条）。**①②是漏报、不会以任何形式报错**，只会安静地给一个漂亮的 0 —— 上一轮那个 0 就是这么来的。补齐后 **7200 条链路、I1=0、I2=0**；⭐**并加了防空转自证**：关掉生产的武装判据 → 探索立刻报出 **I1 违反 60 条**，证明这个 0 是有判别力的 0。**判据（对 S3/S4/S5 全适用）：一个报 0 的探索，只有在证明过它能报非 0 之后才可信** |
