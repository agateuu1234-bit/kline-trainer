...
public struct CrosshairSidebarContent: Equatable, Sendable {
    ...
    /// 千分位整数（locale 无关手工分组，正负皆可）。
    static func groupedInt(_ n: Int64) -> String {
        let neg = n < 0
        var digits = Array(String(n.magnitude))
        var out: [Character] = []
        var count = 0
...
```swift
        /// RFC-C 进入黏滞十字光标：置激活 + arbiter 冻结 + prepare haptic + 首次定位。
        ...
        /// RFC-C 移动光标：刷新 crosshairPoint + 重渲染；吸附 index 变化时震一次（去重）。
        private func moveCrosshair(to location: CGPoint) {
            ...
            guard vp.geometry.candleStep > 0 else { return }      // 空图守卫（Int(NaN) 防崩）
            guard vp.mainChartFrame.contains(location) else { return }
