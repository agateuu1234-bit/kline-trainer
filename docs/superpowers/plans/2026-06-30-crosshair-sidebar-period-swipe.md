...
- `Tests/.../Render/CrosshairLayoutTests.swift` — 新增 frames-path 断言 + **改既有价标断言为左缘**（omit-frames 仅竖线/时签 extent 保旧；价标全局移左，codex R6）。
...
- Produces: `CrosshairLayout.resolve(at:mapper:candles:frames:)` —— `frames: ChartPanelFrames? = nil`。**`frames` 仅控「竖线纵向延展 + 时签底」两项**：`nil` → 竖线/时签限 `mainChartFrame`（旧 extent，**只此两项保旧**）；非 `nil` → 竖线 `mainChart.minY → macdChart.maxY`、时签底贴 `macdChart.maxY`。**价标锚定与 `frames` 无关、全局移左缘 `frame.minX`**（codex R4/R6）→ 既有右缘断言 `priceLabel.rect.maxX==1000` **必改**左缘 `minX==0`（Step 3 注），**不再「既有断言全不变」**。横线仍主图区 + 自由 Y（不变）。
...
Expected: 既有断言（omit frames）全绿（**注：`priceLabelFreeY` 价标断言已按 Step 3 改为左缘 `minX==0`**）+ 新 frames-path 测试（含 `priceLabelOnLeftEdge`）PASS。
...
3d. **drawing × crosshair 双向互斥**（codex R5-M2）：进画线模式时若 crosshair 仍黏滞，先退光标。改 `sync(panel:engine:view:)`（`ChartContainerView.swift:62-75`）——在 `let drawing = isDrawing(...)` 之后、`arbiter.drawingMode = drawing` **之前**插一行：
...
> `setCrosshair` 既有方法（L146-155）保留不动（被 moveCrosshair/exitCrosshair 复用）。`CrosshairLayout.snappedCandleIndex` 是 internal、同模块可见。`view.renderState.visibleCandles` 空时由 candleStep>0 守卫 + resolve clamp 保护。互斥两向：长按进光标查 `!isDrawing`（Step 2）；进画线退光标（3d）。
...
### Task 6: 信息栏数据补全（可见首根真实前收）+ KLineView 绘制悬浮信息卡

**Files:**
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/Render/KLineRenderState.swift`（加 `previousCloseBeforeVisible` 字段）
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/Render/RenderStateBuilder.swift`（加纯 helper + make 注入）
- Modify: `ios/Contracts/Sources/KlineTrainerContracts/Render/KLineView+Crosshair.swift`（传 frames + 画卡）
- Test: `ios/Contracts/Tests/KlineTrainerContractsTests/Render/RenderStateBuilderTests.swift`（前收 helper host 测）

**Interfaces:**
- Consumes: Task 1 `CrosshairSidebarContent`、Task 2 `resolve(...,frames:)`；`renderState.frames`、`renderState.visibleCandles`、`renderState.panel.period`、`renderState.previousCloseBeforeVisible`、`currentPalette`。
- Produces: `RenderStateBuilder.previousCloseBeforeVisible(candles:startIndex:) -> Double?`（纯 helper，host 测）；`KLineRenderState.previousCloseBeforeVisible: Double?`（make 注入，`.empty`=nil）；绘制贯穿整 panel 十字光标 + 悬浮信息卡（颜色：candleUp/candleDown/白/黄）。

> 纯 helper（前收）host 测；KLineView 绘制 UIKit-only，验证 = Catalyst build + Task 7 人工验收。

- [ ] **Step 1: 写「可见首根前收」失败测试**

```swift
// 追加到 RenderStateBuilderTests.swift（@testable import 已有）
...
Run: `cd ios/Contracts && swift test --filter PrevCloseBeforeVisibleTests`
Expected: 编译失败 `type 'RenderStateBuilder' has no member 'previousCloseBeforeVisible'`。

- [ ] **Step 3: 实现 helper + KLineRenderState 字段 + make 注入**

3a. `RenderStateBuilder.swift` 加纯 helper（`make` 上方）：
...
3b. `KLineRenderState.swift` 加字段（`crosshairPoint` 后；init 参数尾部加 `previousCloseBeforeVisible: Double? = nil`**带默认值**减少 call-site churn，并赋值；`.empty` 靠默认或显式 nil）：
...
3c. `RenderStateBuilder.make` 的 `return KLineRenderState(...)` 在 `crosshairPoint: crosshair` 后加：
...
> 注：`candles` = `engine.allCandles[period]`（完整数组），`viewport.startIndex` 滚动后可 >0 → 该值为切片外真实前收。纯 additive 渲染态、零引擎改动、不 bump CONTRACT_VERSION。
...
- [ ] **Step 5: drawCrosshair 传 frames + 装配并画信息卡**
...
- [ ] **Step 7: Catalyst build 验证**

Run: `xcodebuild build-for-testing -scheme KlineTrainer -destination 'platform=macOS,variant=Mac Catalyst' -derivedDataPath /tmp/rfcc-ddt 2>&1 | tail -5`
Expected: `** TEST BUILD SUCCEEDED **`。

- [ ] **Step 8: 跑全量 host 测试核绿（两框架）**
...
```bash
git add ios/Contracts/Sources/KlineTrainerContracts/Render/KLineRenderState.swift ios/Contracts/Sources/KlineTrainerContracts/Render/RenderStateBuilder.swift ios/Contracts/Sources/KlineTrainerContracts/Render/KLineView+Crosshair.swift ios/Contracts/Tests/KlineTrainerContractsTests/Render/RenderStateBuilderTests.swift
git commit -m "feat(rfc-c): 可见首根真实前收(renderState) + KLineView 贯穿光标 + 悬浮信息卡"
```
...
| 16 | 先向右滚动(使有更早历史在屏外)再长按**最左可见**那根 K 线 | 涨跌/涨跌幅显**真实值**(非「—」)、收/光标按真实前收上色 | 最左根显真实涨跌=pass；显「—」/白=fail |
| 17 | 十字光标**黏滞显示时**点画线浮动钮(✎)进入画线模式 | 光标消失、图恢复、之后点图落水平线锚点(非退光标) | 进画线退光标且点图落锚=pass；点图退光标/落锚失败=fail |
| 18 | 小幅横拖一点点再按住进光标→松手→再正常拖图 | 图能正常平移(不卡死)；无残留 pan 状态 | 后续平移正常=pass；卡死/offset 异常=fail |
```
...
**R5（real Codex，verdict=needs-attention，2 medium、已修；R4 verified gone）：**
- **M1 进入光标吞 pan 终止 → engine pan 悬空**（Task 4）：长按与 pan 同时识别，小幅 pan 可在长按前 active（`onPan(.began)`→`engine.beginPan`）；`crosshairMode=true` 后 `handleSinglePan` 早返吞掉 pan `.ended/.cancelled` → engine 收不到 `endPan` → pan/deceleration 状态悬空。→ **修**：`crosshairMode` 改 `didSet`，false→true 转换调既有 `supersedeSinglePanForMultitouch(in: attachedView)`（发残量 + `.cancelled` 给 onPan→engine.cancelPan，无 active pan 时 no-op）。+验收 #18。
- **M2 画线 × 光标互斥只单向**（Task 5）：只在长按 .began 查 `!isDrawing`；光标黏滞时点画线钮 → `arbiter.drawingMode=true` 但 `crosshairMode` 仍 true、优先级更高 → tap 退光标而非落锚、pan/pinch 仍抑制 → 画线工具看着坏。→ **修**：`Coordinator.sync` 在 `arbiter.drawingMode = drawing` 前加 `if drawing && crosshairActive { exitCrosshair() }`（进画线先退光标）。+验收 #17。spec §4.1 标双向互斥。

**R6（real Codex，verdict=needs-attention，1 medium、已修；R5 verified gone）：**
- **M Task 2 nil-frames 契约自相矛盾**：R4 把价标改全局移左缘 + 要求改既有右缘断言，但 Task 2 Interfaces 仍写「nil → 既有断言不变、价签不变」→ 实现者照契约会留错误右缘价标、照 snippet 会破「既有断言不变」。→ **修**：重写 Task 2 Interfaces 契约——**`frames` 仅控竖线/时签 extent**；**价标全局移左缘（与 frames 无关）**；删「既有断言不变」、明示既有右缘断言必改左缘。File-structure note + Step 4 Expected + spec §4.0 同步。纯文档一致性修正（R4 引入、本轮收口）。
