...
| ③ Instruments 帧预算（顺位 12） | `docs/runbooks/2026-06-14-wave3-pr12-frame-budget.md` | Instruments Time Profiler 录制各交互**单帧 `RenderStateBuilder.make` + `KLineView.draw(_:)` 合并峰值 < 4ms**（`____` ms 占位待回填）+ Equatable 短路验证。**测量精度限制见下方 caveat** | ☐ |

> **帧预算 device 测量精度 — 已知限制（codex review R8，accept residual + override；根治超 13c doc-only scope）**：
> - **R8-H1 采样 ≠ 帧相关**：Time Profiler 是**采样器**，分别过滤 `make` / `draw` 取峰值相加**不等于**同一显示帧的真实合并耗时——`make`（`ChartContainerView.updateUIView`）调度 draw 延后、且屏上有**上/下两个图表实例**各自 make/draw，一帧可含多个未配对调用。**严谨**的单帧合并测量需 `os_signpost` 把每个上/下 make/draw 关联到 display frame、取最坏完整帧——属**生产代码 instrumentation**（顺位 12 #104 代码），超 13c doc-only scope → **accept residual**（fast-follow 性能 instrumentation PR）。当前 runbook 的合并峰值 = **指示性上界**（保守），<4ms 有裕量时可作初判，临界值（接近 4ms）须以 signpost 实测复核。
> - **R8-H2 fixture 欠载**：§C seed 各周期蜡烛数按 span 聚合（`.m60`=12 / `.daily`=6，= fresh-session 默认面），**少于** `RenderStateBuilder` 目标 **80-visible-candle** 渲染负载 → 经 §C fixture 的帧预算测的是**欠载图表**，<4ms 不代表满载达标。**满载**测量需各 profiled 周期 ≥80 蜡烛的 perf fixture（13b `DebugFixtureData` 代码增强），超 13c doc-only scope → **accept residual**（fast-follow perf-fixture PR）。**回填帧预算时须同时记录所测周期 + 实际渲染蜡烛数**；蜡烛数 < 80 的测量标「欠载（非满载达标）」，不得作满载 PASS。
> **【RESOLVED 2026-06-15，Wave 3 13c-R2 fast-follow】** perf-fixture 代码增强已 ship：`DebugFixtureData.fullLoadM3Count`(=9600) 经 `AppContainer.seedDebugFixtures` 使用 → §C seed 各周期 ≥ `defaultVisibleCount`(80)、make 默认面板 `.m60`(=480)/`.daily`(=240) ≥ `maxVisibleCount`(240)。**经 §C seed 剖析默认面板现为满载（非欠载）。** 回填仍须记录「所测周期 + 实际渲染蜡烛数」；蜡烛数 ≥80 即**视为满载测量（非欠载）**——但是否 <4ms PASS 仍须按上方 **R8-H1 caveat**（采样≠帧相关，OPEN）复核，满载测量本身**不等于** 4ms 达标。仅当人为切到行数 <80 的极端构造（非 §C 默认）才标欠载。
