...
> ⭐ **交付前 codex 评审又补了一条残留**（控制者在 approve 之后自查发现）：
> **被注释掉的语句也会被当成真语句计数**。例如把一条写全了的插入语句整个注释掉，
> 这条检查仍会把它算作「这里有一条合规语句」。
> ⇒ 后果是那个「每类文件至少要有一条」的锚点**可能被注释里的东西凭空喂饱**（锚点被削弱）。
> ⛔ **不修**，理由已实测：要判断「这句话是不是在注释里」，就得对**整份文件**按 SQL 注释规则跟踪，
> 而作用域里的 `.py` / `.yml` / `.md` **根本不是 SQL**（仓内实测有 14 个文件含没配对的 `/*`）——
> 那条路会把真语句整片吞掉，比现在这个残留糟得多。
> ✅ **方向是安全的**：已实测「被注释掉的合规语句 + 紧跟一条真漏写」⇒ **真漏写照样被抓到**，不会被掩盖。
