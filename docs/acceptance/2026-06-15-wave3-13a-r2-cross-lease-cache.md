...
| 4 | 在测试输出中找到 `retry_crossLease_doesNotDeleteNewerLeaseFile` | 该测试通过；含义：旧 lease 孤儿被拒后，新 lease 占有的 id=42 文件**保留**（`cache.listAvailable` 仍含 id=42；`cache.deletedFilenames` 为空） | □ Pass / □ Fail |
