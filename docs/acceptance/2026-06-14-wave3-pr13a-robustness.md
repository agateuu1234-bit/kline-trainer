...
| **13a-R3：touch-on-use vs 并发 store 的 TOCTOU**——session 打开 reader 后到 `cache.touch` 之间，并发下载 `cache.store` 可 evict/replace 同 id 项 → touch（按 id/filename 解析路径）no-op 或 touch 替换内容。本 PR touch 是 best-effort（no-throws），raced 结果仅 LRU mtime 轻微失准，**非数据丢失**；「resume 引用缺失训练组」是 **pre-existing** eviction-during-session race（与 touch 无关，baseline 既有）。codex 要求的 cache pinning/acquisition API = P5 CacheManager 契约新增 | codex 13a review R7（drilling 我的 §A touch；归因 pre-existing race + 要 P5 新 API）| **OUT of 13a scope**（touch 是良性 best-effort 添加，不恶化既有 race；pinning API 是 P5 契约变更）→ 独立 **P5-cache-pinning RFC**（与活跃 session 的缓存所有权一并设计） |

## codex review 收敛说明（accept + override）

13a **in-scope**（cache touch-on-use + 边界错误 toast）concerns 已在 codex review **R1/R2 收敛**（autosave 重复可见性 generation + 后台失败前台重放 + touch 排序），此后未被再 flag。**R3–R7** 把审查拖入 P2 DownloadAcceptance confirm 状态机 + P5 cache 并发 reliability 下钻——**R4↔R5 自相矛盾**（terminal confirm 失败 reject vs retain）、**R7 循环重提**已 revert 的 R3 + 把 pre-existing race 归因本 PR。依 `feedback_codex_distributed_reliability_drilldown`（P6 先例 8 轮→accept+override）+ `feedback_codex_round6_self_contradiction`（自相矛盾=permanent-bias→override）+ `feedback_big_pr_codex_noncovergence`（>5 轮 escalate）：**7 轮 escalate，剩余 finding 全为 documented residual（13a-R1/R2/R3，OUT of scope，归独立 P2/P5 RFC）→ accept residual + user TTY attest-override + admin merge**（沿用 Wave 0-2 多锚先例）。整体 opus 4.8 xhigh review = APPROVE（0 C/H）。
