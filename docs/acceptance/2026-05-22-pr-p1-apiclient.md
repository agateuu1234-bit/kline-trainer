...
**branch-diff R1**：2 high（terminal URLError + 非预期 4xx 误报 recoverable）→ 全修（TLS/auth/cert + 400/401/403/422 → terminal internalError）（400/403 有显式 test；401/422 等其余非预期 4xx 由同一 failUnexpectedStatus 分类逻辑覆盖）
...
| B2 | 终端运行：`cd ios/Contracts && swift test --filter APIErrorMappingTests 2>&1 \| tail -5` | 含 `Executed 7 tests, with 0 failures` | □ Pass / □ Fail |

---

### C Gate 1 错误翻译证据映射表

> 本节为证据映射参考表（无需执行操作）；可执行核验见 B1/B2 与 D1/D2。

每行对应：注入场景 → 测试函数 → 期望的 AppError。C1–C4 覆盖失败路径用例（DefaultAPIClientTests 失败路径 + APIErrorMappingTests 的 terminal URLError 组）；happy-path 与其余 mapping 用例见 B1/B2/F1/F2/G1/H1，全量回归见 I2b。
...
| HTTP 500 | statusCode=500 | `test_reserve_http_500_throws_serverError` | `.network(.serverError(code: 500))`（5xx → recoverable，可重试） |
...
| 无网络（URLError.notConnectedToInternet，属连通性 URLError） | error=URLError(.notConnectedToInternet) | `test_reserve_offline_throws_network_offline` | `.network(.offline)`（仅瞬断连通性代码 → recoverable，可重试） |
...
| HTTP 500 | statusCode=500 | `test_download_500_throws_serverError` | `.network(.serverError(code: 500))`（5xx → recoverable，可重试） |
| 网络断开（URLError.networkConnectionLost，属连通性 URLError） | error=URLError(.networkConnectionLost) | `test_download_offline_throws_network_offline` | `.network(.offline)`（仅瞬断连通性代码 → recoverable，可重试） |
...
| HTTP 500 | statusCode=500 | `test_confirm_500_throws_serverError` | `.network(.serverError(code: 500))`（5xx → recoverable，可重试） |
...
| I2b | 终端运行：`cd ios/Contracts && swift test 2>&1 \| grep -c "with 0 failures"` | 输出 ≥ 2（至少 DefaultAPIClientTests 与 APIErrorMappingTests 两套件均报 0 failures） | □ Pass / □ Fail |
