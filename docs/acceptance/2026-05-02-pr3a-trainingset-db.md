...
> - **第 6 步**：跑全仓 `swift test`，应等于第 2-5 步覆盖的 28 项 + 历史 KlineTrainerContractsTests 的并集。
...
| 3 | 执行 `swift test --filter DefaultTrainingSetDBFactoryTests` | 9 tests 全 pass（含 metaNullInRequiredColumn / metaWrongTypeInColumn / metaSanityRangeFailure 三个 corrupt meta 防腐 tests） | 末行无 `failed` 字串 |
| 4 | 执行 `swift test --filter DefaultTrainingSetReaderTests` | 15 tests 全 pass（含 wrongType + nullInRequiredColumn + duplicate / nonStrictlyIncreasing endGlobalIndex + m3GlobalIndexNil / Mismatch / higherPeriodOutOfRange + klinesWrongTypeNumeric + m3MissingButHigherPresent + emptyKlinesReturnsEmptyDict 共 10 项 boundary tests） | 末行无 `failed` 字串 |
| 5 | 执行 `swift test --filter HappyPathIntegrationTests` | 1 test pass | 末行无 `failed` 字串 |
| 6 | 执行 `swift test` 跑全仓 | 全部 tests pass（含 KlineTrainerContractsTests 历史项 + 28 个新 Persistence tests） | 末行 `Test Suite 'All tests' passed` |
...
- 若第 1 步 `swift build` 因 GRDB 拉包失败：检查网络；GRDB 6.29.x 包大小 ~3MB 需要短暂等待
