...
> - **第 6 步**：跑全仓 `swift test`，应等于第 2-5 步覆盖的 17 项 + 历史 KlineTrainerContractsTests 的并集。
...
| 4 | 执行 `swift test --filter DefaultTrainingSetReaderTests` | 7 tests 全 pass（含 2 个 corrupt fixture tests：wrongType + nullInRequiredColumn） | 末行无 `failed` 字串 |
| 5 | 执行 `swift test --filter HappyPathIntegrationTests` | 1 test pass | 末行无 `failed` 字串 |
| 6 | 执行 `swift test` 跑全仓 | 全部 tests pass（含 KlineTrainerContractsTests 历史项 + 17 个新 Persistence tests） | 末行 `Test Suite 'All tests' passed` |
