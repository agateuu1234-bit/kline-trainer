...
- **B4-R5（advisory lock conn-scoped failover 极端 defer）**：进程级 lock 随 lock_conn 释放——正常运行已覆盖单例；仅 DB failover/网络断使 lock_conn 掉线而 Python 进程仍存活的极端窗口，第二 scheduler 可能拿锁并发（over-generate 几个训练组，非数据损坏）。codex R7-F2 / branch-diff F1；user explicit 接受残留（2026-05-30，超 3 轮 escalate）。严肃多实例化时改 per-sweep lock + conn-loss 检测。
- **B4-R6（sweep 异常边界已实现，near-term retry defer）**：`_job` 已用 try/except 包住整轮 sweep——瞬时故障（rollback/count/generate 抛异常）记 `logger.exception` 不逃逸、不崩调度器（codex branch-diff F2）。「当天近期自动重排」（near-term reschedule/retry）defer——codex branch-diff R2-medium → R3-high 升级坚持，user explicit 接受残留（2026-05-31，超 3 轮 escalate）。理由：B4 为夜间维护、有 degraded warning 可观测、可靠性加固（same-day retry/backoff）与 B4-R4 部署常驻一并归后续部署 PR。
