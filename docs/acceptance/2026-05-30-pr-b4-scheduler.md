...
- **B4-R6（sweep 异常边界已实现，near-term retry defer）**：`_job` 已用 try/except 包住整轮 sweep——瞬时故障（rollback/count/generate 抛异常）记 `logger.exception` 不逃逸、不崩调度器（codex branch-diff F2）。「当天近期自动重排」（near-term reschedule）非必要 defer——misfire_grace_time=3600 + 次日 cron 已覆盖大多数；瞬时故障当天恢复留待运维或后续部署 PR。
