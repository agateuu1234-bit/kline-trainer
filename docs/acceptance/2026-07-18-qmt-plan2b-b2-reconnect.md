...
| # | 动作 | 预期 | 通过/不通过 |
|---|---|---|---|
| 9 | 执行下面这条（一整条复制）：<br>`if grep -rnE "B4 ?补货已(恢复\|打通)\|库存已可生成\|训练组已能产出\|已恢复出货" docs/acceptance/2026-07-18-qmt-plan2b-b2-reconnect.md; then echo "FAIL: 出现被禁止的过度宣称表述"; exit 1; fi` | **无任何输出**（不打印 FAIL） | ☐ |
| 10 | 通读 PR 说明，确认没有出现「B4 补货已恢复」「库存已可生成」「训练组已能产出」这类说法 | 确实没有；PR 说明里对产出的描述是「代码通路打通」而非「已能出货」 | ☐ |
