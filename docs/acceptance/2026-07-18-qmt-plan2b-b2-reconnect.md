...
| # | 动作 | 预期 | 通过/不通过 |
|---|---|---|---|
| 1 | 在仓库根执行：<br>`cd backend && ../.venv/bin/python -m pytest tests/ -q` | 最后一行是 `243 passed`。**必须是 0 failed、0 skipped**（没有 failed/skipped 字样即可） | ☐ |
| 2 | 单独跑本 PR 新增的集成测：<br>`cd backend && ../.venv/bin/python -m pytest tests/test_b2_reconnect_integration.py -q` | `32 passed`，无 failed / 无 skipped | ☐ |
