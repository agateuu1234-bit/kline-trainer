...
### M11 —— ⭐ 攻【守恒等式本身】：五种腐化 + 双点编辑（交付前收口自查）

**背景**：守恒等式是整支最终评审「重要 1」的修法。⛔ 新判据必须自己也被攻一遍，否则就是「修了一个洞、开了一个新洞」。

**改了什么**：五种单点腐化，每种都**叠加一处真漏写**（`rehearse.sh` 第 2 条语句删掉 `schema_version`）：

| 腐化 | 结果 |
|---|---|
| 两个来源合并（`expected_hits` 也改用 `finditer`） | **1 failed** |
| `bucketed += 1` 挪到「提及」的 `continue` **之后** | **1 failed** |
| 短路 `if bucketed != expected_hits:` | **1 failed** |
| `_scan` 返回前把 `lost` 清空 | **1 failed** |
| 短路 `_enforce` 里的 `if lost:` | **1 failed** |

⇒ **腐化守恒等式本身，不造成静默放行** —— 真漏写仍被主判据（`missing`）抓住。

**再往下量一层（几处编辑才能静默）**：

```
① 只截断（1 处编辑）                 ⇒ 2 failed   （守恒等式 + 合成自测 各响一次）
② 只短路守恒等式（1 处编辑）          ⇒ 1 failed   （主判据仍响）
③ 截断 + 短路守恒等式（【2 处】编辑）  ⇒ 1 failed   ← 仍然红
```

**③ 为什么还是红**：合成自测里那个 **`two.sql`（同一文件两条语句，第 2 条是真漏写）** 用例**独立**接住了截断 —— 它不依赖守恒等式。⇒ 这两道防线**各自独立**，不是同一判据的回声。

⚠️ **一次差点写错的自检**：第一版脚本断言「截断后那串字应出现 1 次」，实跑是 **2 次** ⇒ 报「变异空转」中止。查清是**我自己写在注释里的举例**也含那串字 —— **「含自身的计数」这个坑本片第三次出现**。改成**先量基线、再断言基线 +1** 之后通过。

---

### M12 —— ⭐ 问「四种引号是不是真的枚举全了」：拿**真 SQLite 引擎**当裁判

**背景**：守卫②的前提检查枚举了四种带引号的词法形态。⛔ 「枚举全了」这句话本身是本片连错五次的那类断言，**必须实测**，不能靠记忆。

**跑了什么**：`sqlite3`（Python 内置，真引擎）逐条 `CREATE TABLE` 试。
...
`…` 反引号（标识符）                SQLite ✅ 接受
[…] 方括号（标识符）                SQLite ✅ 接受
$$…$$ 美元引用（PG 的写法）          SQLite ❌ 拒绝：OperationalError: near "$$x": syntax error
```

**结论**：SQLite **恰好四种**，前提检查的枚举**完整**。`$$` 是 PostgreSQL 的美元引用，SQLite **语法层直接拒绝** ⇒ 不构成第五种形态（而且拒绝是**响亮**的，不会静默）。
⭐ 这两份 DDL 是**训练组 `.db` 的 SQLite 建表语句**，所以判据应当按 **SQLite** 的词法枚举，而不是 PostgreSQL。

---

### M13 —— 「次要 3」的改动有没有副作用（三条合法「提及」是否仍判对）

**背景**：为堵「空串恒真」加了 `before and after` 判空。⛔ 收紧判据可能把本该判「提及」的误判成别的。

**跑了什么**：直接 import 守卫模块，对**真实树**重新分类计数。

**原样输出**：

```
分类计数 = {'cols': 12, 'mention': 3}
三条合法「提及」落在：
  backend/tests/test_b2_reconnect_integration.py:208
  backend/tests/test_b2_reconnect_integration.py:699
  backend/tests/test_deployment_source_texts.py:17
```

**结论**：`cols` 仍 12、`mention` 仍 3，**逐条落在与改动前完全相同的三处** ⇒ 无副作用。
...
### M16 / M17 —— ⭐⭐⭐ **嵌套表头注释里藏一个假列清单**（codex attest 第 2 轮）

**来历**：M14/M15 的修法（让正则的「词元间隔」允许注释）**不认嵌套**，被 codex **下一轮**当场打穿。
⭐ 两轮下来，codex **每一轮都挖出真东西，而且第 2 轮挖的正是第 1 轮修法的破绽** —— 这是「换通道」价值最集中的一次体现。

**洞在哪**：

```
INSERT /* outer /* inner */ INTO training_sets (schema_version) */ INTO training_sets (stock_code, file_path) VALUES (1,2)
```
- **PostgreSQL**（`pglast` 实测）：外层注释**整段**被吃掉 ⇒ 真实列清单 `['stock_code','file_path']` ⇒ **真漏写**
- **上一版守卫**：非嵌套的 `/\*.*?\*/` 在**内层** `*/` 收手 ⇒ 把**被注释掉的** `(schema_version)` 当成真列清单读 ⇒ **判「含」⇒ 静默放行**

⭐ **根因是本仓成文教训「订正只落在两份等价副本中的一份」**：我**已经有**嵌套感知的 `_mask_sql_comments`，却在表头另写了一个不认嵌套的正则。

**⛔ 第一版修法被【自己的实测】打回**（值得单独记）：本想「把整份文件按 SQL 注释规则屏蔽掉再匹配」。一跑，守卫在真实树上**变红**并列出 **14 个文件**含没有配对 `*/` 的 `/*` —— Python 的正则字面量、YAML、markdown 正文。
⇒ **那些文件根本不是 SQL，`/*` 在它们里只是普通字符。** 整份屏蔽会把其后内容整片吞掉、真语句跟着消失 ⇒ **修一个洞、开一个更大的洞**。

**最终修法**：只从**每个 `INSERT` 关键字往后走一小段**，手写扫描器逐字符消注释（块注释**嵌套感知**），其余文本一个字都不碰；**未闭合的块注释单独报成一类问题**（响亮，不无声吞掉）。

⚠️ **过程中又自己打回一次**：表名取词写成 `"?name"?`，会把**属于 Python 字符串的**那个引号一起吃掉 ⇒ 合法的「提及」（`if "INSERT INTO training_sets" in query:`）落进【认不出的形状】= **误报**。改成**引号必须成对才吃**。

**拿 `pglast` 当裁判的 9 种表头注释形态差分**（全部一致）：

```
用例                      PG 真相     守卫        判定
对照 正常                   False     False     ✅
INSERT /* */ INTO         False     False     ✅
INSERT INTO /* */ 表名     True      True      ✅
INSERT --x⏎INTO           False     False     ✅
⭐codex 的嵌套构造          False     False     ✅
嵌套 3 层                  False     False     ✅
嵌套且合规                  True      True      ✅
注释夹 schema 前缀          False     False     ✅
引号表名 + 表头注释          False     False     ✅
与 PG 不一致 = 0
```

其中 **5 条已进常驻差分测试**。**已证明能报非 0**：把扫描器的嵌套计数退回「遇到第一个 `*/` 就收手」⇒ 当场 `1 failed`。

**端到端**（把 codex 那条构造注入 `rehearse.sh`）：

```
变异落盘 = True
⇒ 1 failed, 2 passed   类别=列清单里没有 `schema_version`
```

**M16（嵌套注释藏假列清单）⇒ 红；M17（嵌套注释但合规）⇒ 绿**。
**全量回归 29 组变异（24 红 / 5 绿），不符期望 0 组。**

---

### M18 / M19 —— ⭐ 给「未闭合块注释」这条**零命中分支**单独证伪（控制者自查补加）

**为什么必须有**：换成手写扫描器之后新增了一条判据 ——「在表头里撞到**未闭合**的块注释就报」。
它今天在真实树上 **零命中**（因为扫描器只从 `INSERT` 往后走一小段，不碰其余文本）。
⛔ 按本片自己的规矩：**零命中的分支必须单独证伪，否则它可能是恒真的、白写**。
⚠️ 这一组是**控制者自查补的** —— codex 前两轮都没提它，⭐ 但「它没提」不等于「不需要」
（本片成文教训：关于覆盖面的断言，一律要自己实测）。

**M18｜在表头里塞一个没闭合的块注释**

```
INSERT /* 没闭合的注释 INTO training_sets
  (stock_code, stock_name, …, schema_version, file_path, content_hash, …)
```
（⚠️ 注意：这条语句**本身是合规的**——`schema_version` 明明在列清单里。它要验的不是「漏没漏写」，
而是「**注释没闭合导致整条语句隐形**」这件事会不会被报出来。）

```
变异落盘 = True
⇒ 1 failed, 2 passed   类别=【有未闭合的块注释】
```

**M19｜把这条分支短路掉，证明是它在承重**

```
把 `if _unterminated:` 改成 `if False:`，再施加同一条 M18 ⇒ 3 passed
```

**结论**：短路之后**全绿** ⇒ 表头里的未闭合注释会让整条语句**隐形、且无人报告**。
⇒ **这条分支确实在承重**，不是装饰，也不是被别的判据顺手接住的。

⭐ 顺带说明它为什么值得存在：手写扫描器**只消表头那一小段**的注释，正是为了避开
「整份文件屏蔽」那条死路（见 M16/M17）。代价是——扫描器遇到未闭合注释时会**放弃这个候选**。
若不报出来，那就成了一条**新的静默通道**。M18/M19 把这个代价钉死在「响亮」那一侧。

---

### M20 —— ⭐ 攻【手写扫描器】独有的出错面（codex 第 4 轮 approve **之后**，控制者自查）

**为什么 approve 之后还要攻**：前三轮的规律是 —— **codex 每轮只报一种构造，同类还有几种是控制者扩查出来的**（第 1 轮报 `INSERT /* */ INTO`，扩出另两种；第 2 轮报嵌套构造，扩到 3 层嵌套与 schema 前缀）。
⇒ approve 只证明「**它这次没再找到**」，⛔ 不等于「这一类已穷尽」。本片成文教训：关于覆盖面的断言一律自己实测。

**拿 `pglast` 当裁判的 10 组边界**（全部由控制者构造，codex 一条都没提）：

| 用例 | PG 真相 | 守卫 | 判定 |
|---|---|---|---|
| 表名相似 `training_sets_old` | 另一张表 | 零命中 | ✅ 不误伤 |
| 引号相似名 `"training_sets_x"` | 另一张表 | 零命中 | ✅ |
| 别的表 / `public.别的表` | 另一张表 | 零命中 | ✅ |
| `myschema.training_sets` 缺字段 | 是本表、缺 | 判「缺」 | ✅ |
| 行注释在文件末尾**无换行** | PG 拒绝 | 零命中 | ✅ 安全 |
| 块注释**恰在末尾**（未闭合） | PG 拒绝 | 零命中 + **报未闭合** | ✅ 响亮 |
| `INSERT INTO"training_sets"`（无空格） | 是本表、缺 | 判「缺」 | ✅ |
| 两条相邻、前一条不是本表 | 第二条是本表、缺 | 判「缺」 | ✅ |
| **被注释掉的合规语句 + 紧跟真漏写** | 只有 1 条真语句、**缺** | **判「缺」** | ✅ **无静默放行** |

**⚠️ 最后一条查出一处残留**（控制者复核时才看清 —— 第一次探针只看了**第一个**表头，差点误判成缺陷）：

```
/* INSERT INTO training_sets (schema_version) */ INSERT INTO training_sets (stock_code, file_path) VALUES (1,2);
```
- PG 眼里**只有 1 条**语句（前一条整段在注释里），列清单 `[stock_code, file_path]` ⇒ **真漏写**
- 守卫找到 **2 个**表头：被注释掉那条判「含」、真那条判「缺」
- 完整 `_scan` ⇒ `checked=2`、`missing` **非空** ⇒ **真漏写被抓到，没有静默放行** ✅

⇒ **残留**：被注释掉的**合规**语句会被当成真语句**计数**，因而**可能凭空喂饱「格子锚点」**
（例如某个 `(目录, 后缀)` 格子里只剩被注释掉的语句，锚点却显示满足）。
⛔ **不修**：要判断「这个 `INSERT` 关键字是不是在注释里」就得做**整份文件**的注释跟踪，
而那条路已被实测证伪（见 M16/M17：作用域里的 `.py`/`.yml`/`.md` **根本不是 SQL**，
仓内 14 个文件含没配对的 `/*`）。⇒ 方向是**锚点被噪声满足**（弱化），**不是**放过真漏写。
