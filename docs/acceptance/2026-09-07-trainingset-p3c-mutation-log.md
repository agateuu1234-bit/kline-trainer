...
所以，每写完一条新检查，光看它「现在是绿的」不够，还要反过来问一句：**如果我故意把它该抓的那个问题重新制造一次，它会不会真的报错？** 这份文档记的就是这件事——对这一片新增的每一条检查，逐一「故意改坏一处、看它红不红」，一共做了 **12 组**实测。
...
## 编号为什么是 M1–M4、M4b、M5–M9、M10–M11（不是连续的 M1–M12）

这份记录一共 12 组，但编号里有一个 **M4b**，不是顺着数下去的 M5。原因如下：
...
5. **M10、M11** 是**整支最终评审 I1（本片最严重的那条缺陷）的修复波**新增的两组——I1 的裁定要求给新补的两条检查（NAS 副本标注 / P4 前置条件）各配一组能证伪的变异，实测跑完后顺着 M9 之后接着编号，不需要再绕开已写死的号。

**这段由来必须写清楚**，否则读者看到编号从 M4 跳到 M4b 再到 M5，会以为是记录漏了一组，或者编号弄乱了——实际上 12 组一组不少，只是编号的来历比较绕。
...
| M10 | 删掉 NAS handoff 那份副本的「历史记述，非判据」标注 | `final-fix-report.md`「I1」节 | 实施子代理 |
| **M11** | 删掉 `:138` 里的 **P4 前置条件**段落（整支最终评审 I1 的核心证据） | `final-fix-report.md`「I1」节 | 实施子代理 |
...
### M10 —— 删掉 `:136` 的「历史记述，非判据」标注（整支最终评审 I1 修复波新增）

**改了什么**：删掉 NAS handoff 那三个 zip 那一行（`:136`）末尾「（⚠️ **历史记述，非判据** —— 这三个指纹与上一行是同一批 v1 包；2026-09 切片一起它们同样已作废为 v1 审计归档，见本表下方那行订正）」这句说明，其余不动。

**跑了什么**：`cp` 备份 → 删掉该标注 → 清一次字节码缓存 → `cd backend && $PY -m pytest tests/test_deployment_source_texts.py -v` → `cp` 还原 → `diff` 核对 → `git status --short`。

**原样贴出的观测输出**：

```
tests/test_deployment_source_texts.py::test_p7_source_wording_no_longer_offers_the_mac_copy PASSED
tests/test_deployment_source_texts.py::test_rewritten_p7_source_rule_is_present PASSED
tests/test_deployment_source_texts.py::test_mac_copy_archive_note_is_present PASSED
tests/test_deployment_source_texts.py::test_nas_handoff_copy_archive_note_is_present FAILED
tests/test_deployment_source_texts.py::test_p7_source_rule_carries_the_p4_precondition PASSED

E       AssertionError: NAS handoff 那行应带「历史记述，非判据」标注且恰好 1 行，实测 0 行 —— 它与 Mac 那份是同一批 v1 包，少了标注就会有人拿那三个 v1 指纹当判据
1 failed, 4 passed in 0.02s
```

还原后 `diff` 为空，`git status --short` 无本次变异残留。

**结论**：**1 failed, 4 passed**——红的是 `test_nas_handoff_copy_archive_note_is_present`，其余四条绿。这证明**NAS 那份副本的标注也被这条检查钉住了**：原先只有 Mac 那份（`:135`）被钉，而 `:135` 与 `:136` 的三个 CRC32 逐字相同、本是同一批第 1 代包的两份副本——如果只钉住 Mac 那份，NAS 那份的标注被悄悄删掉时不会被任何检查发现。

---

### M11 —— 删掉 `:138` 里的 P4 前置条件段落（整支最终评审 I1 修复波新增）

**改了什么**：把 `:138` 里「⛔ **但该 handoff 目录里当前存放的同样是 v1 包**」到「**不得作为第 2 代产物的来源**」这一整段前置条件删掉，其余原样保留——禁令用词「只能」「v1 审计归档」「不得再 scp 到部署目录」都不动。

**跑了什么**：同 M10（`cp` 备份 → 删段落 → 清一次字节码缓存 → `pytest -v` → `cp` 还原 → `diff` 核对 → `git status --short`）。
...
tests/test_deployment_source_texts.py::test_rewritten_p7_source_rule_is_present PASSED
tests/test_deployment_source_texts.py::test_mac_copy_archive_note_is_present PASSED
tests/test_deployment_source_texts.py::test_nas_handoff_copy_archive_note_is_present PASSED
tests/test_deployment_source_texts.py::test_p7_source_rule_carries_the_p4_precondition FAILED

E       AssertionError: 「P7 只能从 handoff 取」这条规则应带 P4 前置条件且恰好 1 行，实测 0 行 —— 0 行 = 规则又变回无条件，会把操作者指向同样是 v1 的那份副本
1 failed, 4 passed in 0.02s
```

还原后 `diff` 为空，`git status --short` 无本次变异残留。

**结论**：**1 failed, 4 passed**——红的是 `test_p7_source_rule_carries_the_p4_precondition`，其余四条仍绿。这是**整支最终评审 I1 的核心证据**：规则一旦退回无条件版（「只能从 handoff 取」不带任何前置条件），这条检查会当场抓到——**无条件版正是最终评审判定为缺陷的那一版**：规则给出的理由（v1 包与库里新指纹对不上）逐字适用于它自己指定的替代来源（NAS handoff 目录本身也是同一批 v1 包），P4 重建完成之前照无条件规则做会产生同一个故障，而且这次还带着权威背书。
...
- **M11** —— **整支最终评审 I1 的核心证据**：把 `:138` 里那段 P4 前置条件删掉、其余不动，`test_p7_source_rule_carries_the_p4_precondition` 当场报错，其余四条仍绿——证明「规则一旦退回无条件版就会被抓到」。无条件版正是最终评审判定为缺陷的那一版：它给出的理由逐字适用于它自己指定的替代来源，不带这条检查，P4 之后照无条件规则做会产生同一故障，而且这次带着权威背书。
