...
| 4 | 看三节 residual 表（8 行） | A/B/C/D = CLOSED（引 13a #108 / 13b #109）；运行时矩阵 = PARTIAL；**W3-11-R1**（bounce live 接线）= OPEN 且标 **Wave 3 功能完成门 + 关闭前提**（codex review High：承诺交互未上线 → 功能完整性 PENDING-W3-11-R1，区别于 PR11-R1/W1-R2 的 NAS ship 门）；PR11-R1 / W1-R2 = OPEN；人读表与机器块 WAVE3-STATUS（含 `feature-completeness: PENDING-W3-11-R1-bounce-live-wiring`）事实一致 | □ Pass / □ Fail |
