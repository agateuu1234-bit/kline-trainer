...
v1.0 → v1.4 累计 4 轮 codex 对抗性评审 + 56 项修订（统计口径见 `kline_trainer_modules_v1.4.md` 文件头；§15.3 本段仅在 v1.2 时文字化过两轮 / 37 项的旧状态）。后续评审按下列三段触发，不再做 spec 全量评审。
...
| Review 维度 | 性能目标（`kline_trainer_plan_v1.5.md` §一 bitmap cache 触发阈值 4ms）对照；Core Graphics 调用次数 / bitmap cache 引入时机 / CADisplayLink 回调重载 |
