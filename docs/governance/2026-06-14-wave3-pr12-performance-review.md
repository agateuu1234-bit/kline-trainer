...
`KLineView.draw(_:)`
- 位置：`ios/Contracts/Sources/KlineTrainerContracts/Render/KLineView.swift:36-63`（draw 派发块；`drawDrawings` 经顺位 4 #103 注册 `HorizontalLineTool`，`tools: Self.drawingTools`，`:58-59`）
...
|---|---|---|
| `drawCandles` | `:13-27` | per-candle：1 `strokePath`（影线）+ 1 `fill`（实体）+ `setFill`/`setStroke` 颜色态 × 80 → ~80×(2 path + 2 color) ≈ **320 ops** |
...
| **合计** | | ~**600-700+ ops**/帧（**静态估算，非实测**；与 plan L31 估算同阶） |
...
- `engine.drawings.filter { $0.panelPosition == … }`（顺位 4 #103 加，按面板过滤画线）：同文件 `:41`，每帧新建数组（绘线条数通常很小，规模可忽略）

GC/ARC 压力来源候选。**当前规模据 plan L31 无压力**；记录为「device 实测逼近预算时首查点」。

### (b) per-candle 颜色态

`drawCandles`（`:13-27`）循环内：每根蜡烛调用 `color.setFill()` / `color.setStroke()`（涨跌色判断）。涨跌分组批绘是潜在优化——**仅当 Instruments 实测 ≥ 4ms（即未达 <4ms pass）才值得**，否则属于过早优化。
...
> **边界约定**：源 spec 用「pass = 单帧 <4ms」+「超过 4ms（>4ms）引入」，留下恰为 4.0ms 的空隙。本决议门取**闭区间**读法——`<4ms` = pass/no-op，其补集 `≥4ms` = 引入——使 4.0ms 点归属明确、两分支互斥穷尽。语义比 spec 略保守（把恰 4ms 划入「引入」），不放宽。
...
4. **Bitmap Cache**：条件门未触发 → **no-op**；设计草图已写死供 ≥ 4ms 实测时（即未达 <4ms pass）按独立 anchor 引入。
