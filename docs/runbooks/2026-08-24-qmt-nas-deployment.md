...
- ⚠️ `renew` 在延期**之前会重新核实端点本身**（codex 评审 R19）：端点配置**不归这个脚本管**，`open` 之后如果有人改了反代目标、或开了 funnel（那会暴露到**公网**），只看「看门狗还在」就续期，等于把一个不安全的暴露再延长两小时。所以读不到状态、或形态不是我们开的那个 → 一律 `RENEW_REFUSED` 并**当场关闭**，不延期
...
- ⚠️ 看门狗的状态文件放在 `<部署目录>/.expose-state/`（权限 700），**不放 `/tmp`**。原因（codex 评审 R17）：旧设计把「状态文件不在了」当成「你主动撤销了」—— 可是「主动撤销」和「被 `/tmp` 清理掉 / 被误删」**长得一模一样**，后者发生时看门狗会安静退出而端点还开着。现在**状态没了 = 情况不明**：它会先去读端点的真实状态，**只有确认关着才允许退出**，否则强制关闭。
  **实测**：删掉状态文件且端点确实关着 → 记 `WATCHDOG_EXIT_VERIFIED_CLOSED` 后才退出；删掉状态文件且读不到端点状态 → 记 `WATCHDOG_STATE_LOST_BUT_SERVE_OPEN` 并进入关闭重试。
- ⚠️ 若任何一条命令打印 `LOCK_TIMEOUT` —— 说明**另有一个 open / close / 开机守卫正在跑**，本次**什么都没做**（不是失败，也不是成功）。等它跑完再重试。这把互斥锁是为了防止「一边在关、一边在开」互相拆台（codex 评审 R11）
- ⚠️ 若 `LOCK_TIMEOUT` **反复出现**：多半是有一个「关不掉的关闭」在后台一直重试（它会一直占着锁 —— 这是有意的失败关闭行为）。查 `<部署目录>/.expose-state/` 旁边的日志，看有没有成串的 `CLOSE_RETRY`；那说明 docker 或 tailscale 容器出了问题，先把它修好
...
> ⚠️ P10 / P11 / P15 三个脚本都会**先把 `training_sets` 整表锁住**再校验（codex 评审 R13）。原因：它们都是「先校验、后写」，而数据库默认的隔离级别下，**校验之后、写之前**提交的一次预占会被悄悄覆盖 —— 客户端手里还攥着它以为有效的租约，同一批数据却又变成可下载了。关掉对外端点**挡不住**「已经被接受的请求」，也挡不住 NAS 本机直连后端的流量。
> **实测**：另一个事务持锁时，P15 会一直等到对方提交才执行。
...
- 这 3 组数据是在**本地副本上做了临时转换**后生成的、**不是**走生产流水线产出的（残留 W1-R2 只能标「部分满足」）。
- ⚠️ 顺带更正一句常被引用的旧说法：生产路径上那两个缺陷（导出日志里周期列写 `1d` 而代码只认 `daily`，日线被静默跳过；一只股的坏行让整份日志读不了）**已经在 2026-08-24 由 PR #173 修掉了**。但这不改变上一条 —— 这 3 组数据**没有**在修复后的生产路径上重跑过，「生产流水线现在能跑通」仍需另行验证。
