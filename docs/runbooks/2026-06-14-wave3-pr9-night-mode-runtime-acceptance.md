...
| 2 | 进一局训练，观察主图 + 副图（light 模式） | K 线（红涨绿跌）/ MA66（紫）/ BOLL（金虚线）/ MACD（DIF 深线 + DEA 暗琥珀 + 红绿柱）/ 长按十字光标价签框（白底深字）/ 已画水平线（暗橙，codex R2-F1）**均清晰可读**（白底对比足够，真 WCAG AA 目测或取色确认）；买卖标记 B/S 字母（白）在彩色圆点上清晰 | □ pass / □ fail |
