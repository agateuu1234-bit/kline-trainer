...
**演进路径**：v1.0（首次拆分）→ v1.1（整合第 1 轮 Codex 对抗性评审 + Cloud 反评审，21 项修订）→ **v1.2（整合第 2 轮 Codex 评审 + Cloud 反评审，新增 16 项修订）**。累计 **37 项**修订，详见 §十四 评审痕迹。
...
| 组 | 代号 | 模块数 | 语言 | 说明 |
|---|---|---|---|---|
| 契约层 | **M0** | 5 | 多语言 | DB/REST/Swift 模型/错误处理/并发约定 |
| 后端 | **B** | 4 | Python | 导入、生成、服务、调度 |
| iOS 基础 | **F** | 2 | Swift | 数据模型、主题 |
| iOS 图表引擎 | **C** | 8 | Swift/UIKit | 视口、渲染、手势、绘线 |
| iOS 业务逻辑 | **E** | 5 | Swift | 时间、持仓、费用、流控、训练引擎 |
| iOS 持久化 | **P** | 6 | Swift | 网络、验收、两个库、缓存、设置 |
| iOS UI | **U** | 6 | SwiftUI | 首页、训练页、弹窗、HUD |
...
```
   ...
   └──────────────┘                  │                         │
              ...
              ▼                     ▼              ▼          │
       ┌─────────────┐      ┌──────────────┐ ┌─────────────┐  │
       │ C1 完整      │      │ E1 Tick      │ │ P1 APIClient│  │
       │ Chart+View  │      │ E2 Position  │ │ P3 TSetDB   │  │
       │ C2 Animator │      │ E3 TradeCalc │ │ P4 AppDB    │  │
       │ C7 Gesture  │      │ E4 FlowCtrl  │ │ P5 Cache    │  │
       └──────┬──────┘      └──────┬───────┘ │ P6 Settings │  │
              │                    │         └──────┬──────┘  │
              ▼                    │                │         │
       ┌──────────────┐            │                │         │
       │ C3/C4/C5/C6  │            │ P2 下载验收状态机        │
       │  渲染层       │            │ (依赖 P1/P3/P5)         │
       └──────┬───────┘            │                          │
              ▼                    │                          │
       ┌──────────────┐            ▼                          │
       ...
       └──────┬───────┘    │    Engine    │                   │
              ...
              └────────┬──────────┘                           │
                       ▼                                      │
              ┌──────────────────────────────────────┐        │
              │ U1 Home / U2 Training(+scenePhase)   │◀───────┘
              │ U3 Settle / U4 Settings              │
              │ U5 Picker / U6 History               │
              └──────────────────────────────────────┘
```

> **v1.2 说明**：scenePhase 责任链简化为 `U2 监听 → E5.onSceneActivated() → C2.resetOnSceneActive()`，C8 不再兼任 scene 生命周期职责。
...
|---|---|---|---|
| PostgreSQL | NAS | `backend/sql/schema.sql` | 加列只追加，改字段需 migration |
| 训练组 SQLite | 各 `.zip` 内部 | `backend/sql/training_set_schema_v1.sql` | `PRAGMA user_version` 作版本号 |
| app.sqlite | iOS 沙盒 | `ios/sql/app_schema_v1.sql` | GRDB migration 管理 |
...
// LeaseResponse
{
  ...
  "sets": [{
    ...
    "content_hash": string             // v1.2：CRC32 hex（8 字符）
  }]
}
```
...
```swift
// —— 枚举 ——
enum Period: String, Codable, Equatable { case m3 = "3m", m15 = "15m", m60 = "60m", daily, weekly, monthly }
enum TradeDirection: String, Codable, Equatable { case buy, sell }
enum PositionTier: String, Codable, Equatable { case tier1 = "1/5", tier2 = "2/5", tier3 = "3/5", tier4 = "4/5", tier5 = "5/5" }
enum TrainingMode: Equatable { case normal, review, replay }
enum DrawingToolType: String, Codable, Equatable { case ray, trend, horizontal, golden, wave, cycle, time }
enum DisplayMode: String, Codable, Equatable { case light, dark, system }
enum PanelId: Equatable { case upper, lower }
enum SwipeDirection: Equatable { case up, down }
enum PeriodDirection: Equatable { case toLarger, toSmaller }
...
struct FeeSnapshot: Codable, Equatable {
    let commissionRate: Double
    let minCommissionEnabled: Bool
    // stampDutyEnabled 已删除（印花税永久启用）
}

struct TradeOperation: Codable, Equatable {
    let globalTick: Int
    let period: Period
...
struct PendingTraining: Codable, Equatable {
    let trainingSetFilename: String
...
struct TrainingSetMetaItem: Codable {
    let id: Int
    let stockCode: String
    let stockName: String
    let filename: String
    let schemaVersion: Int
    let contentHash: String             // v1.2：CRC32 hex
    enum CodingKeys: String, CodingKey {
        case id
...
struct TrainingSetFile: Equatable {
    let id: Int
    let filename: String
    let localURL: URL
    let schemaVersion: Int
...
```swift
// 顶层错误类型（v1.2 以 case internalError 取代 .unknown）
...
// v1.2：所有 Reason 枚举标 Error（否则无法用 Result<_, Reason>）
enum NetworkReason: Error, Equatable {
    case timeout
    case offline
...
enum PersistenceReason: Error, Equatable {
    case diskFull
    case dbCorrupted
...
enum TradeReason: Error, Equatable {
...
enum TrainingSetReason: Error, Equatable {
    case crcFailed
    case unzipFailed
    case emptyData
...
- **验收**：生成 10 个样本，每个 zip 用 `unzip -v` 查看 CRC 与库里 content_hash 一致

### B3 FastAPI 服务模块 `app/routes.py`（v1.2 锁定 DDL 字段）
...
### C1 图表核心模块 `ChartEngine/Core/`（v1.2 全面修订）
...
```swift
struct PanelViewState: Equatable {
    var period: Period
...
enum ChartInteractionMode: Equatable {
    case autoTracking
    case freeScrolling
    case drawing(snapshot: DrawingSnapshot)
}

// v1.2：非递归冻结快照（代替 v1.1 的 frozenPanel: PanelViewState）
struct FrozenPanelState: Equatable {
    let period: Period
    let visibleCount: Int
    let offset: CGFloat
...
extension PanelViewState {
    /// 合法转换矩阵（v1.2 完整 21 格）
    /// 行 = 当前 interactionMode，列 = action
    ///
    /// |                  | panStarted     | panEnded(v)   | activateDrawing | drawingCommitted | drawingCancelled | tradeTriggered | periodComboSwitched |
    /// |------------------|----------------|---------------|-----------------|------------------|------------------|----------------|---------------------|
    /// | autoTracking     | → freeScroll   | no-op         | → drawing       | ❌ 非法           | ❌ 非法           | → autoTracking | → autoTracking      |
    /// | freeScrolling    | no-op          | no-op+animator| → drawing       | ❌ 非法           | ❌ 非法           | → autoTracking | → autoTracking      |
    /// | drawing          | no-op(截获)    | no-op         | → drawing(换工具)| → autoTracking   | → autoTracking   | → autoTracking | → autoTracking(清空)|
    ///
    /// 标注说明：
    /// - "no-op"：状态不变，无副作用
    /// - "no-op+animator"：状态不变，但触发 DecelerationAnimator（side effect，由调用方执行）
    /// - "截获"：drawing 模式下 pan 由绘线工具消费，不改 interactionMode
    /// - "❌ 非法"：触发 assertionFailure
    mutating func reduce(_ action: ChartAction) -> ChartReduceEffect {
        ...
        case (.autoTracking, .activateDrawing), (.freeScrolling, .activateDrawing):
            interactionMode = .drawing(snapshot: DrawingSnapshot(frozen: freeze(candleRange: computeRange())))
            return .none
        case (.drawing, .activateDrawing):
            return .none  // 切换工具由 DrawingToolManager 处理

        // —— drawingCommitted / drawingCancelled ——
...
enum ChartReduceEffect: Equatable {
    case none
...
struct KLineRenderState: Equatable {
    let panel: PanelViewState
    let frames: ChartPanelFrames
    let viewport: ChartViewport
    let visibleCandles: ArraySlice<KLineCandle>
    let volumeRange: ClosedRange<Double>         // v1.2 新增
    let macdRange: ClosedRange<Double>           // v1.2 新增
...
private func buildRenderState(from engine: TrainingEngine,
    ...
    let volumeRange = 0.0 ... Double(visibleSlice.map(\.volume).max() ?? 1)
    let macdRange = /* min/max over diff/dea/bar */
...
### E5 训练引擎模块（v1.2 新增 onSceneActivated + Preview Fixture）
...
### P2 下载验收状态机 `Services/DownloadAcceptance.swift`
...
final class DownloadAcceptanceRunner {
    init(api: APIClient, cache: CacheManager, tsDB: TrainingSetDB)
...
// 内部流程（v1.2 调整）：
// 1. 下载 zip
// 2. 校验 zip CRC32 == meta.contentHash（v1.2：不再解压算 sha256）
// 3. 解压到临时目录
// 4. P3.openAndVerify 校验 schema_version
// 5. 数据量校验
// 6. P5.store 原子 rename
// 7. P1.confirm（带 lease_id）
// 任何步骤失败 → 清理临时文件 → return .rejected(AppError.*)
```

### P3 训练组数据库 `Services/TrainingSetDB.swift`

```swift
protocol TrainingSetDB {
    func openAndVerify(file: URL) throws -> TrainingSetMeta   // throws AppError
    func loadMeta() throws -> TrainingSetMeta
    func loadAllCandles() throws -> [Period: [KLineCandle]]
}

// 实现约束：每个训练组文件对应一个独立 GRDB DatabaseQueue
```

### P4 应用数据库 `Services/AppDB.swift`（v1.2 统一 DatabaseQueue）
...
// v1.2 并发约束（取代 v1.1 的混乱表述）：
// - 单一 GRDB DatabaseQueue for app.sqlite
// - 读写均通过 queue 串行化
// - 调用方可跨线程调用；结果回 MainActor 更新 @Observable
// - 不使用 DatabasePool
```
...
### P6 设置存储 `Services/SettingsStore.swift`
...
final class SettingsStore {
    ...
    func snapshotFees() -> FeeSnapshot                 // v1.2：U1 启动 Normal 训练时调用
}
```
...
### U1 首页 `HomeView.swift`（v1.2 注释 fees 打包责任）

```swift
struct HomeView: View {
    init(db: AppDB, settings: SettingsStore, cache: CacheManager, api: APIClient)
}

// 启动 Normal 训练时（v1.2 明确责任）：
// let fees = settings.snapshotFees()                   // U1 负责打包
// let flow = NormalFlow(fees: fees, maxTick: ...)
// let engine = TrainingEngine(flow: flow, ...)
```
...
### Wave 0：契约冻结（串行，1 人 ~1.5 周，v1.2 从 1 周调整）

- [ ] **M0.1** 三套 DB schema（含 v1.2 lease 三列、final_tick 列、UNIQUE 约束）
- [ ] **M0.2** OpenAPI 文档（lease_id query、409/404、CRC32 hash）
- [ ] **M0.3** Swift 模型完整实现（Codable/Equatable round-trip + fixture）
- [ ] **M0.4** AppError + 所有 Reason 标 `Error` + 边界翻译表
- [ ] **M0.5** @MainActor 清单 + GRDB DatabaseQueue 约定
- [ ] **F1** Swift 模型实现 + 测试
- [ ] **F2** Theme 框架 + 默认颜色常量
- [ ] **C1 完整交付**（v1.2 不再切片）：
  - 所有类型签名与实现（ChartGeometry/PanelFrames/Viewport/Mapper/IndicatorMapper/PanelViewState/FrozenPanelState/DrawingSnapshot/InteractionMode/ChartAction/reducer/KLineView/KLineRenderState）
  - C3-C6 的 drawXxx extension 方法签名 + **空 stub 实现**（真正实现放 Wave 1）
  - 单元测试：坐标映射 / PriceRange / reducer 21 格矩阵 / renderState Equatable 短路
...
- [ ] `TrainingEngine.preview()` + `SettingsStore.preview()` Fixture 数据

### Wave 2：集成

- [ ] C8 ChartContainerView（纯桥接；computes volumeRange/macdRange 注入 renderState）
- [ ] E5 TrainingEngine（含 onSceneActivated）
- [ ] P2 DownloadAcceptanceRunner
- [ ] U1 HomeView（启动时 snapshotFees）
- [ ] U2 TrainingView（顶层 scenePhase 监听）
- [ ] U4 SettingsPanel
...
### iOS 内部
- [ ] `Models/` 所有类型 + **完整 CodingKeys** + Codable round-trip 测试
- [ ] `AppError` + 所有 `Reason` 枚举 `: Error, Equatable`（含 `case internalError(module, detail)`）
- [ ] M0.4 翻译表（本模块边界内转 AppError）
- [ ] `@MainActor` 清单：E5/P6/F2/C6 DrawingToolManager
...
### 引擎契约
- [ ] **KLineView 本体** + `KLineRenderState`（含 volumeRange/macdRange）+ `ChartPanelFrames`
- [ ] **非递归 `FrozenPanelState`** + `DrawingSnapshot`
- [ ] 所有 C1 值类型 **`Equatable`**
- [ ] **reducer 完整 3×7 矩阵** + 非法 assertionFailure 测试
- [ ] **`IndicatorMapper`** 独立类型 + valueRange 外部注入
...
- [ ] Xcode Target/Group 按 31 模块划分
- [ ] CI：单元测试 + pytest + OpenAPI 校验 + `@MainActor` Threading Checker
...
> **注**：第 38 项合并到第 25 项（M0.4 错误处理统一修订）中，不单独列行。
