...
**演进路径**：v1.0 → v1.1（21 项）→ v1.2（16 项）→ v1.3（17 项）→ **v1.4（第 4 轮收敛：2 条 v1.3 Claude 单方面决定经 codex 对抗后 reject/modify，共 2 项修订）**。累计 **56 项**修订，详见 §十四 评审痕迹。

**v1.3 → v1.4 核心增量**（2 项收敛修订）：
1. **删除 `download_acceptance_journal` 的 `leased` 状态**（reject Claude 原决定）：该状态与 plan §8.2 后端 `reserved` + modules 后端 `reserved` 形成三向名义重复；文档的崩溃恢复规则从未扫描 `leased` → 实际无价值。journal 首个持久化状态改为 `downloaded`（zip 下载完成后才写 journal 行）；GET meta 成功、下载未开始阶段只在内存持 `lease_id`，不写 journal。
2. **拆分 CONTRACT_VERSION bump 策略**（modify Claude 从严原决定）：
   - **必须 bump 顶层 `CONTRACT_VERSION`**：删 state / 改 raw value / 改既有语义 / 改恢复扫描集 / 影响 DDL / 跨系统契约变更
   - **只 bump `P2 journal states` 子版本，不联动顶层**：仅追加本地中间态 + raw value 追加 + 既有 state 语义不变 + DDL 不变 + 不改恢复扫描集 + 不跨 REST 传输；DAO/reader 须对未知 state 显式处理（fail-closed 或忽略）
   - 原"journal state enum 增删均要 bump 顶层"条款被替换
...
#### CONTRACT_VERSION 矩阵（v1.3 新增 / v1.4 拆分 bump 策略）
...
**JournalState enum**（由 P2 定义、P4 DAO 反序列化；v1.4 删 `leased`）：

```
downloaded      -- zip 下载完成；尚未 CRC（journal 首个持久化状态，v1.4）
crcOK           -- CRC32 校验通过
unzipped        -- 解压完成
...
夜间模式 = **light/dark 双 token 集**：现有 13 个 `AppColorRGBA` 默认 token（背景近黑）= **dark/夜间集**；新增 **light/白天集**（背景近白 / 文本近黑 / 语义色保持红涨绿跌 / 辅助线按白底降明度，**具体 RGBA 已落地：`AppPalette.light` 13 token（双 static 集机制；前景 token 经 WCAG 相对亮度对比 ≥3:1 单测闸门；marker 字母 buy 白/sell 黑 vs 涨跌点 ≥4.5:1 小文字阈；真 WCAG AA 设备观感属运行时矩阵）**）。render 层按 `themeController.resolve(trait:)` 返回的 `AppColorScheme` 选 light/dark 集（已落地 = `UIChartPalette.forScheme(_:)`；`KLineView` 经 `registerForTraitChanges` 在 trait 变化重渲染）。`displayMode == .system` 跟随 `UITraitCollection` 变化重解析重渲染（`AppRootView.preferredColorScheme` 据 `display_mode` 驱动全窗 + 嵌入图表 trait）。持久化经 `AppSettings.displayMode`（settings key `display_mode`，**无 schema 改动**）。
...
**v1.4 内部流程**（按 journal 状态推进，确保崩溃恢复；v1.4 删 `leased` 步骤 0）：

| 步骤 | journal state 推进 | 失败分支 |
|---|---|---|
| 0. `api.fetchMeta` 成功：**仅在内存持有 `lease_id`**，不写 journal 行（v1.4 修订）| 无 journal 行 | 直接返回 rejected |
| 1. `api.downloadTrainingSet` 完成后**写入首条 journal 行** | → `downloaded` | rejected（网络，**不写 journal**；服务端 lease TTL 到期自动回滚 `training_sets.status`）|
...
enum P2JournalState: String, Codable, Sendable {
    // v1.4：删除 leased（与 plan §8.2 后端 reserved 形成三向重复；文档恢复集从不扫 leased → 无价值）
    // GET /meta 成功、下载未开始阶段只在内存持 lease_id，不写 journal
    case downloaded        // zip 下载完成，journal 首条持久化行（v1.4）
    case crcOK
    case unzipped
    case dbVerified
    case stored
...
v1.3（累计 54 项修订）
   │
   │ 第 4 轮（v1.4：codex:rescue 对抗性分析 2 条 v1.3 Claude 单方面决定）
   ▼
┌─────────────────────────────────────────────┐
│ 议题 1 `leased` 状态重复：codex reject Claude│
│ 议题 2 CONTRACT_VERSION bump：codex modify   │
│ Claude 接受两条判决；有效修订：2（落 v1.4） │
└────────────┬────────────────────────────────┘
             ▼
v1.4（累计 56 项修订）
```
...
| 41 | R3-P0-4 | P0 | P0 | M0.1/P4/P2 | `download_acceptance_journal` 表 + `P2JournalState` 9 态 + `AcceptanceJournalDAO` + confirmPending 重试（v1.4 后态数缩为 8，删 `leased`）|
...
| **— 第 4 轮（v1.4）—** | | | | | |
| 56 | R4-codex reject | P0 | P1 | M0.1 | 删 `P2JournalState.leased`；journal 首条持久化行改为 `downloaded`；GET /meta 后只在内存持 lease_id 不写 journal（原 v1.3 Claude 单方面决定保留被 codex 证据驳回）|
| 57 | R4-codex modify | P1 | P1 | M0.1 | CONTRACT_VERSION bump 策略拆二：破坏性变更 → 必须 bump 顶层；本地 journal state 兼容新增 → 只 bump 子版本（reader 对未知 state 显式处理）；原 v1.3 Claude"从严"单方面决定被缩小适用范围 |

> **注**：v1.3 共 17 项 Part B 修订（表格行 38-55 = 18 行）；v1.4 共 2 项收敛修订（56-57）。累计 56 项修订。
