...
public struct TrainingView: View {
    ...
    private var cannotPreserveCopy: String {
        ...
        case .trainingSetMissing:
            ...
            //    坏状态里，又是一句半真话。这一支能诚实给出的只有「清理存储 → 重试入账」。
            return "本局的存档还在，但它依赖的训练组数据文件已被清理掉了 —— 现在退出的话这一局将无法继续，所以没有退出。请先清理设备存储空间，再回来重试入账。⚠️ 能看到这个提示说明存储已经写不进去了，此时「放弃本局」同样会失败。"
        case .unreadable:
        ...
        case .none, .usable:
            ...
            //    ⚠️ **三支一视同仁**（2026-09-06 自查订正）：我上一稿以为 `.trainingSetMissing`
            //    那一支「成因是缓存淘汰、存储是好的、所以放弃能成功」—— **那个论证只看了路的一半**。
            //    能走到本弹窗的前提是 `finalize()` 抛错，而它走的是「insertRecord + clearPending
            //    单事务」，**必须写库** ⇒ 进到这里时写库已经失败过一次；而 `.trainingSetMissing`
            //    只要求**读**成功 + 文件不在（读得动、写不动 = 磁盘满的典型形态）。
            //    ⇒ 三支的「放弃本局」全都会失败，不能对任何一支打包票。
