...
public final class TrainingSessionCoordinator {
    ...
    public enum CheckpointStatus: Equatable, Sendable {
        ...
        /// ⚠️ 「清理设备存储空间」对这一种是**部分有效**（2026-09-06 随判据订正）：
        ///    腾空间**不会**让被删的文件回来 —— 但**若**这一次的失败确实是存储写满导致的，
        ///    而重试入账要写库、写库要空间 ⇒ 清理是为了让**入账**写得进去，不是为了找回文件。
        ///    ⛔ 本段曾写着「是无效建议」，那是只针对「找回文件」这一个目的说的；照旧文改代码
        ///    会把新文案当违规改回去（本仓已有「陈旧头注教后人把洞改回来」的先例）。
