...
final class AppDB0005MigrationTests: XCTestCase {
    ...
    // fresh-install 终态：单独用 makeFreshDB（跑完整 migrator）断言 user_version=6（0008 新增）
    func test_fresh_install_full_migrator_user_version_6() throws {
    ...
    func test_0005_backfills_total_capital_from_last_record_and_keeps_other_keys() throws {
        ...
        try AppDBMigrations.makeMigrator().migrate(q)   // 完整 migrator：0005+0006+0007+0008 在此真跑
        XCTAssertEqual(try q.read { try Int.fetchOne($0, sql: "PRAGMA user_version") ?? 0 }, 6)
    ...
    func test_0005_skips_write_on_non_finite_derived_capital() throws {
        ...
        try AppDBMigrations.makeMigrator().migrate(q)
        XCTAssertEqual(try q.read { try Int.fetchOne($0, sql: "PRAGMA user_version") ?? 0 }, 6)   // 仍推进（0008 bump→6）
        let cap = try q.read { try String.fetchOne($0, sql: "SELECT value FROM settings WHERE key='total_capital'") }
