...
final class AppDBMigrationsTests: XCTestCase {
    ...
    // MARK: - migrator 跑过的 PRAGMA user_version（0010 终态 = 8）
    func test_full_migrator_sets_user_version_8() throws {
        let dbURL = try AppDBFixture.makeFreshDB()
        defer { try? FileManager.default.removeItem(at: dbURL.deletingLastPathComponent()) }
        ...
        XCTAssertEqual(version, 8)   // 0001 置 1，0004 bump 至 2，0005（RFC-A A4）bump 至 3，0006（A3）bump 至 4，0007（review-redesign）bump 至 5，0008（整改④）bump 至 6，0009（画线扩充 P1a）bump 至 7，0010（本片）bump 至 8
    }
